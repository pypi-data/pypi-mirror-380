...
class Wording(BasicTypography):
    ...
    def __init__(self, font_family, font_size, font_color, target_width, line_spacing=-1):
        super().__init__(font_family, font_size, font_color, target_width)
        self.rendered_char = {}
        if line_spacing == -1:
    ...
    def generate(self, text):
        ...
        for char in self.word_split(text):
            ...
            if prerender.get_width() + current_width <= self.target_width:
                ...
                current_width = 0
            current_width += prerender.get_width()
        height = len(column) * self.font_size + self.line_spacing * (len(column) - 1)
        ...
        for c in column:
            ...
            current_height += self.font_size + self.line_spacing
        return sur

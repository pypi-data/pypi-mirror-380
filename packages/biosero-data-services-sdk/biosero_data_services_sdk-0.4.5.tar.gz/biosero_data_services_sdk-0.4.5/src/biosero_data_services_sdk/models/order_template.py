...
class OrderTemplate(DataServicesModel):
    ...
    def from_folder(cls, folder: Path) -> Self:
        with (folder / ORDER_TEMPLATE_JSON_FILENAME).open("r") as f:
            data = json.load(f)
    ...
    def dump_to_folder(self, root_folder: Path, *, create_subfolder_using_template_name: bool = False) -> None:
        ...
        with (root_folder / ORDER_TEMPLATE_JSON_FILENAME).open("w") as f:

...
class RedisClient():
    def __init__(self,host,port=6379,password=None,db=0,preFix='',defaltExpire=30):
        '''
        redis连接池
        host: str 为redis地址
        port: int 为redis端口
        ...
        defaltExpire: int 为默认过期时间30秒，单位秒
        '''
        ...
        self.defaltExpire = defaltExpire
    def gen_key(self,key,*args):
        if '{}' in key:
            key =self.keyPrefix+ Gkey(key,*args)
        return key
    ...
    async def set_int(self,key,value,**kwargs):
        ...
        kwargs['ex'] = kwargs.get('ex',self.defaltExpire)
        return await self.client.set(ck,value,**kwargs)
    ...
    async def set(self,key,value,*args,**kwargs):
        ...
        ck = self.keyPrefix+ key
        kwargs['ex'] = kwargs.get('ex',self.defaltExpire)
        return await self.client.set(ck,value,*args,**kwargs)
    async def set_json(self,key,value,*args,**kwargs):
        ...
        ck = self.keyPrefix+ key
        kwargs['ex'] = kwargs.get('ex',self.defaltExpire)
        return await self.client.set(ck,json.dumps(value),*args,**kwargs)

...
if not os.path.exists(main_package_path):
    ...
    print("📌 Must Read 📌- import CleanDB as mdb \n")
    ...
    print("🛟 For Quick guide call CleanDB.help() function\n")
...
def validate_and_register_row_shape(items: list, validation: int | str | None, validation_path: str , list2d = None) -> None:
    ...
    if first_write is True:
        ...
        if is_2d is False and list2d is None: 
            raise ValueError("🟡 1D list dectected, Creating a new file or after a validation reset. Therefore the list2d parameter must be False")

...
class TestEdgeCases:
    ...
    def test_empty_speakers_search(self):
        result = self.service.search_sessions_by_speaker('')
        # Empty speaker search should return sessions with speakers (John Doe session)
        assert len(result['items']) >= 0  # Could be 0 or more depending on implementation

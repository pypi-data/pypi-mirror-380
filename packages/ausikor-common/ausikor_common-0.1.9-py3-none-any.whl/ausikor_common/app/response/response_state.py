...
# Backwards-compatible helpers expected by other modules

def url_not_found(url: str) -> dict:
    return url_not_found_message(url=url)


def method_not_allowed(method: str) -> dict:
    return method_not_allowed_message(method=method)


def server_error(status_code: int, message: str, explanation: str = "") -> dict:
    return server_error_message(status_code=status_code, message=message, explanation=explanation)
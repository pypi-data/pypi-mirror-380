...
__all__ = ("FlowTimsDocking2",)


class FlowTimsDocking2(BaseFlow):
    def __init__(
        self,
        name: str = "timsdocking2",
        *,
        solvent: str = "tip4pew",
        ions: str = "jv",
        ...
        ########## binder ##########
        wnf_binderinput = Filter(
            f"wnf_binderinput_{self.name}",
            artifact_types=(BoxBinderLigandPDB, BinderLigandPDB, LigandLib, LigandFrcmod),
            fail_if_no_artifacts=True,
            single=True,
        )
        wnf_binderbox = Filter(
            f"wnf_binderbox_{self.name}",
            artifact_types=(BoxBinderLigandPDB,),
            fail_if_no_artifacts=True,
            single=True,
        )
        patch_binder = PatchPDBLib(wnid=f"patch_binder_{self.name}", skippable=skippable)
        wnf_binder_libfrcmod = Filter(
            wnid=f"wnf_binder_libfrcmod_{self.name}",
            artifact_types=(
                LigandLib,
                LigandFrcmod,
            ),
        ...
        bl = BoxLigand(
            wnid=f"bl_{self.name}", resolution=resolution, solute_radius=solute_radius, solvent_radius=solvent_radius
        )
        ...
        wnf_binder_rst7 = Filter(
            wnid=f"wnf_binder_rst7{self.name}", artifact_types=(BinderLigandRestart,), skippable=skippable
        )
        ...
        self.dag.add_edge(self.root, wnf_binderinput)
        self.dag.add_edge(wnf_binderinput, patch_binder)
        self.dag.add_edge(patch_binder, bl)
        self.dag.add_edge(wnf_binderinput, wnf_binderbox)
        self.dag.add_edge(wnf_binderbox, bl)
        self.dag.add_edge(patch_binder, wnf_binder_libfrcmod)
        ...
        self.dag.add_edge(bl, gtb)
        self.dag.add_edge(wnf_binder_libfrcmod, gtb)

        self.dag.add_edge(gtb, hmr_binder)
        self.dag.add_edge(gtb, wnf_binder_rst7)

        if ffopt:
            self.dag.add_edge(hmr_complex, ffopt_complex)
            self.dag.add_edge(ffopt_complex, masks)
            self.dag.add_edge(ffopt_complex, init_min)

            self.dag.add_edge(hmr_binder, ffopt_binder)
            self.dag.add_edge(ffopt_binder, self.leaf)
        else:
            self.dag.add_edge(hmr_complex, masks)
            self.dag.add_edge(hmr_complex, init_min)
        ...
        self.dag.add_edge(pt, crs_complex)
        self.dag.add_edge(crs_complex, self.leaf)

        self.dag.add_edge(wnf_binder_rst7, crs_binder)
        self.dag.add_edge(crs_binder, self.leaf)
        self.dag.add_edge(wnf_binder_rst7, self.leaf)

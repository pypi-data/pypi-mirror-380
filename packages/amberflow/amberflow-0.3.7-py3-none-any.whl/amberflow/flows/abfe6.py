...
class FlowABFE6(BaseFlow):
    ...
    nstlim_equil1 : int, optional
        Number of steps for the first equilibration phase (the one that runs at 2fs)(default: 50000).
    nstlim_equil2 : int, optional
        Number of steps for the second equilibration phase (the one that runs at 4fs)(default: 250000).
    ...
    nstlim_trial : int, optional
        Number of steps for the final production MD run of each trial (default: 125).
    ...
    bar_intervall : Optional[int], optional
        Interval for MBAR analysis during production (default: nstlim_trial).
    ...
    max_systems_binder : int, optional
        Maximum number of binder systems to run in parallel. Overrides gpus_binder, unless set to 0 (default: 1).
    gpus_binder: int, optional
        gpus for the binder LambdaMDRuns. Overriden by max_systems_binder (default: 1).
    max_systems_complex : int, optional
        Maximum number of complex systems to run in parallel. Overrides gpus_complex, unless set to 0 (default: 1).
    gpus_complex: int, optional
        gpus for the complex LambdaMDRuns. Overriden by max_systems_complex (default: 1).
    start_trial: int, optional
        The starting trial number (default: 1).
    end_trial: int, optional
        The ending trial number (default: 1).
    ...
    def __init__(
        ...
        *,
        ...
        binder_equil1_params = self._evolve_binder_rmsd_params(equil_params)
        ...
        complex_equil1_params = self._evolve_complex_rmsd_params(equil_params)
        ...
        # 2 #
        binder_equil2_params = attrs.evolve(binder_equil1_params, nstlim=nstlim_equil2, dt=0.004)
        ...
        complex_equil2_params = attrs.evolve(complex_equil1_params, nstlim=nstlim_equil2, dt=0.004)
        ...
        equil_params = attrs.evolve(equil_params, dt=0.004, nstlim=nstlim_trial_equil, irest=0, ntx=1)
        ...
        for j, (tpdt_binder, tpdt_complex) in enumerate(tpdts, start=start_trial):
            ta2d = Amber2Dats(wnid=f"a2d{j}", skippable=skip_analysis)
            self.dag.add_edge(tpdt_binder, ta2d)
            self.dag.add_edge(tpdt_complex, ta2d)
            txml = GetXML(wnid=f"xml{j}", skippable=skip_analysis)
            self.dag.add_edge(ta2d, txml)
            tmbar = EdgeMBAR(wnid=f"edge_mbar{j}", mode=edgembar_mode, skippable=skip_analysis)
        ...
        for j, (tequil_binder, tequil_complex) in enumerate(tequils[1:]):
            blockt = ImpossibleFilter(wnid=f"blockt{start_trial + j + 1}")

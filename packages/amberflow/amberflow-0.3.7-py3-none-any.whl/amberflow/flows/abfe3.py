...
class FlowABFE3(BaseFlow):
    ...
    nstlim_equil : int, optional
        Number of steps for the first equilibration phase (the one that runs at 2fs)(default: 100000).
    nstlim_trial_equil : int, optional
        Number of steps for the equilibration phase of each trial (the one that runs at 4fs) (default: 1000000).
    ...
    def __init__(
        ...
        *,
        ...
        nstlim_equil: int = 100000,
        nstlim_trial_equil: int = 1000000,
        ...
        for tequil_binder, tpdt_binder in tbinder_nodes:
            self.dag.add_node(tequil_binder)
            self.dag.add_node(tpdt_binder)
        for tequil_complex, tpdt_complex in tcomplex_nodes:
            self.dag.add_node(tequil_complex)
            self.dag.add_node(tpdt_complex)
        ...
        # Equilibration
        self.dag.add_edge(heat_binder, equil_binder)
        # Trials
        for tequil_binder, tpdt_binder in tbinder_nodes:
            self.dag.add_edge(equil_binder, tequil_binder)
        ...
        # Equilibration
        self.dag.add_edge(heat_complex, equil_complex)
        # Trials
        for tequil_complex, tpdt_complex in tcomplex_nodes:
            self.dag.add_edge(equil_complex, tequil_complex)

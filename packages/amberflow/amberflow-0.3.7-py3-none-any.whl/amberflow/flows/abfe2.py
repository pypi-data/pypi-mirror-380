...
class FlowABFE2(BaseFlow):
    ...
    nstlim_heat : int, optional
        Number of steps for Lambda heating runs(default: 1000000).
    nstlim_equil1 : int, optional
        Number of steps for the first equilibration phase (the one that runs at 2fs)(default: 1000000).
    nstlim_equil2 : int, optional
        Number of steps for the second equilibration phase (the one that runs at 4fs) (default: 1000000).
    ...
    def __init__(
        ...
        *,
        ...
        nstlim_heat: int = 1000000,
        nstlim_equil1: int = 1000000,
        nstlim_equil2: int = 1000000,
        nstlim_pdt: int = 125,
        ntpr_pdt: int = 125,
        ...
        # 2 #
        equil_params = attrs.evolve(equil_params, dt=0.004, nstlim=nstlim_equil2)
        binder_equil_params = self._evolve_binder_rmsd_params(equil_params)
        ...
        for pdt_binder, _ in pdt_nodes:
            self.dag.add_edge(equil2_binder, pdt_binder)
        ...
        for _, pdt_complex in pdt_nodes:
            self.dag.add_edge(equil2_complex, pdt_complex)

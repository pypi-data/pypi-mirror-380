...
from ibex.openfold.utils.residue_constants import (
    restype_atom14_mask,
    restype_atom14_rigid_group_positions,
    restype_atom14_to_rigid_group,
    restype_rigid_group_default_frame,
)
from ibex.openfold.utils.feats import (
    frames_and_literature_positions_to_atom14_pos,
    torsion_angles_to_frames,
)
from ibex.openfold.utils.precision_utils import is_fp16_enabled
from ibex.openfold.utils.rigid_utils import Rigid, Rotation
from ibex.openfold.utils.tensor_utils import (
    dict_multimap,
    flatten_final_dims,
    permute_final_dims,
)
        ...
        with torch.no_grad():
            if init_fn is not None:
                ...
                if init == "default":
                    ...
                    raise ValueError(f"Invalid init string {init}.")
...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        deepspeed_is_initialized = (
            deepspeed_is_installed and dist.is_initialized()
        )
...
class AngleResnet(nn.Module):
    ...
    def __init__(self, c_in, c_hidden, no_blocks, no_angles, epsilon, use_original_sm):
        ...
        Args:
            ...
            use_original_sm:
                If True implement line 11 of algorithm 20 correctly else use the Ibex implementation.
        """
...
class StructureModule(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            use_original_sm:
                If True use original structure module implementation else use Ibex. If True:
        ...
        self.use_plm_embedding = use_plm_embeddings
        ...
        # remove to match ABodyBuilder2 and because inputs are one hot encodings.
        # self.layer_norm_s = LayerNorm(self.c_s)
        # self.layer_norm_z = LayerNorm(self.c_z)
        if use_plm_embeddings:
            self.plm_in = nn.Sequential(
                nn.Linear(plm_embedding_dim, 256),
                # nn.BatchNorm1d(256),
                nn.ReLU(),
                nn.Dropout(p=0.2),
                nn.Linear(256, plm_input_dim),
                nn.Dropout(p=0.2),
                nn.LayerNorm(plm_input_dim)
            )

            # self.linear_in_node = nn.Sequential(
            #     nn.Linear(self.c_s, self.embed_dim),
            #     nn.ReLU(),
    ...
    def forward(
        ...
        Args:
            evoformer_output_dict:
                Dictionary containing:
                    "single":
                        [*, N_res, C_s] single representation (or [*, N_res, C_s - plm_input_dim] if use_plm_embedding)
                    "pair":
                        [*, N_res, N_res, C_z] pair representation. One hot encoding of relative positions.
                    "plm_embedding": (optional)
                        [*, N_res, plm_embedding_dim] ESM-C small embeddings
            aatype:
        ...
        if self.use_plm_embedding:
            plm_embedding = evoformer_output_dict["plm_embedding"]

            # Reshape to combine batch and residue dimensions
            batch_size, num_residues, embedding_dim = plm_embedding.shape
            plm_embedding = plm_embedding.view(batch_size * num_residues, embedding_dim)

            # Pass through plm_in network
            plm_embedding = self.plm_in(plm_embedding)

            # Reshape back to [batch_size, num_residues, plm_input_dim]
            plm_embedding = plm_embedding.view(batch_size, num_residues, -1)

            s = torch.cat((s, plm_embedding), dim=-1)

        if mask is None:
            # [*, N]
            mask = s.new_ones(s.shape[:-1])

        # Removed to make closer to Ibex and because the inputs are one hot encodings.
        # [*, N, C_s]

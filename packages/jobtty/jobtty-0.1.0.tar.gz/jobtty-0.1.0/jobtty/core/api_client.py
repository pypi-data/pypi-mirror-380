...
class JobttyAPI:
    ...
    def _make_request(self, source: str, endpoint: str, params: Dict = None) -> Optional[Dict]:
        ...
        try:
            ...
            console.print(f"[dim yellow]DEBUG: Making request to: {url}[/dim yellow]")
            console.print(f"[dim yellow]DEBUG: Parameters: {params}[/dim yellow]")
            
            response = self.session.get(url, params=params, headers=headers, timeout=10)
            console.print(f"[dim yellow]DEBUG: Response status: {response.status_code}[/dim yellow]")
            response.raise_for_status()
            json_data = response.json()
            console.print(f"[dim yellow]DEBUG: Response keys: {list(json_data.keys()) if json_data else 'None'}[/dim yellow]")
            return json_data
    ...
    def search_jobs(self, source: str, search_params: Dict) -> List[Dict]:
        ...
        for key, value in search_params.items():
            if value is not None and value != "" and value != []:
                # Keep 'query' parameter as backend expects it
        ...
        if jobs_data:
            ...
            console.print(f"[dim yellow]DEBUG: Jobs array length: {len(jobs_array)}[/dim yellow]")
            console.print(f"[dim yellow]DEBUG: First job: {jobs_array[0] if jobs_array else 'None'}[/dim yellow]")
            jobs = self._normalize_jobtty_jobs(jobs_array)
            console.print(f"[dim yellow]DEBUG: Normalized jobs count: {len(jobs)}[/dim yellow]")
            return jobs

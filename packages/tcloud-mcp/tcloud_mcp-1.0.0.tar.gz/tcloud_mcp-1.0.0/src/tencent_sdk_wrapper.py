...
class TencentSDKWrapper:
    ...
    def get_action_info(self, service: str, action: str) -> Dict[str, Any]:
        ...
        if hasattr(models_module, request_class_name):
            ...
            # Get parameter information from the request class
            # This is a simplified approach - in reality, you'd need to inspect the class more deeply
            info["request_available"] = True

            # Try to create a sample request to see its structure
            try:
                sample_request = request_class()
                info["sample_request"] = sample_request.to_json_string()
            except:
                info["sample_request"] = "{}"
        else:
            info["request_available"] = False

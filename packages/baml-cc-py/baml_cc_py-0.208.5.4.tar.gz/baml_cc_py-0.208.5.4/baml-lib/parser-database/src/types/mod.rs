...
fn visit_client<'db>(idx: ValExpId, client: &'db ast::ValueExprBlock, ctx: &mut Context<'db>) {
    ...
    client
        ...
        .for_each(|(_idx, field)| match field.name() {
            ...
            config => {
                // Collect unknown fields instead of immediately erroring
                unknown_fields.push((field.name().to_string(), field.span().clone()));
            }
        });
    ...
    match provider {
        ...
        None => {
            ...
            // If provider is missing, report all unknown fields as errors
            for (field_name, span) in unknown_fields {
                ctx.push_error(DatamodelError::new_validation_error(
                    &format!("Unknown field `{field_name}` in client"),
                    span,
                ));
            }
        }

    ...
    try:
        ...
        with torch.no_grad():
            for k in trange(numIterations, desc=description):
                r = y_normalized - A_normalized @ theta_flat  # r est de taille (TN,)
                p = A_normalized.T @ r  # p doit être de taille (ZX,), donc on multiplie par A_normalized.T
                rsold = torch.dot(r, r)

                for _ in range(2):
                    Ap = A_normalized @ p  # Ap est de taille (TN,)
                    alpha = rsold / (torch.dot(p, A_normalized.T @ Ap) + 1e-8)  # Correction du produit scalaire
                    theta_flat += alpha * p
                    r -= alpha * Ap
                    rsnew = torch.dot(r, r)
                    if rsnew < 1e-8:
                        break
                    p = A_normalized.T @ r + (rsnew / rsold) * p  # Mise à jour de p
                    rsold = rsnew

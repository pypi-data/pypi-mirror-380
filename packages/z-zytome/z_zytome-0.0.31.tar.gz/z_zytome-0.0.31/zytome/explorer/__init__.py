...
class Dataset(DatasetInterface):
    ...
    def to_tpm(self) -> "Dataset":
        ...
        if sp.issparse(normalized_X):
            ...
            row_sums = check_X.sum(axis=-1, keepdims=True)
            row_sums[row_sums == 0] = 1.0
            sum_normalized_X = normalized_X / row_sums

        tpm_X = sum_normalized_X * 1_000_000

        if sp.issparse(tpm_X):
            nan_mask = np.isnan(tpm_X.data)
            has_nan = nan_mask.any()
        else:
            has_nan = np.isnan(tpm_X).any()

        if has_nan:
            if np.isnan(check_X).any() or np.isinf(check_X).any():
                msg = (
                    "Normalization contains NaN.\n"
                    "Your original values contained NaN or Inf which resulted in this error."
                )
            else:
                msg = (
                    "Normalization contains NaN.\n"
                    "Your original values look okay; the error occurred elsewhere."
                )
            logger.error(msg)
    ...
    def raw_normalized_by_feature_length(self) -> np.ndarray:
        return self.raw / np.where(
            self.feature_lengths[None, :] == 0, 1e-16, self.feature_lengths[None, :]
        )

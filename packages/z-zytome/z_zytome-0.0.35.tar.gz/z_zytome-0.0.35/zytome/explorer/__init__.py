...
class Dataset(DatasetInterface):
    ...
    def raw_normalized_by_feature_length(self):
        ...
        safe_fl = np.where(fl == 0, 1.0, fl)  # replace 0 with 1.0 to avoid huge numbers

        if sp.issparse(self.raw):
            inv_fl = 1.0 / safe_fl  # shape (num_features,)
            normalized = self.raw.copy()
            normalized = normalized.multiply(inv_fl)
            return normalized
        else:

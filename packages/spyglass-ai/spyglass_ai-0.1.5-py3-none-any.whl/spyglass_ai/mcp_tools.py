...
def _wrap_mcp_tool(tool):
    ...
    if hasattr(tool, 'invoke'):
        ...
        def traced_invoke(input_data, config=None, **kwargs):
            with spyglass_tracer.start_as_current_span(f"mcp.tool.{tool.name}.invoke") as span:
                try:
                    # Set tool attributes
    ...
    if hasattr(tool, 'ainvoke'):
        ...
        async def traced_ainvoke(input_data, config=None, **kwargs):
            with spyglass_tracer.start_as_current_span(f"mcp.tool.{tool.name}.ainvoke") as span:
                try:
                    # Set tool attributes
                    _set_tool_attributes(span, tool, {"input": input_data})
...
def wrap_mcp_session(session):
    ...
    if hasattr(session, 'call_tool'):
        ...
        async def traced_call_tool(name, arguments=None):
            with spyglass_tracer.start_as_current_span(f"mcp.session.call_tool.{name}") as span:
                try:

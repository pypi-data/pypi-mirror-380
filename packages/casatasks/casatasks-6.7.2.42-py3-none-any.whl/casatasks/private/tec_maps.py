...
## =============================================================================
    ...
    # Ensure retrieved files are stored locally
    localDir=os.getcwd()+'/'
        ...
        try:
            ...
            if len(glob.glob(igs_file))<1:     # file does not yet exist locally
                ...
                if test_IONEX_connection(get_path):
                    local_get_file=localDir+get_file
                    with open(local_get_file, 'wb') as fp:
                        ftps.retrbinary("RETR " + get_file, fp.write)
                    os.system('gunzip '+local_get_file)
                else:
            ...
            if 'ftps' in locals():
                # only quit ftps connection if it was assigned/constructed successfully above
                ftps.quit()
            print('Failed to successfully connect to and/or retrieve from ftp://gdc.cddis.eosdis.nasa.gov/gps/products/ionex/ with error: ', e)
            print('NB: A working internet connection is required!')
            print('NB: Ionex files may not be available for dates prior to circa June 1998.')
            return 0,0,0,0,0,0,0,0,[0],''
    else:
        print('IGS Final product file: '+igs_file+' already available in current working directory.')
    ...
    igsf=open(igs_file, 'r')
    linestring = igsf.read()
    igsf.close()
...
#        plt.xlabel(r'$\mathrm{Time}$ $\mathrm{(minutes)}$', fontsize=10)
        plt.ylabel('TEC (TECU)', fontsize=10)
#        plt.ylabel(r'$\mathrm{TEC}$ $\mathrm{(TECU)}$', fontsize=10)
        plt.title('TEC values for Long. = '+str(my_long)+' / Lat. = '+str(my_lat),fontsize=10)
#        plt.title(r'$\mathrm{TEC}$ $\mathrm{values}$ $\mathrm{for}$ $\mathrm{Long.}$ $\mathrm{=}$ $\mathrm{'+str(my_long)+'}$ / $\mathrm{Lat.}$ $\mathrm{=}$ $\mathrm{'+str(my_lat)+'}$',
#                  fontsize=10)

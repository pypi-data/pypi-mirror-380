...
class TheoreticalBeam:
    ...
    def get_beamsize_image(self):
        ...
        if len(self.sampling_arcsec) == 1 and \
            ...
            # Gaussian fit to the result convolved with additional kernel
            # to take into account raster pointing pattern.
            # CAS-14608
            # Note that the fitting could fail especially when the
            # pointing pattern is peculiar due to hardware/software problem
            # related to the antenna. In that case, the FWHM estimate will
            # fall back to theoretical beam size.
            try:
                ...
                casalog.post(
                    "Gaussian fit to get effective beam size failed"
                    f" with the following error: {e} \n"
                    "Falling back to the theoretical beam size"
                    " derived from the antenna beam pattern.",
                    priority="WARN"
                )
                fwhm_geo_arcsec = fwhm_arcsec
            finally:
...
def findFWHM(x, y, level=0.5, s=0):
    ...
    if (len(result) == 2):
        ...
        errmsg = "Unsupported FWHM search in CASA. " + \
                 f"More than two crossings ({len(result)}) at level {halfmax} ({level} % of peak)."

...
# it's the right antenna/pad; add the offsets to those already accumulated
    ...
    # If no non-trivial antpos corrections found, BUT start time
    #  is during online tropo correction error period in cycle 16B
    #  (2016/08/09 (57609) - 2016/11/15 (57707), _exclusive_),
    #  invent trivial correction (i.e., zeros) for one antenna,
    #  so that non-empty ant_string and parms will be returned
    #  below, and a caltable (all zeros) will be generated
    #  with the tropo correction keyword trigger (in c++ code).
    #  (This mechanism is implemented here (and not the calling scope)
    #   because this issue is EVLA-specific, and the obs MJD_start_time
    #   is readily available here.  Therefore, least perturbation.)
    #  gmoellen 2025Mar10
    if len(ants)==0 and MJD_start_time > 57609.0 and MJD_start_time < 57707.0:
        ants=[0]
        parms=[0,0,0]

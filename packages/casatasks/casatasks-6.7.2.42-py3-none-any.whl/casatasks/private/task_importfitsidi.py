...
def importfitsidi(fitsidifile,vis,constobsid=None,scanreindexgap_s=None,specframe=None,coordframe=None):
    ...
    try:
        ...
        if mycoordframe not in ['', 'ICRS', 'B1950', 'J2000']:
            raise ValueError('Value '+mycoordframe+' of parameter coordframe invalid. Possible values are ICRS, B1950, J2000')
        ...
        for colname in ['DELAY_DIR', 'PHASE_DIR', 'REFERENCE_DIR']:
            kwds = mytb.getcolkeywords(colname)
            if not mycoordframe=='':
                kwds['MEASINFO']['Ref'] = mycoordframe
            # until casacore gets fixed to use ICRS as its default
            elif mycoordframe=='' and kwds['MEASINFO']['Ref'] == 'J2000':
                kwds['MEASINFO']['Ref'] = 'ICRS'
            mytb.putcolkeywords(colname, kwds)
        mytb.close()

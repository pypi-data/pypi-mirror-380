...
from casatasks.private.imagerhelpers.imager_return_dict import ImagingDict, ConvergenceResult, StopCodes
...
class gclean:
    ...
    _info_group = """imaging"""
    _info_desc = """Radio Interferometric Image Reconstruction"""
...
casatasks/src/private/imagerhelpers/_gclean.py
    ...
    def _deconv_over_fields(self, niter, mask=None, threshold=None, restoration=False, hasit_per_field=None):
        """
        Loop over each field, deconvolve, merge the return dicts, all that good
        stuff. The only configurable option here is self._niter, and
        restoreation, which is different depending on whether we want to
        deconvolve (niter > 0) or update the mask (niter = 0), or restore
        (niter=0, restoration=True).

        The niter passed in is tracked by self._niter for when we need to do deconvolution,
        and is set to 0 otherwise.

        The other parameters that come in from the GUI are used directly within this function,
        such as self._cyclethreshold, self._nsigma, self._gain, etc.
        """

        initialize_merge_dict = False
        counter = 0

        # Loop over each image (outliers included) and deconvolve.
        # This is not necessary for tclean, since we want a joint gridding of all the images
        for fidx, fieldid in enumerate(self._paramlist.allimpars.keys()):
            # Check if convergence info exists
            # If it exists, skip field if it has converged
            if hasit_per_field is not None and restoration == False:
                if hasit_per_field[fidx]:
                    # If convergence is hit, skip this field
                    continue
            elif restoration == True:
                # If restoration is True, we always want to run the deconvolution
                # even if convergence is hit, so we can restore the image.
                # Force niter = 0 regardless of input
                niter = 0
                pass

            if threshold is None:
                # User input threshold over all fields from initial iclean call
                this_threshold = self._threshold
            else:
                # Calculated threshold per field
                this_threshold = threshold[fidx]

            tmpiterpars = self._paramlist.iterpars
            tmpdecpars = self._paramlist.alldecpars[fieldid]

            thismask = tmpdecpars['mask'] if mask is None else mask

            deconv_ret = self._deconvolve(imagename=tmpdecpars['imagename'], startmodel=tmpdecpars['startmodel'],
                                          deconvolver=tmpdecpars['deconvolver'], scales=tmpdecpars['scales'], nterms=tmpdecpars['nterms'],
                                          smallscalebias=tmpdecpars['scalebias'], restoration=restoration, restoringbeam=tmpdecpars['restoringbeam'],
                                          niter = niter, gain=self._gain, threshold=this_threshold, nsigma=self._nsigma,
                                          interactive = False, fullsummary=True, fastnoise=tmpdecpars['fastnoise'], usemask=tmpdecpars['usemask'],
                                          mask = thismask, pbmask=tmpdecpars['pbmask'], sidelobethreshold=self._sidelobethreshold,
                                          noisethreshold=self._noisethreshold, lownoisethreshold=self._lownoisethreshold,
                                          negativethreshold=self._negativethreshold, smoothfactor=tmpdecpars['smoothfactor'],
                                          minbeamfrac=self._minbeamfrac, cutthreshold=tmpdecpars['cutthreshold'],
                                          growiterations=tmpdecpars['growiterations'], dogrowprune=self._dogrowprune,
                                          verbose=tmpdecpars['verbose'])


            # Since we are looping through field_id externally, deconvolve
            # always thinks it's imaging field 0 Fix this manually so when we
            # do the concat below we are not over-writing records
            if fidx > 0 and self._nfields > 1:
                deconv_ret['summaryminor'][fidx] = deconv_ret['summaryminor'].pop(0)
                initialize_merge_dict = True

            if self._nfields > 1 and counter == 0:
                deconv_ret_merge = copy.deepcopy(deconv_ret)

            # Only start merging after the first non-converged field has been processed
            if counter > 0:
                deconv_ret_merge = ImagingDict.concat_multifield_deconv(deconv_ret_merge, deconv_ret)

            counter += 1

        # Move the merged dict back to the original var name
        if self._nfields > 1 and initialize_merge_dict:
            deconv_ret = copy.deepcopy(deconv_ret_merge)
    ...
    def _validate(doc, schema):
        ...
        Args:
            doc: dict, input parameters
            schema: dict, schema for the input parameters
    ...
    def _initialize_convergence_result(self):
        ...
        self.CR.convergence_result = [None,None,None,None,None,{'field_name':{'chan': None, 'major': None},}]
        ...
        #for nn in range(self._nfields):
        #   self.CR.convergence_result.append([None, None, None, None, None, { 'chan': None, 'major': None }])
        self.CR.convergence_result.append(None, None, None, None, None)

        field_dict = {}
        for nn in range(self._nfields):
            field_dict[self._fieldnames[nn]] = {'chan':None, 'major':None}

        self.CR.convergence_result.append(field_dict)
    ...
    def __init__(self, vis='', selectdata=True, field='', spw='', timerange='', uvrange='', antenna='', scan='', observation='', intent='', datacolumn='corrected', imagename='', imsize=[ int(100) ], cell=[  ], phasecenter='', stokes='I', projection='SIN', startmodel='', specmode='mfs', reffreq='', nchan=int(-1), start='', width='', outframe='LSRK', veltype='radio', restfreq=[  ], interpolation='linear', perchanweightdensity=True, gridder='standard', facets=int(1), psfphasecenter='', wprojplanes=int(1), vptable='', mosweight=True, aterm=True, psterm=False, wbawp=True, conjbeams=False, cfcache='', usepointing=False, computepastep=float(360.0), rotatepastep=float(360.0), pointingoffsetsigdev=[  ], pblimit=float(0.2), normtype='flatnoise', deconvolver='hogbom', scales=[  ], nterms=int(2), smallscalebias=float(0.0), fusedthreshold=float(0.0), largestscale=int(-1), restoration=True, restoringbeam=[  ], pbcor=False, outlierfile='', weighting='natural', robust=float(0.5), noise='1.0Jy', npixels=int(0), uvtaper=[ '' ], niter=int(0), gain=float(0.1), threshold=float(0.0), nsigma=float(0.0), cycleniter=int(-1), cyclefactor=float(1.0), minpsffraction=float(0.05), maxpsffraction=float(0.8), nmajor=int(-1), usemask='user', mask='', pbmask=float(0.0), sidelobethreshold=float(3.0), noisethreshold=float(5.0), lownoisethreshold=float(1.5), negativethreshold=float(0.0), smoothfactor=float(1.0), minbeamfrac=float(0.3), cutthreshold=float(0.01), growiterations=int(75), dogrowprune=True, minpercentchange=float(-1.0), verbose=False, fastnoise=True, restart=True, savemodel='none', calcres=True, calcpsf=True, psfcutoff=float(0.35), parallel=False, history_filter=lambda index, arg, history_value: history_value  ):
        ...
        assert self._validate(doc, schema), create_error_string(_pc.errors)
        ...
        self._major_done = 0
        self.hasit = StopCodes(major=0,minor=0) # Convergence flag
        self._has_restored = False
        self._nfields = 1
        self._fieldnames = ['', ] # Default is a list of len 1

        self.stopdescription = '' # Convergence flag
        self._all_input_params = locals()
        del self._all_input_params['self']

        self._initial_mask_exists = []
        ...
        # Read in params and calculate nfields
        self._get_imager_parameters(locals())
        # Initialize convergence result for each field being imaged
        # this has been replaced by the ConvergenceResult class
        #self._initialize_convergence_result()

        self.hasit_per_field = [False for _ in range(self._nfields)]
        self.CR = ConvergenceResult(self._nfields, self._fieldnames)


    def _get_imager_parameters(self, inpparams):
        """
        Initialize ImagerParameters object and set the parameters
        """

        # Drop unnecessary params
        del inpparams['self']
        del inpparams['schema']
        del inpparams['history_filter']

        # deal with parameters that are not the same name
        inpparams, defparm = sanitize_tclean_inputs(inpparams)

        # Stolen from task_tclean.py
        bparm = {k: inpparams[k] if k in inpparams else defparm[k] for k in defparm.keys()}
        self._paramlist = ImagerParameters(**bparm)

        self._nfields = len(self._paramlist.allimpars.keys())
        self._fieldnames = [val['imagename'] for key, val in self._paramlist.allimpars.items()]

        return self._paramlist


    def __update_convergence(self, field=0):
        """
        ...
        Inputs:
            field: int, field index to update convergence for. Default is 0, which is the main field.
                   If there are multiple fields, this will be the index of the field to update.
        Returns:
            outrec: dict, a nested dictionary with the following structure:
            {
                channel_id: {
                    stokes_id: {
                        summary_key: [values, one per minor cycle]
                    }
                }
            }
        """
            ...
            for ss in range(self.global_imdict.nstokes):
                ...
                for key in keys:
                    ...
                    if key == 'iterDone':
                        ...
                        outrec[nn][ss]['iterations'] = np.cumsum(self.global_imdict.get_key(key, field=field, stokes=ss, chan=nn))
                        # Replace iterDone with iterations
                        #outrec[nn][ss]['iterations'] =  self.global_imdict.get_key(key, stokes=ss, chan=nn)
                    else:
                        outrec[nn][ss][key] = self.global_imdict.get_key(key, field=field, stokes=ss, chan=nn)

        return outrec



    def __add_per_major_items( self, tclean_ret, major_ret):
        ...
        for fidx, fname in enumerate(self._fieldnames):
            if 'cyclethreshold' in tclean_ret:
                if all([mj['major'] is None for mj in major_ret.values()]):
                    rdict[fname] = dict(major=dict(cyclethreshold=[tclean_ret['cyclethreshold']]), chan=self.__update_convergence(field=fidx))
                else:
                    rdict[fname] = dict(major=dict(cyclethreshold=major_ret[fname]['major']['cyclethreshold'] + [tclean_ret['cyclethreshold']]), chan=self.__update_convergence(field=fidx))
            else:
                # It really shouldn't get to this point...
                rdict = dict(major=dict([mj['major']['cyclethreshold'] for mj in major_ret.values()] + [tclean_ret['cyclethreshold']]), chan=self.__update_convergence(field=fidx))

        return rdict



    def _calc_deconv_controls(self, imdict, field = 0, niterleft=0, threshold=0, cycleniter=-1, hasit_per_field=None):
        """
        ...
        # Each field needs it's own cyclethreshold
        cyclethresh_per_field = []
        for fidx, fieldid in enumerate(self._paramlist.allimpars.keys()):
            if hasit_per_field is not None:
                if hasit_per_field[fidx]:
                    # If convergence is hit, skip this field
                    cyclethresh_per_field.append(None)
                    continue

            if fidx not in imdict.returndict['summaryminor']:
                # If the field is not in the summaryminor, then we cannot calculate the cyclethreshold
                # So set to the default for now
                # This can happen if the field has not been imaged yet.
                cyclethresh_per_field.append(self._threshold)
                continue

            cyclethreshold = psffrac * imdict.get_peakres(field=fidx)
            cyclethreshold = max(cyclethreshold, threshold)
            cyclethresh_per_field.append(cyclethreshold)

        return int(use_cycleniter), cyclethresh_per_field
    ...
    def _check_initial_mask(self):
        ...
        image_products = self.image_products()
        ...
        for product in image_products:
            if self._usemask == 'user' and self._mask == '':
                ...
                if os.path.exists(maskname):
                    self._initial_mask_exists.append(True)
                else:
                    self._initial_mask_exists.append(False)
    ...
    def _fix_initial_mask(self):
        ...
        if self._usemask == 'user' and self._mask == '':
            for idx, product in enumerate(image_products):
                maskname = product['maskname']

                # This means the mask was newly created by deconvolve, so flip it
                if os.path.exists(maskname) and self._initial_mask_exists[idx] is False:
                    ia.open(maskname)
                    ia.set(0.0)
                    ia.close()


    def _update_peakres_masksum_multifield(self):
        """
        Update peak residual value in the return dict for each image on disk -
        main field and outliers
        """
        ...
        for idx, product in enumerate(image_products):
            residname = product['residualname']
            maskname = product['maskname']
            ...
            statsdict['idx'] = idx
            statsdict['imagename'] = product['imagename']
            statsdict['peakres'] = peakres
            statsdict['masksum'] = masksum

            imstats.append(statsdict)

        #return peakres, masksum
        return imstats


    def image_products(self, imageidx = None):
        """
        Function that returns the image names for all fields.

        Optionally if imageidx is specified, return the image names
        only for that specific index.

        By default it returns a list of dicts, where each element in the list is a
        field. The dicts have the following keys:

        - imageidx: index of the field (0 for the main field, 1 for the first outlier etc.)
        - imagename: name of the image
        - maskname: name of the mask
        - residualname: name of the residual
        - imagepath: path to the image
        """

        # Use pysynthimager to get the names of the outlierfields
        # Populate paths and names, generate list
        # Use this function instead of self._imagename wherever possible.

        allimpars = self._paramlist.allimpars
        imlist = []

        for key, value in allimpars.items():
            ...
            fullpath = os.path.abspath(value['imagename'])
            imname = os.path.basename(fullpath)
            deconvolver = value['deconvolver']
            ...
            imdict['name'] = imname
            imdict['imagename'] = imname + f'.image{suffix}'
            imdict['imagepath'] = os.path.dirname(fullpath)

            imdict['maskname'] = imname + '.mask'
            imdict['residualname'] = imname + f'.residual{suffix}'
    ...
    def _generate_convergence_log(self, hasit_per_field, stopcode_min_per_field, stopdesc_per_field):
        """
        Generate a string that summarizes the convergence information for all fields, that
        can be passed in to the logger function.

        Inputs:
            hasit_per_field: list of bools, whether convergence was hit for each field
            stopcode_min_per_field: list of ints, the stop code for each field
            stopdesc_per_field: list of str, the stop description for each field

        Returns:
            logstr : str, a string that summarizes the convergence information for all fields
        """

        logstr = ''
        logstr += f"#################################### Convergence Log ###################################<br>"
        logstr += f"# {'Field Name':^20s} | {'Description':^46s} | Code (maj, min)<br>"
        logstr += "#----------------------------------------------------------------------------------------<br>"
        for fidx, fieldname in enumerate(self._fieldnames):
            if hasit_per_field[fidx]:
                logstr += f"# {fieldname:^20s} | {stopdesc_per_field[fidx]:^46s} | ({hasit_per_field[fidx]}, {stopcode_min_per_field[fidx]})<br>"
            else:
                logstr += f"# {fieldname:^20s} | {'Not converged':^46s} | ({hasit_per_field[fidx]}, {stopcode_min_per_field[fidx]})<br>"
        logstr += "########################################################################################<br>"
        logstr = logstr.replace(" ", "&nbsp;")  # Replace spaces with non-breaking spaces for HTML rendering
        return logstr
    ...
    def __next__( self ):
        ...
        tclean_ret = {}
        deconv_ret = {}
        stopcode_min_per_field = [0 for _ in range(self._nfields)]
        stopdesc_per_field = ['' for _ in range(self._nfields)]

        #                      vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv------------>>> is done to produce an initial dirty image
        if self._niter < 1 and self.CR.convergence_result[2] is not None:
            self.CR.convergence_result = ( f'nothing to run, niter == {self._niter}',
                                         self.CR.convergence_result[1],
                                         self._major_done,
                                         self._nmajor,
                                         self._niter,
                                         self.CR.convergence_result[5] )
            return self.CR.convergence_result
        else:
            ### CALL SEQUENCE:
            ...
            try:
                if self.CR.convergence_result[1] is None:
                    ...
                    # This loops over all fields
                    self._check_initial_mask()

                    # deconv args are parsed correctly inside this function
                    deconv_ret = self._deconv_over_fields(niter=0)
                    ...
                    # imstats is a list of dicts
                    imstats = self._update_peakres_masksum_multifield()

                    self.hasit, self.stopdescription, self.hasit_per_field, __, __ = self.global_imdict.check_convergence(self._niter, self._threshold, self._nmajor, imstats=imstats)

                    # Has not, i.e., not converged
                    if not self.hasit.major:
                        use_cycleniter, cyclethresh_per_field = self._calc_deconv_controls(self.current_imdict, niterleft=self._niter, threshold=self._threshold, cycleniter=self._cycleniter)

                        # deconv args are parsed correctly inside this function
                        # This will skip fields that have already converged
                        deconv_ret = self._deconv_over_fields(niter=use_cycleniter, mask='', threshold=cyclethresh_per_field, hasit_per_field=self.hasit_per_field)

                        # Joint major cycle over all fields
                        # Run the major cycle
                        tclean_ret = self._tclean( vis=self._vis, imagename=self._imagename, imsize=self._imsize, cell=self._cell,
                                           ...
                                           minpercentchange=self._minpercentchange, fastnoise=self._fastnoise, savemodel='none',
                                           maxpsffraction=self._maxpsffraction,
                                           minpsffraction=self._minpsffraction, parallel=self._parallel, fullsummary=True, outlierfile=self._outlierfile)
                        ...
                        if deconv_ret['stopcode'] == 7:   ## Tell the convergence checker that the mask is zero and iterations were skipped
                            self.hasit, self.stopdescription, self.hasit_per_field, stopcode_min_per_field, stopdesc_per_field = self.global_imdict.check_convergence(self._niter, self._threshold, self._nmajor, cycleniter=cycleniterleft)
                        else:
                            imstats = self._update_peakres_masksum_multifield()
                            self.hasit, self.stopdescription, self.hasit_per_field, stopcode_min_per_field, stopdesc_per_field = self.global_imdict.check_convergence(self._niter, self._threshold, self._nmajor, cycleniter=cycleniterleft, imstats=imstats)

                    # XXX : This is breaking, fix this to get the logging working correctly.
                    logstr = self._generate_convergence_log(self.hasit_per_field, stopcode_min_per_field, stopdesc_per_field)
                    self._exe_cmds.append(logstr)
                    self._exe_cmds_per_iter[-1] += 1
                    ...
                    if not self.hasit.major and self._usemask == 'auto-multithresh':
                        ...
                        # deconv args are parsed correctly inside this function
                        deconv_ret = self._deconv_over_fields(niter=0, mask='', hasit_per_field=self.hasit_per_field)
                ...
                if len(self.global_imdict.returndict) > 0 and 'summaryminor' in self.global_imdict.returndict and sum(map(len,self.global_imdict.returndict['summaryminor'].values())) > 0:
                    self.CR.convergence_result = ( self.global_imdict.returndict['stopDescription'] if 'stopDescription' in self.global_imdict.returndict else '',
                                                 ...
                                                 self.__add_per_major_items( self.global_imdict.returndict,
                                                                             #self.CR.convergence_result[5]['major'],
                                                                             self.CR.convergence_result[5]))
                                                                             #self.__update_convergence( ) ) )
                else:
                    self.CR.convergence_result = ( f'tclean returned an empty result',
                                                 self.CR.convergence_result[1],
                                                 self._major_done,
                                                 self._nmajor,
                                                 self._niter,
                ...
                self.CR.convergence_result = ( str(e),
                                             -1,
                                             self._major_done,
                                             self._nmajor,
                                             self._niter,
    ...
    def __decrement_counts( self ):
        ...
        if not any (self.hasit):  ##If not yet converged.
    ...
    def reset(self):
        ...
        self._finalized = False
        self.CR.convergence_result = ( None,
                                     self.CR.convergence_result[1],
                                     self._major_done,
                                     self._nmajor,
                                     self._niter,
    ...
    def restore(self):
        ...
        If savemodel is specified, first predict the model column with tclean,
        then run restore with deconvolve.
        """

        casalog.post('Restoring final image (iterating over all fields)', 'INFO')
        print("Restoring the final image (iterating over all fields)")

        if self._savemodel != 'none':
            # Run the major cycle to predict the model column
            tclean_ret = self._tclean( vis=self._vis, imagename=self._imagename, imsize=self._imsize, cell=self._cell,
                               phasecenter=self._phasecenter, stokes=self._stokes, specmode=self._specmode, reffreq=self._reffreq,
                               gridder=self._gridder, wprojplanes=self._wprojplanes, mosweight=self._mosweight, psterm=self._psterm,
                               wbawp=self._wbawp, conjbeams=self._conjbeams, usepointing=self._usepointing, interpolation=self._interpolation,
                               ...
                               maxpsffraction=self._maxpsffraction, outlierfile=self._outlierfile,
                               minpsffraction=self._minpsffraction, parallel=self._parallel, fullsummary=True )


        deconv_ret = self._deconv_over_fields(niter=0, mask='', restoration=True)

        self._has_restored = True

        # Write history into relevant images for all fields
        for fidx, field in enumerate(self._fieldnames):
            #params = cleanhelper.get_func_params(self.__init__, self._all_input_params)
            cleanhelper.write_tclean_history(self._paramlist.allimpars[f"{fidx}"]['imagename'], "InteractiveClean", tuple(self._all_input_params['doc'].items()), casalog)

        return self.global_imdict.returndict

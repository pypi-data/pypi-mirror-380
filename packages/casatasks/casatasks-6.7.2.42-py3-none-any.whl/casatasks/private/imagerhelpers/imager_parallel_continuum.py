...
class PyParallelContSynthesisImager(PySynthesisImager):
    ...
    def resetSaveModelParams(self, params=None):
        ...
        for n in self.allselpars:  # for all nodes
            for v in self.allselpars[n]:  # for all MSes
                ...
                self.allselpars[n][v]["usescratch"] = mainparams[v][
                    "usescratch"
                ]
    ...
    def initializeImagersBase(self, thisSelPars, partialSelPars):
        ...
        if partialSelPars == False:  ## Init only on the zero'th node
            ...
            for fld in range(0, self.NF):
                ...
                self.toolsi.defineimage(
                    impars=tmpimpars, gridpars=self.allgridpars[str(fld)]
                )
            ...
            for fld in range(0, self.NF):
                self.allimpars[str(fld)]["csys"] = self.coordsyspars[str(fld)][
                    "coordsys"
                ].copy()
            ...
            for node in nodes:
                ...
                for fld in range(0, self.NF):
                    if self.NN > 1:
                        nimpars[str(fld)]["imagename"] = (
                            self.PH.getpartimagename(
                                nimpars[str(fld)]["imagename"], node
                            )
                        )
                    ...
                    joblist.append(
                        self.PH.runcmd(
                            "toolsi.defineimage( impars="
                            + str(nimpars[str(fld)])
                            + ", gridpars="
                            + str(ngridpars[str(fld)])
                            + ")",
                            node,
                        )
                    )
    ...
    def initializeImagers(self):
        ...
        if self.allgridpars["0"]["gridder"].startswith("awpr"):
            ...
            cfcExists = True
        if (
            self.allgridpars["0"]["gridder"] == "awproject"
            or self.allgridpars["0"]["gridder"] == "awprojectft"
        ):
            if cfCacheName == "":
            ...
            cfcExists = os.path.exists(cfCacheName) and os.path.isdir(
                cfCacheName
            )
            if cfcExists:
                ...
                if nCFs == 0:
                    casalog.post(
                        cfCacheName
                        + " exists, but is empty.  Attempt is being made to fill it now.",
                        "WARN",
                    )
                    cfcExists = False
        # casalog.post("##########################################")
        ...
        if (
            (
                self.weightpars["type"].count("briggs")
                or self.weightpars["type"].count("uniform")
            )
            > 0
        ) and (self.weightpars["multifield"]):
        ...
        for immod in range(0, self.NF):
            ...
            if self.NN > 1:
                ...
                for node in self.listOfNodes:
                    partnames.append(
                        self.PH.getpartimagename(
                            self.allimpars[str(immod)]["imagename"], node
                        )
                    )
                    ...
                    self.PH.deletepartimages(
                        self.allimpars[str(immod)]["imagename"], node
                    )  # To ensure restarts work properly.
    ...
    def setWeighting(self):
        ...
        if (
            ...
            for fld in range(0, self.NF):
                self.toolsi.defineimage(
                    impars=self.allimpars[str(fld)],
                    gridpars=self.allgridpars[str(fld)],
                )
            self.toolsi.setweighting(**self.weightpars)
            ###master create the weight density for all fields
            ...
            for node in self.listOfNodes:
                joblist.append(
                    self.PH.runcmd(
                        "toolsi.setweightdensity('" + str(destWgtim) + "')",
                        node,
                    )
                )
            self.PH.checkJobs(joblist)
        ...
        #### end of multifield or mosweight
            ...
            for node in self.listOfNodes:
                ...
                joblist.append(
                    self.PH.runcmd(
                        "toolsi.setweighting( **" + str(self.weightpars) + ")",
                        node,
                    )
                )
            self.PH.checkJobs(joblist)
            ...
            if (
                self.NF == 1
            ):  # and self.allimpars['0']['stokes']=="I":   ## Remove after gridded wts appear for all fields correctly (i.e. new FTM).

                if not (
                    (self.weightpars["type"] == "natural")
                    or (self.weightpars["type"] == "radial")
                ):  ## For natural and radial, this array isn't created at all.
                    ## Remove when we switch to new FTM

                    casalog.post(
                        "Gathering/Merging/Scattering Weight Density for PSF generation",
                        "INFO",
                    )
                    ...
                    # for node in self.listOfNodes:
                    #    joblist.append(
                    #        self.PH.runcmd("toolsi.getweightdensity()", node)
                    #    )
                    # self.PH.checkJobs(joblist)
                    ...
                    resname = self.localnormpars["imagename"] + ".residual"
                    sumgridname = resname.replace(".residual", ".gridwt")
                    ################

                    if os.path.exists(sumgridname + "_temp") and (
                        os.path.exists(resname)
                        or os.path.exists(resname + ".tt0")
                    ):  # a restart
                    ...
                    #############################
                        ...
                        try:
                            joblist = []
                            if (
                                self.localnormpars["calcres"]
                                and self.localnormpars["calcpsf"]
                            ):
                                ##SIImageStore ..keeps comparing shape
                                ##when we try to make a gridwt
                                psfname = resname.replace(".residual", ".psf")
                                pbname = resname.replace(".residual", ".pb")
                                shutil.rmtree(resname, True)
                                shutil.rmtree(resname + ".tt0", True)
                                shutil.rmtree(psfname, True)
                                shutil.rmtree(psfname + ".tt0", True)
                                shutil.rmtree(pbname, True)
                                shutil.rmtree(pbname + ".tt0", True)

                            for node in self.listOfNodes:
                                joblist.append(
                                    self.PH.runcmd(
                                        "toolsi.getweightdensity()", node
                                    )
                                )
                            self.PH.checkJobs(joblist)
                            locpstool = synthesisnormalizer()
                            locpstool.setupnormalizer(
                                normpars=self.localnormpars
                            )
                        ...
                        # print("%%%%%%%%", sumgridname)
                            ...
                            if os.path.exists(resname) or os.path.exists(
                                resname + ".tt0"
                            ):
                                raise Exception(
                                    f"restarting Briggs Style weighting for parallel mfs imaging without the {sumgridname}_temp file on disk try restarting with calcres and calcpsf=True"
                                ) from inst
                            else:
                    ...
                    if not os.path.exists(
                        resname
                    ):  # not a restart so psf shape may be different if full pol...delete it
                ...
                if not (
                    ...
                    casalog.post(
                        "Parallel-Continuum-multifield  with briggs weighting will give different weighting schemes with number of processes used",
                        "WARN",
                    )
    ...
    def runMajorCycleCore(self, lastcycle):
        ...
        for node in self.listOfNodes:
            joblist.append(
                self.PH.runcmd(
                    "toolsi.executemajorcycle(controls={'lastcycle':"
                    + str(lastcycle)
                    + "})",
                    node,
                )

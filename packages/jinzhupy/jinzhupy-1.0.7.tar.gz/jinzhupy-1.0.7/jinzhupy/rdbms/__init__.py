...
class DatabaseManager:
    ...
    async def bulk_insert_orm(self, items, key=None):
        if not key:
            key = self.default_db_key
        conn = await self.get_connection(key)
        return await conn.bulk_insert_orm(items)

    async def bulk_insert_orm_with_session(self, session: AsyncSession, items, key=None):
        if not key:
            key = self.default_db_key
        conn = await self.get_connection(key)
        return await conn.bulk_insert_orm_with_session(session, items)

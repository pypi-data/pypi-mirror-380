...
class xngpt:
    ...
    def generate(prompt: str, links: list = None):
        """
        واجهة API لإنشاء أو تعديل الصور عبر gemini.php
        ------------------------------------------------
        prompt : str
            النص (الوصف أو التعليمات)
        links : list[str] | None
            إذا كان None → إنشاء صور جديدة
            إذا كانت قائمة روابط → تعديل الصور (حد أقصى 10 روابط)
        """
        try:
            api_url = "https://tofey.serv00.net/api/gemini.php"
            data = {"text": prompt}

            # إذا تعديل صور (links موجودة)
            if links:
                if isinstance(links, list):
                    links = ",".join(links[:10])  # يقبل 10 روابط فقط
                data["links"] = links

            # إرسال الطلب
            res = requests.post(api_url, data=data, timeout=120)

            # التحقق من الاستجابة
            if res.status_code == 200:
                try:
                    return res.json()  # إذا رجع JSON
                except Exception:

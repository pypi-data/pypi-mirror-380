...
def create_remote_config(domain, port):
    ...
    # Use echo + tee to create file remotely
    cmd = f"echo '{config_content}' | ssh {USER}@{HOST} \"sudo tee {remote_conf_path} > /dev/null\""
    ...
    cmd = f"ssh {USER}@{HOST} \"sudo ln -sf {remote_conf_path} {NGINX_SITES_ENABLED}/{domain}.conf\""

...
def create_remote_config(domain, port):
    """Generate config and send it to server"""
    ...
    # Step 1: Write the .conf file into sites-available
    cmd = f"ssh {USER}@{HOST} \"sudo tee {remote_conf_path} > /dev/null\" <<'EOF'\n{config_content}\nEOF"
    print(f"👉 Writing config to {remote_conf_path}")
    result = subprocess.run(cmd, shell=True, text=True)
    if result.returncode != 0:
        print("❌ Failed to write config file on server.")
        sys.exit(1)

    # Step 2: Create/refresh symlink in sites-enabled
    cmd = f"ssh {USER}@{HOST} \"sudo ln -sf {remote_conf_path} {remote_enabled_path}\""
    print(f"👉 Linking {remote_enabled_path} → {remote_conf_path}")
    result = subprocess.run(cmd, shell=True, text=True)
    if result.returncode != 0:

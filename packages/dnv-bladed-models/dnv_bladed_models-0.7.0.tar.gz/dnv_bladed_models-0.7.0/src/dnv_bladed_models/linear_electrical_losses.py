...
class LinearElectricalLosses(ElectricalLosses):
    ...
    """
    ElectricalLossesType: Literal['LinearElectricalLosses'] = Field(alias="ElectricalLossesType", default='LinearElectricalLosses', allow_mutation=False, const=True) # type: ignore
    PowerLossAtNoLoad: float = Field(alias="PowerLossAtNoLoad", default=None)
    Efficiency: float = Field(alias="Efficiency", default=None)

    _relative_schema_path = 'Components/Generator/ElectricalLosses/LinearElectricalLosses.json'
    _type_info = TypeInfo(
        set([]),
        set([]),
        set([]),

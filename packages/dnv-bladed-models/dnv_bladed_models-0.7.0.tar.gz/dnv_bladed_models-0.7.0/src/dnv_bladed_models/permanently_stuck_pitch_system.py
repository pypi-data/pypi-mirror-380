...
class PermanentlyStuckPitchSystem(Event):
    ...
    """
    EventType: Literal['PermanentlyStuckPitchSystem'] = Field(alias="EventType", default='PermanentlyStuckPitchSystem', allow_mutation=False, const=True) # Not supported yet # type: ignore
    StuckAtAngle: float = Field(alias="StuckAtAngle", default=None) # Not supported yet

    _relative_schema_path = 'TimeDomainSimulation/Event/PermanentlyStuckPitchSystem.json'
    _type_info = TypeInfo(
        set([]),
        set([]),
        set([]),
        'EventType').merge(Event._type_info)

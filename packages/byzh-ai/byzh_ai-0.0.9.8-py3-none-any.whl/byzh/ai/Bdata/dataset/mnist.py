...
class B_Download_KMNIST:
    ...
    def download_HF(self):
        """
        使用 Hugging Face datasets 下载 KMNIST
        :return: X_train, y_train, X_test, y_test
        """
        save_paths = [
            os.path.join(self.save_dir, f'{self.name}_X_train.pt'),
        ...
        # HF 数据集
        from datasets import load_dataset
        train_ds = load_dataset("rois-codh/kmnist", split="train")
        test_ds = load_dataset("rois-codh/kmnist", split="test")
...
class B_Download_EMNIST:
    ...
    def download_TV(self):
        ...
        # 标准化
        X_train, X_test = b_data_standard2d([X_train, X_test], template_data=X_train,
    ...
    def download_HF(self):
        """
        使用 Hugging Face datasets 下载 EMNIST
        注意：HF 上的 EMNIST 资源不是官方标准版，有些 split 可能缺少
        推荐 torchvision 版本
        """
        save_paths = [
            os.path.join(self.save_dir, f'{self.name}_X_train.pt'),
        ...
        # HF 数据集（部分 split 可能不可用）
        from datasets import load_dataset
        dataset_id = f"emnist/{self.split}"  # 一般写法

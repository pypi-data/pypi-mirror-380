...
class CompressedLocalFile : public File {
    ...
    std::string format;
    z_stream zlib_inner_stream;
    std::vector<uint8_t> zlib_outer_buffer;
    static constexpr size_t CHUNK_SIZE = 16384;

    void init_zlib() {
        memset(&zlib_inner_stream, 0, sizeof(zlib_inner_stream));
        if (mode == "r") {
            int window_bits;
            if (format == "gzip") {
            ...
            if (inflateInit2(&zlib_inner_stream, window_bits) != Z_OK) {
            ...
            if (deflateInit2(&zlib_inner_stream, Z_DEFAULT_COMPRESSION, Z_DEFLATED, window_bits, 8, Z_DEFAULT_STRATEGY) != Z_OK) {
                throw std::runtime_error("failed to initialize compression stream");
            }
        }
        zlib_outer_buffer.resize(CHUNK_SIZE);
    }

    void cleanup_zlib() {
        if (mode == "r") {
            inflateEnd(&zlib_inner_stream);
        } else {
            deflateEnd(&zlib_inner_stream);
        }
    }

    void seek(int64_t offset, std::ios::seekdir dir = std::ios::beg) {
        if (offset != 0 || dir != std::ios::beg) {
            throw std::runtime_error("seeking in compressed files is not supported (only seek(0) allowed)");
        }
        file_handle->seekg(offset, dir);
        if (file_handle->fail()) {

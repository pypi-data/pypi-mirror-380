...
class MCPServerBuilder:
    ...
    def build_executable(self, script_path: Path, onefile: bool = True, optimize: bool = False) -> bool:
        """构建可执行文件"""
        ...
        if optimize:
            print("   🚀 Using optimized build settings...")
        ...
        try:
            ...
            if optimize:
                ...
                if platform.system() != "Windows":
                    cmd.append("--strip")
                # 暂时禁用UPX压缩，因为可能导致兼容性问题
                # cmd.append("--upx-dir=/usr/bin")
                
                # 排除不必要的模块 - 保守策略，避免排除第三方库依赖的标准库
                exclude_modules = [
                    "tkinter", "turtle", "test", "doctest",
                    "pdb", "profile", "pstats", "cProfile", "trace",
                    "distutils", "setuptools", "pip", "wheel"
                    # 注意：不排除 email, xml, html, urllib, http 等，因为很多第三方库依赖它们
                ]
            ...
            if optimize:
                # 优化模式：只添加必要的隐藏导入
                essential_packages = {"aiohttp", "pydantic", "click", "colorama", "psutil"}
                for req in requirements:
                    pkg_name = req.split('==')[0].split('>=')[0].split('<=')[0].strip()
                    if pkg_name != "mcp-framework" and pkg_name in essential_packages:
                        cmd.extend(["--hidden-import", pkg_name])
                
                # 只添加核心 MCP Framework 模块
                core_mcp_imports = [
...
"""
    ...
    def build_all(self, clean: bool = True, test: bool = True, 
                  onefile: bool = True, include_source: bool = False, optimize: bool = False) -> bool:
        """构建所有服务器"""
        if clean:
            self.clean()
        ...
        for script_path in servers:
            ...
            # 构建可执行文件
            if not self.build_executable(script_path, onefile=onefile, optimize=optimize):
...
def main():
    ...
    parser.add_argument("--optimize", action="store_true", help="Enable optimized build for faster startup")

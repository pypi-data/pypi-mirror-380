...
from .db_process_pool import get_db_pooled_client, cleanup_db_process
...
class SimpleClient:
    ...
    async def _ensure_ready(self):
        ...
        if not self._is_ready:
            if self.use_process_pool:
                # 使用数据库进程池
                self._client, self._pool_key = await get_db_pooled_client(
    ...
    async def cleanup(self):
        """清理客户端资源"""
        if self.use_process_pool and self._pool_key:
            # 使用进程池时，释放客户端回池中
            from .db_process_pool import get_db_process_pool
            pool = get_db_process_pool()
            await pool.release_client(self._pool_key)
            self._pool_key = None
            self._client = None
        elif self._client and not self.use_process_pool:
            # 传统方式：直接关闭连接
            try:
...
async def quick_call(server_script: str, 
    ...
    Returns:
        Dict[str, Any]: 工具执行结果
    """
    async with SimpleClient(server_script, alias, config_dir, use_process_pool=use_process_pool) as client:
...
async def quick_get(server_script: str,
    ...
    Returns:
        Any: 配置值
    """
    async with SimpleClient(server_script, alias=alias, config_dir=config_dir, use_process_pool=use_process_pool) as client:
...
async def quick_set(server_script: str,
                   ...
                   alias: Optional[str] = None,
                   config_dir: Optional[str] = None,
                   use_process_pool: bool = True) -> bool:
    """
    快速设置配置值
...
async def quick_tools(server_script: str,
                     alias: Optional[str] = None,
                     use_process_pool: bool = True) -> List[str]:
    """
    快速获取工具列表
    ...
    Returns:
        List[str]: 工具名称列表
    """
    async with SimpleClient(server_script, alias=alias, use_process_pool=use_process_pool) as client:

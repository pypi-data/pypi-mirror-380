...
class GelAnnotationGUI(QWidget):
    ...
    def _save_png(self):
        ...
        defaultname = str(p.with_suffix("")) + "_annotated.png"

        print (f"defaultname: {defaultname}\t original path: {self.image_path}")
        fname, _ = QFileDialog.getSaveFileName(
            self, "Save annotated PNG", defaultname, "PNG (*.png)"
        )
        if not fname:
            return
        ...
        overlay_rgb, result = run_detection_pipeline(
            ...
            user_probable_wells=user_probables, target_height=115 * self.spin_num_rows.value(), user_reject_wells=user_reject,

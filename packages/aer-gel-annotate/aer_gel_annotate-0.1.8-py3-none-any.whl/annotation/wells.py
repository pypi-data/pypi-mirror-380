...
def finalize_wells(
    ...
    if User_REJECT:
        ...
        kept_final = []
        for pt in final_centroids:
            if is_reject(pt):
                reject_centroids.append(pt)
            else:
                kept_final.append(pt)
        final_centroids = kept_final

        kept_ladder = []
        for pt in ladder_centroids:
            if is_reject(pt):
                reject_centroids.append(pt)
            else:
                kept_ladder.append(pt)
        ladder_centroids = kept_ladder

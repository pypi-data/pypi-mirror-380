...
matplotlib.use("Agg")  # IMPORTANT: keep Matplotlib headless inside PyQt. (Also, this setting has to be before other Matplotlib imports).
...
def run_detection_pipeline(
    ...
    *,
    ...
    verbose=1,
    ladder=True,
        # below variables are tuned for our geldoc+imge, gel size combiniation.
    target_width=700,
    target_height=700,
    min_width=16,
    max_width=30,
    ...
    max_height=200,
    min_row_spacing=107,
    threshold_method="local", #this is the local gradient algorithm that is used to detect wells, i have tried 3-4 others and this one worked best for grayscale gels
...
def render_overlay(
    ...
    *,
    ...
    if manual_yes is None: manual_yes = []
    if manual_no is None: manual_no = []
    trace=True if os.environ["DEBUG"]=="True" else False
    h, w = img_corrected.shape  # height, width in pixels
    dpi = 80  # or whatever you prefer
    figsize = ((w / dpi), (h / dpi)+1) if pad else  ((w / dpi), (h / dpi))  # converting width, height in inches using dpi
    ...
    if ladder and ladder_wells:
        ...
        ladder_info = ladder_reader(
            ...
            smoothing_sigma=1, draw_trace_on_image=trace, use_smoothed=False
        )
    ...
    if detect_bars:
        ...
        band_calls = detect_pcr_bands(
            ...
            draw_trace_on_image=trace, use_smoothed=True,
            return_results=True
        )

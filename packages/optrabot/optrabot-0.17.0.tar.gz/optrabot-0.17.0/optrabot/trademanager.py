		...
		async with self._lock:
			...
			for current_leg in managed_trade.current_legs:
				if current_leg.action == OrderAction.BUY and current_leg.bidPrice <= 0:
					...
					continue
				closing_order_legs.append(current_leg)
			closing_price = OptionHelper.roundToTickSize(abs(managed_trade.current_price), 5)
			price_effect = PriceEffect.DEBIT if managed_trade.template.is_credit_trade() else PriceEffect.CREDIT
			closing_order = Order(symbol=managed_trade.trade.symbol, legs=closing_order_legs, action=OrderAction.SELL_TO_CLOSE, quantity=managed_trade.template.amount, type=OrderType.LIMIT, price=closing_price, price_effect=price_effect)

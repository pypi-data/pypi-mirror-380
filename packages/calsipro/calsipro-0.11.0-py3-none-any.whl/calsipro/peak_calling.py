...
def _call_peaks(time, signal, baseline=None, peak_detected=None, sigma=3):
    ...
    for i in range(1, len(time)-1):
        ...
        # looks like a peak
        if (np.abs(signal[i]-b[i-1]+d[i-1]) > 3*np.sqrt(1/bp[i-1])) or ((signal[i] > b[i-1]+d[i-1]) and ((np.abs(drift_estimate-d[i-1]) > 3*np.sqrt(1/dp[i-1])))):
            peak_detected[i] = True
            dp[i] = dp[i-1] * peak_p
            d[i] = d[i-1]
            bp[i] = bp[i-1] * peak_p
...
def call_peaks(time, signal, threshold=3):
    ...
    peaks = [peak for peak in peaks if np.max(normalised_signal[peak[0]:peak[1]]) > average_change]
    return peaks, b

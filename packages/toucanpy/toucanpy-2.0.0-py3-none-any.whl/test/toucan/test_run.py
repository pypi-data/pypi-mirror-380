...
class TestRunScript(unittest.TestCase):
    def setUp(self):
        ...
        self.patches = [
            ...
            patch.object(self.runner, "file_results", self.all_pass_results),
    ...
    @patch.object(ToucanRunner, '_get_file_results')
    def test_run_toucan_format(self, get_file_results, store_support, get_installed_version, file_from_emulator,
    ...
    def test_get_file_results(self, mock_store_results, mock_get_file):
        ...
        self.runner._get_file_results(self.dummy_format)
    ...
    @patch.object(ToucanRunner, '_get_file_results')
    @patch('toucan.runner.start_emulators')
    def test_run_toucan_format_no_newer_version(self, emulators, get_file_results, version_supported, store_support,
                                                update_app,
    ...
    def test_store_support(self, open_patch):
        self.runner._store_support(DummyFormat())
        open_patch.assert_called_once()

    @patch('toucan.runner.open')
    def test_store_support_no_support(self, open_patch):
        with patch.object(self.runner, "file_results", self.some_fail_results):
            self.runner._store_support(DummyFormat())

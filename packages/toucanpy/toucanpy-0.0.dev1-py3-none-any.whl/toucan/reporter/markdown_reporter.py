...
from toucan.utils import logger, PACKAGE_ROOT
from toucan.reporter.application_result import ApplicationResult, Status
from toucan.reporter.git_utils import git_checkout_and_pull, commit_push_daily
...
class MarkdownReporter(Reporter):
    ...
    """
    def __init__(self, report_root: Path, report_file_name: str = "README.md", git_pull_and_push: bool = False,
    ...
    def _check_all_applications_unique(results: list[ApplicationResult]):
        ...
        """
        if len(results) != len(set([r.app_name for r in results])):
    ...
    def _daily_summary(self, results: list[ApplicationResult]):
        ...
        replacements = {"CURRENT_DATE": self.current_date,
    ...
    def _load_previous_section(today_table_entry: DataFrame, previous_section_path: Path) -> DataFrame:
        ...
        if os.path.exists(previous_section_path):
            ...
            logger.warning(f"No previous section could be found at {previous_section_path}. Creating an empty DataFrame.")
    ...
    def _application_report(self, result: ApplicationResult):
        ...
        if not result.has_new_version():
            ...
            recognition = '✅' if result.all_files_recognized() else '❌' if result.all_files_recognized() is False else '❔'
            support = '✅' if result.all_parsing_rules_pass() else '❌' if result.all_parsing_rules_pass() is False else '❔'

        previous_section = self._load_previous_section(self._get_app_summary_table(result), app_previous_section_path)
        aggregated_previous_section = self.aggregate_by_date(previous_section)
        # floatfmt is required to preserve the version numbers correctly. Tabulate rounds off floats, even if they are strings
        previous_section_md = aggregated_previous_section.to_markdown(floatfmt=".1f")
        section_today = aggregated_previous_section.head(1).to_markdown(floatfmt=".1f")
    ...
    def _recognition_entry(result: ApplicationResult) -> str:
        ...
        if not result.has_results():
            ...
            return (''.join('❌ ' + toucan_file.file_path.name + '<br/>' for toucan_file in result.all_unrecognized_files()) +

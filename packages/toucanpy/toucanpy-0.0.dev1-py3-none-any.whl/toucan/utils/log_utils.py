...
def load_default_log_config(log_level: int = logging.INFO,
                            log_dir: Path = RESULTS_ROOT / "log",
                            ignore_already_set: bool = False):
    global _LOGGING_ALREADY_CONFIGURED
    if _LOGGING_ALREADY_CONFIGURED and not ignore_already_set:
        raise Exception(f'Logging configuration cannot be called twice.')
    ...
    if _LOGGING_ALREADY_CONFIGURED and ignore_already_set:
        logging.getLogger(__name__).warning('Logging config was loaded while another config was loaded before. '

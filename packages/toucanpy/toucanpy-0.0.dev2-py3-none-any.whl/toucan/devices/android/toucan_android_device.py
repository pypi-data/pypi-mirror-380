...
class ToucanAndroidDevice(BaseEmulator):
    ...
    def __init__(self, avd_name: str, max_wait_time: int, interactive: bool = False) -> None:
        ...
        self.emulator: AndroidEmulator = AndroidEmulator(
            interactive=interactive,
            avd_device_name=self.avd_device_name
        )
        self.adb_device: Optional[AdbDevice] = None
        self.is_started = False
        self.max_wait_time = max_wait_time

    def _check_if_device_is_already_started(self) -> Optional[AdbDevice]:
        ...
        for adb_name in AdbDevice.list_devices():
            ...
            if emu_avd_name_result.success and emu_avd_name_result.stdout:
                result = AvdDevice(emu_avd_name_result.stdout.splitlines()[0], adb_name,
                                   AdbDevice.get_device_status(adb_name))
    ...
    def _wait_until_avd_is_available_on_adb(self) -> Optional[AdbDevice]:
        total_wait_time = 0
    ...
    def start_emulator(self, snapshot: Optional[str] = None, log=True) -> AdbDevice:
        """
        ...
        if self._check_if_device_is_already_started():
            logger.info(f'{self.avd_device_name} was already booted, shutting down because we need to control the '
                        f'process')
        ...
        if not self.is_started:
            raise Exception(
                f'Device {self.avd_device_name} is available over adb but cannot fully boot. Adb name: {self.adb_device.device}')
    ...
    def snapshots_list(self, snapshot_prefix: Optional[str]) -> List[str]:
        """

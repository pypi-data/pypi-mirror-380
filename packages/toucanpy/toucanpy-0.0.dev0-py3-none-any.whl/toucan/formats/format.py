...
class ToucanFile:
    ...
    Every ToucanFile defines three aspects:
        ...
        * The expected file type: this is the file type we expect the recognition code to return, eg a file command-based
          implementation would return "SQLite 3.x database" for the Whatsapp contacts DB
    ...
    def pull(self, adb_device: AdbDevice, destination_folder: str) -> Path:
        ...
        if not pulled_file.success:  # TODO TOUCAN-167: in case any pull fails, we should stop analysis of this ToucanFile and continue with the next one.
    ...
    def __eq__(self, other):
        ...
        return self.file_path == other.file_path and self.expected_type == other.expected_type and self.parsing_rules == other.parsing_rules
...
class ToucanSqliteFile(ToucanFile):
    def pull(self, adb_device: AdbDevice, destination_folder: str) -> Path:
        ...
        for pulled_file in pulled_files:
            if pulled_file.success:  # TODO TOUCAN-167: in case any pull fails, we should stop analysis of this ToucanFile and continue with the next one.
        ...
        logger.info(f'Vacuuming pulled files. main file: {filename}, all files: {[basename(f.path) for f in pulled_files]}')
    ...
    def _vacuum(filename: str, destination_folder: str):
        ...
        try:
            result = subprocess.run(['sqlite3', absolute_path, 'VACUUM;'])
            result.check_returncode()
        except Exception as e:
            logger.exception(e)
            logger.warning(f'Vacuuming on command line failed, trying again through Python sqlite3')
...
class ToucanFileBuilder:
    ...
    def build(self) -> ToucanFile:
        ...
        if self.is_sqlite_file:
            return ToucanSqliteFile(file_path=self.file_path,
                                    expected_type=self.expected_type,
                                    parsing_rules=MappingProxyType(self.rules))
        else:
            return ToucanFile(file_path=self.file_path,
                              expected_type=self.expected_type,
                              parsing_rules=MappingProxyType(self.rules))

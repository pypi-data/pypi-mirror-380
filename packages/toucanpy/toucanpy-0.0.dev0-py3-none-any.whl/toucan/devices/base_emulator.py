...
class BaseEmulator(BaseDevice, ABC):
    ...
    @abstractmethod
    def start_emulator(self, custom_snapshot: Optional[str] = None, log=True):
        """
        Start the emulator.
    ...
    def reboot_emulator(self, snapshot: Optional[str] = None) -> bool:
        """
        Reboot the device.

        Note that this is *not* an abstract method: the called methods `start_emulator` and `shutdown_emulator` always have an
        implementation, because they are abstract methods.
    ...
    @abstractmethod
    def snapshots_list(self, filename_glob_pattern: Optional[str]) -> List[str]:
        """
    ...
    def snapshot_revert_to_latest_successful(self):
        ...
        if revert.success:
            ...
            logger.error(f"Failed to revert to the latest snapshot (has Toucan already run successfully at least once?")

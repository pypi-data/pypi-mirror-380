...
def get_devices() -> [str]:
    ...
    devices = subprocess.getoutput(f'{EMULATOR} -list-avds').splitlines()
    logger.info(f'{len(devices)} devices found: {devices}')
    return devices


def pick_first_available_device() -> Optional[str]:
    """
...
def _check_device_name(device_name: str) -> Optional[str]:
    """
...
class AndroidEmulator:
    ...
    def emulator_command(self, command: str, log: bool) -> subprocess.Popen:
        """
            This function executes emulator commands using subprocess.Popen and returns a subprocess object. This is valuable for accessing the process ID (PID), logs, and error information.
        ...
        with open(_get_log_file_path(), "w") as log_file:
            android_process = subprocess.Popen(
                [command],
                stdout=log_file if log else None, stderr=log_file if log else None,
    ...
    def reboot_emulator(self, snapshot: Optional[str] = None) -> subprocess.Popen:
        """
        ...
        logger.info(f"Wait a few seconds before restarting...")
        sleep(2)

...
from toucan.devices.emulator_service import shut_down_emulator, start_emulators, get_file_from_emulator, update_app
from toucan.config import ToucanConfig
from toucan.formats.format import ToucanFormat
from toucan.utils import versions, RESULTS_ROOT
...
class ToucanRunner:
    def __init__(self, formats: list[ToucanFormat], reporter: Reporter, config: ToucanConfig):
        ...
        self.alice: Optional[ToucanAndroidDevice] = None
        self.bob: Optional[ToucanAndroidDevice] = None
    ...
    def run(self, interactive: bool = False, debug: bool = False):
        ...
        :param interactive: if True, the emulator will open a window showing the phone display. Otherwise, the emulator will run headless.
        ...
        try:
            ...
            # Publish results
            if not debug:
                logger.info(f'Publishing all results to the Toucan daily')
                self.reporter.report_results(results)
            else:
                logger.info(f'Debug run of Toucan completed, not pushing application results to daily summary')
    ...
    def run_toucan_format(self,
                          toucan_format: ToucanFormat,
                          debug: bool = False
                          ) -> ApplicationResult:
        """
        ...
        try:
            ...
            self._get_file_results(toucan_format)
            self._store_support(toucan_format)
            return ApplicationResult(toucan_format.app_name, self.latest_version, self.file_results)
        except Exception as e:
            logger.error(f"Something went wrong while running Toucan for application {toucan_format.app_name}: {e}",
                         exc_info=True)
            if not debug and self.toucan_snapshot_name:
    ...
    def _store_support(self, toucan_format: ToucanFormat):
        """
        Store a file if the application is still supported.

        :param toucan_format: The application that was checked
        """
        if any(not recog_result or not all(parse_result.values()) for recog_result, parse_result in self.file_results.values()):
            return

        # If all checks returned True, create empty file to signal that we still have support
        with open(self.result_folder / f'{toucan_format.app_name}_supported.txt', 'w') as file:
            pass
    ...
    def _handle_no_new_version(self, toucan_format):
        ...
        if not versions.is_newer_version(self.latest_version, self.current_version):
            ...
            if not self.latest_version == self.current_version:
                logger.info(
                    f'Latest version {self.latest_version} is lower than the version of previous run, skipping {toucan_format.app_name}.')
            ...
            if self._version_supported(toucan_format.app_name):
                logger.info(
                    f'Latest version {self.latest_version} was supported in previous run, skipping {toucan_format.app_name}.')
    ...
    def _get_file_results(self, toucan_format: ToucanFormat):
        for toucan_file in toucan_format.toucan_files:
            logger.info(f'Pulling file {toucan_file.file_path}...')
            destination_file = get_file_from_emulator(self.alice, toucan_file, str(self.result_folder), toucan_format.app_name,
                                                      self.latest_version)
            ...
            logger.info(f'Storing recognition and parsing results')
            ...
            self.file_results[toucan_file] = (toucan_file.verify_type(recognition_result),

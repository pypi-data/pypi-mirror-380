...
def git_checkout_and_pull(repo_root, branch):
    _execute_shell_command(
        f"{repo_root}/utils/shell_scripts/git_checkout.sh {repo_root} {branch}")


def commit_push_daily(project_root, report_name):
    _execute_shell_command(f"{project_root}/utils/shell_scripts/git_add_commit_push_daily_summary.sh {project_root} {report_name}")
...
def _execute_shell_command(command):
    ...
    if output.returncode != 0:
        logger.error(f"Command `{command}` exited with return code {output.returncode} and the following output:\n"

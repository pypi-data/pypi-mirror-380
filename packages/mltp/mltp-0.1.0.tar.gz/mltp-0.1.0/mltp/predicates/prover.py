...
from mltp.logic_utils import fresh_variable_name_generator, is_z_and_number
from .syntax import ForbiddenVariableError, Term, Formula
from .proofs import *
...
class Prover:
    ...
    def _add_line(self, line: Proof.Line) -> int:
        ...
        assert line.is_valid(self._assumptions, self._lines, line_number), print(self._assumptions, self._lines, line_number)
        if self._print_as_proof_forms:
    ...
    def add_existential_derivation(self, consequent: Union[Formula, str],
        ...
        assert quantified.variable not in consequent.free_variables(), print(f'\nThe variable quantified in line1, {quantified.variable}, is in the consequent {consequent}\n')
        assert line_number2 < len(self._lines)
        conditional = self._lines[line_number2].formula
        assert conditional == Formula('->', quantified.statement, consequent), print(f'\nConditional is {conditional},\nshould be      {Formula("->", quantified.statement, consequent)}\n')
    ...
    def add_free_instantiation(self, instantiation: Union[Formula, str],
        ...
        if nested:
            for variable in original.variables(): #I moved this check to only complex cases where next(fresh_variable_name_generator) is actually used
                assert not is_z_and_number(variable), print('instantiation contains a z-and-number variable and is complex. I may not be able to freely instantiate without conflicts.')
            temp_variables = {}
            new_variables = {}

...
from mltp.logic_utils import frozen, memoized_parameterless_method
...
#             self.root, self.first, self.second = root, first, second
    ...
    def substitute_variables(self, substitution_map: Mapping[str, Formula]) -> Formula:
        ...
        for variable in substitution_map:
            if isinstance(substitution_map[variable], str):
                substitution_map[variable] = Formula.parse(substitution_map[variable])
...
def negate_formula(formula: Formula) -> Formula:
    if is_variable(formula.root) or is_binary(formula.root):
        return Formula('~', formula)
    elif is_constant(formula.root):
        if formula == Formula.parse('T'):
            return Formula.parse('F')
        else:
            return Formula.parse('T')
    else:

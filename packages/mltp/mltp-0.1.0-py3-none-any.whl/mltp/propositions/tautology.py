...
def prove_tautology_full(tautology: Formula, model: Model = frozendict()) -> Proof:
    """Proves the given tautology from the formulas that capture the given
    model.
    ...
    """
    assert is_tautology(tautology)
    assert tautology.operators().issubset({'T', 'F', '->', '~', '&', '|'})
    assert is_model(model)
    assert sorted(tautology.variables())[:len(model)] == sorted(model.keys())

    if sorted(tautology.variables()) == sorted(model.keys()):
        return prove_in_model_full(tautology, model)
    else:
        model = dict(model)
        model_copy = {i: model[i] for i in model.keys()}
        next_variable = sorted(tautology.variables())[len(model)]
        ...
        return reduce_assumption(prove_tautology_full(tautology, model),
                                 prove_tautology_full(tautology, model_copy))
    # Task 6.3a

def proof_or_counterexample_full(formula: Formula) -> Union[Proof, Model]:
    """Either proves the given formula or finds a model in which it does not
    hold.
    ...
    assert formula.operators().issubset({'T', 'F', '->', '~', '&', '|'})
    if is_tautology(formula):
        return prove_tautology_full(formula)
    else:
        for i in all_models(formula.variables()):
            if not evaluate(formula, i):
                return i
...
def prove_sound_inference_full(rule: InferenceRule) -> Proof:
    """Proves the given sound inference rule.
    ...
    rules = AXIOMATIC_SYSTEM
    lines = list(prove_tautology_full(encoding).lines)
...
def model_or_inconsistency_full(formulae: Sequence[Formula]) -> Union[Model, Proof]:
    """Either finds a model in which all the given formulas hold, or proves
    ``'~(p->p)'`` from these formulas.
    ...
    if models:
        ...
        return prove_sound_inference_full(InferenceRule(formulae, Formula.parse('~(p->p)')))
    # Task 6.5
...
# formula = to_implies_not(Formula.parse('(((((((x11|x12)|x13)&(~x11|~x12))&(~x11|~x13))&(~x12|~x13))&(((((x21|x22)|x23)&(~x21|~x22))&(~x21|~x23))&(~x22|~x23)))&(((((x31|x32)|x33)&(~x31|~x32))&(~x31|~x33))&(~x32|~x33)))'))
# allmodels = all_models(formula.variables())
# for i in range(10):
#     proof1 = prove_in_model(formula, next(allmodels))
#     print(proof1.statement, '\n')

#--------------

# formula = to_implies_not(Formula.parse('((((((((((((((((((((((((((((((((((((((((((((((((x11|x12)|x13)&(~x11|~x12))&(~x11|~x13))&(~x12|~x13))&(((((x21|x22)|x23)&(~x21|~x22))&(~x21|~x23))&(~x22|~x23)))&(((((x31|x32)|x33)&(~x31|~x32))&(~x31|~x33))&(~x32|~x33)))&(((((x41|x42)|x43)&(~x41|~x42))&(~x41|~x43))&(~x42|~x43)))&(((((x51|x52)|x53)&(~x51|~x52))&(~x51|~x53))&(~x52|~x53)))&(((((x61|x62)|x63)&(~x61|~x62))&(~x61|~x63))&(~x62|~x63)))&(((((x71|x72)|x73)&(~x71|~x72))&(~x71|~x73))&(~x72|~x73)))&(((((x81|x82)|x83)&(~x81|~x82))&(~x81|~x83))&(~x82|~x83)))&(~x21|~x41))&(~x22|~x42))&(~x23|~x43))&(~x51|~x81))&(~x52|~x82))&(~x53|~x83))&(~x41|~x61))&(~x42|~x62))&(~x43|~x63))&(~x11|~x41))&(~x12|~x42))&(~x13|~x43))&(~x51|~x71))&(~x52|~x72))&(~x53|~x73))&(~x21|~x31))&(~x22|~x32))&(~x23|~x33))&(~x11|~x71))&(~x12|~x72))&(~x13|~x73))&(~x41|~x51))&(~x42|~x52))&(~x43|~x53))&(~x71|~x81))&(~x72|~x82))&(~x73|~x83))&(~x11|~x61))&(~x12|~x62))&(~x13|~x63))&(~x41|~x71))&(~x42|~x72))&(~x43|~x73))&(~x31|~x51))&(~x32|~x52))&(~x33|~x53))'))
# print(formula, '\n\n\n')
...
# proof1 = prove_in_model(formula, assignment)
# print('\n', len(proof1.lines), len((proof1.clean()).lines))

...
from typing import AbstractSet, FrozenSet, Generic, Mapping, Tuple, TypeVar, Set, Mapping
from datetime import datetime
from mltp.logic_utils import frozen, frozendict
from .syntax import ForbiddenVariableError, Term, Formula, is_constant, is_variable, is_function, is_equality, is_relation, is_unary, is_binary, is_quantifier
from mltp.propositions.semantics import print_truth_table as print_propositional_truth_table
...
class Model(Generic[T]):
    ...
    def __init__(self, universe: AbstractSet[T],
            ...
            if len(relation_interpretation) == 0:
                ...
                for arguments in relation_interpretation:
                    ...
                    for argument in arguments:
                        assert argument in universe, print(f'argument {argument} not in universe, type: {type(argument)}')
        ...
        for function in function_interpretations:
            ...
            assert arity > 0
            assert len(function_interpretation) == len(universe)**arity, print(f'\nFunction interpretations are: {function_interpretation}, \n universe is {universe}\n')
...
# minimum = 0
# maximum = 3
# universe = set(str(i) for i in range(minimum,maximum+1)) | {'undefined'}
# relation_interpretations = {'L':set(),'GorE':set()}
# function_interpretations = {'plus':dict(), 'minus':dict(), 'times':dict(), 'fdividedby': dict()}
# for i in universe:
#     for j in universe:
#         if i == 'undefined' or j == 'undefined':
#             function_interpretations['plus'][(i,j)] = 'undefined'
#             function_interpretations['minus'][(i,j)] = 'undefined'
#             function_interpretations['times'][(i,j)] = 'undefined'
#             function_interpretations['fdividedby'][(i,j)] = 'undefined'
#             continue
#         if int(i)+int(j) <= maximum:
#             function_interpretations['plus'][(i,j)] = str(int(i)+int(j))
#         else:
#             function_interpretations['plus'][(i,j)] = 'undefined'

#         if int(i)-int(j) >= minimum:
#             function_interpretations['minus'][(i,j)] = str(int(i)-int(j))
#         else:
#             function_interpretations['minus'][(i,j)] = 'undefined'

#         if int(i)*int(j) <= maximum:
#             function_interpretations['times'][(i,j)] = str(int(i)*int(j))
#         else:
#             function_interpretations['times'][(i,j)] = 'undefined'

#         if j != '0' and int(i) % int(j) == 0:
#             function_interpretations['fdividedby'][(i,j)] = str(int(int(i)/int(j)))
#         else:
#             function_interpretations['fdividedby'][(i,j)] = 'undefined'

#         if i >= j:
#             relation_interpretations['GorE'].add((i,j))
#         else:
#             relation_interpretations['L'].add((i,j))

# model = Model(
#                 universe,                                                                       #universe
#                 {(i if is_constant(i) else make_constant(i,universe)):i for i in universe},     #constant interpretations
#                 relation_interpretations,                                                       #relation interpretations
#                 function_interpretations                                                        #function interpretations
#                 )

# for i in range(1,6):
#     formula = make_formula_of_size(i)

#     startTime = datetime.now()
#     for j in range(10):
#         model.evaluate_formula(formula)
...
#     print(f'at length {i}, original is {time1} and numpy fromiter is {time2}')

...
from mltp.propositions.semantics import is_tautology as is_propositional_tautology
from mltp.propositions.syntax import is_variable as is_propositional_variable
from .syntax import ForbiddenVariableError, Term, Formula, is_constant, is_variable, is_function, is_relation, is_equality, is_binary, is_unary, is_quantifier
...
class Schema:
    ...
    def __init__(self, formula: Formula,
        ...
        """
        if isinstance(formula, Schema):
            self.formula = formula.formula
            self.templates = formula.templates
        else:
...
class Proof:
        ...
        def is_valid(self, assumptions: AbstractSet[Schema],
            ...
            if formula_is_valid and line_number > self.antecedent_line_number and line_number > self.conditional_line_number:
                ...
                if not formula_is_valid:
                    print(f'Invalid formula {self.formula}.\nAntecedent is   {lines[self.antecedent_line_number].formula},\nconditional is  {lines[self.conditional_line_number].formula}\n')
                if not line_number > self.antecedent_line_number:
                    print(f'\nline is after antecedent: {line_number > self.antecedent_line_number}')
                if not line_number > self.conditional_line_number:
    ...
    def clean(self, report=False) -> Proof:
        ...
        if report:
            print('Removed ', len(self.lines)-len(proof.lines), ' lines')
        return proof
        # personal task

from mltp.propositions.proofs import Proof as PropositionalProof, \
                                InferenceRule as PropositionalInferenceRule, \
                                SpecializationMap as \
                                PropositionalSpecializationMap
from mltp.propositions.axiomatic_systems import AXIOMATIC_SYSTEM as \
                                           PROPOSITIONAL_AXIOMATIC_SYSTEM, \
                                           MP, I0, I1, D, I2, N, NI, NN, R
from mltp.propositions.tautology import prove_tautology as \
...
# Benardete's Paradox:
# reflexive = Formula.parse('Ax[LE(x,x)]')
# transitive = Formula.parse('Ax[Ay[Az[((LE(x,y)&LE(y,z))->LE(x,z))]]]')
# asymetric = Formula.parse('Ax[Ay[((LE(x,y)&LE(y,x))->x=y)]]')
# strongly_connected = Formula.parse('Ax[Ay[(LE(x,y)|LE(y,x))]]')
# beginningless = Formula.parse('Ax[Ey[LE(y,x)]]')
# anbc = Formula.parse('Ax[Ay[(P(x)<->(LE(y,x)->~P(y)))]]')

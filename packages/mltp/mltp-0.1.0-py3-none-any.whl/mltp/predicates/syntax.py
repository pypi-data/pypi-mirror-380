...
from mltp.logic_utils import fresh_variable_name_generator, frozen, memoized_parameterless_method
from mltp.propositions.syntax import Formula as PropositionalFormula, is_variable as is_propositional_variable
# from predicates.operators import *
...
class Formula:
    ...
    def parse(string: str) -> Formula:
        ...
        assert string == str(formula), print(f"Ensure that string {string} is a well-formed formula. It doesn't match {str(formula)}")
        return formula
        # Task 7.4b
    ...
    def operators(self) -> Set[Tuple[str, int]]:
        """Finds all relation names in the current formula, along with their
        arities.
        ...
        root = self.root
        if is_quantifier(root):
            return Formula.operators(self.statement)

        elif is_unary(root):
            return {root} | Formula.operators(self.first)

        elif is_binary(root):
            return {root} | Formula.operators(self.first) | Formula.operators(self.second)

...
class CoarseGrainer(object):
    ...
    def _coarse_grain_molecule(self, molecule: Molecule, residue_name: str) -> Molecule:
        ...
        **NOTE** This is a core method which performs coarse-graining molecule-wise.
        ...
        for i, bead in enumerate(beads):
            ...
            # print(f"\nProcessing bead {bead_name} # {i+1} :")
            # print("----------------------------------------")
            # print(" Atom Index  Name | Type | Elem | Mass | Weight | Position")
            # print("Atom contributions:")
            # print("Atom Name | Weight | Position | Mass | Weighted Position")
            ...
            for atom in molecule:
                if atom.name in bead_atoms:
                    ...
                    # if atom.isMassElem:
                    #     batom.setMass(atom.getMass()*weight)
                    #     # print(f"\nSUCCESS: set element mass for atom '{batom.name}' ...")
                    # else:
                    #     batom.setMassElem()
                    #     batom.setMass(batom.getMass() * weight)
                    #     if not batom.isMassElem:
                    #         print(f"WARNING: failed to set element mass for atom '{batom.name}' ...")
                    #
            ...
            if canSetElemCSL and cg_atom.setElems(bead["atoms"]):
                ...
                #     print(f"\ncoarse_grain_molecule(): Setting ElemCSL {bead_name} :\n"
                #           f"{cg_atom.getElemCSL()} : \n "
                #           f"{bead['atoms']} \n "
                #           f"{bead_csl} -> {sum(bead_csl)}\n "
            ...
            # print(f"\nAdded CG {cg_atom}")
            # print(f"\nAdded CG {cg_atom} with calculated mass {bead_mass}")
    ...
    def water_cg_molset(
        ...
        logger.info(
            f"Neighbours' stats for wmolset:"
            f"\n min = {minnbs} ({nminnb}), "
            f"max = {maxnbs} ({nmaxnb}), "
    ...
    #                 cg_molset.addItem(cg_mol)
    #                 n_done += 1
    #                 print(f"\rProcessed {residue_name} molecules: {n_done}/{n_sets}\n") #, end="")
    #

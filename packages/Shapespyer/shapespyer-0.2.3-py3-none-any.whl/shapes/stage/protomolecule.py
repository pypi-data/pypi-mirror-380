...
class Molecule(AtomSet):
    ...
    def getBoneRvecIntToExt(self, be_verbose=False):
        ...
        if len(self.items) > 0:
            ...
            logger.debug(
                f"\n({self.bone_int} -> {self.bone_ext}): Rvec[{self.indx}] "
                f"= {rvec}"
            )
        else:
            logger.debug(
                f"\n({self.bone_int} -> {self.bone_ext}): no items set yet"
                " - skipping ..."
            )
        return rvec
    ...
    def getBoneRvecExtToInt(self, be_verbose=False):
        ...
        if len(self.items) > 0:
            ...
            logger.debug(
                f"\n({self.bone_ext} <- {self.bone_int}): "
                f"Rvec[{self.indx}] = {rvec}"
            )
        else:
            logger.debug(
                f"\n({self.bone_ext} <- {self.bone_int}): no items set "
                "yet - skipping ..."
            )
        return rvec
    ...
    def alignBoneToVec(
        ...
        # avec must be (cosa * cost, sina * cost, sint), i.e. it is supposed to be normalised!
        mint = self.bone_int
        # mext = self.bone_ext
        ...
        for i in range(matms):
            ...
            if be_verbose:
                ...
                if diff * diff > TINY:
                    logger.warning(
                        f"\nVector diff upon rotation ({i}) = {diff}\n"
                    )
        ...
        if is_flatxz:
            ...
            for i in range(matms):
                ...
                if be_verbose:
                    ...
                    if diff * diff > TINY:
                        logger.warning(
                            f"\nVector diff upon rotation about Z: ({i}) "
                            f"= {diff}\n"
                        )
                self.items[i].setRvec(list(vec2))
        # place the first molecule in correct orientation
        vorg = self.items[mint].getRvec()
        vec0 = self.getBoneRvecIntToExt()
    ...
    def guessBondsFromDistances(self, crdScale: float = 1.0):
        ...
        # logger.info(f"\n{class_method}: "
        #      f"listAtomDist ({len(listAtomDist)}) = {distMax} ", *listAtomDist, sep='\n')
        # logger.info(f"\n{class_method}: "
        #      f"dictValences ({len(dictValences)}) =\n", array(dictValences))
        # logger.info(f"\n{class_method}: "
        #      f"atomGroup ({len(atomGroup)}) = ", *atomGroup, sep='\n')
        # sys.exit(0)
        ...
        # logger.info(f"\n{class_method}: "
        #       f"atomGroups ({len(atomGroup)}) = ", *atomGroup, sep='\n')
        ...
        logger.info(f"\n\nFound bonds ({len(molBonds)}) = ")
        logger.info("\n".join(map(str, molBonds)))
    ...
    def guessBondsFromAtomNames(self, crdScale: float = 1.0):
                ...
                if atomElem in eb[0]:
                    ...
                    if atomElem != "H":
                        ...
                        #    molGroups[-1].append((iatm, atomElem))
                            ...
                            if len(molGroups) > 0:
                                # logger.info(f"Looking for bondElem '{bondElem}' in molGroup = \n{molGroups}")
        ...
        # logger.info(f"\n{class_method}: "
        #      f"Found atom groups 0 ({len(molGroups)}) = ",*molGroups, sep='\n')
        ...
        logger.info(f"\nFound atom groups ({len(molGroups)}) = ")
        logger.info("\n".join(map(str, molGroups)))
    ...
    def assignBonds(self, crdScale: float = 1.0):
        ...
        # logger.info(f"\n{class_method}: listAtomDist = ", listAtomDist)
        # sys.exit(0)
        ...
        for grp in range(len(molChain)):
            ...
            # logger.info(f"\n{class_method}: Atom group, len(group), grp = ", group, len(group), grp)
            ...
            for indx1 in [0, len(group) - 1]:
                for indx2 in range(1, len(group)):
                    ...
                    # logger.info(f"\n{class_method}: valency1, valency2 = ", valency1, valency2)
                    # logger.info(f"\n{class_method}: Now looking at indx1, indx2 = ", indx1, indx2)
                ...
                for eb in listAtomDist:
                    ...
                    if valency1 > 0 and valency2 > 0:
                        if apair == list(
                            ...
                            logger.info(
                                f"\n{class_method}: apair, minRank, molValence[aindx1][2], molValence[aindx2][2] = \n"
                            )
                            logger.info(apair)
                            logger.info(minRank)
                            ...
                            if (
                                ...
                                continue
                                # logger.info(f"{class_method}: Min Rank for atom pair = \n", apair, aindx1, aindx2, minRank)
                        else:
                            continue
                    else:
                        continue
                # logger.info(f"\n{class_method}: Min Rank for atom pair = \n", aindx1, aindx2, molValence[aindx1][2], molValence[aindx2][2])
        logger.info(f"\nFound bonds ({len(bonds)}) = ")
        logger.info("\n".join(map(str, bonds)))
    ...
    def setSmlBonds(self, topology: list):
        ...
        logger.info(f"\nFound bonds ({len(self.totalBnd)}) =")
        logger.info("\n".join(map(str, self.totalBnd)))
    ...
    def setSmlAngles(self, topology: list):
        ...
        if self.totalBnd is not None:
            ...
            for mi in range(len(topology)):
                ...
                # logger.info(f"\n{class_method}: Angle Triplets for atom '{me[0]}' ({mi}): {angTriplet}")

                # MD: Determining mean bond angles:
                if me[1]["isaroma"] and mbonds > 3:
                    logger.info(
                        f"{class_method}: Incorrect bond number {mbonds} for atom '{me[0]}' "
            ...
            logger.info("\nNo bonds set for molecule")

        logger.debug(f"\nFound angles ({len(self.totalAng)}) =")
        logger.debug("\n".join(map(str, self.totalAng)))
    ...
    def setTopBonds(self, bndDictList: list, verbose=False):
        ...
        logger.debug(f"\nFound TopBonds ({len(self.topBonds)}) =")
        logger.debug("\n".join(map(str, self.topBonds)))
    ...
    def setTopAngles(self, angDictList: list, verbose=False):
        ...
        logger.debug(f"\nFound TopAngles ({len(self.topAngls)}) =")
        logger.debug("\n".join(map(str, self.topAngls)))
    ...
    def setBonesFromSmiles(self, smilesTopology: list, verbose=False):
        ...
        if smilesTopology is None:
            logger.info("\nInput SMILES topology is None! - skipping ...\n")
            return
        else:
            ...
            logger.debug(f"\nSetting SMILES topology ({len(self.smileTop)}) =")
            logger.debug("\n".join(map(str, self.smileTop)))

        # logger.info(f"\n{class_method}: Working out bonds, angles, etc ...")
        ...
        logger.debug(
            f"\nInitial list of backbone atoms ({len(self.bonesTop)}) ="
        )
        logger.debug("\n".join(map(str, self.bonesTop)))
    ...
    def setBonesBnd(self, verbose=False):
        ...
        logger.debug(f"\nBackbone bonds ({len(self.bonesBnd)}) =")
        logger.debug("\n".join(map(str, self.bonesBnd)))
    ...
    def setBonesAng(self, verbose=False):
        ...
        logger.debug(f"\nBackbone angles ({len(self.bonesAng)}) =")
        logger.debug("\n".join(map(str, self.bonesAng)))
    ...
    def setSmilesTopology(
        ...
        if smilesTopology is None:
            ...
            logger.debug(f"\nBackbone atoms ({len(self.bonesTop)}) =")
            logger.debug("\n".join(map(str, self.bonesTop)))

        logger.debug("\nWorking out bonds, angles, etc ...")
        ...
        # logger.debug(f"\nList of bone atoms ({len(bonesAtm)}) =")
        # logger.debug("\n".join(map(str, bonesAtm)))

        logger.debug(f"\nTotal list of atoms ({len(molAtoms)}) =")
        logger.debug("\n".join(map(str, molAtoms)))

        logger.debug(f"\nTotal list of bonds({len(bondsTot)}) =")
        logger.debug("\n".join(map(str, bondsTot)))
        # sys.exit(0)
        ...
        logger.debug(f"\nTotal list of angles({len(anglesTot)}) =")
        logger.debug("\n".join(map(str, anglesTot)))
        ...
        logger.debug(f"\nFinal list of bonds ({len(self.topBonds)}) =")
        logger.debug("\n".join(map(str, self.topBonds)))
        # *molBonds, sep='\n')
        # *self.topBonds, sep = '\n')  # *molBonds, sep='\n')

        logger.debug(f"\nFinal list of angles ({len(self.topAngls)}) =")
        logger.debug("\n".join(map(str, self.topAngls)))
        # *molAngls, sep='\n')

        logger.debug(f"\nFinal molecule topology ({len(self.topology)}) =")
        logger.debug("\n".join(map(str, self.topology)))
        # *molAtoms, sep='\n')
        # logger.info(f"\n{class_method}: Final SMILES topology ({len(self.smileTop)}) =",
        #         *self.smileTop, sep='\n')
        # sys.exit(0)
    ...
    def writeITP(self, fname: str = "") -> None:
        ...
        if os.path.isfile(tplName):
            logger.info(f"\n File '{tplName}' exists - overwriting!..")
    ...
    def writeTop2YAML(self, fname: str = "") -> None:
        ...
        if self.getTopology() is not None:
            ...
            if os.path.isfile(yamlName):
                logger.info(f"\nFile '{yamlName}' exists - overwriting!..")
            ...
            with open(yamlName, "w") as yamlFile:
                ...
                # logger.info(f"\n YAML bonds = {yaml_bonds}")
                # logger.info(f"\n YAML angles = {yaml_angles}")

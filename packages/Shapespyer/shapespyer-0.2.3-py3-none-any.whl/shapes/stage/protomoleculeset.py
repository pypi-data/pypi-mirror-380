...
class MoleculeSet(object):
    ...
    def addItem(self, mol=None, be_verbose=False) -> None:
        if isinstance(mol, Molecule):
            ...
            logger.debug(f"Input {mol} does not qualify as "
    ...
    def imagesVec3(self,
        ...
        # print(f"\nimagesVec3(): {len(image_map)} points "
    ...
    def findNeighboursKDT(self,
        ...
        # etime = time.time()
        # print(f"\nfindNeighboursKDT(): Elapsed time for ssd_dist {sec2hms(etime-stime)}\n\n"
        #       f"{kdt_rnnb[:10]}")
        return kdt_rnnb
    ...
    def findNeighboursSSD(self,
        ...
        # self.dnnbs = ssd_dnnb
        # etime = time.time()
        # print(f"\nfindNeighboursSSD(): Elapsed time for ssd_dist {sec2hms(etime-stime)}\n\n"
        #       f"{ssd_rnnb[:10]}")
        return ssd_rnnb
    ...
    def findClusters(self, box):
        ...
        logger.info(f"Found {len(clusters)} clusters of sizes:\n"
                    f"counts = {counts} ->\n"
                    f"{nclust} => {np.sum(nclust)} =?= {self.nitems} molecules in total")
        #print(f"\nfindClusters(): self = {self}\n")
        ...
        for ic, clust in enumerate(clusters):
            ...
            if abs(dvec[0]) + abs(dvec[1]) + abs(dvec[2]) > TINY:
                logger.info(
                    f"\nnow applying PBC to cluster {ic} @ {rvec0} -> {rvec1}"
    ...
    def updateRvecs(self, **kwargs) -> None:  # center of mass
        ...
        if self.nitems > 0:
            ...
            if "box" in kwargs.keys():
                ...
                # print(f"\n{class_method}(): "
                #       f"Check: box = {box} & isMolPBC = {is_MolPBC} "
    ...
    def radialDensities(
        ...
        logger.info(f"\nCollecting histograms for {len(atms)} atoms on "
                    f"species {clist}:\n")
        # f"{axyz[:10]}")
        ...
        for ih, hist in enumerate(hlist):
            if ih > 0:
                if hist[0][1] in gntot:
                    ...
                    if gntot.count(hist[0][1]) > 1:
                        logger.warning(
                            f"\nMore than one histogram for group '{hist[0][1]}' "
            ...
            logger.debug(f"\nHistogram for group '{hist[0][1]}' @ atom {hist[0][0]} "
                         f"on species {self.name} => {np.sum(hist[1])} counts:\n",
                         f"{hist[1].T}\n")
                ...
                for ic in range(len(elems)):
                    if elems[ic].isdigit():
                        ...
                        if ic == len(elems) - 1:
                            ...
                            mass = emass[elems[ic]]
                logger.debug(f"\nHistogram for group '{gntot[ih]}' of {np.sum(hist)}"
                             f" counts, mass = {gmass}, CSL = {gchsl}:\n",
        ...
        if isinstance(bname, str):
            ...
            if any(checkA):  # AB: Atom contributions
                ...
                for ih, hist in enumerate(halst):
                    natot += np.sum(hist[1])
                    logger.debug(f"Histogram for atoms '{hist[0]}' of {np.sum(hist[1])}"

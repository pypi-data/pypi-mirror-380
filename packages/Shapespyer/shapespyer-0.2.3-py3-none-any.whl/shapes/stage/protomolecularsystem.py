...
class MolecularSystem(object):
    ...
    def getSpecies(self, i: int = 0) -> list:  # i-th unique Mol. Set
        if i > len(self.items) - 1:
            logger.info(f"\nSpecies index {i} > {len(self.items)} - 1")
        return self.items[i]
    ...
    def Densities(self,
        ...
        if not dlist or len(dlist) < 1:
            logger.warning("\nno density type specified - skipping!")
            return None
        elif 'all' not in dlist:
            dlist.extend(['all'])
            # print(f"\n{self.__class__.__name__}.Densities(): Added 'all' to dlist = {dlist}")
        ...
        if is_zden:
            ...
            # print(f"{self.__class__.__name__}.Densities(): z_bin * n_zbin = "
        ...
        if (len(atoms_mass.values()) > 0 and
            ...
            logger.info(f"Cannot use ill-defined atom or bead masses: \n"
        ...
        if 'nsld' in dlist:
            if any( [ abs(ecsl) < TINY for ecsl in elems_csl.values() ] ):
                raise ValueError(f"\n{self.__class__.__name__}.Densities(): "
                                 ...
                                 f"{elems_csl} ...")
                # print(f"\n{self.__class__.__name__}.WARNING: Skipping 'nsld' calculation ...")
            else:
                logger.info(f"\nWill use ElemCSL: \n{elems_csl} ...")

        etime = time.time()
        logger.info(f"\nElapsed time before atom checks: {etime - stime}")
        stime = etime
        ...
        #     print(f"\n{self.__class__.__name__}.Densities(): Will calculate "
        #           f"ATM density contributions (i.e. atoms only) ...")
            ...
            for ia, aname in enumerate(anms):
                ...
                if aname[0] == 'H':  # add hydrogen to the COM group
                    ...
                    if aname not in atmlist:
                        logger.info(f"\nSeeding a group around atom {aname} {ia}...")
        ...
        etime = time.time()
        logger.info(f"\nElapsed time after atom binning: {etime - stime}")
        stime = etime
        ...
        if not is_cg:
            ...
            etime = time.time()
            logger.info(f"\nElapsed time after groups binning: {etime - stime}")
            stime = etime
        ...
        if any(checkA):  # AB: Atom contributions
            ...
            for ih, hist in enumerate(halst):
                natot += sum(hist[1])
                logger.debug(
                    f"Histogram for atoms '{hist[0]}' of {sum(hist[1])} counts:\n",
                    f"{np.column_stack((drange, hist[1]+0.0))}"
                )
                aname = hist[0][0]

"""Storing, automatically validating options used for shapes generation.
...
class ShapeType(ValidatedAttrs, Enum):
    ...
    is_pinned : bool
        Is shape type `balp`, `vesp`, `lat2` or `waves`.
    is_lattice : bool
    ...
    def is_pinned(self) -> bool:
        """Check if the shape type contains pins.

        Returns
        -------
        bool
            Is shape type `balp`, `vesp`, `lat2`, `waves` or `smiles`.
        """
        return self.name in (
            "BALP",
            "VESP",
...
class ShapeOptions(ValidatedAttrs):
    ...
    def __init__(self) -> None:
        ...
        self._offset = Defaults.Shape.OFFSET
        self.sbuff: float = Defaults.Shape.SBUFF
    ...
    def layers(self, value: Layers | int | list[float | int]) -> None:
        ...
        if (
            ...
            # not (self.type.is_vesicle and not self.type.is_pinned)
            # AB: the above condition does not make sense!
        ):
            new_layers.quantity = 1
        elif (
            ...
            # self.type.is_vesicle and not self.type.is_pinned
            # AB: the above condition does not make sense!
        ):
...
class Options:
    """A container for other *Options classes"""
...
class OptionsSerDes:
    ...
    def serialize(
        ...
        return {
            ...
            keys.SHAPE: {
                ...
                keys.Shape.OFFSET: opts.shape.offset,
                keys.Shape.SBUFF: opts.shape.sbuff,
            },
            keys.ANGLE: {
    ...
    def dump_validated_yaml(
        ...
        if os.path.isfile(file_name):
            logger.warning(f"dump Yaml file {file_name} exists, overwriting!")
    ...
    def deserialize(
        ...
        print(f"\nDEBUG: Yaml read options.other = {other} ...\n" )
        ...
        # print(f"\nDEBUG: Yaml read options.shape.layers = {layers} ...\n" )
        ...
        opts.shape.offset = shape.get(keys.Shape.OFFSET, Defaults.Shape.OFFSET)
        opts.shape.sbuff = shape.get(keys.Shape.SBUFF, Defaults.Shape.SBUFF)
        ...
        if base.get(keys.Base.ORIGIN):
            opts.base.origin = ShapeOrigin[base.get(keys.Base.ORIGIN).upper()]

...
class CellParameters:
    ...
    def from_matrix(self,
        ...
        if self._populated:
            print(f"\nWARNING: resetting cell parameters from matrix!")
    ...
    def to_matrix(self, as_nda: bool = False) -> list[Vec3] | np.ndarray:
        ...
        print(f"\nThe cell converted to matrix -> {cmatrix}")
...
class Frame(ABC):
    ...
    def min_max_zdim(frame_coordinates):
        ...
        zdim = zmax - zmin
        # print(f"Max - Min coords => Bounds : {zmax} - {zmin} => {zdim}\n")
...
class FrameMolSys(object):
    ...
    def update_molsys(self,
        ...
        if self.frame.cell_params:
            ...
            if self._molsys_shift_flag > 0:
                ...
                if max(cmaxs) > 0.75*bmax:
                    ...
                    print(f"Centering the system at the origin is ON (required)\n")
                else:
                    hbox = Vec3(0.0, 0.0, 0.0)
                    self._molsys_shift_flag = -1
                    print(f"Centering the system at the origin is OFF (unnecessary)\n")
        else:
        ...
        if is_MolPBC:
            print(f"Applying Mol.COM.PBC: {is_MolPBC}, "
                  f"box shift = {hbox}")
            atom_idx  = 0
            ...
            for molset in self.molsys.items:
                ...
                if molset.items[0].name in self.rnames:
                    ...
                    print(f"\n Skipping unrecognised residue: {molset.items[0].name} "
                          f"not in {self.rnames}")

            # self.molsys.refresh()
        else:
            print(f"Skipping Mol.COM.PBC: {not is_MolPBC}, "
                  f"box shift = {hbox}")
            atom_idx  = 0
            scaledBox = lscale*(Vec3(*cbox))
            for molset in self.molsys.items:

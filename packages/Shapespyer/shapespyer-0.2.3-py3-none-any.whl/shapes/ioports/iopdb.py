    ...
    def readInMols(self, inp_data: dict = None, is_close: bool = True) -> None:
        ...
        if self._lnum == nrems:
            ...
            while True:
                ...
                # logger.debug(f"{self.__class__.__name__}.readInMols(): Read-in resix = {resix} "
                ...
                if is_molin:
                    ...
                    # logger.debug(f"{self.__class__.__name__}.readInMols(): Read-in resix = "+str(resix)+", resnm = "+resnm+", atnm = "+atms[i]+ \
                    #      ", atmid = "+str(atmix)+"\n")
            ...
            if mmols > 0:
                ...
                logger.debug(
                    f"\n Read-in Mmols = {mmols}, "
                    f"Matms = {mnatm}, MolNames = {molnm}"
                )
            else:
                logger.error(
                    f"\n Read-in Mmols = {mmols}, "
                    f"no molecule '{resname}' with resid in {resids} found "
                    "- FULL STOP!!!"
                )
                sys.exit(2)
        ...
        if matms != natms:
            logger.debug(
                f"\n Total number of atoms: {matms} =/= {natms} number of "
                "atoms kept..."
            )
        ...
        # print(f"\n{self.__class__.__name__}.readInMols(): {cell} "
        #       f"-> proper: {cell.is_proper()}; "

...
class Layer(object):
    """
    Class **Layer(object)** - generates a set of molecules arranged in a 'bilayer' or 'monolayer' configuration.
    ...
    def make(
        ...
        a Layer object (``self``) with molecules placed accordingly.
        ...
        if frcs is not None:
            ...
            if isinstance(frcs, list):
                ...
                frcs = self.frcs
            logger.info(f"\nNOTE: The given list of fractions: {self.frcs}")
        ...
        for m in range(len(mols_inp)):
            ...
            for n in range(len(mols_inp[m])):
                ...
                #print(f"\nZ-aligned molecule Rvec() = {mols_inp[m][n].getRvec()} ")
                # AB: get the Z shift
        ...
        logger.info(f"\nNOTE: Fractions in leaflet 1: {frcs}")
        ...
        for m in range(len(mols_inp)):
            for n in range(len(mols_inp[m])):
                ...
                #print(f"Z-aligned molecule' Rvec() = {mols_inp[m][n].getRvec()}\n")

        if self.frcs and isinstance(self.frcs[0], list) and len(self.frcs) > 1:
            frcs = self.frcs[1]

        nmols1 = [ len(mols) for mols in mols_out]
        logger.info(f"NOTE: Molecules in leaflet 1: {nmols1}")

        logger.info(f"\nNOTE: Fractions in leaflet 2: {frcs}")
        ...
        logger.info(f"NOTE: Molecules in leaflet 2: {nmols2}\n")
    ...
    def create_monolayer(self, nside, dmin, offset, mols_inp, mols_out, frcs):
        ...
        if len(mols_inp) > 1:
            ...
            if frcs is None or len(frcs) == 0:
                ...
                logger.info(f"NOTE: using fractions (uniform): {frcs}\n")
            else:
                # ensure flat list
                # if isinstance(frcs, list) and len(frcs) > 0 and isinstance(frcs[0], list):
                #     frcs = frcs[0]
                logger.info(f"NOTE: using fractions (given): {frcs}\n")

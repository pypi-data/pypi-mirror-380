...
def main():
    ...
    logger.info("Coarse-graining bead mapping\n"
                "============================\n"
...
# prepare masses and moments for beads
    ...
    logger.info("Information from mapping file ({0:s}):\n".format(map))
...
# selecting user-provided molecule name and (optionally) molecule/residue number
    ...
    if gro or pdb:
        logger.info("Looking for molecule named {0:s}, molecule number {1:d}\n".format(mole, molenum))
    else:
        logger.info("Assuming configuration consists of one instance of molecule named {0:s}\n".format(mole))
...
# centre-of-mass, sorting out any periodic boundaries
    ...
    # minimum image convention for periodic boundaries
        ...
        logger.info("\nBead masses and positions adjusted for centre-of-mass (nm):\n")
        for i in range(num_beads):
            logger.info("Bead {0:d}, name {1:s}, mass {2:f} u, position = ({3:f}, {4:f}, {5:f})".format(i+1, bead_list[i], bead_mass[i], bead_positions[i][0], bead_positions[i][1], bead_positions[i][2]))
...
# to 0.1 nm converts to angstroms
    ...
    if lscale != 1.0:
        logger.info("Bead masses and positions in requested length units ({0:f} nm):\n".format(lscale))
        if mscale != 1.0:
            for i in range(num_beads):

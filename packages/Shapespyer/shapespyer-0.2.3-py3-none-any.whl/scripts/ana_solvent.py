...
#            clust.append(str(arg.rstrip()))
        ...
        logger.error("Unrecognised option(s): "+arg+"\n")
        logger.info("Try: "+script+" --help\n")
        sys.exit(1)

   iext = finp[-4:]
   logger.info("Input file extension: '"+iext+"'\n")
   if (iext not in exts) : # and finp!='CONFIG' :
      if iext[1]=='.' :
         logger.error("Unsupported input extension: '"+iext+"' [.gro]\n")
      else :
         logger.error("Unsupported input file-name: '"+finp+"'\n")
      sys.exit(2)
   #else :
   #   print("Using input file: '"+finp+"'\n")
   ...
   #print("Output file extension: '"+oext+"'\n")

   if len(oext)>0 and (oext not in exts) :
      logger.error("Unsupported output extension: '"+oext+"' [.gro/.xyz/?.pdb; N/A for DL_POLY CONFIG]\n")
      sys.exit(3)
   elif len(oext)<1 and fout!='CONFIG' :
      logger.error("Unsupported output file-name: '"+fout+"' [no extension => DL_POLY 'CONFIG']\n")
      sys.exit(3)
   #else :
   #   print("Using output file: '"+fout+"' (extension: '"+oext+"')\n")
   ...
   if os.path.isdir(dinp) :
      ...
      if os.path.isfile(dfinp) :
         ...
         #   natms = matms
            ...
            logger.error("Unrecongnised input format: '"+fout+"' [no extension => DL_POLY CONFIG]\n")
            sys.exit(2)
...
#               axyz_out[i][2] += gbox[2]*0.5
         ...
         #   print("GRO input '"+finp+"' => DLP output '"+fout+"'\n")
            ...
            logger.error("Unrecongnised output format: '"+fout+"'\n")
            sys.exit(3)
      else :
         logger.error("input file not found: '{}'".format(dfinp))
         sys.exit(2)
   else :
      logger.error("directory not found: '{}'".format(dinp))
      sys.exit(2)

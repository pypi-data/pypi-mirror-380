...
def ana_traj(
    ...
    if "dnames" in scheme_keys:
        ...
        if den_names:
            if scheme["zbin"] > TINY:
                logger.info(f"\nana_traj(): Will calculate Z-densities: {den_names} ...")
            else:
                logger.info(f"\nana_traj(): Will calculate R-densities: {den_names} ...")
    ...
    if fpath_trj.endswith(".dcd"):
        ...
        for iframe, (cell_params, coordinates) in enumerate(dcd_inp):
            ...
            logger.info(f"\nProcessing frame {iframe} for {len(res_names)} "
            ...
            if origin:
                logger.info(f"\nElapsed time before moving solutes of length {len(solutes.items)} "
                ...
                shift = origin - rcom
                molsys.moveBy(shift)
                logger.debug(f"\nElapsed time moving solutes to the origin = {origin}: "
                      f"{sec2hms(time()-sstime)}\n")

            logger.debug(f"\nElapsed time preparing the frame for analysis: "
                  f"{sec2hms(time()-stime)}\n")
            ...
            logger.debug(f"\nTotal time working on the frame: "
                  f"{sec2hms(time()-stime)}\n")
        ...
        if den_names and len(histograms) > 0:
            ...
            for ih, hist in enumerate(histograms):
                if hist:
                    ...
                    #print(f"\nHistograms {ih} '{hist.keys()}'")
                else:
                    logger.info(f"\nHistogram {ih} (None?) :{hist}")
...
def main(argv: list[str]):
    ...
    #     description="Calculate R-densities for a (coarse-grained) "
    #                 "molecular system using either "
    ...
    parser = argparse.ArgumentParser(
        ...
        usage=tb.BOLD
              + sname
              + " -i <INP> -r <REF> [-o <OUT>] -d <DNAMES> [-r <RNAMES>] [-s <SNAMES>] [extra options]"
              + tb.END,
    ...
    if args.wname:
        ...
        if args.cg and args.wnmap:
            ...
            logger.info(f"{sname}:: Extra options for CG water:\n{wpars}\n")
    ...
    dnames = None,
    if not args.dtypes:
        logger.info(f"\n{sname}:: No density types specified: {dnames} - skipping ...\n")
        args.grid = None
        #args.zbin = 0.0
        # if args.zbin or args.grid:
        #     raise ValueError(f"\n{sname}:: "
        #                      f"Either specify --dtypes for density calculations "
        #                      f"or do not use --grid nor --zbin options.")
    else:
        dnames = InputParser.list_of_strings(args.dtypes)
        logger.info(f"\n{sname}:: Specified density types: {dnames}\n")
        ...
        if float(args.zbin) > TINY:
            ...
            if grid[0] > grid[1]:
                # print(f"\n{sname}:: Swapping grid bounds: {grid[0]} <-> {grid[1]} ")
                grid1   = grid[0]
                grid[0] = grid[1]
                grid[1] = grid1
            logger.info(f"\n{sname}:: Will use R-grid: {grid}\n")
        elif dnames:  # and not args.zbin:
            raise ValueError(f"\n{sname}:: "
                             f"Need to specify --grid or --zbin for density calculations!")
        #     grid = [0.0,10.0,0.2]
        #     print(f"\n{sname}:: Using default grid: {grid}")

    schema = dict(
        ifirst = int(args.begin),
        frqncy = int(args.freq),
        dnames = dnames, #list_of_strings(args.dtypes),
    ...
    # print(f"Will use Grid: {schema['grid']} for densities in {schema['dnames']} "
    #       f"for atoms on {schema['rnames']} (clustering {schema['snames']} & CG = {schema['cgobj']})")

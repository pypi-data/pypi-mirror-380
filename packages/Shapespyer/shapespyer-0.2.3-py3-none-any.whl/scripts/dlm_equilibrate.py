...
def main():
    ...
    logger.info("DL_MESO_DPD System Equilibration\n"
                "================================\n"
    ...
    if command==None: 
        ...
        if stepnumber>0:
            ...
            if args["--visual"] and os.path.isfile(equil+"/export"):
                ...
                # close VTF file
                fw.close()
                logger.info("Written final snapshot to {0:s} (open in VMD to visualise)\n".format(equil+"/equilibrated-{0:d}ps.vtf".format(round(tscale*tstep*float(stepnumber)))))

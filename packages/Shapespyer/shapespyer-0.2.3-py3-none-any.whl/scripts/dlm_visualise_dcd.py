...
def main():
    ...
    if args["--water"] != None:
        logger.info("\nBasing DPD mass, length and time units on {0:f} water molecules per bead".format(water))
        ...
        if mscale == 1.0 and float(args["--lscale"])==0.1:
            logger.info("\nUsing original DPD mass, length and time units\n")
            tscale = 1.0
        else:
            ...
            logger.info("DPD length unit = {0:f} nm ({1:f} angstroms)".format(0.1*lscale, lscale))
            logger.info("DPD time unit = {0:f} ps".format(tscale))
    logger.info("Assumed DPD simulation timestep = {0:f} ps\n".format(dt*tscale))

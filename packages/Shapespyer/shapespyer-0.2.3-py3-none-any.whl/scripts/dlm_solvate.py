...
def main():
    ...
    logger.info("DL_MESO_DPD System Solvation\n"
                "============================\n"
    ...
    logger.info("\nNumber of beads provided in structure = {0:d}".format(len(particledata)))
    logger.info("Original volume of simulation box = {0:f} ({1:f} nm^3)".format(volume0, volume0*dpdlen*dpdlen*dpdlen))
    logger.info("Minimum possible volume for structure = {0:f} ({1:f} nm^3)\n".format(minvolume, minvolume*dpdlen*dpdlen*dpdlen))
    ...
    conc0 = 0.0
    conctype = ''
    logger.info("Contents of structure\n---------------------")
    molbead = 0
    ...
    logger.info("Adding salt to system\n---------------------")
    ...
    logger.info("Adding solvent (and counterion) beads to system\n"
                "-----------------------------------------------")
    ...
    if speciesprop[spec][2] != 0.0:
        ...
        logger.info("Going to add {0:d} beads of solvent {1:s} to system\n".format(nsbeads, speciesname[spec]))

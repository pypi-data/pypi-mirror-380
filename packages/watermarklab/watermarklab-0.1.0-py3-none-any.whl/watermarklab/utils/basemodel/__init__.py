...
class BaseWatermarkModel(ABC, nn.Module):
    ...
    def embed(self, cover_list: List[Any], secrets: List[Any]) -> Result:
        """
        Embeds a watermark into cover images or prompts.

        This is the primary method for watermark insertion. Subclasses must implement
        how the secret is encoded into the image (e.g., frequency domain, diffusion process).

        Args:
            cover_list (List[Any]): List of cover images (PGW) or prompts (IGW).
            secrets (List[Any]): List of secret bit sequences to embed.

        Returns:
            Result: Object containing:
                - stego_img: Watermarked images
                - clean_img: Original images (for IGW)
                - emb_bits: Embedded bits (for metric computation)
                - other_result: Timing, losses, etc.

        Example:
            result = model.embed([img1, img2], [[1,0,1], [0,1,0]])
        """
        pass
    ...
    def recover(self, stego_list: List[ndarray]) -> Result:
        ...
        Returns:
            Result: Object containing:
                ...
                - other_result: Metrics like recovery PSNR
        """
        pass

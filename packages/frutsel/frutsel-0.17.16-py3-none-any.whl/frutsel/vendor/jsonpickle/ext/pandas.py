...
def make_read_csv_params(meta, context):
    ...
    return (
        dict(dtype=dtype, header=header, parse_dates=parse_dates, converters=converters),
        timedeltas,
        parse_datetime_v2,
    )
...
class PandasDfHandler(BaseHandler):
    ...
    def flatten(self, obj, data):
        ...
        for col in obj.columns:
            ...
            if dtype_name == 'object':
                ...
                if col_data.apply(lambda x: isinstance(x, (list, dict, set, tuple, np.ndarray))).any():
    ...
    def restore(self, obj):
        ...
        try:
            ...
            warnings.warn(
                ('jsonpickle versions at and above v3.4 have a different encoding scheme for pandas dataframes. Please update your jsonpickle and re-encode these objects!'),
                stacklevel=6,
            )
            return self.restore_v3_3(obj)
        ...
        if meta.get('is_multicolumns', False):
            columns = pd.MultiIndex.from_tuples(columns_decoded, names=meta.get('column_names'))
        else:
        ...
        for col in df.columns:
            ...
            try:
                ...
                msg = f'jsonpickle was unable to properly deserialize the column {col} into its inferred dtype. Please file a bugreport on the jsonpickle GitHub! '
                warnings.warn(msg)

        # decode and set the index
        index_values = decode(meta['index'], keys=True)
        if meta.get('is_multiindex', False):
            index = pd.MultiIndex.from_tuples(index_values, names=meta.get('index_names'))
        else:
    ...
    def restore_v3_3(self, data):
        ...
        df = pd.read_csv(StringIO(csv), **params) if data['values'].strip() else pd.DataFrame()
...
class PandasIndexHandler(BaseHandler):
    ...
    def restore(self, data):
        ...
        name_bundle = {'name': (tuple if v is not None else lambda x: x)(v) for k, v in meta.items() if k in {'name', 'names'}}

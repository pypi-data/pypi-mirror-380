...
class NumpyNDArrayHandler(NumpyBaseHandler):
    ...
    def restore(self, data):
        ...
        arr = np.array(values, dtype=self.restore_dtype(data), order=data.get('order', 'C'))
...
class NumpyNDArrayHandlerView(NumpyNDArrayHandlerBinary):
    ...
    def flatten(self, obj, data):
        ...
        if base is None and obj.flags.forc:
            ...
            if self.mode == 'warn':
                msg = 'ndarray is defined by reference to an object we do not know how to serialize. A deep copy is serialized instead, breaking memory aliasing.'
                warnings.warn(msg)
    ...
    def restore(self, data):
        ...
        if base is None:
            ...
            if not isinstance(base, np.ndarray):
                ...
                base = np.array(base)
            assert base.flags.forc, 'Current implementation assumes base is C or F contiguous'

...
def encode(
    ...
    :param unpicklable: If set to ``False`` then the output will not contain the
        ...
        If you want the output to not include the dtype for numpy arrays, add::
            ...
            jsonpickle.register(numpy.generic, UnpicklableNumpyGenericHandler, base=True)
...
class Pickler:
    ...
    def _sort_attrs(self, obj):
        if hasattr(obj, '__slots__') and self.warn:
            ...
            raise TypeError('Objects with __slots__ cannot have their keys reliably sorted  by jsonpickle! Please sort the keys in the __slots__ definition instead.')
    ...
    def _flatten_impl(self, obj):
        ...
        if type(obj) in (str, bool, int, float, type(None)) or (self._use_decimal and isinstance(obj, decimal.Decimal)):
            return obj
        #########################################
    ...
    def _flatten_key_value_pair(self, k, v, data):
        ...
        # attribute lookups
        if self.handle_readonly and k in {attr for attr, val in inspect.getmembers(self._original_object)} and util.is_readonly(self._original_object, k, v):
            return data
    ...
    def _flatten_properties(self, obj, data, allslots=None):
        ...
        properties = [x[0] for x in inspect.getmembers(obj.__class__) if valid_property(x)]
    ...
    def _flatten_obj_instance(self, obj):
        ...
        if self.unpicklable:
            ...
            if reduce_val and isinstance(reduce_val, str):
                ...
                if not (state and has_own_getstate and not hasattr(obj, '__setstate__') and not isinstance(obj, dict)):
        ...
        if has_class and not isinstance(obj, types.ModuleType):
            ...
            if has_getnewargs_ex:
                data[tags.NEWARGSEX] = [self._flatten(arg) for arg in obj.__getnewargs_ex__()]
    ...
    def _get_flattener(self, obj):
        if type(obj) in (list, dict):
            if self._mkref(obj):
                return self._list_recurse if type(obj) is list else self._flatten_dict_obj
            else:
                return self._getref
        ...
        # We handle tuples and sets by encoding them in a "(tuple|set)dict"
            ...
            return lambda obj: {tags.TUPLE if type(obj) is tuple else tags.SET: [self._flatten(v) for v in obj]}

...
def decode(
    ...
    if isinstance(on_missing, str):
        ...
        warnings.warn('Unpickler.on_missing must be a string or a function! It will be ignored!')
...
class Unpickler:
    ...
    def _restore_list(self, obj):
        ...
        proxies = [(parent, idx, value, method) for idx, value in enumerate(parent) if isinstance(value, _Proxy)]
    ...
    def _process_missing(self, class_name):
        ...
        if self.on_missing == 'ignore':
            ...
            raise errors.ClassNotFoundError('Unpickler.restore_object could not find %s!' % class_name)
    ...
    def _restore_from_dict(self, obj, instance, ignorereserved=True, restore_dict_items=True):
    ...
    def _restore_state(self, obj, instance):
        ...
        if hasattr(instance, '__setstate__'):
            ...
            # and no slots
            instance = self._restore_from_dict(state, instance, ignorereserved=False, restore_dict_items=False)
        elif has_slots:
            instance = self._restore_from_dict(state[1], instance, ignorereserved=False, restore_dict_items=False)
            if has_slots_and_dict:
                instance = self._restore_from_dict(state[0], instance, ignorereserved=False, restore_dict_items=False)
    ...
    def _restore_object_instance(self, obj, cls, class_name=''):
        ...
        if _safe_hasattr(instance, 'default_factory') and isinstance(instance.default_factory, _Proxy):

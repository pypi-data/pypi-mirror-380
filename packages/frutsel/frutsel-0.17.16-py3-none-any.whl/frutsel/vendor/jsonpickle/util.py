...
def is_sequence_subclass(obj):
    ...
    *obj* must be a subclass and not the actual builtin, such
    ...
    True
    """
    return hasattr(obj, '__class__') and issubclass(obj.__class__, SEQUENCES) and not is_sequence(obj)
...
def is_module_function(obj):
    ...
    return (hasattr(obj, '__class__') and isinstance(obj, (types.FunctionType, types.BuiltinFunctionType)) and hasattr(obj, '__module__') and hasattr(obj, '__name__') and obj.__name__ != '<lambda>') or is_cython_function(obj)
...
def is_reducible(obj):
    ...
    if is_collections(obj) and not isinstance(obj, collections.defaultdict):
        return True
    if type(obj) in NON_REDUCIBLE_TYPES or obj is object or is_dictionary_subclass(obj) or isinstance(obj, types.ModuleType) or is_reducible_sequence_subclass(obj) or is_list_like(obj) or isinstance(getattr(obj, '__slots__', None), _ITERATOR_TYPE) or (is_type(obj) and obj.__module__ == 'datetime'):
        return False
    return True
...
def is_cython_function(obj):
    ...
    return callable(obj) and hasattr(obj, '__repr__') and repr(obj).startswith('<cyfunction ')

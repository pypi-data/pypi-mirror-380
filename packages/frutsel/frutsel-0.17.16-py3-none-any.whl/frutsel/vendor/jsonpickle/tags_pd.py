...
def get_all_numpy_dtype_strings():
    ...
    try:
        ...
        dt_variants = list(dict.fromkeys(['datetime64[' + re.search(r'\[(.*?)\]', var).group(1) + ']' for var in char_codes._DT64Codes.__args__ if re.search(r'\[(.*?)\]', var)]))
        td_variants = list(dict.fromkeys(['timedelta64[' + re.search(r'\[(.*?)\]', var).group(1) + ']' for var in char_codes._TD64Codes.__args__ if re.search(r'\[(.*?)\]', var)]))
...
np_dtypes = list(dict.fromkeys([dtype for dtype in get_all_numpy_dtype_strings() if isinstance(dtype, str)]))

pd_dtypes = list(dict.fromkeys([dtype for dtype in get_all_pandas_dtype_strings() if isinstance(dtype, str)]))

...
class FastRulesEngine:
    ...
    def _evaluate_internal(
        ...
        # Track fuzzy matching statistics for telemetry
            ...
            if current_policy_match_score == 0.0 and self.fuzzy_matcher:
                ...
                if domains:
                    ...
                    if fuzzy_matches:
                        ...
                        # Track fuzzy match statistics for telemetry
        ...
        # Add fuzzy matching telemetry to current span if available
        if fuzzy_matches_count > 0:
            try:
                from opentelemetry import trace
                span = trace.get_current_span()
                if span.is_recording():
                    span.set_attribute("guardrails.fuzzy_matches_count", fuzzy_matches_count)
                    span.set_attribute("guardrails.fuzzy_matched_tokens", ",".join(fuzzy_matched_tokens))
                    logger.debug(f"Added fuzzy match telemetry: {fuzzy_matches_count} matches")
            except Exception as e:
                logger.debug(f"Failed to add fuzzy match telemetry: {e}")

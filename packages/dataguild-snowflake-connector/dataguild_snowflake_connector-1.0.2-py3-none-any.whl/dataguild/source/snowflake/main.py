    ...
    def _extract_metadata(self) -> Iterable[MetadataWorkUnit]:
        ...
        try:
            ...
            if safe_config_get(self.config, 'include_atlan_compatible_metadata', True):
                try:
                    from dataguild.source.snowflake.atlan_compatible_extractor import AtlanCompatibleSnowflakeExtractor
                    
                    atlan_extractor = AtlanCompatibleSnowflakeExtractor(
                        config=self.config,
                        report=self.report,
                        connection=self.connection,
                        filters=self.filters,
                    ...
                    with self.report.new_stage("*: Atlan-Compatible Comprehensive Metadata Extraction"):
                        yield from atlan_extractor.get_comprehensive_metadata_workunits()
                except Exception as e:
                    logger.error(f"Error in Atlan-compatible metadata extraction: {e}")

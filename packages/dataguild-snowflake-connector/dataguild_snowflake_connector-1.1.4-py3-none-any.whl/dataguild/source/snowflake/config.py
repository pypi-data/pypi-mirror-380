...
from dataguild.emitter.snowflake_integration import SnowflakeEmissionConfig
...
class SnowflakeV2Config(
    ...
    emitter_config: Optional[SnowflakeEmissionConfig] = Field(
        default=None,
        description="Configuration for metadata emission to external systems (REST/Kafka).",
    )
    ...
    extract_tags_as_structured_properties: bool = Field(
        default=False,
        description="If enabled along with `extract_tags`, extracts snowflake's key-value tags as enterprise structured properties instead of enterprise tags.",
    )
    ...
    enable_ai_intelligence: bool = Field(
        default=True,
        description="Enable AI-powered metadata intelligence with Gemma integration.",
    )
    
    gemma_base_url: str = Field(
        default="http://localhost:11434",
        description="Base URL for Ollama-hosted Gemma AI server.",
    )
    
    gemma_api_key: Optional[str] = Field(
        default=None,
        description="API key for Gemma AI server (usually not needed for Ollama).",
    )
    
    gemma_model_name: str = Field(
        default="gemma:2b",
        description="Ollama model name for AI intelligence (e.g., 'gemma:7b', 'gemma:2b').",
    )
    
    ai_classification_enabled: bool = Field(
        default=True,
        description="Enable AI-powered metadata classification.",
    )
    
    ai_quality_scoring_enabled: bool = Field(
        default=True,
        description="Enable AI-powered data quality scoring.",
    )
    
    ai_optimization_suggestions_enabled: bool = Field(
        default=True,
        description="Enable AI-powered optimization suggestions.",
    )
    ...
    ai_business_insights_enabled: bool = Field(
        default=True,
        description="Enable AI-powered business insights generation.",
    )
    ...
    def validate_configuration(self) -> bool:
        ...
        try:
            ...
            if not isinstance(self.account_id, str) or len(self.account_id) < 3:
                raise ValueError("account_id must be a non-empty string with at least 3 characters")
            
            # Validate username format
            if not isinstance(self.username, str) or len(self.username) < 1:
                raise ValueError("username must be a non-empty string")
            
            # Validate password format (handle both str and SecretStr)
            password_value = str(self.password) if hasattr(self.password, '__str__') else self.password
            if not isinstance(password_value, str) or len(password_value) < 1:
            ...
            # Validate database pattern (AllowDenyPattern object)
            if hasattr(self, 'database_pattern') and self.database_pattern:
                # database_pattern is an AllowDenyPattern object, not a dict
                if not hasattr(self.database_pattern, 'allow') or not hasattr(self.database_pattern, 'deny'):
                    raise ValueError("database_pattern must be a valid AllowDenyPattern object")
            
            # Validate schema pattern (AllowDenyPattern object)
            if hasattr(self, 'schema_pattern') and self.schema_pattern:
                if not hasattr(self.schema_pattern, 'allow') or not hasattr(self.schema_pattern, 'deny'):
                    raise ValueError("schema_pattern must be a valid AllowDenyPattern object")
            
            # Validate table pattern (AllowDenyPattern object)
            if hasattr(self, 'table_pattern') and self.table_pattern:
                if not hasattr(self.table_pattern, 'allow') or not hasattr(self.table_pattern, 'deny'):
                    raise ValueError("table_pattern must be a valid AllowDenyPattern object")
            
            # Validate view pattern (AllowDenyPattern object)
            if hasattr(self, 'view_pattern') and self.view_pattern:
                if not hasattr(self.view_pattern, 'allow') or not hasattr(self.view_pattern, 'deny'):
                    raise ValueError("view_pattern must be a valid AllowDenyPattern object")
            
            # Validate profiling configuration
            if hasattr(self, 'profiling') and self.profiling:
                if not isinstance(self.profiling, dict):
                    raise ValueError("profiling must be a dictionary")
                
                if 'enabled' in self.profiling and not isinstance(self.profiling['enabled'], bool):
                    raise ValueError("profiling.enabled must be a boolean")
                
                if 'profile_table_row_limit' in self.profiling:
                    if not isinstance(self.profiling['profile_table_row_limit'], int) or self.profiling['profile_table_row_limit'] < 0:
                        raise ValueError("profiling.profile_table_row_limit must be a non-negative integer")
                
                if 'profile_table_size_limit' in self.profiling:
                    if not isinstance(self.profiling['profile_table_size_limit'], int) or self.profiling['profile_table_size_limit'] < 0:
                        raise ValueError("profiling.profile_table_size_limit must be a non-negative integer")
            ...
            if hasattr(self, 'usage') and self.usage:
                ...
                if 'enabled' in self.usage and not isinstance(self.usage['enabled'], bool):
                    raise ValueError("usage.enabled must be a boolean")
                
                if 'start_time' in self.usage and 'end_time' in self.usage:
                    if self.usage['start_time'] and self.usage['end_time'] and self.usage['end_time'] <= self.usage['start_time']:
                        raise ValueError("usage.end_time must be after usage.start_time")
            
            # Validate stateful ingestion configuration
            if hasattr(self, 'stateful_ingestion') and self.stateful_ingestion:
                if not isinstance(self.stateful_ingestion, dict):
                    raise ValueError("stateful_ingestion must be a dictionary")
                
                if 'enabled' in self.stateful_ingestion and not isinstance(self.stateful_ingestion['enabled'], bool):
                    raise ValueError("stateful_ingestion.enabled must be a boolean")
                
                if 'remove_stale_metadata' in self.stateful_ingestion and not isinstance(self.stateful_ingestion['remove_stale_metadata'], bool):
                    raise ValueError("stateful_ingestion.remove_stale_metadata must be a boolean")
            ...
            if hasattr(self, 'classification') and self.classification:
                ...
                if 'enabled' in self.classification and not isinstance(self.classification['enabled'], bool):
                    raise ValueError("classification.enabled must be a boolean")
            
            # Validate incremental properties configuration
            if hasattr(self, 'incremental_properties') and self.incremental_properties:
                if not isinstance(self.incremental_properties, dict):
                    raise ValueError("incremental_properties must be a dictionary")
                
                if 'enabled' in self.incremental_properties and not isinstance(self.incremental_properties['enabled'], bool):
                    raise ValueError("incremental_properties.enabled must be a boolean")
    ...
    def get_connection(self) -> "SnowflakeConnection":
        """Get a Snowflake connection using the configuration."""
        try:
            ...
            conn_params = {
                ...
                'user': self.username,
                'password': str(self.password),  # Convert SecretStr to string
            ...
            snowflake_conn = snowflake.connector.connect(**conn_params)
            
            # Wrap in DataGuild SnowflakeConnection
            return SnowflakeConnection(snowflake_conn)
            
        except Exception as e:
            logger.error(f"Failed to create Snowflake connection: {e}")
            raise SnowflakeConnectorError(f"Connection creation failed: {e}")

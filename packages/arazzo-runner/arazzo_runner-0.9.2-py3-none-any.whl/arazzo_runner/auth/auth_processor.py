...
from arazzo_runner.auth.models import AuthType, EnvVarKeys, SecurityOption, SecurityRequirement
from arazzo_runner.executor.operation_finder import OperationFinder
from arazzo_runner.models import ArazzoDoc, OpenAPIDoc
from arazzo_runner.utils import create_env_var_name, extract_api_title_prefix
...
class AuthProcessor:
    ...
    def _normalize_openapi_spec(self, openapi_spec: dict[str, Any]) -> dict[str, Any]:
        ...
        if isinstance(processed_spec, dict) and (
            "swagger" in processed_spec or "openapi" in processed_spec
        ):
            ...
            if "components" not in processed_spec and "securityDefinitions" in processed_spec:
                ...
                processed_spec["components"] = {
                    "securitySchemes": processed_spec["securityDefinitions"]
                }
                ...
                processed_spec["components"] = {
                    "securitySchemes": processed_spec["securitySchemes"]
                }
    ...
    def process_api_auth(
        ...
        if not openapi_specs:
            ...
            return {"auth_requirements": [], "env_mappings": {}, "auth_workflows": []}
        ...
        for source_id, spec in processed_specs.items():
            try:
                ...
                logger.warning(
                    f"Error extracting auth requirements from spec with ID {source_id}: {str(e)}"
                )
    ...
    def generate_env_mappings(
        ...
        for auth_requirement in auth_requirements:
            ...
            env_var_prefix = create_env_var_name(
                var_name=security_scheme_name, prefix=api_title_prefix
            )
            ...
            if auth_requirement.auth_type == AuthType.API_KEY:
                ...
                http_auth_type = (
                    "basic"
                    if "basic" in auth_requirement.schemes
                    else (
                        "bearer"
                        if "bearer" in auth_requirement.schemes
                        or "Bearer" in auth_requirement.schemes
                        else "generic"
                    )
                )
            ...
            for workflow in workflows:
                ...
                if is_auth_workflow:
                    ...
                    for output_name in outputs:
                        if any(
                            kw in output_name.lower()
                            for kw in ["token", "access", "auth", "bearer"]
                        ):
    ...
    def get_security_requirements_for_workflow(
        workflow_id: str, arazzo_spec: ArazzoDoc, source_descriptions: dict[str, OpenAPIDoc]
        ...
        for source, options in by_source.items():
            ...
            by_source[source] = (
                [SecurityOption(requirements=merged_requirements)] if merged_requirements else []
            )
        return by_source

    @staticmethod
    def get_security_requirements_for_openapi_operation(
        openapi_spec: OpenAPIDoc, http_method: str, path: str
    ) -> list[SecurityOption]:
        """

#!/usr/bin/env python3
"""
Authentication Models for Arazzo Runner.
...
class OAuth2Flow(BaseModel):
    ...
    scopes: dict[str, str] = Field(
        default_factory=dict, description="Available scopes and their descriptions"
    )
...
SecuritySchemeUnion = ApiKeyScheme | HttpAuthScheme | OAuth2Scheme | OpenIDScheme | CustomScheme
...
def auth_requirement_to_schema(req: "AuthRequirement") -> SecuritySchemeUnion:
    ...
    """
    # Common attributes
    common = {"name": req.name, "description": req.description, "required": req.required}
    ...
    if req.auth_type == AuthType.API_KEY:
        ...
        return ApiKeyScheme(**common, location=location, parameter_name=req.name)
        ...
        return HttpAuthScheme(**common, scheme=scheme)
        ...
        if flow_type == OAuth2FlowType.AUTHORIZATION_CODE:
            ...
            flows.implicit = ImplicitFlow(authorization_url=auth_urls.authorization)
        elif flow_type == OAuth2FlowType.PASSWORD:
            flows.password = PasswordFlow(token_url=auth_urls.token)
        elif flow_type == OAuth2FlowType.CLIENT_CREDENTIALS:
            flows.client_credentials = ClientCredentialsFlow(token_url=auth_urls.token)

        return OAuth2Scheme(**common, flows=flows)

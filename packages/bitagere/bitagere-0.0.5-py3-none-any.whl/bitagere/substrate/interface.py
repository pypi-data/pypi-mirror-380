    ...
    def _substrate_call(self, func_name: str, *args, **kwargs):
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                last_exception = e
                logger.warning(
                    "Attempt %d/%d to ensure connection before '%s' failed: %s",
                    attempt,
                    self.max_retries,
                    func_name,
                    e,
                ...
                time.sleep(min(self.retry_delay_seconds * attempt, 30))
                continue
            ...
            if not self.substrate:
                ...
                last_exception = ConnectionError(msg)
                time.sleep(min(self.retry_delay_seconds * attempt, 30))
                continue
            ...
            try:
                ...
                last_exception = e
                logger.warning(
                    "Connection-level error during '%s' (attempt %d/%d): %s",
                    func_name,
                    attempt,
                    self.max_retries,
                    e,
                ...
                logger.error(
                    "Substrate operation error during '%s': %s",
                    func_name,
                    e,
                    exc_info=True,
                )
        ...
        error_message = (
            f"AgereInterface: Failed to execute '{func_name}' after {self.max_retries} attempts."
        )

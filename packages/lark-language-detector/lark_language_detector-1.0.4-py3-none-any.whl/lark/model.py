...
class BatchBoundaryPredictor(nn.Module):
    ...
    def __init__(self, d_model, init_tau=0.4, min_tau=0.0001,
                 anneal_rate=1e-4, dtype=torch.bfloat16):
        super().__init__()
    ...
    def forward(self, x: Tensor, pad_mask: Tensor = None,
                hard: bool = True, training: bool = False) -> Tensor:
        logits = self.mlp(x).squeeze(-1)  # [B, T]
        if training:
            self._update_tau()

        y = gumbel_sigmoid(logits, temp=float(self.tau), hard=hard)
...
class LarkModel(nn.Module):
    ...
    def forward(self, x_bytes: Tensor, pad_mask: Tensor = None,
                hard: bool = True, training: bool = True) -> Tensor:
        h = self.encoder(x_bytes, pad_mask)
        hard_boundary = self.predictor(h, pad_mask, hard=hard, training=training)

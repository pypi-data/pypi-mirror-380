...
class Vault:
    ...
    def get_client_from_model(self, model: str):
        ...
        # Check OpenAI first since it's most common and always available
        if model in self.openai.model_token_limits:
            return LLMClient(self.openai, **client_kwargs)
        elif self.anthropic_key and model in self.anthropic.model_token_limits:
            return LLMClient(self.anthropic, **client_kwargs)
        elif self.cerebras_key and model in self.cerebras.model_token_limits:
            return LLMClient(self.cerebras, **client_kwargs)
        elif self.grok_key and model in self.grok.model_token_limits:
            return LLMClient(self.grok, **client_kwargs)
        elif self.gemini_key and model in self.gemini.model_token_limits:

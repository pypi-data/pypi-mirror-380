...
class LLMClient:
    ...
    """    
        ...
        # Pending fetches for lazy loading prompts
        self._pending_fetches = {}
        self._fetch_executor = None
        self._fetch_futures = {}
        self._prompts_finalized = False

    def set_pending_fetch(self, key, fetch_func):
        """Set a pending fetch operation for a prompt/personality message"""
        self._pending_fetches[key] = fetch_func
        
    def _ensure_prompts_ready(self):
        """Ensure all pending fetches are resolved before using prompts"""
        # If prompts are already finalized, don't do anything
        if self._prompts_finalized:
            return
            
        # If there are no pending fetches, mark as finalized and return
        if not self._pending_fetches:
            self._prompts_finalized = True
            return
            
        # Execute all pending fetches if not already started
        if not self._fetch_futures:
            from concurrent.futures import ThreadPoolExecutor
            self._fetch_executor = ThreadPoolExecutor(max_workers=3)
            for key, fetch_func in self._pending_fetches.items():
                self._fetch_futures[key] = self._fetch_executor.submit(fetch_func)
        
        # Wait for and apply results
        for key, future in self._fetch_futures.items():
            try:
                result = future.result()
                setattr(self, key, result)
            except Exception:
                pass  # Keep default values on error
        
        # Clear pending fetches and update prompts
    ...
    def set_prompts(self):
        self.context_prompt = self.main_prompt_with_context + '\n' + f'{self.personality_message}' + '\n'
        self.prompt = self.main_prompt + '\n\n' + f'{self.personality_message}' + '\n'
        # If set_prompts is called directly (e.g., from vault.py with custom values),
        # mark prompts as finalized to prevent cloud fetches from overriding
        if not self._pending_fetches:
            self._prompts_finalized = True

    ...
    def load_ai(self, model: str = None) -> None:
        ...
        if not self.ai_loaded or model:
            ...
            try: 
                ...
                # If custom prompts were provided during init, use them
                if cstm_mnwc:
                    self._ai.main_prompt_with_context = self.main_prompt_with_context
                else:
                    # Otherwise, set up async fetch for cloud-stored prompt
                    self._ai.set_pending_fetch('main_prompt_with_context', lambda: self.fetch_custom_prompt())
                
                if cstm_mn:
                    self._ai.main_prompt = self.main_prompt
                else:
                    # Otherwise, set up async fetch for cloud-stored prompt
                    self._ai.set_pending_fetch('main_prompt', lambda: self.fetch_custom_prompt(context=False))
                
                if cstm_pm:
                    self._ai.personality_message = self.personality_message
                else:
                    # Otherwise, set up async fetch for cloud-stored personality
                    self._ai.set_pending_fetch('personality_message', lambda: self.fetch_personality_message())
                
                # If any custom values were set directly, update prompts now
                if cstm_mn or cstm_mnwc or cstm_pm:

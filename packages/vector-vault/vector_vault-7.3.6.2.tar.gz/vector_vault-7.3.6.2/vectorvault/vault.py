    ...
    def load_ai(self, model: str = None) -> None:
        ...
        if not self.ai_loaded or model:
            ...
            try: 
                ...
                need_ctx = not cstm_mnwc
                need_main = not cstm_mn
                need_pm = not cstm_pm

                results = {'ctx': None, 'main': None, 'pm': None}
                threads = []

                if need_ctx:
                    def _fetch_ctx():
                        try:
                            results['ctx'] = self.fetch_custom_prompt()
                        except Exception:
                            results['ctx'] = None
                    threads.append(T(target=_fetch_ctx))
                if need_main:
                    def _fetch_main():
                        try:
                            results['main'] = self.fetch_custom_prompt(context=False)
                        except Exception:
                            results['main'] = None
                    threads.append(T(target=_fetch_main))
                if need_pm:
                    def _fetch_pm():
                        try:
                            results['pm'] = self.fetch_personality_message()
                        except Exception:
                            results['pm'] = None
                    threads.append(T(target=_fetch_pm))
                ...
                self._ai.main_prompt_with_context = self.main_prompt_with_context if cstm_mnwc else results['ctx']
                self._ai.main_prompt = self.main_prompt if cstm_mn else results['main']
                self._ai.personality_message = self.personality_message if cstm_pm else results['pm']

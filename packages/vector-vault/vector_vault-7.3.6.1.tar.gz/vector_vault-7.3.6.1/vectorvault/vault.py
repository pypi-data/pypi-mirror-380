        ...
        if not self.ai_loaded or model:
            ...
            try: 
                ...
                with ThreadPoolExecutor(max_workers=3) as executor:
                    if not cstm_mnwc:
                        future_to_key[executor.submit(self.fetch_custom_prompt)] = 'mpwc'  # context prompt
                    if not cstm_mn:
                        future_to_key[executor.submit(self.fetch_custom_prompt, context=False)] = 'mp'  # no-context prompt
                    if not cstm_pm:
                        future_to_key[executor.submit(self.fetch_personality_message)] = 'pm'  # personality message

                    for future in as_completed(future_to_key):
                        key = future_to_key[future]
                        fetched_values[key] = future.result()

                # Choose provided values when customized; otherwise use fetched values with safe fallbacks
                mpwc_val = self.main_prompt_with_context if cstm_mnwc else fetched_values.get('mpwc', self._ai.main_prompt_with_context)
                mp_val = self.main_prompt if cstm_mn else fetched_values.get('mp', self._ai.main_prompt)
                pm_val = self.personality_message if cstm_pm else fetched_values.get('pm', self._ai.personality_message)

                self._ai.main_prompt_with_context = mpwc_val
                self._ai.main_prompt = mp_val
                self._ai.personality_message = pm_val

...
class GitBranch(BaseModel):
    ...
    # run before validation so we convert "" -> None
...
class GitPush(BaseModel):
    repo_path: str
    remote_name: str = "origin"  # Default remote name is 'origin'
    branch_name: Optional[str] = None  # Optional branch name to push
...
def git_push(repo: git.Repo, remote_name: str = "origin", branch_name: Optional[str] = None) -> str:
    remote = repo.remotes[remote_name]
    push_target = branch_name if branch_name else repo.active_branch.name
    response = remote.push(push_target)

    # Check for any push errors or rejections
    for res in response:
        if res.flags & (git.PushInfo.ERROR | git.PushInfo.REJECTED):
            return f"Push failed: {res.summary}"
    return f"Successfully pushed to {remote_name}/{push_target}"

...
def plot_grid_osm(
    ...
    # create bounding polygon, this will be used to determine the size of the basemap
    # to be downloaded
    outline_polygon = box(
        grid.x.min(), grid.x.max(), grid.y.min(), grid.y.max()
    )
    gdf_outline = gpd.GeoDataFrame(geometry=[outline_polygon], crs=grid_epsg)
    gdf = gdf_outline.to_crs(epsg=3857) # Convert to Web Mercator
    gdf.plot(ax=axes, facecolor="none", edgecolor="None", linewidth=2)

    # Add a map tile layer that looks like Google Maps
    cx.add_basemap(
        axes,
    ...
    # plot grid on map: convert from grid_epsg to web mercator
    transformer = Transformer.from_crs(grid_epsg, "EPSG:3857", always_xy=True)
    X, Y = np.meshgrid(
        grid.x.data,
        grid.y.data,
    )

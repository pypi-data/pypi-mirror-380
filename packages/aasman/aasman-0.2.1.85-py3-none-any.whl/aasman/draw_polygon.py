...
def draw_polygon(
    ...
    if faces:
        for f in faces:
            ...
            scene.play(Create(poly), run_time=rt/(len(faces) if len(faces) > 1 else 1))
...
def label_vertices(scene, threeDAxes, vertices, labels=None, shift_dirs=None, font_size=24, label_color=WHITE):
    """
    Hiển thị nhãn cho các vertices.

    - vertices: list tọa độ [[x,y,z], ...]
    - labels: list tên label tương ứng (nếu None → dùng số index)
    - shift_dirs: list vectors để shift label tránh trùng điểm
    """
    if labels is None:
        labels = [str(i) for i in range(len(vertices))]

    if shift_dirs is None:
        shift_dirs = [UP*0.3 + LEFT*0.3]*len(vertices)

    for i, v in enumerate(vertices):
        pos = threeDAxes.c2p(*v)
        shift = shift_dirs[i] if i < len(shift_dirs) else UP*0.3
        label = MathTex(labels[i], color=label_color).scale(font_size / 24)
        label.move_to(pos + shift)
        scene.play(Create(label), run_time=0.5)
...
def parse_sequences_from_nuad_txt(path: str) -> List[Sequence]:
    """
    Parse sequences from a NUAD-style plain text file where each non-empty line
    contains three whitespace-separated fields:

        <name> <ignored_group> <sequence...>

    - The second column is ignored
    - The first and second columns contain no spaces
    - The sequence field may contain spaces and is preserved as-is (minus EOL)
    ...
    with open(path, "r", encoding="utf-8") as handle:
        for raw_line in handle:
            ...
            if not line or not line.strip():
                continue
            # Split into at most 3 parts: name, group (ignored), sequence (rest)
            parts = line.split(None, 2)
            if len(parts) < 3:
                # Skip lines that don't match expected 3-field format
                continue
            name, _ignored_group, sequence = parts[0], parts[1], parts[2]
            # Preserve internal spaces in sequence exactly as provided
            results.append(Sequence(name=name, sequence=sequence))
    return results

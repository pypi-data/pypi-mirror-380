...
class DoclingClient:
    ...
    # ---- Async task APIs (submit + poll + result) ----
    def submit_file_async_task(
        self,
        file_path: Union[str, Path, BinaryIO],
        options: Optional[ConvertDocumentsRequestOptions] = None,
        target: Optional[TargetRequest] = None,
    ) -> TaskStatusResponse:
        """Submit a file to the async convert endpoint and return a TaskStatusResponse with task_id."""
        if options is None:
            options = ConvertDocumentsRequestOptions()
        if target is None:
            target = InBodyTarget()
        ...
        if isinstance(file_path, (str, Path)):
            ...
            if not p.exists():
                raise DoclingError(f"File not found: {file_path}")
            ftuple = (p.name, p.open('rb'), 'application/octet-stream')
        else:
            ftuple = ("document", file_path, 'application/octet-stream')
        ...
        with httpx.Client(timeout=self.timeout) as client:
            ...
            return TaskStatusResponse(**data_json)

    async def submit_file_async_task_async(
        self,
        file_path: Union[str, Path, BinaryIO],
        options: Optional[ConvertDocumentsRequestOptions] = None,
        target: Optional[TargetRequest] = None,
    ) -> TaskStatusResponse:
        """Async variant for submitting a file to async convert endpoint."""
        if options is None:
            options = ConvertDocumentsRequestOptions()
        if target is None:
            target = InBodyTarget()
        ...
        if isinstance(file_path, (str, Path)):
            ...
            if not p.exists():
                raise DoclingError(f"File not found: {file_path}")
            file_tuple = (p.name, p.open('rb'), 'application/octet-stream')
        else:
            file_tuple = ("document", file_path, 'application/octet-stream')
    ...
    def poll_status(self, task_id: str, wait_seconds: float = 0) -> TaskStatusResponse:
        """Poll task status using /v1/status/poll/{task_id}?wait=..."""
        with httpx.Client(timeout=self.timeout) as client:
            response = client.get(
    ...
    async def poll_status_async(self, task_id: str, wait_seconds: float = 0) -> TaskStatusResponse:
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            response = await client.get(
                f"{self.base_url}/v1/status/poll/{task_id}",
                params={"wait": wait_seconds},
    ...
    def fetch_result(self, task_id: str) -> Dict[str, Any]:
        """Fetch the final task result (convert or chunk response)."""
        with httpx.Client(timeout=self.timeout) as client:
            response = client.get(
    ...
    async def fetch_result_async(self, task_id: str) -> Dict[str, Any]:
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            response = await client.get(
                f"{self.base_url}/v1/result/{task_id}",
                headers=self._get_headers(),

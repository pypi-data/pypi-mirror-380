...
class LogCls(SingletonMixin):
    ...
    def __init__(
        ...
        Args:
            level: 日志级别，默认为10(DEBUG级别)，也可以是字符串如"DEBUG"
            ...
            log_file_name: 日志文件名，默认为xt_YYYYMMDD.log
        """
        self.loger = logger.bind()  # pyright:ignore[ reportUnannotatedClassAttribute]
        self.loger.remove()
        ...
        if log_dir is None:
            ...
            logs_dir: str = os.path.abspath(log_dir)
        ...
        log_file: str = os.path.join(logs_dir, log_file_name)

        # 保存配置信息
        self.current_level: int = level
        self.log_file: str = log_file
        self._file_id: int | None = None  # loguru handler ID 是整数类型
        self._console_id: int | None = None  # loguru handler ID 是整数类型

        # 保存参数到实例变量
        self.log_file_rotation_size: str = log_file_rotation_size
        self.log_file_retention_days: str = log_file_retention_days
    ...
    def __getattr__(self, attr: str) -> Callable[..., object]:
        ...
        try:
            return getattr(self.loger, attr)  # pyright: ignore[reportAny]

...
def get_function_location(func: Callable[..., object]) -> str:
    ...
    if hasattr(original_func, '__code__'):
        ...
        return f'{code.co_filename}:{code.co_firstlineno}@{original_func.__name__} | '

    # 回退方案：使用inspect.getfile
    try:
        file_path = inspect.getfile(original_func)
        _source_lines, line_no = inspect.getsourcelines(original_func)
        return f'{file_path}:{line_no}@{original_func.__name__} | '
    except Exception:
        # 最后的回退方案
        return f'{original_func.__module__ or "any"}:0@{original_func.__name__ or "any"} | '
...
class SimulationRunner:
    ...
    def _resolve_steer_endpoint(self, endpoint: str) -> str:
        base = os.getenv("COLLINEAR_STEER_URL", "https://steer.collinear.ai/")
        if not base:
    ...
    def _build_steer_payload(
        ...
        *,
        ...
        messages = self._transform_conversation_for_steer(conversation)
    ...
    def _transform_conversation_for_steer(
    ...
    def list_traits(self) -> list[str]:
        ...
        Resolves the traits endpoint to the same host as the Steer API by reusing
        ``COLLINEAR_STEER_URL``. When unset, defaults to
        ``https://steer.collinear.com/traits``.

        Network errors or unexpected payloads result in an empty list.
        """
        try:
            steer_url = os.getenv("COLLINEAR_STEER_URL")
            if steer_url:
                traits_url = self._resolve_steer_endpoint("traits")
            else:

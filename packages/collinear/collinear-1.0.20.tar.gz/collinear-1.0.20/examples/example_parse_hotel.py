from __future__ import annotations

import os
from pathlib import Path

from dotenv import load_dotenv

from collinear.client import Client


def main() -> None:
    env_path = Path(__file__).resolve().parents[1] / ".env"
    load_dotenv(env_path, override=True)

    api_key = os.environ["OPENAI_API_KEY"]
    trait_key = os.environ["TRAIT_KEY"]

    client = Client(
        assistant_model_url=os.getenv("OPENAI_BASE_URL", "https://api.openai.com/v1"),
        assistant_model_api_key=api_key,
        assistant_model_name=os.getenv("OPENAI_ASSISTANT_MODEL", "gpt-4o-mini"),
        steer_api_key=trait_key,
    )

    runner = client.simulation_runner
    runner.ASSISTANT_PROMPT_TEMPLATE = (
        "You are a hotel front-desk/concierge assistant. Be efficient and polite. "
        "Respond only with the next assistant message. Keep it 1–2 sentences."
    )

    sims = client.simulate(
        steer_config={
            "ages": ["25-34", "35-44"],
            "genders": ["male", "female"],
            "occupations": ["Employed", "Student"],
            "intents": ["Reservations & Booking", "Hotel Amenities"],
            "traits": {"impatience": ["medium", "high"], "confusion": ["low"]},
            "locations": ["United States"],
            "languages": ["English"],
            "tasks": ["hotel concierge"],
        },
        k=3,
        num_exchanges=2,
        batch_delay=0.2,
    )

    for idx, sim in enumerate(sims, start=1):
        print(f"Conversation {idx}")
        for message in sim.conv_prefix:
            role = message.get("role", "")
            content = message.get("content", "")
            if content:
                print(f"{role}: {content}")
        print(f"assistant: {sim.response}\n")


if __name__ == "__main__":
    main()

"""High-volume simulation demo (concise).

Loads `.env`, runs many simulations with concurrency, and prints transcripts.
"""

from __future__ import annotations

import os
from pathlib import Path

from dotenv import load_dotenv

from collinear.client import Client


K = 10
NUM_EXCHANGES = 12
MAX_CONCURRENCY = 8
BATCH_DELAY = 0.2


def main() -> None:
    env_path = Path(__file__).resolve().parents[1] / ".env"
    load_dotenv(env_path, override=True)

    api_key = os.environ["OPENAI_API_KEY"]
    trait_key = os.environ["TRAIT_KEY"]

    client = Client(
        assistant_model_url=os.getenv("OPENAI_BASE_URL", "https://api.openai.com/v1"),
        assistant_model_api_key=api_key,
        assistant_model_name=os.getenv("OPENAI_ASSISTANT_MODEL", "gpt-4o-mini"),
        steer_api_key=trait_key,
    )

    sims = client.simulate(
        steer_config={
            "ages": ["25-34", "35-44", "45-54"],
            "genders": ["male", "female"],
            "occupations": ["Employed", "Student"],
            "intents": ["Cancel service", "Upgrade plan", "Resolve billing issue"],
            "traits": {"impatience": ["medium", "high"], "confusion": ["low", "high"]},
            "locations": ["United States"],
            "languages": ["English"],
            "tasks": ["telecom support"],
        },
        k=K,
        num_exchanges=NUM_EXCHANGES,
        batch_delay=BATCH_DELAY,
        max_concurrency=MAX_CONCURRENCY,
        progress=True,
    )

    print(f"\nCompleted {len(sims)} simulations\n")

    for idx, sim in enumerate(sims, start=1):
        print(f"Conversation {idx}")
        for message in sim.conv_prefix:
            role = message.get("role", "")
            content = message.get("content", "")
            if content:
                print(f"{role}: {content}")
        print(f"assistant: {sim.response}\n")


if __name__ == "__main__":
    main()

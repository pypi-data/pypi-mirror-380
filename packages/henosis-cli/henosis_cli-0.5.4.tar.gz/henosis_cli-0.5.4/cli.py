...
class ChatCLI:
    ...
    def _render_tool_call(self, name: str, args: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self.ui.print(f"{model_prefix}: ⇒ " + summary, style=self.ui.theme["tool_call"])
                    ...
                    async for event, data_raw in parse_sse_lines(resp, debug=(lambda s: self.ui.print(f"[debug] {s}", style=self.ui.theme["dim"])) if DEBUG_SSE else None):
                        ...
                        if event == "session.started":
                                    ...
                                    if self.show_tool_calls:
                                        ...
                                        if not ok:
                                            ...
                                            if isinstance(err, str) and err.strip():
                                                ...
                                                self.ui.print(f"{model_prefix}: {label} — " + (f"[bold red]failure[/bold red]: [red]{reason}[/red]" if self.ui.rich else f"\x1b[31mfailure\x1b[0m: {reason}"))
                                            else:
                                                self.ui.print(f"{model_prefix}: {label} — " + ("[bold red]failure[/bold red]" if self.ui.rich else "\x1b[31mfailure\x1b[0m"))
                                        else:
                                            self.ui.print(f"{model_prefix}: {label} — " + ("[green]success[/green]" if self.ui.rich else "\x1b[32msuccess\x1b[0m"))
                                except Exception:
                            ...
                            try:
                                ...
                                rem_line = ""
                            box_lines.append(
                                f"model: {model_used or '(unknown)'} | cost charged: ${float(self._last_commit_cost_usd):.6f} | chat cost: ${float(self.server_cumulative_cost_usd):.6f}{rem_line}"
                            )
                            # If this is an OpenAI reasoning-capable model, surface the reasoning effort used
                            try:
                                if self._is_openai_reasoning_model(model_used):
                                    box_lines.append(f"Reasoning effort: {self.reasoning_effort}")
                            except Exception:
                                pass

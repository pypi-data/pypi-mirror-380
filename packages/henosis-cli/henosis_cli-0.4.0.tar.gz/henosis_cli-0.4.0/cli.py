    ...
    async def maybe_check_for_updates(self) -> None:
        ...
        try:
            if os.getenv("HENOSIS_CLI_NO_UPDATE_CHECK", "").strip() in ("1", "true", "yes"):  # opt-out
                return
            # Resolve current version from installed metadata; fallback to embedded default
            pkg_name = "henosis-cli"
            try:
                current = importlib.metadata.version(pkg_name)
            except importlib.metadata.PackageNotFoundError:
                # Fallback: try to read pyproject nearby when running from source
                current = None
                try:
            ...
            if not _newer(current, latest):
                return

            # Prompt user
            self.ui.warn(f"A newer henosis-cli is available: {current} -> {latest}")
            do_update = self.ui.confirm("Update now? (recommended)", default=True)
            if not do_update:
                # Show how to update manually
                self.ui.print("To update later:")
                self.ui.print("- pipx:    pipx upgrade henosis-cli", style=self.ui.theme["dim"])  # type: ignore
                self.ui.print(f"- pip:     {os.path.basename(sys.executable)} -m pip install -U henosis-cli", style=self.ui.theme["dim"])  # type: ignore
                return

            # Try to update using pip for the current interpreter
            try:
                self.ui.info("Running: python -m pip install -U henosis-cli")
                py = sys.executable
                proc = await asyncio.create_subprocess_exec(
                    py, "-m", "pip", "install", "-U", "henosis-cli",
                ...
                if code == 0:
                    self.ui.success("henosis-cli updated successfully.")
                    # Suggest restart
                    self.ui.print("Please restart the CLI to use the new version.", style=self.ui.theme["info"])  # type: ignore
                    # Offer to exit now
                    if self.ui.confirm("Exit now and restart?", default=True):
                        print("Goodbye.")
                        raise SystemExit(0)
                else:
                    self.ui.warn(f"Update command returned {code}.")
                    # Show a short preview of stderr
                    try:
                        if err:
                            msg = (err.decode("utf-8", errors="ignore") or "").strip().splitlines()
                            preview = "\n".join(msg[:20])
                            if preview:
                                self.ui.print(preview, style=self.ui.theme["dim"])  # type: ignore
                    except Exception:
                        pass
                    self.ui.print("You can try one of these manually:")
                    self.ui.print("- pipx:    pipx upgrade henosis-cli", style=self.ui.theme["dim"])  # type: ignore
                    self.ui.print(f"- pip:     {os.path.basename(sys.executable)} -m pip install -U henosis-cli", style=self.ui.theme["dim"])  # type: ignore
            except Exception as e:
                self.ui.warn(f"Automatic update failed: {e}")
                self.ui.print("Try manually:")
                self.ui.print("- pipx:    pipx upgrade henosis-cli", style=self.ui.theme["dim"])  # type: ignore
                self.ui.print(f"- pip:     {os.path.basename(sys.executable)} -m pip install -U henosis-cli", style=self.ui.theme["dim"])  # type: ignore
        except SystemExit:
            raise
            ...
            if r.status_code == 200:
                ...
                try:
                    ...
                    try:
                        ...
                        rem_str = None
                line = (
                    f"[server] model: {model_used or '(unknown)'} | cost charged: ${cost_f:.6f} | server cumulative: ${self.server_cumulative_cost_usd:.6f}"
                    + (rem_str or "")
                )
                ...
                async with client.stream("POST", self.stream_url, json=req_payload) as resp:
                    ...
                    async for event, data_raw in parse_sse_lines(resp):
                        ...
                        if event == "session.started":
                            ...
                            if not self.ui.verbose:
                                try:
                                    if self.show_tool_calls:
                                        ...
                                        # For failures, append a short reason when present
                                        if not ok:
                                            err = result.get("error")
                                            if isinstance(err, str) and err.strip():
                                                # keep it short
                                                reason = self._clip(err, 160)
                                                self.ui.print(f"{label} — " + (f"[bold red]failure[/bold red]: [red]{reason}[/red]" if self.ui.rich else f"\x1b[31mfailure\x1b[0m: {reason}"))
                                            else:
                                                self.ui.print(f"{label} — " + ("[bold red]failure[/bold red]" if self.ui.rich else "\x1b[31mfailure\x1b[0m"))
                                        else:
                                            self.ui.print(f"{label} — " + ("[green]success[/green]" if self.ui.rich else "\x1b[32msuccess\x1b[0m"))
                                except Exception:
                            ...
                            # Use server's cumulative when provided (authoritative), otherwise maintain client-side
                            cum_source = "client"
                            ...
                            if server_cum_total > 0:
                                # Server provided cumulative - use it directly (authoritative)
                                ...
                                # No server cumulative - maintain client-side running total
                            ...
                            # Show cumulative totals (session-wide) and model used
                            self.ui.print(
                                f"[completed] model: {model_used or '(unknown)'} | "
                            ...
                            self.ui.print(
                                f"[turn] model: {model_used or '(unknown)'} | total tokens used last turn: in {int(turn_in)} + out {int(turn_out)} = {int(turn_total)}",
...
async def amain():
    ...
    # Check for updates (non-fatal, quick)
    try:

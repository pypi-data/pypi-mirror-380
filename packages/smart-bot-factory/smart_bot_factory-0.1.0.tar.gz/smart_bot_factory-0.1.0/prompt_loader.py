...
class PromptLoader:
    ...
    def __init__(self, prompts_dir: str, prompt_files: List[str]):
            ...
            logger.info(f"Инициализирован загрузчик промптов: {self.prompts_dir.absolute()}")
...
"""
    ...
    async def load_welcome_message(self) -> str:
        ...
        try:
            ...
            if not self.welcome_file.exists():
                error_msg = f"Файл приветствия не найден: {self.welcome_file.absolute()}"
                logger.error(error_msg)
            ...
            if not content.strip():
                error_msg = f"Файл приветствия пуст: {self.welcome_file.absolute()}"
                logger.error(error_msg)
                raise ValueError(error_msg)
    ...
    async def _load_file(self, filename: str) -> str:
        ...
        try:
            if not file_path.exists():
                error_msg = f"Файл промпта не найден: {file_path.absolute()}"
                logger.error(error_msg)
                raise FileNotFoundError(error_msg)
            ...
            logger.error(f"Ошибка чтения файла {file_path.absolute()}: {e}")
            raise
    ...
    def get_prompt_info(self) -> Dict[str, any]:
        ...
        return {
            'prompts_dir': str(self.prompts_dir.absolute()),

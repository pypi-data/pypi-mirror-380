...
class IAMClient:
    ...
    def __init__(
        ...
        Args:
            ...
            app_slug: Application slug (required for authentication operations)
            org_slug: Organization slug (required for authentication operations)
        ...
        self._last_token_introspection: Optional[Dict[str, Any]] = None
    ...
    async def authenticate(
        ...
        Args:
            ...
            app_slug: Application slug (uses client's app_slug if not provided)
            organization_slug: Organization slug (uses client's org_slug if not provided)

        Returns:
            TokenInfo: Authentication token information

        Raises:
            ValueError: If required slugs are not provided and not set in client
            AuthenticationError: If authentication fails
        """
        # Use provided slugs or fall back to client configuration
        final_app_slug = app_slug or self.app_slug
        final_org_slug = organization_slug or self.org_slug

        if not final_app_slug:
            raise ValueError(
                "app_slug is required for authentication - provide it as parameter or set in client"
            )
        if not final_org_slug:
            raise ValueError(
                "organization_slug is required for authentication - provide it as parameter or set in client"
            )

        data = {
            "identifier": username,
            "password": password,
            "app_slug": final_app_slug if final_app_slug else self.app_id,
            "organization_slug": final_org_slug if final_org_slug else self.org_id,
        }
    ...
    async def validate_token(self, token: str) -> UserResponse:
        ...
        if not token:
            raise AuthenticationError("Token is required for validation")

        await self._ensure_client()

        introspection_payload: Dict[str, Any] = {"token": token}
        if self.app_slug or self.app_id:
            introspection_payload["app_slug"] = self.app_slug or self.app_id
        if self.org_slug or self.org_id:
            introspection_payload["organization_slug"] = self.org_slug or self.org_id

        introspect_url = urljoin(self.base_url + "/", "auth/me")

        try:
            response = await self._client.post(
                introspect_url, json=introspection_payload
            )
        except httpx.TimeoutException as exc:
            raise IAMTimeoutError("IAM token introspection timed out") from exc
        except httpx.RequestError as exc:
            raise IAMConnectionError("Failed to connect to IAM service") from exc
        ...
        introspection = response.json() if response.content else {}
        if not introspection.get("active"):
            raise AuthenticationError("Token is inactive")

        user_id = introspection.get("user_id") or introspection.get("subject")
        if not user_id:
            raise AuthenticationError(
                "Token validation response missing user identifier"
            )

        self._last_token_introspection = introspection

        user_url = urljoin(self.base_url + "/", f"users/{user_id}")
        headers = {"Authorization": f"Bearer {token}"}

        try:
            user_response = await self._client.get(user_url, headers=headers)
        except httpx.TimeoutException as exc:
            raise IAMTimeoutError("Fetching IAM user info timed out") from exc
        except httpx.RequestError as exc:
            raise IAMConnectionError("Failed to fetch user info from IAM") from exc

        if user_response.status_code == 200:
            return UserResponse(**user_response.json())
        if user_response.status_code in (401, 403):
            raise AuthenticationError("Unauthorized to fetch user information")
        if user_response.status_code == 404:
            raise AuthenticationError("User not found in IAM")

        raise IAMError(f"Failed to fetch user information: {user_response.status_code}")
    ...
    @property
    def last_token_introspection(self) -> Optional[Dict[str, Any]]:
        """Return the most recent token introspection payload."""
        return self._last_token_introspection

...
class Message(models.Model):
    ...
    def _process_chain_with_smart_tool_grouping(self, chain, msg_to_dict):
        """
        Process message chain with smart tool call grouping.
        Groups parallel tool calls (same reply_to_message_id) into single LLM messages.
        Everything works as before except when encountering tool_call messages with siblings.
        """
        messages = []
        i = 0
        ...
        while i < len(chain):
            ...
            if msg.media_type == 'tool_call':
                # Look for sibling tool calls (same reply_to_message_id)
                siblings = []
                reply_to_id = msg.reply_to_message_id

                if reply_to_id:
                    # Find all tool_call messages in the chain that reply to the same message
                    for j in range(i, len(chain)):
                        chain_msg = chain[j]
                        if (chain_msg.media_type == 'tool_call' and
                            chain_msg.reply_to_message_id == reply_to_id):
                            siblings.append(chain_msg)

                # If we have multiple siblings, group them
                if len(siblings) > 1:
                    tool_calls_list = []
                    for sibling in siblings:
                        tool_call_data = sibling.raw.get('tool_call', {})
                        ...
                        tool_calls_list.append({
                            "id": tool_call_data.get('id', f"call_{sibling.id}"),
                            "type": "function",
                            "function": {
                    ...
                    # Create grouped assistant message with all parallel tool calls
                    grouped_msg = {
                        "role": "assistant",
                        "content": None,
                    ...
                    # Skip ahead past all the sibling tool calls
                    sibling_ids = {s.id for s in siblings}
                    while i < len(chain) and chain[i].id in sibling_ids:
                        i += 1
                    continue
                else:
                    # Single tool call, process normally
                    messages.append(msg_to_dict(msg))
            else:
                # Regular message (including tool_response) - process normally
                messages.append(msg_to_dict(msg))
    ...
    def as_llm_chat(self, depth=10, mode="chat", system_instruction=None, multimodal=True):
        ...
        if mode == "chat":
            ...
            messages = self._process_chain_with_smart_tool_grouping(selected, msg_to_dict)
        elif mode == "thread":
            chain = []
            cur = self
            ...
            messages = self._process_chain_with_smart_tool_grouping(chain, msg_to_dict)
        else:

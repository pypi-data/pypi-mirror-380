...
def chat_history_view(request, chat_id):
    ...
    for message in message_list:
        try:
            message.llm_chat_json = json.dumps(message.as_llm_chat(mode='chat'), indent=2, ensure_ascii=False)
            message.llm_thread_json = json.dumps(message.as_llm_chat(mode='thread'), indent=2, ensure_ascii=False)
        except Exception:

...
class Message(models.Model):
        ...
        while i < len(chain):
            ...
            if (msg.is_outgoing and msg.response_to_tool_call and 
                ...
                if msg.media_type not in ['tool_call', 'tool_response']:
                    print(f"DEBUG: Adding regular message {msg.id} to output")
                    messages.append(msg_to_dict(msg))
                else:
                    print(f"DEBUG: Skipping {msg.media_type} message {msg.id}")
    ...
    def as_llm_chat(self, depth=10, mode="chat", system_instruction=None, multimodal=True):
        ...
        if mode == "chat":
            ...
            chain.reverse()
            # For thread mode, use simpler processing - include all non-tool messages
            messages = []
            for msg in chain:
                if msg.media_type not in ['tool_call', 'tool_response']:
                    messages.append(msg_to_dict(msg))

...
class ScraperClient:
    ...
    @limiter
    async def get_BS4(self, domain: str, url: URL, client_session: ClientSession, headers_inc: Optional[Dict] = None) -> BeautifulSoup:
    ...
    @limiter
    async def get_name_from_url(self, domain: str, url: URL, client_session: ClientSession, headers_inc: Optional[Dict] = None) -> str:
        headers = {**self._headers, **headers_inc} if headers_inc else self._headers
        async with client_session.get(url, headers=headers, ssl=self.client_manager.ssl_context,
                                      proxy=self.client_manager.proxy) as response:
            try:
    ...
    async def get_json(self, domain: str, url: URL, params: Optional[Dict] = None, headers_inc: Optional[Dict] = None, client_session: ClientSession = None) -> Dict:
        ...
        async with client_session.get(url, headers=headers, ssl=self.client_manager.ssl_context,
            ...
            content_type = response.headers.get('Content-Type')
            assert content_type is not None
            if  'json' not in content_type.lower() and 'kemono' not in response.url.host :
    ...
    @limiter
    async def get_text(self, domain: str, url: URL, client_session: ClientSession, headers_inc: Optional[Dict] = None) -> str:

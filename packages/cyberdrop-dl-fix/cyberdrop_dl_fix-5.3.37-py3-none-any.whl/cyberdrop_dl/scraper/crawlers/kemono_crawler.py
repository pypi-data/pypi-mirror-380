...
class KemonoCrawler(Crawler):
    def __init__(self, manager: Manager):
        ...
        # Custom headers for kemono requests
        self.kemono_headers = {"Accept": "text/css"}
    ...
    async def profile(self, scrape_item: ScrapeItem) -> None:
        ...
        api_call = self.api_url / service / "user" / user/ "posts"
        while True:
            async with self.request_limiter:
                JSON_Resp = await self.client.get_json(self.domain, api_call.with_query({"o": offset}), headers_inc=self.kemono_headers)
                offset += 50
                if not JSON_Resp:
                    break
    ...
    async def post(self, scrape_item: ScrapeItem) -> None:
        ...
        async with self.request_limiter:
            post = await self.client.get_json(self.domain, api_call, headers_inc=self.kemono_headers)
        await self.handle_post_content(scrape_item, post, user, user_str)
    ...
    async def get_user_str_from_post(self, scrape_item: ScrapeItem) -> str:
        ...
        async with self.request_limiter:
            soup = await self.client.get_BS4(self.domain, scrape_item.url, headers_inc=self.kemono_headers)
    ...
    async def get_user_str_from_profile_api(self, url: str) -> str:
        ...
        async with self.request_limiter:
            name = await self.client.get_name_from_url(self.domain, url, headers_inc=self.kemono_headers)
            return name

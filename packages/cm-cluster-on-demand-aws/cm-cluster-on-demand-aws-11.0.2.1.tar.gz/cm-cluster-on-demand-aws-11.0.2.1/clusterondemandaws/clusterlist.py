    ...
    def run(self):
        ...
        for cluster in clusters:
            ...
            if cluster.is_ha:
                ...
                def is_active_headnode(head_node):
                    return head_node == cluster.active_head_node

                row += ["\n".join(
                    [
                        cluster.primary_head_node.id
                        + (" (A)" if cluster.primary_head_node else "missing")
                        + (" *" if is_active_headnode(cluster.primary_head_node) else "")
                    ]
                    +
                    [
                        cluster.secondary_head_node.id
                        + (" (B)" if cluster.secondary_head_node else "missing")

...
class TemporalFeatures1D(ConvBase):
    ...
    def forward(self, x: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        wvf_k = self.proj_wvf(F.selu(fwd) * bwd.tanh())
        wvb_k = self.proj_wvb(F.celu(fwd) * bwd.cos())
        wv_fb_g = fwd.sigmoid() * bwd.tanh()
        wv_o = torch.cat([wvf_k, wvb_k, wv_fb_g], dim=1)
        return self.proj_o(wv_o)


# Similar to attention but its another thing
class ConvUndefAttn(Model):
    def __init__(
        self, feature_size: int, d_model: int = 128, hidden_channels: int = 128
    ):
        super().__init__()
        ...
        self.qW = nn.Conv1d(d_model, feature_size, 1)
        self.kW = nn.Conv1d(d_model, feature_size, 1)
        self.vV = nn.Conv1d(d_model, feature_size, 1)
        self.oW = nn.Linear(feature_size, feature_size)
        self.init_linear()
        self.init_convs()

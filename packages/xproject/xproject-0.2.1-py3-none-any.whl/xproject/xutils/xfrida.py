...
class Frida:
    ...
    dir_path = os.path.join(os.path.dirname(file_path), os.path.splitext(os.path.basename(file_path))[0])
    ...
    def _create_frida_data(self) -> FridaData | None:
        try:
            ...
            pc_frida_server_xz_file_path = os.path.join(self.dir_path, frida_server_xz_file_name)

            frida_server_file_name = re.sub(r"\.xz$", "", frida_server_xz_file_name, re.DOTALL)
            pc_frida_server_file_path = os.path.join(self.dir_path, frida_server_file_name)

            mobile_frida_server_file_path = (
                self.mobile_dir_path + frida_server_file_name
                if self.mobile_dir_path.endswith("/") else
                self.mobile_dir_path + "/" + frida_server_file_name
            )

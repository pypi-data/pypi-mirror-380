...
DEFAULT_ROUTER_URL = "http://127.0.0.1:9000"
...
class RouterConfig:
    ...
    def from_payload(cls, payload: Dict[str, Any]) -> "RouterConfig":
        # Allow environment variable overrides so containers can use internal DNS
        # without requiring Router config to change (useful for compose/dev).
        return cls(
            kv_url=os.getenv("KV_URL")
            or os.getenv("KV_SERVICE_ADDRESS")
            or payload["kvUrl"],
            temporal_address=os.getenv("TEMPORAL_ADDRESS")
            or payload["temporalAddress"],
            nats_url=os.getenv("NATS_URL") or payload["natsUrl"],
            gateway_url=os.getenv("GATEWAY_URL") or payload["gatewayUrl"],
            router_url=payload.get("routerUrl")
            or os.getenv("ROUTER_URL", DEFAULT_ROUTER_URL),
        )
...
class RouterClient:
    ...
    async def refresh_now(self) -> None:
        LOGGER.info("[router-client] refresh_now() called")
        try:
            await self._refresh_snapshot(initial=False)
            LOGGER.info("[router-client] refresh_now() completed")
    ...
    async def _refresh_snapshot(self, *, initial: bool) -> None:
        LOGGER.info("[router-client] _refresh_snapshot starting (initial=%s)", initial)
        http = await self._get_http()
        LOGGER.info("[router-client] fetching config")
        config_data = await self._fetch_config(http)
        LOGGER.info("[router-client] fetching routes")
        routes_data = await self._fetch_routes(http)
        LOGGER.info("[router-client] applying snapshot")
        await self._apply_snapshot(config_data, routes_data, initial=initial)
        LOGGER.info("[router-client] _refresh_snapshot completed")
...
async def ensure_router_client(router_url: Optional[str] = None) -> RouterClient:
    ...
    router_url = router_url or os.getenv("ROUTER_URL") or DEFAULT_ROUTER_URL
    async with _router_lock:
        if _router_client is None:
            _router_client = RouterClient(router_url)

...
class EventBusSettings:
    ...
    def from_env(cls) -> "EventBusSettings":
        ...
        return cls(
            nats_url=os.getenv("EVENTBUS_NATS_URL", DEFAULT_NATS_URL),
        ...
        try:
            if getattr(self._nats, "is_connected", False):
                try:
                    ...
                    if ConnectionClosedError is None or isinstance(
    ...
    async def publish(
        ...
        *,
        ...
        if self._nats is None or not getattr(self._nats, "is_connected", False):
            raise RuntimeError("EventBus client is not connected.")

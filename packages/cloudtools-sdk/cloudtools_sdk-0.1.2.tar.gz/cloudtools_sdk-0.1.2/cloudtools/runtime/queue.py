...
DEFAULT_WORKFLOW_TIMEOUT = timedelta(minutes=5)
...
async def _workflow_run(self, payload: Any) -> Any:
    ...
    # Get timeout for this specific task
    task_name = type(self).task_name
    task_func = service.tasks.get(task_name)
    timeout = runtime.config.workflow_run_timeout  # Default timeout
    
    if task_func and hasattr(task_func, "__cloudtools_task_timeout__"):
        timeout = getattr(task_func, "__cloudtools_task_timeout__")
    
    # For task timeouts, we need to use workflow.await_condition with a timeout
    # This is a simplified approach - in practice, Temporal handles workflow timeouts
    # at the workflow execution level, not at the task level
    return await service._invoke(task_callable, payload, ctx)

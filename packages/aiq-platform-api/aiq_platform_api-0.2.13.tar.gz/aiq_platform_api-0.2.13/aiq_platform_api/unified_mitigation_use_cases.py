...
def test_get_rules_by_integration_type(client: AttackIQRestClient):
    """Test getting rules filtered by integration type."""
    # Test with the confirmed integration types from API URLs and integration-monorepo
    integration_types = ["sentinel", "chronicle", "splunk_es", "splunk", "elastic", "qradar"]

    for integration_type in integration_types:
        logger.info(f"\n--- Testing rules for integration type: {integration_type} ---")
        try:
            count = 0
            for rule in UnifiedMitigationUtils.get_rules_by_integration_type(client, integration_type, limit=3):
                count += 1
                integration_info = rule.get("integration", {})
                integration_name = integration_info.get("name", "N/A") if integration_info else "N/A"
            ...
            if count == 0:
                logger.info(f"No rules found for integration type: {integration_type}")
            else:
                logger.info(f"Total {integration_type} rules found: {count}")

        except Exception as e:
            logger.error(f"Failed to get rules for {integration_type}: {e}")
...
def run_test(choice: "TestChoice", client: AttackIQRestClient, mitigation_id: Optional[str] = None):
    ...
    test_functions = {
        ...
        TestChoice.GET_RULES_BY_INTEGRATION_TYPE: lambda: test_get_rules_by_integration_type(client),
...
if __name__ == "__main__":
    ...
    class TestChoice(Enum):
        ...
        GET_RULES_BY_INTEGRATION_TYPE = "get_rules_by_integration_type"
    ...
    choice: TestChoice = TestChoice.GET_RULES_BY_INTEGRATION_TYPE

...
class UnifiedMitigationUtils:
    ...
    @staticmethod
    def get_rules_by_integration_type(
        client: AttackIQRestClient,
        integration_type: str,
        params: Optional[Dict[str, Any]] = None,
        limit: Optional[int] = 10,
    ) -> Generator[Dict[str, Any], None, None]:
        """Get unified mitigation rules filtered by integration type."""
        integration_name = INTEGRATION_NAMES[integration_type]
        logger.info(f"Getting rules for integration type '{integration_type}' -> '{integration_name}'")
    ...
    def get_detection_results(
        ...
        endpoint = "v1/unified_mitigation_detection_results"
        if params is None:
            params = {}
        params["unified_mitigation"] = mitigation_id
        params["ordering"] = "-modified"  # Most recent first
    ...
    def get_latest_detection_result(client: AttackIQRestClient, mitigation_id: str) -> Optional[Dict[str, Any]]:
        """Get the most recent detection result for a mitigation rule."""
        results = list(UnifiedMitigationUtils.get_detection_results(client, mitigation_id, limit=1))
        if results:
            return results[0]
        return None
    ...
    def get_associated_assessment(client: AttackIQRestClient, mitigation_id: str) -> Optional[Dict[str, Any]]:
        ...
        params = {"unified_mitigation": mitigation_id}
        associations = list(UnifiedMitigationProjectUtils.list_associations(client, params=params, limit=1))

        if not associations:
            return None

        assessment_id = associations[0]["project"]
    ...
    def set_detection_status(
        ...
        Only updates the most recent detection result. If no assessment run exists,
        ...
        if not latest_result:
            logger.warning(
                ...
                "Assessment has not been run yet or rule is not associated with any assessment."
            )
            return None
        ...
        logger.info(
            f"Updating detection status for mitigation {mitigation_id}, "
            f"result {latest_result['id']}, run {latest_result.get('project_run_id')}"
        )
    ...
    def get_analyst_verdict(client: AttackIQRestClient, mitigation_id: str) -> Optional[str]:
        ...
        verdict = latest_result.get("detection_outcome")
        if verdict:

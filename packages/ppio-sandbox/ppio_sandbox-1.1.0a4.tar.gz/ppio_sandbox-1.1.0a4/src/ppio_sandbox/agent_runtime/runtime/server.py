...
class AgentRuntimeServer:
    ...
    def _normalize_ping_status(self, status_value: Any) -> PingStatus:
        """规范化 ping 状态值"""
        if isinstance(status_value, PingStatus):
            return status_value
        ...
        try:
            if self._ping_func:
                ...
                if isinstance(result, dict):
                    # 规范化状态值
                    if 'status' in result:
                        result['status'] = self._normalize_ping_status(result['status'])

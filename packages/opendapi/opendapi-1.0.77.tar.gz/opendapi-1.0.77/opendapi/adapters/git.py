...
def get_merge_base(cwd: str, current_ref: str, base_ref: str) -> str:
    ...
    # we will use the origin base_ref in case the base ref wasn't fetched in the current environment
    origin_ref = (
        base_ref if base_ref.lower().startswith("origin/") else f"origin/{base_ref}"
    )
    merge_base = (
        run_git_command(cwd, ["git", "merge-base", current_ref, origin_ref])
        .decode("utf-8")
        .strip()
    )
    logger.info("Merge base of %s and %s is %s", current_ref, origin_ref, merge_base)
    return merge_base

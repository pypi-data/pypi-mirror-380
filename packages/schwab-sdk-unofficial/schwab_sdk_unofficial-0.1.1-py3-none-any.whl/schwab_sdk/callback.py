...
<html lang="es">
  ...
  <body>
    <h1>✅ Credenciales recibidas correctamente</h1>
...
<html lang="es">
  <head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <title>Finalizando inicio de sesión…</title>
    <style>body{font-family:system-ui,Segoe UI,Roboto,Arial,sans-serif;margin:2rem;line-height:1.5}</style>
  </head>
  <body>
    <h1>Procesando los datos…</h1>
    <p>Si esta página no cierra automáticamente, ya puedes cerrarla.</p>
    <script>
      (function(){
        try {
          ...
          if (Object.keys(params).length > 0) {
            fetch(window.location.pathname, {
              ...
              document.body.innerHTML = '<h1>✅ Datos enviados. Puedes cerrar esta ventana.</h1>';
            }).catch(function(){
              document.body.innerHTML = '<h1>⚠️ No se pudieron enviar los datos automáticamente.</h1><p>Por favor, cierra esta ventana y vuelve a intentarlo.</p>';
            });
          }
        } catch (e) { /* ignore */ }
      })();
...
class _WSGIServerThread(threading.Thread):
    """Runner HTTP con apagado limpio (usado sólo si force_https=False)."""
...
class CallbackServer:
    """
    Servidor de callback OAuth con soporte de producción simple y páginas HTML personalizables.

    Flujo: start() -> wait() -> shutdown().

    wait() devuelve un dict como:
        {
          "params": {...},
          "method": "GET" | "POST",
          "path": "/callback",
        ...
        *,
        ...
        if self.force_https:
            if self.server == "gevent" and not (self.ssl_cert and self.ssl_key):
                raise RuntimeError("gevent requiere ssl_cert y ssl_key (no soporta adhoc)")
            if self.server in {"auto", "werkzeug"} and not (self.adhoc_ssl or (self.ssl_cert and self.ssl_key)):
                raise RuntimeError("HTTPS obligatorio: usa adhoc_ssl=True o provee ssl_cert y ssl_key")
    ...
    def start(self) -> None:
        """Arranca el servidor en un hilo de fondo."""
        # HTTPS branch
        ...
        # HTTP branch (solo si force_https=False)
    ...
    def wait(self, timeout: Optional[float] = 180.0) -> Optional[Dict[str, Any]]:
        """Bloquea hasta recibir el callback válido (success/error) o agotar timeout. Devuelve dict o None."""
    ...
    def _shutdown_via_request(self) -> None:
        # Llama a la ruta interna de apagado usando HTTPS/HTTP local
...
def run_callback_server(
    ...
    *,
    ...
    """
    Levanta el servidor, espera exactamente un resultado (success o error) y lo apaga.
    Devuelve dict o None al agotar el timeout.

    `server="auto"` intenta usar gevent si hay certs y la librería está instalada;
    de lo contrario cae a Werkzeug (o HTTP si `force_https=False`).
    """
    srv = CallbackServer(
        host=host,
        port=port,
...
def _cli() -> None:
    ...
    if args.ssl_cert and not args.ssl_key:
        parser.error("--ssl-key es requerido cuando se usa --ssl-cert")

    # gevent checks
    if args.server == "gevent" and not args.ssl_cert:
        parser.error("--server gevent requiere --ssl-cert y --ssl-key (no soporta adhoc)")
    if args.server == "gevent" and _GEventWSGIServer is None:
        parser.error("gevent no está instalado. Instálalo con: pip install gevent")

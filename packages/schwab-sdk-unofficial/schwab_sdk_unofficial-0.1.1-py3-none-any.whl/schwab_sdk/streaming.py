"""
Schwab SDK - Streaming (esqueleto inicial)

Admin (connect/login/logout), utilidades (subscribe/add/unsubscribe/view),
callbacks (on_data/on_response/on_notify) y resuscripción básica.

Servicios específicos se agregarán en siguientes iteraciones.
"""
...
class Streaming:
    """
    Cliente de Streaming WebSocket para Schwab.

    Foco inicial:
    - ADMIN: connect/login/logout
    - Utilidades comunes: subscribe/add/unsubscribe/view
    - Callbacks: on_data/on_response/on_notify
    - Resuscripción tras reconexión
    """
    ...
    def __init__(self, client) -> None:
        ...
        self._subscriptions: List[Dict[str, Any]] = []  # historial de comandos SUBS/ADD/VIEW
        self._last_login_payload: Optional[Dict[str, Any]] = None
        self._request_counter: int = 1

        # Streamer info (dinámico desde userPreference)
    ...
    def _ensure_streamer_info(self) -> None:
        ...
        if not self._streamer_url:
            # Fallback a constante si no disponible (no recomendado)
    ...
    def connect(self) -> None:
        if websocket is None:
            raise RuntimeError("La librería websocket-client no está instalada. 'pip install websocket-client'")

        if self._ws is not None:
            return

        self._should_run = True
        # Asegurar streamer info y URL dinámica
        ...
        def _runner() -> None:
            while self._should_run:
                try:
                    ...
                    pass
                if self._should_run:
                    time.sleep(2)  # backoff simple antes de reintentar

        self._thread = threading.Thread(target=_runner, daemon=True)
        self._thread.start()

    def wait_until_connected(self, timeout: float = 10.0) -> bool:
        """Espera a que el socket esté abierto o agote timeout."""
    ...
    def login(
        ...
        *,
        ...
        # Preparar authorization: usar access token actual si no se provee
        if authorization is None:
            token = self.client.get_access_token()
            if not token:
                # Intentar refresh inmediato y reintentar obtener token
                try:
                    refreshed = False
                    if hasattr(self.client, "refresh_token_now"):
                    ...
                    token = None
            if not token:
                raise RuntimeError("No hay access token para streaming. Ejecuta client.login() para autenticarte.")
            # WS LOGIN requiere Authorization sin prefijo Bearer
            authorization = token

        # Permitir obtener customer/correl desde userPreference si faltan
        if not (customer_id and correl_id and channel and function_id):
            # Usar streamer info cacheada
            self._ensure_streamer_info()
        ...
        if not self.wait_until_connected(10.0):
            raise RuntimeError("WebSocket no se conectó en el tiempo esperado")
        self._send(payload)
    ...
    def account_activity(self, fields: Optional[List[int]] = None, request_id: Optional[str] = None) -> None:
        if fields is None:
            fields = [0, 1, 2]
        # Obtener account hash si no está cacheado
        ...
        if not account_hash:
            try:
                ...
                account_hash = None
        if not account_hash:
            raise RuntimeError("No se pudo obtener account hash para ACCT_ACTIVITY")
        self._common_request("SUBS", "ACCT_ACTIVITY", [account_hash], fields, request_id)

    # ===== UNSUBSCRIBE HELPER POR SERVICIO =====
    ...
    def _common_request(
        ...
        # Asegurar streamer IDs
        self._ensure_streamer_info()

        # Correl id por petición
    ...
    def _send(self, payload: Dict[str, Any]) -> None:
        ...
        try:
            ...
            raise RuntimeError(f"Error enviando mensaje WS: {e}")

    def _on_open(self, _ws) -> None:  # noqa: N802 (lib externa usa nombres camelCase)
        # Al abrir, si había login previo pendiente, reenviarlo
    ...
    def _on_error(self, _ws, error) -> None:  # noqa: N802
        # Errores se reportan vía on_response si están serializados; aquí son de transporte
    ...
    def _on_message(self, _ws, message: str) -> None:  # noqa: N802
        ...
        # El servidor Schwab puede enviar diferentes estructuras
    ...
    def _resubscribe_all(self) -> None:
        # Reenviar todas las suscripciones guardadas (SUBS/ADD/VIEW)
        # Evita re-LOGOUT/LOGIN aquí; sólo reenvía comandos de mercado

        ...
        while remaining > 0:
            ...
            for i, p in enumerate(pairs):
                ...
                if not isinstance(p, dict):
                    print(f"WARNING: pairs[{i}] is {type(p)}, expected dict. Value: {p}")
                    continue
                if 'input' not in p:
                    print(f"WARNING: pairs[{i}] is missing input. Value: {p}")
                    continue
                ...
                if "output_json" in p:
                    ...
                    print(f"WARNING: pairs[{i}] is missing output. Value: {p}")
                    continue
                if inp:
                    inputs.append(inp)
                    outputs.append(out)
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument("--out-prefix", default="samples_morecategories_81920examples", help="Prefix for split files (e.g., samples_train.jsonl)")

    # Spec sources
    parser.add_argument("--specs-jsonl", default="data/specs_morecategories_81920examples.jsonl", help="Specs JSONL path (default: data/specs.jsonl)")

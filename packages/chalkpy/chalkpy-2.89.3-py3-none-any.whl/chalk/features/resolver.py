...
class StreamResolver(Resolver[P, T]):
    def __init__(
        ...
        *,
        ...
        message_producer: StreamResolverMessageProducer | None,
    ):
        super().__init__(
            function_definition=function_definition,
            function_captured_globals=function_captured_globals,
        ...
        self.message_producer_parsed: StreamResolverMessageProducerParsed | None = (
            StreamResolverMessageProducerParsed.from_unparsed(message_producer)
...
def make_stream_resolver(
    *,
    ...
    owner: Optional[str] = None,
    doc: str | None = None,
    message_producer: StreamResolverMessageProducer | None = None,
) -> StreamResolver:
    ...
    message_producer
        ...
        Read more at https://docs.chalk.ai/api-docs#StreamResolverMessageProducer
...
class StreamResolverMessageProducer:
    """Sends stream messages to a stream source of your choice.

    Messages are arrow tables serialized in IPC format with the specified output columns
    """
...
class StreamResolverMessageProducerParsed:
    ...
    @classmethod
    def from_unparsed(cls, stream_resolver_message_producer: StreamResolverMessageProducer):
        if not isinstance(
            stream_resolver_message_producer.send_to, StreamSource
        ):  # pyright: ignore[reportUnnecessaryIsInstance]
            raise ValueError("Expected a StreamSource for argument 'send_to' on StreamResolverMessageProducer")
        if not isinstance(
            stream_resolver_message_producer.output_features, collections.abc.Iterable
        ):  # pyright: ignore[reportUnnecessaryIsInstance]
            raise ValueError("Expected an iterable for argument 'output_features' on StreamResolverMessageProducer")
        output_features_str = [str(f) for f in stream_resolver_message_producer.output_features]
        if len(output_features_str) == 0:
            raise ValueError("Expected at least one output feature for StreamResolverMessageProducer")
        if stream_resolver_message_producer.feature_expressions is not None:
            if not isinstance(
                stream_resolver_message_producer.feature_expressions, collections.abc.Mapping
            ):  # pyright: ignore[reportUnnecessaryIsInstance]
                raise ValueError(
                    "Expected a mapping for argument 'feature_expressions' on StreamResolverMessageProducer"
                )
            feature_expressions_dict = {
                str(k): v for k, v in stream_resolver_message_producer.feature_expressions.items()
            }
        else:
            feature_expressions_dict = None
        return cls(
            send_to=stream_resolver_message_producer.send_to,

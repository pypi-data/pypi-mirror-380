...
#### Stability
To attempt forcing that the identified linear model is stable, i.e., that matrix $A$ has all eigenvalues inside the unit disk, there are two methods. The first method uses $A$/max(max(abs(eig($A$))),1) as the state-transition matrix and is simply used by setting the flag `stability` at construction:
...
before calling the `fit` function, which introduces instead a custom regularization penalty designed to enforce the constraint $\|A\|_2<1$.

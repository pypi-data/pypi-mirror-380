...
class SchemaFieldExtractor:
    ...
    def extract_bigquery_fields(ddl: str) -> set:
        ...
        pattern = r'\s*(\w+)\s+(?:STRING(?:\(\d+\))?|FLOAT64|INT64|NUMERIC|BOOLEAN|DATE|TIMESTAMP|TIME|JSON)'
    ...
    def extract_bigquery_field_type(ddl: str, field_name: str) -> str:
        ...
        # Use the same improved field definition extraction approach
        field_def = SchemaFieldExtractor.extract_bigquery_field_definition(ddl, field_name)
        if not field_def:
            return ""
        
        # Extract type from the field definition
        type_pattern = r'\b(STRING|FLOAT64|INT64|NUMERIC|BOOLEAN|DATE|TIMESTAMP|TIME|JSON)(?:\(\d+\))?'
        match = re.search(type_pattern, field_def, re.IGNORECASE)

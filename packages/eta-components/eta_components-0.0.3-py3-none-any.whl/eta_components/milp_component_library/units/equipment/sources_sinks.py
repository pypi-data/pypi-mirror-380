...
class _BaseSourceSink(BaseStandaloneUnit, ABC):
    ...
    def _validate_capacity(self, capacity: float | dict):
        if self._sign_of_capacity is None:
            return
        if isinstance(capacity, int | float):
            capacity = {None: capacity}

        if not all(self._sign_of_capacity(val, 0) for val in capacity.values()):
            warnings.warn(
                f"Not all values for {self.name} are {self._sign_of_capacity.__name__} than 0. Check your "

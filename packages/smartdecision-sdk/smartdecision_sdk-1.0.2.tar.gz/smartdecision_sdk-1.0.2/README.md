...
- 🐛 **Fixed Event Loop Issues**: Resolved "asyncio.run() cannot be called from running event loop" errors
- 🐛 **Fixed Event Loop Closed**: Fixed "Event loop is closed" errors when making multiple requests
- 🌐 **Updated Default Domain**: Changed default API endpoint from `api.smartdecision.ai` to `api.smartdec.ai`
- 🔧 **Improved Sync Methods**: Enhanced synchronous method handling with proper event loop management
- ✅ **Better Error Handling**: Improved error messages and event loop conflict detection
- 🧪 **Enhanced Testing**: All sync, async, and context manager functionality now works correctly

...
def rioxarray_get_projection_info(
    ...
    if src_dst.rio.crs is not None:
        # EPSG
        epsg = (
            src_dst.rio.crs.to_epsg() if src_dst.rio.crs.is_epsg_code else None
        )
...
def create_stac_item(
    ...
    with ExitStack() as ctx:
        ...
        media_type = (
            get_media_type(dataset)
            if asset_media_type == "auto"
            else asset_media_type
        )

        if (
            "start_datetime" not in properties
            and "end_datetime" not in properties
        ):

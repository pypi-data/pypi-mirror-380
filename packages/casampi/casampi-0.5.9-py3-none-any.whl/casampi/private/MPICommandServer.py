    ...
    class __MPICommandServerImpl:
        ...
        def start_virtual_frame_buffer(self):
            ...
            def run_xauth_xvfb(xauth_dir, xauthfile, port, cookie):
                ...
                # CAS-14034: Exclude LD_LIBRARY_PATH from the Xvfb call.
                # This is due to a conflict with the libgpg-error.so.0 that is provided by
                # the system installation of Xvfb and libgpg-error.so.0 packaged with Casa.
                env_no_ld_library_path = os.environ.copy()
                try:
                   del env_no_ld_library_path["LD_LIBRARY_PATH"]
                except KeyError:
                    pass
                ...
                self.__virtual_frame_buffer_process = subprocess.Popen(
                    ...
                    shell=False, env=env_no_ld_library_path)

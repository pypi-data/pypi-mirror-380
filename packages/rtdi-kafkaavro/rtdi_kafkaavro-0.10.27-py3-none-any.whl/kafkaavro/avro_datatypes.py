...
class AvroEnum(AvroPrimitive):
    ...
    def create_schema_dict(self) -> dict:
        return {"type": "enum", "logicalType": "ENUM", "symbols": self.get_symbol_json()}
...
def get_datatype(schema: dict):
    ...
    match t:
        case "record":
            r = RecordSchema(schema["name"], schema["namespace"], schema.get("doc"))
    ...
    if l is not None:
        match l:
            ...
            case "ENUM":
                return AvroEnum(schema.get("symbols")), False
            case "FIXED":
                return AvroFixed(schema.get("size")), False
            case "FLOAT":

...
class SupportsEncode(Protocol):
    ...
    def encode(self, text: str) -> np.ndarray:  # pragma: no cover - protocol definition
        ...

    def encode_batch(self, texts: Sequence[str]) -> np.ndarray:  # pragma: no cover - protocol definition
        ...
...
def _ensure_matrix(vectors: Sequence[Sequence[float]]) -> np.ndarray:
    if not vectors:
        return np.zeros((0, 0), dtype=np.float32)
...
def create_encoder(name: str, *, dim: int, model: str | None = None) -> SupportsEncode:
    ...
    if backend in {"openai", "openai-api"}:
        ...
        try:
            ...
            logger.warning("openai backend requested but 'openai' package not installed; falling back to hashing encoder")
            return HashingEncoder(dim=dim)
    if backend in {"sentence-transformer", "sentence_transformer", "hf"}:
        ...
        try:
            ...
            logger.warning(
                "sentence-transformer backend requested but dependency missing; falling back to hashing encoder"
            )
            return HashingEncoder(dim=dim)

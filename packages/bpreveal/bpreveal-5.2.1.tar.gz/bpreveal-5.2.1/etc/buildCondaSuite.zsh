...
runAndCheck ${CONDA_BIN} install --yes -c conda-forge "numpy"
...
runAndCheck ${PIP_BIN} install --no-input modisco
...
if [ "$INSTALL_DEVTOOLS" = true ] ; then
    ...
    runAndCheck ${CONDA_BIN} install --yes -c conda-forge auditwheel twine
fi

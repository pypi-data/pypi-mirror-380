...
class ManagedCursor(object):
    ...
    async def execute(self, sql: str, params: None):
        ...
        return await asyncio.get_event_loop().run_in_executor(None, lambda cursor: cursor.execute(sql, params),
                                                              self.cursor)

    async def executemany(self, sql: str, seq_of_parameters):
        return await asyncio.get_event_loop().run_in_executor(None,
                                                              lambda cursor: cursor.executemany(sql, seq_of_parameters),
                                                              self.cursor)
    ...
    async def rowcount(self):
        return await asyncio.get_event_loop().run_in_executor(None, lambda cursor: cursor.rowcount, self.cursor)

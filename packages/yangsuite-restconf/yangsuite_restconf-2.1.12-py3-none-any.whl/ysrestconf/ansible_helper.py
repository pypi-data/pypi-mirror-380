...
class Node(object):
    """Node in XML document tree, the XML tag encapsulates a value,
    or multiple values/children"""
    ...
    def __init__(self, wrap='', tag='', data=None, repeat_iterables_parent=True):
        ...
        # Wrap is the starting and ending tags for any Node element
        # and its children (if children exist)
    ...
    def sanitize_element(element):
        ...
        if element and isinstance(element, str):
            ...
            return ''.join(
                ...
                + ['_' if not (NAME_START_CHAR_RE.match(c)
                               or NAME_CHAR_RE.match(c))
                    else c
                    for c in element]
            )
...
class DictionaryToXML(object):
    ...
    def __init__(self, data):
        ...
        self.xml_str = f'''
            <data>
                {self.convert(ordereddict_data)}
            </data>
        '''

    def _join_strings(self):
        """Returns a function that given a list of strings, will return that list
        as a single, indented, string"""

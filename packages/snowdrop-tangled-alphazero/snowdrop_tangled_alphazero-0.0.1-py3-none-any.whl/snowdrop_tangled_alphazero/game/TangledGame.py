""" minimal Tangled variant to test MCTS """
import numpy as np
...
class TangledGame(Game):
    def __init__(self, graph_args, fixed_token_game):
        ...
        gp = GraphProperties()
        self.graph_properties = gp.graph_database[self.graph_number]   # todo check
        self.fixed_token_game = fixed_token_game

        data_directory = str(files('snowdrop_adjudicators').joinpath('data'))
        # script_dir = os.path.dirname(os.path.abspath(__file__))  # Get the directory of the current script
        # data_directory = os.path.join(script_dir, '..', 'data')

        args = {'data_dir': data_directory,
                'graph_number': self.graph_number,
                'epsilon': graph_args["adjudicator_args"]['epsilon']}

        # it is possible that the adjudicator_to_use passed in here is already an adjudicator and not a label.
        # todo: modify so that adjudicators are getting the correct inputs
        if isinstance(graph_args["adjudicator_to_use"], str):
            self.adjudicator = None

            if graph_args["adjudicator_to_use"] == 'simulated_annealing':
                ...
                if graph_args["adjudicator_to_use"] == 'quantum_annealing':
                    ...
                    if graph_args["adjudicator_to_use"] == 'lookup_table':
                        self.adjudicator = LookupTableAdjudicator()
                    else:
                        if graph_args["adjudicator_to_use"] == 'schrodinger_equation':
            ...
            self.adjudicator = graph_args["adjudicator_to_use"]     # todo: check is this correct?
    ...
    def getGameEnded(self, board, player):
        ...
        if selected_edges.count(0) == 0 and 1 in vertex_states and -1 in vertex_states:
            ...
            # todo: change so that all qa samples are saved in a LUT; I think this is the only place adjudicate is called

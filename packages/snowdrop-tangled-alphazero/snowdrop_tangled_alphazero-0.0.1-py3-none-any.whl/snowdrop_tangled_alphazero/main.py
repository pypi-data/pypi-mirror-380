...
# for paper, only change graph_number * keep all other params the same *
# make sure your neural net is the right size in AlphaZeroNet.py
...
def main():
    ...
    graph_args : dict[str, Any] = {
        'graph_number': 11,   # 11 P_3, 2 K_3, 20 diamond, 19 barbell, 18 3-prism, 12 moser, 24 C_60
        'adjudicator_to_use': 'lookup_table'} # ['lookup_table', 'simulated_annealing', 'quantum_annealing']
    ...
    if graph_args["adjudicator_to_use"] in ['lookup_table']:
    ...
    args = {
        ...
        'numIters': 3,    # number of self-play/nn training/competition cycles
        'numEps': 3200,     # Number of complete self-play games to simulate during a new iteration; number of training
        ...
        'numMCTSSimsComp': 5,  # Moves for MCTS to simulate during competitive play
        ...
        'arenaCompare': 100,  # Half the number of games to play during arena play to determine if new net will be
    ...
    args["checkpoints"] += graph_args["adjudicator_to_use"] + '/'

    if graph_args["adjudicator_to_use"] in ['lookup_table']:
        args["checkpoints"] += lookup_args["solver"] + '/'

    if graph_args["adjudicator_to_use"] in ['quantum_annealing']:
    ...
    if graph_args["adjudicator_to_use"] in ['simulated_annealing']:
    ...
    if args["load_model"]:
        ...
        model = AlphaZeroNet(board_size=graph_args["get_board_size"], action_size=graph_args["get_action_size"])
    ...
    for i in range(1, args["numIters"] + 1):
        ...
        if graph_args["adjudicator_to_use"] == "quantum_annealing":

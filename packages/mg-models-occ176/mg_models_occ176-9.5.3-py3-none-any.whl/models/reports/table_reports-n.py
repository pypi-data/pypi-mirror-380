...
def affid_report(db, group_by=[], filters={}, reporting_query=False, order_by=False, inc_test_cc=False, **kwargs):
    ...
    qry = f"""
          ...
          FROM ( SELECT
                ...
                sum(CASE
                        WHEN a.decline_reason IS NULL AND bc_inferred = '0' /*AND pp_customer = '0'*/ THEN order_total
                ...
                count(CASE
                          WHEN attempt_count = '0' AND bc_inferred = '1' /*AND not cap_fill*/ AND a.decline_reason IS NULL /*AND pp_customer = '0'*/
                              THEN 1 END)                      AS "natural_approval",

                count(CASE
                          WHEN attempt_count = '0' AND bc_inferred = '1' AND a.decline_reason IS NULL /*AND pp_customer = '0'*/
...
def approval_report(db, group_by=[], filters={}, reporting_query=False, order_by=False, inc_test_cc=False, **kwargs):
    ...
    sdate, edate = reporting_query['start_date'], reporting_query['end_date']
    qry = f""" select * from(
        select e.corporation_name,  d.processor,  c.mid_number, b.cc_type,
...
from (select crm_id, order_id, retry_attempt_count as attempt, decline_reason, bc_inferred
      ...
      where month_date >= '{sdate}'::date
        and month_date <= '{edate}'::date) a
         ...
         inner join (select gateway_id, mid_number, mid_id, crm_id from ui_54407332_clients.steps) c
        ...
        group by e.corporation_name, d.processor,  c.mid_number, b.cc_type) a
        {f_whr}

...
class KCL:
    ...
    def search_block(self, point):
        ...
        """
        block_start = self._block_data_offset
        block_data = self._data[block_start:]
        
        px, py, pz = point
        minx, miny, minz = self._area_min_pos

        x = int(px - minx)
        if (x & self._area_x_width_mask) != 0:
            return None

        y = int(py - miny)
        if (y & self._area_y_width_mask) != 0:
            return None

        z = int(pz - minz)
        if (z & self._area_z_width_mask) != 0:
            return None

        # initialize root
        shift = self._block_width_shift
        cur_block_offset = 0  # root at start of block_data

        index = 4 * (((z >> shift) << self._area_xy_blocks_shift)
            | ((y >> shift) << self._area_x_blocks_shift)
                    | (x >> shift))

...
# 自定义带 TCP Keep-Alive 的适配器，开启池阻塞避免过量新建连接
# 直接定义最小 socket 选项，避免依赖 urllib3 内部常量导致兼容性问题
...
class KeepAliveHTTPAdapter(requests.adapters.HTTPAdapter):
    ...
    def init_poolmanager(self, connections, maxsize, block=False, **pool_kwargs):
        pool_kwargs["socket_options"] = KEEPALIVE_SOCKET_OPTIONS
        return super().init_poolmanager(connections, maxsize, block=block, **pool_kwargs)

    def proxy_manager_for(self, proxy, **proxy_kwargs):
        proxy_kwargs["socket_options"] = KEEPALIVE_SOCKET_OPTIONS
...
# 配置连接池与 Keep-Alive 行为
adapter = KeepAliveHTTPAdapter()
session.mount('http://', adapter)
session.mount('https://', adapter)

# 设置默认请求头，显式启用 Keep-Alive 与压缩
...
class pro_api:
    ...
    def query(self,api_name, fields='', **kwargs):
        ...
        with session.post(url, json=params, timeout=DEFAULT_TIMEOUT) as response:

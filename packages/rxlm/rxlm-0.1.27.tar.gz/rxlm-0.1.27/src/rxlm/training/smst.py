...
class SupervisedMemoryAwareTrainer(BaseTrainer):
                    ...
                    for inner_step_idx in range(number_of_inner_steps):
                        ...
                        if self.use_amp:
                            with torch.amp.autocast(device_type=self.device.type, dtype=self.dtype):
                                ...
                                loss, _ = self.compute_loss(train_batch, query_lens=query_lens)
                        else:
                            train_batch = {
                                'prev': smart_concat(prev_query, prev_answer, max_length=self.max_seq_len,
                                                     pad_token_id=self.pad_token_id),
    ...
    def compute_loss(self, batch: dict[str, dict[str, torch.Tensor]], query_lens: torch.Tensor = None) -> tuple[torch.Tensor, torch.Tensor]:

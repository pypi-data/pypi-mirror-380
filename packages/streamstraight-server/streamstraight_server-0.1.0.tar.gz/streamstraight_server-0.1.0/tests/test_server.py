...
from streamstraight_python.constants import get_package_version
from streamstraight_python import ServerOptionsDict, StreamOptionsDict
from streamstraight_python.protocol import (
    ProducerClientToServerEvents,
    ProducerServerToClientEvents,
)
from streamstraight_python.server import StreamstraightServer, StreamstraightServerError
...
def patch_socketio(monkeypatch):
    ...
    monkeypatch.setattr(
        "streamstraight_python.server.socketio.AsyncClient",
        lambda: fake_client,
    )
    return fake_client
...
async def test_stream_sends_chunks_and_end(patch_socketio):
    ...
    chunk_events = [event for event in fake_client.emitted if event[0] == ProducerClientToServerEvents.CHUNK]

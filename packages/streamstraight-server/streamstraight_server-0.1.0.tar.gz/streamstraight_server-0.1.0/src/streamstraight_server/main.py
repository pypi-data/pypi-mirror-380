...
async def streamstraight_server(
    ...
    normalized_server = normalize_server_options(server_options)
    normalized_stream = normalize_stream_options(stream_options)

    server: StreamstraightServer[C] = StreamstraightServer(normalized_server)
    await server.connect(normalized_stream)
    return server

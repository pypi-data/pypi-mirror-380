...
@patch('cat_win.src.service.diffviewer.DiffViewer.getxymax', getxymax)
@patch('cat_win.src.service.diffviewer.curses', mm)
...
class TestEditor(TestCase):
    ...
    @patch('cat_win.src.service.diffviewer.DiffViewer._setup_file', lambda *args: None)
    def test__open(self):
        diffviewer = DiffViewer([__file__, __file__], [__file__, __file__])
        diffviewer.curse_window = MagicMock()
        with patch('cat_win.src.service.diffviewer.DiffViewer._run', lambda *args: None):
            self.assertEqual(diffviewer._open(), None)

    @patch('cat_win.src.service.diffviewer.CURSES_MODULE_ERROR', new=True)
    @patch('cat_win.src.service.diffviewer.on_windows_os', new=True)
    def test_diffviewer_no_curses_error(self):
        with patch('sys.stderr', new=StdOutMock()) as fake_out:
            DiffViewer.open([], [])
    ...
    def test_diffviewer_set_flags(self):
        backup_a = DiffViewer.debug_mode
        backup_b = DiffViewer.file_encoding
        DiffViewer.set_flags(True, 'utf-16')
        self.assertEqual(DiffViewer.debug_mode, True)
        self.assertEqual(DiffViewer.file_encoding, 'utf-16')
        DiffViewer.set_flags(backup_a, backup_b)

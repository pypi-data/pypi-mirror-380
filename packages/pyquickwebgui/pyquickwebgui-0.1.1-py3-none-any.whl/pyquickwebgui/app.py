...
class QuikeUI:
    ...
    Attributes:
        server_type (Union[str, Server_type_enum, Callable[[Any], None]]): 服务器类型或可调用对象，默认使用 FastAPI。
    ...
    server_type: Union[str, Server_type_enum, Callable[[Any], None]] = Server_type_enum.FASTAPI.value
    ...
    def __post_init__(self):
        ...
        # 枚举转换字符串值
        if isinstance(self.server_type, Server_type_enum):
            self.server_type = self.server_type.value
        ...
        if self.port is None:
            self.port = (
                ...
                else self.config.get_free_port()
            )

        # 更新全局变量，记录当前使用的端口
        self.config.FLASKWEBGUI_USED_PORT = self.port

        # 如果服务器参数为字符串，则通过调度器获取默认服务器配置
        if isinstance(self.server_type, str):
            default_server = self.config.webserver_dispacher[self.server_type]
    ...
    def load_rootplugins(self):
        ...
        rootplugins = []
        ROOTPLUGINS_DIR = os.path.join(current_dir, self.config.ROOTPLUGINS_DIR)
        ...
        for filename in os.listdir(ROOTPLUGINS_DIR):
            if filename.endswith(".py"):
                ...
                for item_name in dir(module):
                    if str(item_name).lower().find(self.config.PLUGIN_CLASS_NAME) > -1 and \
                            str(item_name).lower().find(self.config.BASE_PLUGIN_CLASS_NAME) == -1:
    ...
    def load_plugins(self):
        ...
        for filename in os.listdir(self.config.PLUGINS_DIR):
            if filename.endswith(".py"):
                path = os.path.join(self.config.PLUGINS_DIR, filename)
                name = filename[:-3]
                spec = importlib.util.spec_from_file_location(name, path)
                module = importlib.util.module_from_spec(spec)
                spec.loader.exec_module(module)
    ...
    def run_server(self):
        ...
        启动服务器进程。
        """
        self.log.info("=============>启动服务器进程 server_type={}".format(self.server_type))
        try:
            ...
            self.log.error("run_server error:{}".format(e) , exc_info = True )
    ...
    def run_browser(self):
        ...
        if self.only_webserver:
            return
        self.log.info("=============>启动浏览器进程 browser_type={}".format(self.browser_type))

        _browser_cls =  self.config.browser_dispacher[self.browser_type]
        self.browser_obj = _browser_cls(log=self.log)
        view_kwargs = self.browser_obj.get_view_kwargs(self.server_kwargs)
        self.browser_obj.view(self ,view_kwargs)

...
from sifts.analysis.code_parser import (
    ...
    _extract_code_and_imports,
    analyze_method_node,
    get_embedding_predictions_batch,
...
async def analyze_project(
    *,
    ...
    # Pre-calculate snippet hashes for batch query
    snippet_hashes = []
    for where, method_node in function_pairs:
        # Extract code to calculate hash (similar to _prepare_snippet logic)
        ...
        if extraction is not None:
            ...
            snippet_hashes.append(snippet_hash)

    # Get embedding predictions in batch
    embedding_predictions = await get_embedding_predictions_batch(
        context=context,
        snippet_hashes=snippet_hashes,
    )

    LOGGER.info(
        "Found predictions for %s out of %s snippets",
        len(embedding_predictions),
        len(snippet_hashes),
    )
    if not embedding_predictions:
        return
...
async def analyze_prompt(
    ...
    for node in await get_valid_functions(
        ...
        async for result in analyze_method_node(
            ...
            config=config,
            db_backend=db_backend,
            embedding_predictions={},  # Empty dict for prompt analysis
        ):
            if result is not None:
                yield result

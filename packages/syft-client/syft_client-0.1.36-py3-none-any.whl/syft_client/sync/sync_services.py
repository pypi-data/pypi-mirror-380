...
class SyncServiceManager:
    ...
    def _check_and_link_watcher(self):
        ...
        server_name = f"watcher_sender_{self.client.email.replace('@', '_').replace('.', '_')}"
    ...
    def _check_and_link_receiver(self):
        ...
        # Create expected server name
        server_name = f"receiver_{self.client.email.replace('@', '_').replace('.', '_')}"
    ...
    def ensure_receiver_running(self, verbose: bool = False) -> bool:
        ...
        try:
            ...
            # Get auto_accept setting from client
            auto_accept = getattr(self.client, 'accept_requests', True)
            
            # Create and start receiver
            server = create_receiver_endpoint(
                self.client.email, 
                verbose=verbose,
    ...
    def stop_watcher(self, verbose: bool = True) -> bool:
        ...
        if self._watcher:
            try:
                from ..sync.watcher import destroy_watcher_endpoint
                
                # Use the proper destroy function
                destroy_watcher_endpoint(self.client.email, verbose=verbose)
    ...
    def stop_receiver(self, verbose: bool = True) -> bool:
        ...
        if self._receiver:
            try:
                from ..sync.receiver import destroy_receiver_endpoint
                
                # Use the proper destroy function
                destroy_receiver_endpoint(self.client.email, verbose=verbose)

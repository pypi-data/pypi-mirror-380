...
class GDriveFilesTransport(BaseTransportLayer, BaseTransport):
    ...
    def remove_peer(self, email: str, verbose: bool = True) -> bool:
        ...
        try:
            ...
            # If no permissions were revoked but folders exist, try to delete them
            if folders_processed == 0:
                ...
                if outbox_id:
                    try:
                        ...
                        if file_info.get('ownedByMe', False):
                            # Move to trash
                            self.drive_service.files().update(fileId=outbox_id, body={'trashed': True}).execute()
                            folders_deleted += 1
                            if verbose:
                ...
                if archive_id:
                    try:
                        ...
                        if file_info.get('ownedByMe', False):
                            # Move to trash
                            self.drive_service.files().update(fileId=archive_id, body={'trashed': True}).execute()
                            folders_deleted += 1
                            if verbose:
                ...
                if pending_id:
                    try:
                        ...
                        if file_info.get('ownedByMe', False):
                            # Move to trash
                            self.drive_service.files().update(fileId=pending_id, body={'trashed': True}).execute()
                            folders_deleted += 1
                            if verbose:

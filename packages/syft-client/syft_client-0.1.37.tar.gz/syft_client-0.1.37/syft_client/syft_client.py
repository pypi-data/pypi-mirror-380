...
class SyftClient:
    ...
    def _login(self, provider: Optional[str] = None, verbose: bool = False, init_transport: bool = True, wizard: Optional[bool] = None, accept_requests: Optional[bool] = None) -> None:
        """
        ...
        Args:
            ...
            accept_requests: Whether to auto-accept peer requests (None=prompt, True=accept, False=skip)
        ...
        try:
            ...
            try:
                ...
                if hasattr(self, 'sync') and hasattr(self.sync, 'peers_manager'):
                    ...
                    # Handle accept_requests based on value
                    if unique_peer_emails:
                        # If accept_requests is None, prompt the user
                        if accept_requests is None:
                            # Clear progress line and prompt
                            sys.stdout.write(f"\r{' ' * 80}\r")
                            sys.stdout.flush()
                            
                            print(f"\n📬 You have {len(unique_peer_emails)} pending peer request{'s' if len(unique_peer_emails) != 1 else ''}:")
                            for email in sorted(unique_peer_emails):
                                print(f"   • {email}")
                            
                            # Prompt with timeout (Unix-like systems only)
                            import platform
                            
                            if platform.system() != 'Windows':
                                # Unix-like systems support signal.alarm
                                import signal
                                ...
                                try:
                                    response = input("\nAccept all requests? (y/N) [5s timeout]: ").strip().lower()
                                    signal.alarm(0)  # Cancel the alarm
                                    accept_requests = response in ['y', 'yes']
                                except (TimeoutError, EOFError):
                                    signal.alarm(0)  # Cancel the alarm
                                    print("\n⏰ Timed out - not accepting requests")
                                    accept_requests = False
                                except KeyboardInterrupt:
                                    signal.alarm(0)  # Cancel the alarm
                                    print("\n❌ Cancelled - not accepting requests")
                                    accept_requests = False
                            else:
                                # Windows doesn't support signal.alarm, just use regular input
                                try:
                                    response = input("\nAccept all requests? (y/N): ").strip().lower()
                                    accept_requests = response in ['y', 'yes']
    ...
    def login(email: Optional[str] = None, provider: Optional[str] = None, 
              ...
              wizard: Optional[bool] = None, accept_requests: Optional[bool] = None, **kwargs) -> 'SyftClient':
        """
        ...
        Args:
            ...
            accept_requests: If True, automatically accept all pending peer requests. If False, skip. If None (default), prompts user when requests exist.

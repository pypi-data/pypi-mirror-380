...
class Peer:
    ...
    def delete(self, verbose: bool = True) -> bool:
        """
        Delete this peer from all transports
        
        Args:
            verbose: Whether to print progress
            
        Returns:
            True if peer was deleted successfully
        """
        if not hasattr(self, '_client') or not self._client:
            raise ValueError("Peer must be accessed through client.peers to use delete()")
        
        # Call the peer manager's remove_peer method
        return self._client.sync.peers_manager.remove_peer(self.email)

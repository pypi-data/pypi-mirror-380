...
class GDriveFilesTransport(BaseTransportLayer, BaseTransport):
    ...
    def remove_peer(self, email: str, verbose: bool = True) -> bool:
        ...
        try:
            ...
            # Always try to delete folders we own
            folders_deleted = 0
            
            # Try to delete outbox folder if we own it
            if outbox_id:
                try:
                    # Check if we own this folder
                    file_info = self.drive_service.files().get(fileId=outbox_id, fields="ownedByMe").execute()
                    if file_info.get('ownedByMe', False):
                        # Permanently delete the folder
                        self.drive_service.files().delete(fileId=outbox_id).execute()
                        folders_deleted += 1
                        if verbose:
                            print(f"✅ Deleted outbox folder for {email}")
                except:
                    pass
                
                # Try to delete archive folder if we own it
                if archive_id:
                    try:
                        # Check if we own this folder
                        file_info = self.drive_service.files().get(fileId=archive_id, fields="ownedByMe").execute()
                        if file_info.get('ownedByMe', False):
                            # Permanently delete the folder
                            self.drive_service.files().delete(fileId=archive_id).execute()
                            folders_deleted += 1
                            if verbose:
                                print(f"✅ Deleted archive folder for {email}")
                    except:
                        pass
                
                # Also try to delete pending folder
                ...
                if pending_id:
                    try:
                        # Check if we own this folder
                        file_info = self.drive_service.files().get(fileId=pending_id, fields="ownedByMe").execute()
                        if file_info.get('ownedByMe', False):
                            # Permanently delete the folder
                            self.drive_service.files().delete(fileId=pending_id).execute()
                            folders_deleted += 1
                            if verbose:
                                print(f"✅ Deleted pending folder for {email}")
                    except:
                        pass
                
            # Report results
            total_actions = folders_processed + folders_deleted
            if total_actions > 0:
                if verbose:
                    if folders_processed > 0 and folders_deleted > 0:
                        print(f"✅ Removed {email} from contacts (revoked {folders_processed} permission(s), deleted {folders_deleted} folder(s))")
                    elif folders_processed > 0:
                        print(f"✅ Removed {email} from contacts (revoked access to {folders_processed} folder(s))")
                    else:
                        print(f"✅ Removed {email} from contacts (deleted {folders_deleted} folder(s))")
                return True
            else:
                if verbose:
    ...
    def list_peers(self) -> List[str]:
        ...
        try:
            ...
            # Query for folders matching our pattern that WE own
            query = f"name contains '{prefix}' and name contains '{suffix}' and '{self._syftbox_folder_id}' in parents and mimeType='application/vnd.google-apps.folder' and trashed=false and 'me' in owners"
            results = self.drive_service.files().list(
                q=query,

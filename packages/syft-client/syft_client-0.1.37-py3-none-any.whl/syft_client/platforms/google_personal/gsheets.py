    ...
    def remove_peer(self, email: str, verbose: bool = True) -> bool:
        ...
        try:
            ...
            if sheet_id and self.drive_service:
                ...
                # Also try to delete the sheet if we own it
                try:
                    # Check if we own this sheet
                    file_info = self.drive_service.files().get(fileId=sheet_id, fields="ownedByMe").execute()
                    if file_info.get('ownedByMe', False):
                        # Permanently delete the sheet
                        self.drive_service.files().delete(fileId=sheet_id).execute()
                        removed = True
                        if verbose:
                            print(f"✅ Deleted message sheet for {email}")
                except:
                    pass
        ...
        try:
            ...
            for file in results.get('files', []):
                ...
                if len(parts) == 2 and parts[1].endswith('_messages'):
                    ...
                    # First replace _at_ with @, then only replace remaining underscores with dots
                    their_email = their_email.replace('_at_', '@')
                    # Now replace underscores with dots, but only for the domain part
                    if '@' in their_email:
                        local, domain = their_email.split('@', 1)
                        domain = domain.replace('_', '.')
                        their_email = f"{local}@{domain}"
                    contacts.add(their_email)
            
            # Don't include incoming sheets - those are peer requests, not established peers

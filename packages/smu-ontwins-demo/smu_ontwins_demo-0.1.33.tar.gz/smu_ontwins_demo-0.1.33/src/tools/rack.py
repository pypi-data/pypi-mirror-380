...
class Racks:
    ...
    def _get_cell_position(self, rack: int, level: int, cell: int):
        idx = (cell-1) + (self.cfg.cells_per_level * (level-1)) + (self.cfg.cells_per_level * self.cfg.cells_per_level * (rack-1))
        position = self.cfg.rack_entities[idx]["properties"]["position"] if self.cfg.rack_entities[idx] else [0, 0, 0]
        return position

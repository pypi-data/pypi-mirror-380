...
def _spark_asof_join_features_tempo(
    ...
    # 为特征表的列添加别名以避免列名冲突
    feature_df_aliased = feature_df.select(
        *[F.col(col).alias(f"feature_{col}") for col in feature_df.columns]
    )
    
    # 1. 只保留键列和时间戳列
    df_keys = df.select(lookup_keys + [timestamp_key])
    feature_keys = feature_df_aliased.select(
        [F.col(f"feature_{k}").alias(k) for k in lookup_keys] + 
        [F.col(f"feature_{timestamp_key}").alias(timestamp_key)]
    )

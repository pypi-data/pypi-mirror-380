...
class FeatureStoreClient:
    ...
    def __init__(self, spark: Optional[SparkSession] = None, online_config: Optional[RedisStoreConfig] = None):
        """
        :param spark: 已初始化的SparkSession对象
        :param online_config: 在线特征表配置
        """
        if spark is None:
            spark = SparkSession.builder.getOrCreate()
        self._spark = spark
        self._spark_client = SparkClient(spark)
        self._feature_table_client = FeatureTableClient(spark, online_config=online_config)

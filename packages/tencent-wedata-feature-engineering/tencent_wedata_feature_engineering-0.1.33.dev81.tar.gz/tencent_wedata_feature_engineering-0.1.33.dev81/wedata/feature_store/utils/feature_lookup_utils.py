...
def _spark_asof_join_features_tempo(
    ...
    """
    自定义实现 as-of 连接（修复列名冲突版）
    """
    from pyspark.sql.functions import col as _c

    # 1. 给特征表所有列加前缀，确保与主表列不重名
    prefix = "__ft_"
    feature_df = feature_df.select(
        [_c(c).alias(prefix + c) for c in feature_df.columns]
    )

    # 2. 提取主表键列 & 时间戳列（保持原名）
    df_keys = df.select(*lookup_keys, timestamp_key)

    # 3. 提取特征表键列 & 时间戳列（用前缀列，但别名回原名以便写条件）
    ft_keys = feature_df.select(
        *[_c(prefix + k).alias(k) for k in lookup_keys],
        _c(prefix + timestamp_key).alias(timestamp_key)
    )

    # 4. 组装连接条件
    cond = [_c("df." + k) == _c("ft." + k) for k in lookup_keys]
    cond = reduce(lambda a, b: a & b, cond)
    cond &= _c("df." + timestamp_key) >= _c("ft." + timestamp_key)
    if lookback_window is not None:
        cond &= (
            unix_timestamp(_c("df." + timestamp_key)) -
            unix_timestamp(_c("ft." + timestamp_key))
        ) <= lookback_window

    # 5. 先做一次 left semi-join 找出每个主表行对应的最新特征时间戳
    joined = (
        df_keys.alias("df")
        .join(ft_keys.alias("ft"), cond, "left")
        .select(
            *[_c("df." + k).alias(k) for k in lookup_keys],
            _c("df." + timestamp_key).alias(timestamp_key),
            _c("ft." + timestamp_key).alias("__max_ft_ts")
        )
        .groupBy(*lookup_keys, timestamp_key)
        .agg(F.max("__max_ft_ts").alias("__max_ft_ts"))
    )

    # 6. 把最新特征拼回来（此时特征列仍带前缀，不会冲突）
    result = (
        df.join(
            joined,
        ...
        .join(
            feature_df,
            (
                [_c("df." + k) == _c(prefix + k) for k in lookup_keys] +
                [_c("df." + timestamp_key) == _c(prefix + timestamp_key)]
            ),
            "left"
        )
        .drop(*[prefix + k for k in lookup_keys + [timestamp_key]])  # 去掉冗余键列
    )

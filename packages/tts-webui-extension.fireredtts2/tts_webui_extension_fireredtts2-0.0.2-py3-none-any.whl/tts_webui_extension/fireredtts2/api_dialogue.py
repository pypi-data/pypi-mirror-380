...
def dialogue_synthesis_function(
    ...
    if voice_mode == 0:
        ...
        if not check_monologue_text(spk1_prompt_text, "[S1]"):
            ...
            return None
        if not check_monologue_text(spk2_prompt_text, "[S2]") and num_speakers >= 2:
            gr.Warning(message="Invalid speaker 2 prompt text.")
            return None
        if not check_monologue_text(spk3_prompt_text, "[S3]") and num_speakers >= 3:
            gr.Warning(message="Invalid speaker 3 prompt text.")
            return None
        if not check_monologue_text(spk4_prompt_text, "[S4]") and num_speakers >= 4:
            gr.Warning(message="Invalid speaker 4 prompt text.")
            return None
    # Check dialogue text

...
class NodeDownlinkMessage(DownlinkMessage):
    ...
    def issued_commands(self):
        ...
        return [
            DownlinkMessage(
                self.session, x, self.instance, self._get("{}/issuedCommand/{}".format(self.client_edge_url, x))
            )
    ...
    def get_status(self):
        ...
        self._data = self._get(
            "".join([self._af_client_edge_url, "/symbleDownlinkStatus", "/", self.target, "/", self.subject_id])
        )
        return self._data
...
class Node(AirfinderSubject):
    ...
    def _send_message(self, payload, time_to_live_s, access_point=None):
        ...
        if access_point:
            ...
            url = "{}/multicastCommand/{}".format(self._af_client_edge_url, self.subject_id)
            params = {"payload": payload.hex(), "ttlMsecs": int(time_to_live_s * 1e3)}
            data = self._post(url, params=params)
            self.last_downlink = NodeDownlinkMessage(
                self.session, data.get("symbleMessageId"), self.instance, _data=data
            )

...
class LocationDownlinkMessageSpecV5(DownlinkMessageSpec):
    """
    TODO: IDK, maybe if there's time? Old spec.

    Message Spec for Location Beacon msgSpecVersion 5
    No idea why they named the other classes after FW versions.
...
class LocationDownlinkMessageSpecV6(DownlinkMessageSpec):
    """
    Message Spec for Location Beacon msgSpecVersion 5
    No idea why they named the other classes after FW versions.
    ...
    header = {"def": ["msg_type", "msg_spec_vers"], "struct": ">BB", "defaults": [0x01, 0x06]}

    msg_types = {
        "Configuration": {
            "def": ["mask",             # 2B mask               default: 0
                    "adv_en",           # 1B mask               default: 1
                    "schedule",         # 21B mask              default: FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF
                    "heartbeat",        # 4B unsigned int       default: 900 made this up
                    "rssi_adj",         # 1B unsigned int       default: 10
                    "loc_weight",       # 1B unsigned int       default: 0
                    "behavior",         # 1B mask               default: IDK, 0?
                    "tx_pwr",           # 1B signed int         default: -10
                    "hw_id",            # 1B mask               default: 8
                    "adv_rate"          # 1B unsigned int,      default: 70
                    ],
            "struct": ">HB21sIBBBbBB",
            "defaults": [0x00, 0x01, bytearray(b"\xff" * 21), 900, 10, 0, 0, -10, 8, 70],  # why is this hex?
        },
        "Reset": {"def": ["reset_code"], "struct": ">I", "defaults": [0xD5837ED6]}
    }
...
class Location(Node):
    ...
    application = "e9dcd73daa71fb635f36" # This parameter is redundant and unnecessary. See self.app_tok
    ...
    def _get_spec(cls, vers):
        ...
        if vers.major == 0:
            if vers.minor == 1:
                ...
                print(" *** MESSAGE SPEC v5 NOT CURRENTLY IMPLEMENTED *** ")
                return LocationDownlinkMessageSpecV5()  # Not implemented
            elif vers.minor == 6:
                return LocationDownlinkMessageSpecV6()  # half implemented
            else:
...
class Location2(Location):
    ...
    af_subject_name = "location"
    application = "6fea3361a4ea2c8417e9"  # This parameter is redundant and unnecessary. See self.app_tok

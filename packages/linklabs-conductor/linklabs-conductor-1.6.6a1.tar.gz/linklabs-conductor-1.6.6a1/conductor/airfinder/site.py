...
class Site(AirfinderSubject):
    ...
    def get_areas(self):
        ...
        return [
            Area(self.session, x.get("id"), self.instance, _data=x)
            for x in self._get_registered_assets_by_site("areas")
        ]
    ...
    def get_gateways(self):
        ...
        """
        return [
            Gateway(self.session, x.get("nodeAddress"), self.instance, _data=x)
            for x in self._get_registered_assets_by_site("gateways")
        ]
    ...
    def get_gateways_by_status(self):
        ...
        for x in data:
            ret[x.get("health")].append(Gateway(self.session, x["node"]["nodeAddress"], self.instance, x["node"]))
        return ret
    ...
    def get_access_points(self):
        ...
        for x in assets:
            ...
            if cls:
                aps.append(cls(session=self.session, subject_id=subject_id, instance=self.instance, _data=x))
            else:
                LOG.error("{} {}".format(subject_id, x["registrationToken"]))
                aps.append(AccessPoint(session=self.session, subject_id=subject_id, instance=self.instance, _data=x))
        return aps
        ...
        for x in data:
            ...
            if cls:
                ret[x.get("health")].append(
                    cls(session=self.session, subject_id=subject_id, instance=self.instance, _data=x["node"])
                )
            else:
                ret[x.get("health")].append(
                    AccessPoint(session=self.session, subject_id=subject_id, instance=self.instance, _data=x["node"])
                )
        return ret
    ...
    def remove_node(self, mac_id):
        ...
        data = {"nodeAddress": [str(mac_id)], "siteId": self.subject_id}
    ...
    def remove_location(self, locations):
        ...
        data = {"nodeAddresses": [locations], "siteId": self.subject_id}
        resp = self.session.delete(url, json=data)
        resp.raise_for_status()
        return resp.json()
    ...
    def get_locations_by_status(self):
        ...
        for x in data:
            ...
            ret[x.get("health")].append(obj(self.session, subject_id, self.instance, x["node"]))
        return ret
    ...
    def get_site_users(self):
        ...
        return [SiteUser(self.session, x.get("id"), _data=x) for x in self._get_registered_assets_by_site("users")]

...
class ConductorAccount(UplinkSubject):
    ...
    def __init__(self, username, instance=PRODUCTION, password=None, account_name=None):
        pwd_prompt = "Conductor password for {} on {}:".format(username, instance)
        ...
        super(ConductorAccount, self).__init__(self.session, self.account_id, self.instance, _data=account)
    ...
    def register_gateway(self, gateway, network_token):
        ...
        data = {
            "account": {"href": self._href, "desc": ""},
            "token": {"href": network_token._href, "desc": ""},
    ...
    def get_gateways(self):
        ...
        """
        return [
            Gateway(self.session, x["nodeAddress"], self.instance, _data=x)
    ...
    def register_module(self, module_addr, app_token):
        ...
        data = {
            "account": {"href": self.account_id, "desc": ""},
            "token": {"href": str(app_token), "desc": ""},
    ...
    def get_modules(self):
        ...
        return [
            (LTEmModule if is_lte(x["nodeAddress"]) else Module)(self.session, x["nodeAddress"], self.instance, _data=x)
    ...
    def get_application_tokens(self):
        ...
        return [
            AppToken(self.session, x["hash"], self.instance, _data=x)
    ...
    def get_network_tokens(self):
        ...
        """
        return [
            NetToken(self.session, x["hash"], self.instance, _data=x)
    ...
    def create_asset_group(self, name):
        ...
        data = {"account": {"href": self.account_id, "desc": ""}, "assetGroupName": name}
        asset = self._post(url, json=data)
    ...
    def get_asset_groups(self):
        ...
        """
        return [
            AssetGroup(self.session, x["id"], self.instance, _data=x)
    ...
    def get_downlink_message(self, issuance_id):
        ...
        return DownlinkMessage(self.session, asset["hash"], self.instance, _x=asset)
    ...
    def get_event_count(self, start, stop=None):
        ...
        url = "{}/activity/account/{}/{}/{}".format(
            self.network_asset_url, self.account_id, format_time(stop), format_time(start)
        )
        ...
        for d in filtered_data:
            ...
            res = AccountActivity(d.get("eventCount"), parse_time(ts) if ts else None, d.get("subjectId"))
            results.append(res)
        if results:

...
def get_sentiment_analysis(
    ...
    try:
        ...
        try:
            ...
            # Prepare market data with fallback values
            market_data = {
                "volume_ratio": float(getattr(index_data, "volume_ratio", 1.0)) if index_data else 1.0,
                "advancing": breadth.advancing if breadth else 2000,
                "declining": breadth.declining if breadth else 2000,
                "total_stocks": breadth.total_stocks if breadth else 4000,
                "change_pct": float(getattr(index_data, "change_pct", 0.0)) if index_data else 0.0,
                "amplitude": float(getattr(index_data, "amplitude", 1.0)) if index_data else 1.0,
                "north_net": (capital_flow.north_net or 0) if capital_flow else 0,
                "main_net": (capital_flow.main_net or 0) if capital_flow else 0,
            }

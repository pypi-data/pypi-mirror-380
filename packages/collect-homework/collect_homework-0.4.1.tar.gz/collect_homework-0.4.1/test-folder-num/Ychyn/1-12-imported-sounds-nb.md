...
# la_1seconde = ...
la_1seconde = np.sin(2*np.pi*LA*la_1seconde)
```
...
#     ...
def sine(freq, duration=1, amplitude=1.):
    la_1seconde = np.arange(int(RATE*duration))/RATE
    la_1seconde = amplitude*np.sin(2*np.pi*freq*la_1seconde)
    return la_1seconde
```
...
def sine_linear(freq1, freq2, duration):
    la_1seconde = np.arange(int(RATE*duration))/RATE
    la_1seconde = np.sin(2*np.pi*(freq1*(duration-la_1seconde)/duration + freq2 *la_1seconde/duration))
    return la_1seconde
```
...
# pour écouter
MyAudio(sine_linear(440, 1660, 3))
```
...
crescendo_la_1seconde = np.arange(RATE)/RATE
crescendo_la_1seconde = (1 + 5*crescendo_la_1seconde)*np.sin(2*np.pi*LA*crescendo_la_1seconde)
```
...
# votre code pour 2.
def crescendo_sine(freq, duration):
    crescendo = np.arange(int(RATE*duration))/RATE
    crescendo = (1 + 10*crescendo)*np.sin(2*np.pi*freq*crescendo)
    return crescendo
```
...
def crescendo_sine(freq, duration, increase=True):
    ...
    if increase:
        ...
        crescendo = (1 + 10*(duration - crescendo))*np.sin(2*np.pi*freq*crescendo)
    return crescendo
```
...
```{code-cell} ipython3
# votre code pour 4.
def crescendo_malin(son, facteur,increase=True):
    temps = np.arange(len(son))/RATE
    if increase:
        crescendo = (1 + facteur*temps)*son
    else:
        crescendo = (1 + facteur*(temps[-1]-temps))*son
    return crescendo
```
...
```{code-cell} ipython3
# votre code
la_do = np.concatenate((sine(LA),sine(LA),sine(DO)))
```
...
```{code-cell} ipython3
# à vous
#unsigned_16 = np.arange(-5,5,dtype=np.uint16)
#print(unsigned_16)
signed_16 = np.arange(32764,32771,dtype=np.int16)
print(signed_16)
```
...
def float_to_int16(as_float):
    as_int=(32767*as_float).astype(np.int16)
    return as_int
```

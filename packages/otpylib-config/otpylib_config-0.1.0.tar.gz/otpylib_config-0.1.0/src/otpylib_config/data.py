...
class ConfigManagerState:
    ...
    # Configuration state
    config: Dict[Any, Any] = field(default_factory=dict)  # Maps path atoms to values
    subscribers: Dict[Any, List[Callable]] = field(default_factory=dict)  # Maps pattern atoms to callbacks
    
    # Source management
    sources: List[Any] = field(default_factory=list)  # ConfigSource objects
    last_reload: float = 0
...
class ConfigSpec:
    ...
    id: str
    sources: List[Any]  # List of ConfigSource objects
...
CONFIG_MGR_SERVER = "config_manager"

...
from .data import ConfigManagerState, ReloadResult, ConfigResult
from .atoms import config_path_atom, path_from_atom
...
async def ensure_config_value(path: str, value: Any, state: ConfigManagerState) -> Result[Dict[str, Any], str]:
    ...
    Returns info about the change including old value for subscriber notification.
    """
    try:
        path_atom = config_path_atom(path)
        ...
        match existing_value:
            case None:
                ...
                return Ok({
                    ...
                    "new_value": value,
                    "changed": True,
                    "message": f"Configuration key '{path}' created"
                })
            ...
            case existing if existing == value:
                ...
                return Ok({
                    ...
                    "new_value": value,
                    "changed": False,
                    "message": f"Configuration key '{path}' already set correctly"
                })
            ...
            case existing:
                ...
                return Ok({
                    ...
                    "new_value": value,
                    "changed": True,
                    "message": f"Configuration key '{path}' updated"
                })
...
async def ensure_subscription(pattern: str, callback: Callable, state: ConfigManagerState) -> Result[Dict[str, Any], str]:
    """
    ...
    try:
        ...
        if callback in state.subscribers[pattern_atom]:
            return Ok({
                "pattern": pattern,
...
async def ensure_subscription_absent(pattern: str, callback: Callable, state: ConfigManagerState) -> Result[Dict[str, Any], str]:
    """
    ...
    try:
        ...
        if callback in state.subscribers[pattern_atom]:
            state.subscribers[pattern_atom].remove(callback)
    ...
    try:
        ...
        for source in state.sources:
            try:
                ...
                if config is not None:  # None means no changes
        ...
        # Convert new config to atom keys
        _flatten_config_to_atoms(merged_config, new_atom_config)
        
        # Count changes
        config_changes = _count_config_differences(old_atom_config, new_atom_config)
...
def get_matching_subscribers(path: str, state: ConfigManagerState) -> List[Callable]:
    """
    Get all subscriber callbacks that match the given configuration path.
    Uses pattern matching to find relevant subscribers.
    """
    matching_callbacks = []
    
    for pattern_atom, callbacks_list in state.subscribers.items():
        pattern_str = path_from_atom(pattern_atom)
        
        if _path_matches_pattern(path, pattern_str):
            matching_callbacks.extend(callbacks_list)

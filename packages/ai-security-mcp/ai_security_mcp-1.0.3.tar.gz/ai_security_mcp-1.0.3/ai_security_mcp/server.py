...
class MCPServer:
    ...
    async def _call_cloud_scan_api(self, scan_path: str, agents_filter: List[str], output_format: str) -> Dict[str, Any]:
        """Call FastMCP service using MCP JSON-RPC protocol"""
        try:
            # Prepare MCP JSON-RPC request for security scan
            mcp_request = {
                "jsonrpc": "2.0",
                "method": "tools/call",
                "params": {
                    ...
                    "arguments": {
                        "target_content": f"scanning repository at {scan_path}",
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        if result and "result" in result:
                            ...
                            return self._convert_fastmcp_response(result["result"])
                        else:
                            logger.warning(f"Unexpected FastMCP response: {result}")
                            return await self._get_demo_scan_results(scan_path)
                    else:
                        logger.warning(f"FastMCP request failed with status {response.status}")
                        return await self._get_demo_scan_results(scan_path)
    ...
    async def _health_check(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        """Check health of FastMCP service"""
        try:
            # Make a simple MCP health check request
            mcp_request = {
                "jsonrpc": "2.0",
                "method": "tools/call",
                "params": {

...
class MCPServer:
    ...
    def __init__(self, name: str = "ai-security-scanner"):
        ...
        self.auth_backend = "https://ai-security-mcp-simple-amkk3ticjq-uc.a.run.app"
        ...
        try:
            ...
            if not self.session_initialized:
                if not await self._initialize_fastmcp_session():
                    logger.error("Failed to initialize FastMCP session - service may be unavailable")
                    return {
                        "error": "Could not connect to AI Security Scanner service",
                        "status": "failed",
                        "mode": "service_unavailable",
                        "message": "The AI Security Scanner cloud service is currently unavailable. Please try again later or contact support.",
                        "support": "https://ai-threat-scanner.com/support"
                    }

            # Step 2: Read file content for scanning
            try:
                import os
                if os.path.isfile(scan_path):
                    with open(scan_path, 'r', encoding='utf-8', errors='ignore') as f:
                        target_content = f.read()
                    logger.info(f"Read {len(target_content)} characters from {scan_path}")
                else:
                    # For directory scanning, create a representative sample
                    target_content = f"Repository scan requested for directory: {scan_path}"
                    logger.info(f"Directory scan mode for {scan_path}")
            except Exception as e:
                logger.warning(f"Could not read file content: {e}, using placeholder")
                target_content = f"Scan requested for: {scan_path}"

            # Step 3: Make tool call using the persistent session
            mcp_request = {
                "jsonrpc": "2.0",
                "method": "tools/call",
                "params": {
            ...
            if self.session_id:
                ...
                logger.info(f"Including session ID in tool call: {self.session_id}")
            ...
            async with self.fastmcp_session.post(
                ...
                if response.status == 200:
                    ...
                    if result and "result" in result:
                        ...
                        return {
                            "error": "Unexpected FastMCP response",
                            "status": "failed",
                            "mode": "fastmcp_parsing_error",
                    ...
                    if "session" in response_text.lower() or response.status == 400:
                        logger.info("Resetting session due to error")
                        self.session_initialized = False
                        self.session_id = None
                        if self.fastmcp_session:
                            await self.fastmcp_session.close()
                            self.fastmcp_session = None
                    return {
                        "error": f"FastMCP service returned {response.status}",
                        "status": "failed",
                        "mode": "fastmcp_http_error",
            ...
            if self.fastmcp_session:
                await self.fastmcp_session.close()
                self.fastmcp_session = None
            # Return connection error instead of demo data
            return {
                "error": f"FastMCP connection failed: {str(e)}",
                "status": "failed",
                "mode": "fastmcp_network_error",

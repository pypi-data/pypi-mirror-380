...
class VisualFeaturesProcessor(DatametricProcessor):
    ...
    def __init__(self, name: str = "visual_metrique", config: dict[str, Any] | None = None) -> None:
        ...
        if not hasattr(self, "outputs_features") or not self.outputs_features:
            self.outputs_features = self.DEFAULT_OUTPUTS.copy()
        ...
        self.clip_percentiles: tuple[float, float] | None = (
            tuple(cfg.get("clip_percentiles")) if cfg.get("clip_percentiles") is not None else None
        )
        self.laplacian_kernel: str = str(cfg.get("laplacian_kernel", "3x3"))

        # check if the transformation is defined in the processor
        if not isinstance(self.outputs_features, dict):
            raise ValueError(f"[{self.name}] 'output_columns' must be a dict of metric->column_name")
        for k in ("luminosity", "contrast", "blur", "entropy"):
            if k not in self.outputs_features:
                self.outputs_features[k] = self.DEFAULT_OUTPUTS[k]
    ...
    def compute_features(
        ...
        features = {}
        features[self.outputs_features["luminosity"]] = self._compute_luminosity_feature(gray_images)
        features[self.outputs_features["contrast"]] = self._compute_contrast_feature(gray_images)
        features[self.outputs_features["blur"]] = self._compute_blur_feature(gray_images)
        features[self.outputs_features["entropy"]] = self._compute_entropy_feature(gray_images)
        return features
    ...
    def _to_gray_np(self, x: Any) -> np.ndarray:
        ...
        if isinstance(x, Image.Image):
            ...
            if Path.is_file(x):
                ...
                raise ValueError(f"Path does not exist: {x}")

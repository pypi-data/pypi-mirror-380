...
class ProductionEtcdClient(EtcdClient):
    ...
    def _coerce_config_value(self, internal_name: str, value):
        ...
        if internal_name == "max_connections":
            try:
                return max(1, min(100, int(str(value)))) if value else 10
            ...
            try:
                interval = int(str(value)) if value else 30
...
class StagingEtcdClient(EtcdClient):

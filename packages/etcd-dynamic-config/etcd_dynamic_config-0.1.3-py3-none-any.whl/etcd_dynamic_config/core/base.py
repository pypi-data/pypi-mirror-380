...
class BaseEtcdClient:
    ...
    def _build_etcd_key_map_from_yaml(self) -> Dict[str, str]:
        ...
        etcd_keys = self._yaml_config.get("etcd_key_map", [])
        base_prefix = self.get_config_prefix()

        key_map = {}
        for item in etcd_keys:
            if not isinstance(item, dict) or "key" not in item or "name" not in item:
                continue
    ...
    def _build_env_var_map_from_yaml(self) -> Dict[str, str]:
        ...
        env_vars = self._yaml_config.get("env_var_map", [])

        env_map = {}
        for item in env_vars:
            if not isinstance(item, dict) or "name" not in item or "env" not in item:
                continue
            internal_name = item["name"]
            env_var = item["env"]
            env_map[internal_name] = env_var

...
class TestBaseEtcdClient:
    ...
    def create_test_config(self):
        ...
        config = {
            "etcd_key_map": [
                {"key": "DatabaseUrl", "name": "database_url", "type": "str"},
                {"key": "DebugMode", "name": "debug_mode", "type": "bool"},
                {"key": "MaxConnections", "name": "max_connections", "type": "int"},
                {"key": "Timeout", "name": "timeout", "type": "float"},
            ],
            "env_var_map": [
                {"name": "database_url", "env": "DATABASE_URL", "type": "str"},
                {"name": "debug_mode", "env": "DEBUG_MODE", "type": "bool"},
                {"name": "max_connections", "env": "MAX_CONNECTIONS", "type": "int"},
                {"name": "timeout", "env": "TIMEOUT", "type": "float"},
            ],
        }
    ...
    def test_key_map_generation(self):
        ...
        try:
            ...
            # Check mappings
            assert key_map["/APPS/TestService/DatabaseUrl"] == "database_url"
            assert key_map["/APPS/TestService/DebugMode"] == "debug_mode"
    ...
    def test_env_var_map_generation(self):
        ...
        try:
            ...
            # Check mappings
            assert env_map["database_url"] == "DATABASE_URL"
            assert env_map["debug_mode"] == "DEBUG_MODE"
            assert env_map["max_connections"] == "MAX_CONNECTIONS"
    ...
    def test_type_coercion(self):
        ...
        try:
            ...
            result = client._coerce_config_value("database_url", "postgres://localhost")
            assert result == "postgres://localhost"
            assert isinstance(result, str)

            # Test boolean coercion
            result = client._coerce_config_value("debug_mode", "true")
            assert result is True
            assert isinstance(result, bool)

            result = client._coerce_config_value("debug_mode", "false")
            assert result is False

            result = client._coerce_config_value("debug_mode", "1")
            assert result is True

            # Test integer coercion
            result = client._coerce_config_value("max_connections", "10")
            assert result == 10

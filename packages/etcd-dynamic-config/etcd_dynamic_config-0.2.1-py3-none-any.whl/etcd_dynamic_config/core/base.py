...
class EtcdClient:
    ...
    def __init__(
        ...
        etcd_key_map: Optional[Dict[str, str]] = None,
        ...
        Args:
            ...
            etcd_key_map: A dictionary mapping etcd keys to internal config names.
            env_var_map: A dictionary mapping internal config names to environment variables.
        """
        ...
        self._etcd_key_map = (
            etcd_key_map if etcd_key_map is not None else self._build_etcd_key_map()
        )
        self._env_var_map = (
            env_var_map if env_var_map is not None else self._build_env_var_map()
        )
    ...
    def get_config_prefix(self) -> str:
        ...
        """
        root = self._root_key or "/APPS/Default"
        root = root.strip()
    ...
    def connect(self) -> Optional[Any]:
        ...
        if self._client is None:
            if not self.validate_connection_settings() or not self._endpoint:
                return None
    ...
    def start_watch_prefix(
        ...
        try:
            from etcd3 import utils as _utils
            from etcd3.stateful.watch import (  # type: ignore
                Watcher as StatefulWatcher,
            )

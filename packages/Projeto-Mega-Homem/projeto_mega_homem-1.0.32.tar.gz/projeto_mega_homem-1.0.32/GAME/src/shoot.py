...
class Shoot(Collision):
    def __init__(self, x, y, width=6 * 3, height=6 * 3):
        ...
        self.buster_sprite = [GAME.src.global_var.megaman_sprites["Mega_Buster"]]

    def lemon_shoot(self, shoots, mega):
        if GAME.src.global_var.shoots < 3:
            GAME.src.global_var.shoots += 1
        ...
        for i in range(len(shoots) - 1, -1, -1):
            if shoots[i][0].enable_sprite:
                ...
                self.screen.blit(
                    ...
                    (
                        shoots[i][0].shoot_x - GAME.src.global_var.camera_x,
                        shoots[i][0].shoot_y - GAME.src.global_var.camera_y,
                    ),
                )
    ...
    def delete_shoot(self, shoots, shoot):
        for i in range(len(shoots) - 1, -1, -1):
            if shoots[i] == shoot:
                shoots.remove(shoot)
        GAME.src.global_var.shoots -= 1
        if GAME.src.global_var.shoots < 0:
            GAME.src.global_var.shoots = 0

...
def cam_vertical_move(pos_relativa_y):
    if pos_relativa_y < -85:
        GAME.src.global_var.camera_y -= 768
...
def cam_move(
    ...
    middle = 360
    pos_relativa_x = pos[0] - GAME.src.global_var.camera_x
    pos_relativa_y = pos[1] - GAME.src.global_var.camera_y

    if segment == "Cutman_Stage_Segment_1":
        if right and pos_relativa_x > middle and pos[0] < 2736:
            GAME.src.global_var.camera_x += speed_x
        elif not right and pos_relativa_x < middle:
            if pos[0] > 340:
                GAME.src.global_var.camera_x -= speed_x
            if GAME.src.global_var.camera_x < 0:
                GAME.src.global_var.camera_x = 0
        ...
        if (
            ...
            and pos[0] <= 6550 + 1028 * GAME.src.global_var.first_door_open
        ):
            GAME.src.global_var.camera_x += speed_x
        ...
        if (
            right
            and pos_relativa_x > middle
            and pos[0] < 8038 + 768 + 48 + 756 * GAME.src.global_var.second_door_open
        ):
            GAME.src.global_var.camera_x += speed_x

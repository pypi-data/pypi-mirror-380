            ...
            for enemy in enemies:
                if enemy.collision.colliderect(shoot[0].collision):
                    if not enemy.defending:
                        enemy.health -= 1
                        GAME.src.sounds.enemy_damage.play()
                    else:
                        GAME.src.sounds.reflect_fire.play()
...
class Blaster(Enemy, Projectile):
    def __init__(
        ...
        self.sprites = [
            GAME.src.global_var.blaster_sprites["Blaster_0"],
            GAME.src.global_var.blaster_sprites["Blaster_Attack_0"],
            GAME.src.global_var.blaster_sprites["Blaster_Attack_1"],
            GAME.src.global_var.blaster_sprites["Blaster_Attack_2"],
            GAME.src.global_var.blaster_sprites["Blaster_Attack_1"],
            GAME.src.global_var.blaster_sprites["Blaster_Attack_0"],
        ]
    ...
    def attack(self, enemy, kind):
        ...
        GAME.src.sounds.enemy_fire.play()
...
class Helicopter(Enemy):
    def __init__(self, x=600, y=0, width=13 * 3, height=15 * 3, health=1, damage=3):
        ...
        self.y_coll = self.y
        self.sprites = [
            GAME.src.global_var.helicopter_sprites["Fly_1"],
            GAME.src.global_var.helicopter_sprites["Fly_2"],
        ]
    ...
    def respawn_bunby(self, segment, rand_enemies):
        spawn_x = GAME.src.global_var.camera_x
        spawn_y = GAME.src.global_var.camera_y
        if (
...
class Octopus(Enemy):
    def __init__(
        ...
        self.sprites = [
            GAME.src.global_var.octopus_sprites["Octopus_Sleep"],
            GAME.src.global_var.octopus_sprites["Octopus_Move"],
        ]
...
class Big_eye(Enemy):
    def __init__(self, x, y, width=48, height=48 * 3, health=30, damage=7):
        ...
        self.sprites = [
            GAME.src.global_var.bigeye_sprites["Grounded"],
            GAME.src.global_var.bigeye_sprites["Jump"],
        ]
    ...
    def run(
        ...
        if not GAME.src.global_var.first_door_open:
            for i in range(len(enemies) - 1, -1, -1):
                enemies[i].can_respawn = False
            self.in_screen(enemies)
            self.check_health(enemies)

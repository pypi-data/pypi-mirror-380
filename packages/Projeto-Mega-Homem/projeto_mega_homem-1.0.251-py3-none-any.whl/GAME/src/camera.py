...
def cam_move(
    ...
    middle = 360
    pos_relativa_x = pos[0] - src.global_var.camera_x
    pos_relativa_y = pos[1] - src.global_var.camera_y

    if segment == "Cutman_Stage_Segment_1":
        if right and pos_relativa_x > middle and pos[0] < 2736:
            src.global_var.camera_x += speed_x
        elif not right and pos_relativa_x < middle:
            if pos[0] > 340:
                src.global_var.camera_x -= speed_x
            if src.global_var.camera_x < 0:
                src.global_var.camera_x = 0
        ...
        if (
            ...
            and pos[0] <= 6550 + 1028 * src.global_var.first_door_open
        ):
            src.global_var.camera_x += speed_x
        ...
        if (
            right
            and pos_relativa_x > middle
            and pos[0] < 8038 + 768 + 48 + 756 * src.global_var.second_door_open
        ):
            src.global_var.camera_x += speed_x

...
import GAME.src.enemy_spawns
from GAME.src.screen_config import Screen
from GAME.src.global_var import stage_sprites
from GAME.src.stage_coll import Stage_collisions
...
class Stage(Stage_collisions):
    ...
    def handle_coll(self):
        debug = GAME.src.global_var.debug_mode
        self.update_coll()
    ...
    def spawn(self, segment, enemies, name):
        if name == "blaster":
            if (
                ...
                GAME.src.enemy_spawns.spawn_blasters(segment, enemies)
        if name == "octopus":
            if (
                segment == "Cutman_Stage_Segment_3"
                or segment == "Cutman_Stage_Segment_4"
            ):
                GAME.src.enemy_spawns.spawn_octopus(segment, enemies)
        if name == "big_eye":
            if segment == "Cutman_Stage_Segment_7":
                GAME.src.enemy_spawns.spawn_big_eye(segment, enemies)

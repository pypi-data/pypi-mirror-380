...
from src.tittle import tittle_screen
from src.ending_screen import ending_screen
from src.megaman import Megaman
from src.screen_config import Screen
from src.shoot import Shoot
from src.enemy import Helicopter, Blaster, Octopus, Big_eye
from src.cutman import Cutman
...
def main():
    ...
    while running:
        ...
        for event in events:
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_p:
                    if src.global_var.debug_mode is False:
                        src.global_var.debug_mode = True
                    else:
                        src.global_var.debug_mode = False
                if event.key == pygame.K_o:
                    if src.global_var.disable_bunby_spawn is False:
                        src.global_var.disable_bunby_spawn = True
                    else:
                        src.global_var.disable_bunby_spawn = False
        ...
        if mega.y - src.global_var.camera_y > 1080:
            mega.health = 0
        ...
        if not src.global_var.disable_bunby_spawn:
            if timer <= 0:
                random_enemies = bundy.respawn_bunby(segment, random_enemies)
                timer = 3
        if mega.x >= 9622:
            src.global_var.enable_boss = True

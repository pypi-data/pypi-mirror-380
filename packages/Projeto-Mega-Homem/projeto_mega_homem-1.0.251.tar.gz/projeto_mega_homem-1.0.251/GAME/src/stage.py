...
import src.enemy_spawns
from src.screen_config import Screen
from src.global_var import stage_sprites
from src.stage_coll import Stage_collisions
...
class Stage(Stage_collisions):
    ...
    def handle_coll(self):
        debug = src.global_var.debug_mode
        self.update_coll()
    ...
    def spawn(self, segment, enemies, name):
        if name == "blaster":
            if (
                ...
                src.enemy_spawns.spawn_blasters(segment, enemies)
        if name == "octopus":
            if (
                segment == "Cutman_Stage_Segment_3"
                or segment == "Cutman_Stage_Segment_4"
            ):
                src.enemy_spawns.spawn_octopus(segment, enemies)
        if name == "big_eye":
            if segment == "Cutman_Stage_Segment_7":
                src.enemy_spawns.spawn_big_eye(segment, enemies)

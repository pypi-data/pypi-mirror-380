...
from src.screen_config import Screen
from src.character_attributes import Atributtes
from src.life_bar import Life_bar
import src.sounds
...
class Megaman(Atributtes, Life_bar, Collision):
    def __init__(self, x, y, health=28, width=14 * 3, height=23 * 3):
        ...
        self.walk_sprites = [
            src.global_var.megaman_sprites["Megaman_Walk_1"],
            src.global_var.megaman_sprites["Megaman_Walk_2"],
            src.global_var.megaman_sprites["Megaman_Walk_3"],
            src.global_var.megaman_sprites["Megaman_Walk_2"],
            src.global_var.megaman_sprites["Mega_Shoot_walk_1"],
            src.global_var.megaman_sprites["Mega_Shoot_walk_2"],
            src.global_var.megaman_sprites["Mega_Shoot_walk_3"],
            src.global_var.megaman_sprites["Mega_Shoot_walk_2"],
        ]
        self.idle_sprites = [
            src.global_var.megaman_sprites["Mega_Stand_0"],
            src.global_var.megaman_sprites["Mega_Shoot_idle"],
        ]

        self.jump_sprites = [
            src.global_var.megaman_sprites["Mega_Jump"],
            src.global_var.megaman_sprites["Mega_Shoot_jump"],
        ]

        self.stair_sprite = [
            src.global_var.megaman_sprites["Megaman_Ladder_1"],
            src.global_var.megaman_sprites["Mega_Shoot_stair"],
        ]

        self.knockback_sprite = src.global_var.megaman_sprites["Mega_Pain"]
    ...
    def colliding(self, collision):
        cx = src.global_var.camera_x
        cy = src.global_var.camera_y
    ...
    def is_idle(self):
        if not (self.keys_pressed[pygame.K_d] or self.keys_pressed[pygame.K_a]) or (
            ...
            if not self.on_stair:
                self.animation_index = 0
            self.y_coll = self.y + pixel_offset_y - src.global_var.camera_y
            self.x_coll = self.x + pixel_offset + 4 - src.global_var.camera_x

    def move_right(self):
        cy = src.global_var.camera_y
        if (
            self.keys_pressed[pygame.K_d]
            and self.x - src.global_var.camera_x < 720 - 58
            and not self.stunned
        ):
            if not self.on_stair:
                self.moving = True
                self.x += self.speed
                self.x_coll = self.x + pixel_offset - src.global_var.camera_x
            self.left = True
        self.is_idle()
    ...
    def move_left(self):
        ...
        if (
            self.keys_pressed[pygame.K_a]
            and self.x - src.global_var.camera_x > -10
            and not self.stunned
        ):
            if not self.on_stair:
                self.moving = True
                self.x -= self.speed
                self.x_coll = self.x + pixel_offset - src.global_var.camera_x + 3
            self.left = False
        self.is_idle()
        self.y_coll = self.y + 1 - cy
    ...
    def vertical_move(self, offset=0):
        if not self.stopped:
            self.y += self.y_speed
            self.y_coll = self.y + offset - src.global_var.camera_y
    ...
    def animations(self):
        ...
        if not self.knockback_inx % 4:
            self.display_to_blit.blit(
                ...
                (
                    self.x - src.global_var.camera_x - self.shooting_offset,
                    self.y - src.global_var.camera_y - self.y_offset,
                ),
            )
            self.shooting_offset = 0
            self.y_offset = 0
    ...
    def respawn(self):
        ...
        self.mid_transition = False
        self.alive = True
        src.global_var.enable_boss = False
        self.x = self.init_x
        self.y = self.init_y
        src.global_var.camera_x = 0 + 6903 * src.global_var.checkpoint
        src.global_var.camera_y = 0 - 3800 * src.global_var.checkpoint
        pygame.mixer.music.load("./audio/music/Cutman_Stage_Theme.mp3")
        pygame.mixer.music.play(-1)

    def take_damage(self, damage):
        if self.alive and not self.invincible:
            src.sounds.damage.play()
            self.on_stair = False
            self.health -= damage
    ...
    def handle_death(self):
        if self.health <= 0:
            if self.death_timer == 0:
                ...
                src.sounds.death.play()
            self.death_timer += 1
            self.alive = False
            if self.death_timer >= 50:
                self.display_to_blit.fill("black")
                self.mid_transition = True
                src.global_var.first_door_open = False
                src.global_var.second_door_open = False
    ...
    def run(self):
        self.stopped = src.global_var.stop_time
        self.handle_death()
        if self.invincible:
            self.knockback()
        if self.alive:

...
from GAME.src.rolling_cutter import Rolling_Cutter
...
class Cutman(Enemy, Life_bar):
    def __init__(self, x, y, width=48, height=72, max_health=28, damage=6):
        ...
        self.idle_spr = [
            GAME.src.global_var.cutman_sprites["Cutman_Idle_us"],
            GAME.src.global_var.cutman_sprites["Cutman_Idle_1_us"],
            GAME.src.global_var.cutman_sprites["Cutman_Idle"],
            GAME.src.global_var.cutman_sprites["Cutman_Idle_1"],
        ]

        self.walking_spr = [
            GAME.src.global_var.cutman_sprites["Cutman_Walk_1_us"],
            GAME.src.global_var.cutman_sprites["Cutman_Walk_2_us"],
            GAME.src.global_var.cutman_sprites["Cutman_Walk_3_us"],
            GAME.src.global_var.cutman_sprites["Cutman_Walk_2_us"],
            GAME.src.global_var.cutman_sprites["Cutman_Walk_1"],
            GAME.src.global_var.cutman_sprites["Cutman_Walk_2"],
            GAME.src.global_var.cutman_sprites["Cutman_Walk_3"],
            GAME.src.global_var.cutman_sprites["Cutman_Walk_2"],
        ]

        self.jumping_spr = [
            GAME.src.global_var.cutman_sprites["Cutman_Jump_us"],
            GAME.src.global_var.cutman_sprites["Cutman_Jump"],
        ]

        self.shoot_spr = [
            GAME.src.global_var.cutman_sprites["Cutman_Shoot_1"],
            GAME.src.global_var.cutman_sprites["Cutman_Shoot_2"],
        ]
    ...
    def follow(self, megaman):
        if 0 > self.x - GAME.src.global_var.camera_x:
            self.direction = True
        elif self.x - GAME.src.global_var.camera_x > 600:
            self.direction = False
        self.walking = True
        if self.direction:
            self.speed = 5
    ...
    def __on_startup__(self):
        self.starting = False
        pygame.mixer.music.load(path + "/audio/music/Boss_Theme.mp3")

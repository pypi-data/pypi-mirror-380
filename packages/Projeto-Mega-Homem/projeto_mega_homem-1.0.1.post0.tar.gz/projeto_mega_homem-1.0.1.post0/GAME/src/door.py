...
class Door(Collision, Screen):
    def __init__(self, x, y, kind, width=48, height=12 * 16):
        ...
        self.sprite = GAME.src.global_var.stage_sprites["Door"]
    ...
    def open(self, megaman):
        if not self.already_open and self.collision.colliderect(megaman):
            ...
            if self.kind:
                GAME.src.global_var.first_door_open = True
                GAME.src.global_var.checkpoint = True
            else:
                GAME.src.global_var.second_door_open = True
    ...
    def door_anim(self):
        ...
        if self.anim_inx == 300:
            self.open_inx += 4
            GAME.src.sounds.door_open.play()
            self.anim_inx = 0
        if self.open_inx == 16:
            self.opening = False
            GAME.src.global_var.stop_time = False

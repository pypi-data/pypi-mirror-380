...
class Coordinator:
    ...
    async def kick_booked_resource(self, resource):
        ...
        await self.return_resource(resource, target_framework)
        return 0, ""
    ...
    def get_framework_instance(self, framework_name):
        for framework in self.__framework_plugins:
            if framework.__module__.split(".")[-1] == framework_name:
                return framework
        return None

    async def __set_resource_status_post(
        self, resource, status, caller, purpose, owner
    ):
        framework = self.get_framework_instance(status)
        if framework:
            if not owner:
                owner = await framework.get_resource_owner(resource)
            if purpose:
                self.logger.info(
                    "%s(%s): resource acquired(%s) by %s",
                    resource,
                    status,
                    purpose,
                    owner,
                ...
                self.logger.info(
                    "[metric] %s(%s): resource acquired by %s", resource, status, owner
                )
        if status == "fc":
            self.logger.info("[metric] %s(%s): resource released", resource, caller)

        await self.publisher_queue.put({resource: status})

    def __set_resource_status(self, resource, status, caller="", purpose="", owner=""):
        self.__managed_resources_status[resource] = status

        self.logger.info("* %s now belongs to %s", resource, status)
        asyncio.create_task(
            self.__set_resource_status_post(resource, status, caller, purpose, owner)
        )

    async def return_resource(self, resource, context):
        if (
            self.__managed_resources_status.get(resource, "")
            in Config.registered_frameworks
        ):
            self.__set_resource_status(
                resource, "fc", caller=context.__module__.split(".")[-1]
            )
    ...
    def accept_resource(self, resource, context, purpose="", owner=""):
        ...
        self.__set_resource_status(
            resource, context.__module__.split(".")[-1], purpose=purpose, owner=owner
        )

...
class DockerEnvironment(MetaflowEnvironment):
    ...
    def _bake(self, steps) -> Dict[str, FastBakeryApiResponse]:
        ...
        def prepare_step(step):
            ...
            pypi_set_python = next(
                (
                    d.attributes["python"]
                    for d in step.decorators
                    if isinstance(d, PyPIStepDecorator)
                ),
                None,
            )
            # Since the introduction of custom decorators and new packaging support in Metaflow
            # @pypi/@pypi_base etc can be set at runtime and can also be overriden at runtime.
            # So at time of execution here, we can assume the python version set in the decorator land
            # is the one that has been finally resolved. So we just coalesce the conda_set_python and pypi_set_python.
            def coalesce(*args):
                for arg in args:
                    if arg is not None:
                        return arg
            ...
            python = coalesce(conda_set_python, pypi_set_python)

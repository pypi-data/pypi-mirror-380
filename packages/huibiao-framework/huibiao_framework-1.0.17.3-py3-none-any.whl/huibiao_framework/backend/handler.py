...
class TimingMetric:
    ...
    @classmethod
    def update_timing_and_exception_handler_buckets(cls, buckets: List[float]):
...
def timing_and_exception_handler(func):
    ...
    async def wrapper(*args, **kwargs):
        ...
        try:
            ...
            TimingMetric.get_metric_obj().add(func.__name__, elapsed_time)
            return result
        except Exception as e:
            # 计算耗时
            elapsed_time = time.perf_counter() - start_time
            TimingMetric.get_metric_obj().add_error(func.__name__)
            # 记录异常信息和完整堆栈
            logger.error(

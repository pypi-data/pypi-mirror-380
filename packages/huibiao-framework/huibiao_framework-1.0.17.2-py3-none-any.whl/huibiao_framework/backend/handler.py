...
from huibiao_framework.backend.metrics.request_metrics import request_operation_metrics_factory


metric = request_operation_metrics_factory()
...
def timing_and_exception_handler(func):
    ...
    async def wrapper(*args, **kwargs):
        ...
        try:
            ...
            metric.add(func.__name__, elapsed_time)
            return result
        except Exception as e:
            # 计算耗时
            elapsed_time = time.perf_counter() - start_time
            metric.add_error(func.__name__)
            # 记录异常信息和完整堆栈
            logger.error(

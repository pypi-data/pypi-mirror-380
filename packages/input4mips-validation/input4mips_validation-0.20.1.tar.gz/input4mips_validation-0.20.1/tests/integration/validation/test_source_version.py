"""
Integration tests of the source version validation
"""
...
def test_invalid_source_version_raises(test_cvs):
    """
    Test that an invalid value raises

    For a full list of edge case tests,
    see `tests/unit/validation/test_source_version_validation.py`.
    """
    source_id = "CR-CMIP-0-2-0"
    invalid_value = "0.3.0"

    assert (
        invalid_value != test_cvs.source_id_entries[source_id].values.source_version
    ...
    valid_disk_ready_ds.attrs["source_version"] = invalid_value
    ...
    error_msg = re.escape(
        ...
        "The value provided for source_version was '0.3.0'. "
        "According to the CVs, source_version depends on the value of source_id. "
        "As a result, source_version must be '0.2.0'."
    )
    with pytest.raises(ValidationResultsStoreError, match=error_msg):
        res.raise_if_errors()


def test_no_source_version_raises(test_cvs):
    out_path, valid_disk_ready_ds = get_valid_out_path_and_disk_ready_ds(
        cv_source=test_cvs
    )

    valid_disk_ready_ds.attrs.pop("source_version")
    ...
    error_msg = re.escape("MissingAttributeError: 'source_version'")
    with pytest.raises(ValidationResultsStoreError, match=error_msg):
        res.raise_if_errors()

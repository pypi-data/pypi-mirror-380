...
def main(directory, fix, single_thread, full, pattern=None):
    if fix:
        # Run black with line length 120 to match flake8 config
        run_command(["python3", "-m", "black", "--line-length=120", directory])
        # Run isort to fix import sorting
        run_command(["python3", "-m", "isort", "--profile=black", "--line-length=120", directory])

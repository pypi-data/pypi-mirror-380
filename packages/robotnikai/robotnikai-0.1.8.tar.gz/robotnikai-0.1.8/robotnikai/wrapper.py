...
class TablesApi(
    ...
    def graphql(
        ...
        if not graph_ql_request:
            if not query:
                raise ValueError("Either 'query' or 'graph_ql_request' must be provided")
        ...
        if self.integrations_api.connection_id:
            ...
            try:
                if first_variable_key and "connection_id" not in graph_ql_request.variables[first_variable_key]:
                    for object in graph_ql_request.variables[first_variable_key]:
                        if isinstance(object, dict):
                            object["connection_id"] = self.integrations_api.connection_id

                    logger.info(
                        f"Auto-injected connection_id ({self.integrations_api.connection_id}) to graphql variables"
                    )
            except Exception as e:
                logger.debug(f"Error injecting connection_id to graphql variables: {str(e)}")

...
class TimeTrackerGUI:
    ...
    def check_for_updates(self):
        ...
        # Run upgrade on background thread
        threading.Thread(target=self._run_upgrade_and_prompt_restart, daemon=True).start()

    def _version_tuple(self, v: str):
        return tuple(int(p) if p.isdigit() else 0 for p in v.replace("-", ".").split("."))

    def _run_upgrade_and_prompt_restart(self):
        """Attempt to upgrade via pipx first, then pip; prompt to restart."""
        ok, output = self._attempt_upgrade()
        self.root.after(0, lambda: self._post_upgrade_dialog(ok, output))

    def _post_upgrade_dialog(self, ok: bool, output: str):
        if ok:
    ...
    def _attempt_upgrade(self):
        ...
        # Fallback to pip for the active interpreter
    ...
    def _restart_app(self):
        ...
        try:
            _subprocess.Popen([sys.executable, "-m", "timetracking"])  # launch new instance
        except Exception:
            pass
        self.root.quit()

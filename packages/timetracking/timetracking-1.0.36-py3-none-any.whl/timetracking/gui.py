...
class TimeTrackerGUI:
    ...
    def add_project(self):
        ...
        print(f"DEBUG: add_project - name: '{name}'")
        print(f"DEBUG: add_project - description: '{description}'")
        print(f"DEBUG: add_project - email: '{email}'")
        print(f"DEBUG: add_project - rate_str: '{rate_str}'")
        print(f"DEBUG: add_project - currency: '{currency}'")
    ...
    def check_for_updates(self):
        ...
        try:
            current_version = _metadata.version("timetracking")
        except Exception:
            current_version = "0.0.0"

        latest = None
        try:
            with _urlreq.urlopen("https://pypi.org/pypi/timetracking/json", timeout=5) as resp:
                data = resp.read()
            info = _json.loads(data.decode("utf-8"))
            latest = info.get("info", {}).get("version")
        except Exception:
            messagebox.showerror("Update", "Unable to check for updates right now.")
            return

        if not latest:
            messagebox.showerror("Update", "Could not determine latest version.")
            return
        ...
        if self._version_tuple(latest) <= self._version_tuple(current_version):
            messagebox.showinfo("Update", f"You are up to date (v{current_version}).")
            return

        if not messagebox.askyesno("Update Available", f"A new version is available (v{latest}).\n\nCurrent: v{current_version}\n\nUpdate now?"):
            return

        # Run upgrade on background thread with target version
        threading.Thread(target=self._run_upgrade_and_prompt_restart, args=(latest,), daemon=True).start()

    def _version_tuple(self, v: str):
        return tuple(int(p) if p.isdigit() else 0 for p in v.replace("-", ".").split("."))

    def _run_upgrade_and_prompt_restart(self, target_version=None):
        """Attempt to upgrade via pipx first, then pip; prompt to restart."""
        ok, output = self._attempt_upgrade(target_version)
        self.root.after(0, lambda: self._post_upgrade_dialog(ok, output, target_version))

    def _post_upgrade_dialog(self, ok: bool, output: str, target_version=None):
        if ok:
            # Verify the installed version matches target
            if target_version:
                try:
                    installed_version = _metadata.version("timetracking")
                    if self._version_tuple(installed_version) < self._version_tuple(target_version):
                        # Version still behind, try one more time with exact version
                        threading.Thread(target=self._run_upgrade_and_prompt_restart, args=(target_version,), daemon=True).start()
                        return
                except Exception:
                    pass  # Continue with restart if verification fails
            
            if messagebox.askyesno("Update", "Update completed. Restart the app now?"):
                self._restart_app()
        else:
            messagebox.showerror("Update Failed", f"The update did not complete.\n\nDetails:\n{output[:2000]}")

    def _attempt_upgrade(self, target_version=None):
        """Upgrade to the exact target version without cache to avoid double-update issues."""
        cmds = []
        
        if target_version:
            # Install exact version without cache
            if _shutil.which("pipx"):
                cmds.append(["pipx", "install", f"timetracking=={target_version}", "--pip-args=--no-cache-dir"])
            cmds.append([sys.executable, "-m", "pip", "install", "--no-cache-dir", f"timetracking=={target_version}"])
        else:
            # Fallback to general upgrade
            if _shutil.which("pipx"):
                cmds.append(["pipx", "upgrade", "timetracking"])
            cmds.append([sys.executable, "-m", "pip", "install", "-U", "timetracking"])
    ...
    def _restart_app(self):
        """Restart the application using the console entry-point."""
        try:
            import os
            import shutil
            ...
            # Method 1: Use pipx command if available
            timetracking_cmd = shutil.which('timetracking')
            if timetracking_cmd:
                restart_commands.append(['timetracking'])
            
            # Method 2: Try to find the pipx executable and run timetracking through it
            pipx_cmd = shutil.which('pipx')
            if pipx_cmd:
                restart_commands.append([pipx_cmd, 'run', 'timetracking'])
            
            # Method 3: Fallback to direct module execution
            restart_commands.append([sys.executable, "-m", "timetracking"])
...
class EmailDialog:
    ...
    # Removed duplicate update utilities from EmailDialog; handled by TimeTrackerGUI.
...
class EditEntryDialog:
    ...
    def setup_ui(self):
        ...
        print(f"DEBUG: EditEntryDialog - Entry data: {self.entry}")
        print(f"DEBUG: EditEntryDialog - Entry length: {len(self.entry) if self.entry else 'None'}")
        
        # Validate entry data
        if not self.entry or len(self.entry) < 6:
            print(f"DEBUG: EditEntryDialog - Invalid entry data: {self.entry}")
        ...
        entry_id_value = str(self.entry[0])
        print(f"DEBUG: EditEntryDialog - Entry ID value: '{entry_id_value}'")
        self.entry_id_var = tk.StringVar(value=entry_id_value)
        self.entry_id_widget = ttk.Entry(main_frame, textvariable=self.entry_id_var, state="readonly", width=20)
        self.entry_id_widget.grid(row=0, column=1, sticky=(tk.W, tk.E), pady=(0, 5))
        print(f"DEBUG: EditEntryDialog - Entry ID widget created, value: '{self.entry_id_var.get()}'")
        ...
        # Description
        ttk.Label(main_frame, text="Description:").grid(row=2, column=0, sticky=tk.W, pady=(0, 5))
        description_value = self.entry[3] or ""
        print(f"DEBUG: EditEntryDialog - Description value: '{description_value}'")
        self.description_var = tk.StringVar(value=description_value)
        self.description_widget = ttk.Entry(main_frame, textvariable=self.description_var, width=40)
        self.description_widget.grid(row=2, column=1, sticky=(tk.W, tk.E), pady=(0, 5))
        print(f"DEBUG: EditEntryDialog - Description widget created, value: '{self.description_var.get()}'")
        
        # Start time
        ttk.Label(main_frame, text="Start Time:").grid(row=3, column=0, sticky=tk.W, pady=(0, 5))
        start_time_value = self.entry[4]
        print(f"DEBUG: EditEntryDialog - Start time value: '{start_time_value}'")
        start_dt = datetime.fromisoformat(start_time_value)
        start_date_str = start_dt.strftime('%Y-%m-%d')
        start_time_str = start_dt.strftime('%H:%M:%S')
        print(f"DEBUG: EditEntryDialog - Start date: '{start_date_str}', Start time: '{start_time_str}'")
        self.start_date_var = tk.StringVar(value=start_date_str)
        self.start_time_var = tk.StringVar(value=start_time_str)
        
        start_frame = ttk.Frame(main_frame)
        start_frame.grid(row=3, column=1, sticky=(tk.W, tk.E), pady=(0, 5))
        self.start_date_widget = ttk.Entry(start_frame, textvariable=self.start_date_var, width=12)
        self.start_date_widget.pack(side=tk.LEFT, padx=(0, 5))
        self.start_time_widget = ttk.Entry(start_frame, textvariable=self.start_time_var, width=12)
        self.start_time_widget.pack(side=tk.LEFT)
        ...
        if self.entry[5] and self.entry[5].strip():  # If end_time exists and is not empty
            end_time_value = self.entry[5]
            print(f"DEBUG: EditEntryDialog - End time value: '{end_time_value}'")
            end_dt = datetime.fromisoformat(end_time_value)
            end_date_str = end_dt.strftime('%Y-%m-%d')
            end_time_str = end_dt.strftime('%H:%M:%S')
            print(f"DEBUG: EditEntryDialog - End date: '{end_date_str}', End time: '{end_time_str}'")
            self.end_date_var = tk.StringVar(value=end_date_str)
            self.end_time_var = tk.StringVar(value=end_time_str)
        else:
            print(f"DEBUG: EditEntryDialog - No end time, setting empty values")
            self.end_date_var = tk.StringVar(value="")
            self.end_time_var = tk.StringVar(value="")
        
        end_frame = ttk.Frame(main_frame)
        end_frame.grid(row=4, column=1, sticky=(tk.W, tk.E), pady=(0, 5))
        self.end_date_widget = ttk.Entry(end_frame, textvariable=self.end_date_var, width=12)
        self.end_date_widget.pack(side=tk.LEFT, padx=(0, 5))
        self.end_time_widget = ttk.Entry(end_frame, textvariable=self.end_time_var, width=12)
        self.end_time_widget.pack(side=tk.LEFT)
        ...
        self.duration_widget = ttk.Entry(main_frame, textvariable=self.duration_var, state="readonly", width=20)
        self.duration_widget.grid(row=5, column=1, sticky=(tk.W, tk.E), pady=(0, 5))
        
        # Calculate initial duration
        self.update_duration()
        
        # Debug: Check StringVar values after all widgets are created
        print(f"DEBUG: EditEntryDialog - Final StringVar values:")
        print(f"  Entry ID: '{self.entry_id_var.get()}'")
        print(f"  Description: '{self.description_var.get()}'")
        print(f"  Start Date: '{self.start_date_var.get()}'")
        print(f"  Start Time: '{self.start_time_var.get()}'")
        print(f"  End Date: '{self.end_date_var.get()}'")
        print(f"  End Time: '{self.end_time_var.get()}'")
        print(f"  Duration: '{self.duration_var.get()}'")
        
        # Force UI update after a short delay
        self.dialog.after(100, self._force_ui_update)
    ...
    def _force_ui_update(self):
        """Force UI update to ensure StringVar values are displayed"""
        print("DEBUG: EditEntryDialog - Forcing UI update")
        print(f"  Entry ID after update: '{self.entry_id_var.get()}'")
        print(f"  Description after update: '{self.description_var.get()}'")
        print(f"  Start Date after update: '{self.start_date_var.get()}'")
        print(f"  Start Time after update: '{self.start_time_var.get()}'")
        print(f"  End Date after update: '{self.end_date_var.get()}'")
        print(f"  End Time after update: '{self.end_time_var.get()}'")
        print(f"  Duration after update: '{self.duration_var.get()}'")
        
        # Try alternative approach: directly set Entry widget values
        print("DEBUG: EditEntryDialog - Trying direct widget value setting")
        try:
            # Update Entry ID widget (readonly)
            if hasattr(self, 'entry_id_widget'):
                self.entry_id_widget.config(state="normal")
                self.entry_id_widget.delete(0, tk.END)
                self.entry_id_widget.insert(0, self.entry_id_var.get())
                self.entry_id_widget.config(state="readonly")
                print(f"  Directly set Entry ID widget: '{self.entry_id_widget.get()}'")
            
            # Update Description widget
            if hasattr(self, 'description_widget'):
                self.description_widget.delete(0, tk.END)
                self.description_widget.insert(0, self.description_var.get())
                print(f"  Directly set Description widget: '{self.description_widget.get()}'")
            
            # Update Start Date widget
            if hasattr(self, 'start_date_widget'):
                self.start_date_widget.delete(0, tk.END)
                self.start_date_widget.insert(0, self.start_date_var.get())
                print(f"  Directly set Start Date widget: '{self.start_date_widget.get()}'")
            
            # Update Start Time widget
            if hasattr(self, 'start_time_widget'):
                self.start_time_widget.delete(0, tk.END)
                self.start_time_widget.insert(0, self.start_time_var.get())
                print(f"  Directly set Start Time widget: '{self.start_time_widget.get()}'")
            
            # Update End Date widget
            if hasattr(self, 'end_date_widget'):
                self.end_date_widget.delete(0, tk.END)
                self.end_date_widget.insert(0, self.end_date_var.get())
                print(f"  Directly set End Date widget: '{self.end_date_widget.get()}'")
            
            # Update End Time widget
            if hasattr(self, 'end_time_widget'):
                self.end_time_widget.delete(0, tk.END)
                self.end_time_widget.insert(0, self.end_time_var.get())
                print(f"  Directly set End Time widget: '{self.end_time_widget.get()}'")
            
            # Update Duration widget (readonly)
            if hasattr(self, 'duration_widget'):
                self.duration_widget.config(state="normal")
                self.duration_widget.delete(0, tk.END)
                self.duration_widget.insert(0, self.duration_var.get())
                self.duration_widget.config(state="readonly")
                print(f"  Directly set Duration widget: '{self.duration_widget.get()}'")
                
        except Exception as e:
            print(f"  Error in direct widget setting: {e}")
    ...
    def update_duration(self, *args):
        ...
        try:
            ...
            print(f"DEBUG: update_duration - start_date: '{start_date}', start_time: '{start_time}'")
            
            if not start_date or not start_time:
                self.duration_var.set("Invalid start time")
                print(f"DEBUG: update_duration - Invalid start time")
                return
            ...
            print(f"DEBUG: update_duration - end_date: '{end_date}', end_time: '{end_time}'")
            
            if not end_date or not end_time:
                self.duration_var.set("Running")
                print(f"DEBUG: update_duration - Running timer")
                return
            ...
            if end_dt < start_dt:
                ...
                print(f"DEBUG: update_duration - Invalid (end < start)")
                return
            ...
            print(f"DEBUG: update_duration - calculated duration: '{duration_str}'")
            ...
            if hasattr(self, 'duration_widget'):
                self.duration_widget.config(state="normal")
                self.duration_widget.delete(0, tk.END)
                self.duration_widget.insert(0, duration_str)
                self.duration_widget.config(state="readonly")
                print(f"DEBUG: update_duration - directly set duration widget: '{self.duration_widget.get()}'")

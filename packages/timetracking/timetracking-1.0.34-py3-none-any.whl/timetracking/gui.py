...
class EditEntryDialog:
    ...
    def _force_ui_update(self):
        ...
        try:
            # Find and update Entry widgets directly
            for widget in self.dialog.winfo_children():
                if isinstance(widget, ttk.Frame):
                    for child in widget.winfo_children():
                        if isinstance(child, ttk.Entry):
                            if hasattr(child, 'cget') and child.cget('textvariable') == str(self.entry_id_var):
                                child.delete(0, tk.END)
                                child.insert(0, self.entry_id_var.get())
                                print(f"  Directly set Entry ID widget: '{child.get()}'")
                            elif hasattr(child, 'cget') and child.cget('textvariable') == str(self.description_var):
                                child.delete(0, tk.END)
                                child.insert(0, self.description_var.get())
                                print(f"  Directly set Description widget: '{child.get()}'")
        except Exception as e:

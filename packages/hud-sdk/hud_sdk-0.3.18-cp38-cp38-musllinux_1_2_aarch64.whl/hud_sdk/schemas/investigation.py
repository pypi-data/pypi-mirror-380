...
class InvestigationExceptionInfo(Schema):
    def __init__(self, exception: Exception, execution_flow: List[Tuple[str, str]]):
        self.name = exception.__class__.__name__
        self.message = str(exception)
        self.stack_trace = get_exception_stack_trace(exception)
        self.execution_flow = [
            ExecutionFlowItem(item[0], item[1]) for item in execution_flow
        ]

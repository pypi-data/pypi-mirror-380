...
class SubscriptionServiceTests(TransactionCase):
    ...
    def test_update_subscription_description(self, MockSubscription):
        phone_number = "685725100"
        MockSubscription.get.return_value = Mock(spec=["subscription"])
        MockSubscription.get.return_value.subscription = Mock(
            spec=["update_subscription_description"]
        )

        SubscriptionService(
            self.contract,
        ).update_subscription_description(phone_number)

        MockSubscription.get.assert_called_once_with(self.contract.code)

        MockSubscription.get.return_value.subscription.update_subscription_description.assert_called_once_with(  # noqa
            phone_number
        )

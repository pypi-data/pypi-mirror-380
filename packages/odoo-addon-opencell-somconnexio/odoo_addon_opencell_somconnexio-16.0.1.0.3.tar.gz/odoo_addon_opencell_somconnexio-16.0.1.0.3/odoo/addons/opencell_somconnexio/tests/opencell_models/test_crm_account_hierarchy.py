...
class CRMAccountHierarchyTests(SCTestCase):
    def setUp(self):
        ...
        self.contract_with_3_emails = self.env["contract.contract"].create(
            {
                ...
                "email_ids": [
                    (
                        6,
                        0,
                        ([child_email.id, second_child_email.id, third_child_email.id]),
                    )
                ],
                "mandate_id": self.mandate.id,
            }
        ...
        self.crm_account_hierarchy_from_partner = CRMAccountHierarchyFromPartner(
            self.partner, "Code"
        )

    def test_email(self):
        self.assertEqual(
            self.contract.email_ids.email, self.crm_account_hierarchy.email
        )
    ...
    def test_multiple_emails(self):
        ...
        self.assertEqual(
            crm_account_hierarchy.email, self.contract_with_3_emails.email_ids[0].email
        )
        self.assertIn(
    ...
    def test_contact_information(self):
        ...
        self.assertEqual(
            self.crm_account_hierarchy.contactInformation, expected_contact_information
        )

    def test_code(self):
        self.assertEqual("Code", self.crm_account_hierarchy.code)

    def test_crmAccountType(self):
        self.assertEqual("CA_UA", self.crm_account_hierarchy.crmAccountType)

    def test_phone(self):
        self.assertEqual(
            self.contract.partner_id.mobile, self.crm_account_hierarchy.phone
        )

    def test_crmParentCode(self):
        self.assertEqual(
            self.contract.partner_id.ref, self.crm_account_hierarchy.crmParentCode
        )

    def test_language(self):
        self.assertEqual("CAT", self.crm_account_hierarchy.language)

    def test_customerCategory(self):
        self.assertEqual("CLIENT", self.crm_account_hierarchy.customerCategory)

    def test_currency(self):
        self.assertEqual("EUR", self.crm_account_hierarchy.currency)

    def test_billingCycle(self):
        self.assertEqual("BC_SC_MONTHLY_1ST", self.crm_account_hierarchy.billingCycle)
    ...
    def test_emailTemplate(self):
        self.assertEqual(
            "EMAIL_TEMPLATE_TEST", self.crm_account_hierarchy.emailTemplate
        )
...
class CRMAccountHierarchyFromPartnerTests(SCTestCase):
    def setUp(self):
        super().setUp()
        fake = Faker("es-ES")
        self.res_partner_bank = {
        ...
        self.crm_account_hierarchy = CRMAccountHierarchyFromPartner(
            self.partner, "Code"
        )

    def test_address(self):
        expected_address_format = {
            "address1": self.partner.full_street,
            "zipCode": self.partner.zip,
            "city": self.partner.city,
            "state": self.partner.state_id.name,
            "country": self.partner.country_id.code,
        }
        self.assertEqual(expected_address_format, self.crm_account_hierarchy.address)
    ...
    def test_canary_country_code(self):
        self.partner.state_id = self.ref("base.state_es_tf")
        crm_account_hierarchy = CRMAccountHierarchyFromPartner(self.partner, "Code")
        self.assertEqual(crm_account_hierarchy.country, "ES_CN")
        self.partner.state_id = self.ref("base.state_es_gc")
        crm_account_hierarchy = CRMAccountHierarchyFromPartner(self.partner, "Code")
        self.assertEqual(crm_account_hierarchy.country, "ES_CN")

    def test_canary_country_code_is_in_from_partner_to_dict(self):
        self.partner.state_id = self.ref("base.state_es_tf")
        crm_account_hierarchy = CRMAccountHierarchyFromPartner(self.partner, "Code")
        crm_account_hierarchy_dict = crm_account_hierarchy.to_dict()
        self.assertIn("country", crm_account_hierarchy_dict)
        self.assertEqual(crm_account_hierarchy_dict["country"], "ES_CN")
        self.partner.state_id = self.ref("base.state_es_gc")
        crm_account_hierarchy = CRMAccountHierarchyFromPartner(self.partner, "Code")
        crm_account_hierarchy_dict = crm_account_hierarchy.to_dict()
        self.assertIn("country", crm_account_hierarchy_dict)

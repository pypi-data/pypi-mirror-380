...
def add_table(name: str, project: str) -> None:
    ...
    if not utilities.exists():
        print(f"Error: Project '{project}' does not have a Utilities folder")
        sys.exit(1)
    ...
    if table_file.exists():
        print(f"Error: Table '{name}' already exists in project '{project}'")
        sys.exit(1)
...
def {name}():
    ...
    if init_file.exists():
        ...
        new_import = f"from .tables.{name} import {name}"
        if new_import not in existing_content:
            updated_content = existing_content + f"\n{new_import}"
            init_file.write_text(updated_content)
    else:
        # Create new __init__.py with just this import
        init_file.write_text(f"from .tables.{name} import {name}")

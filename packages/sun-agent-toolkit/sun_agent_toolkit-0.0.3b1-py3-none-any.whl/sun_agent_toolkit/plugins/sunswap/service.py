...
class SunSwapService:
    ...
    def swap_tokens(self, params: dict[str, Any], wallet_client: TronWalletBase) -> dict[str, Any]:
        from_token = cast(str, params["fromToken"])
        to_token = cast(str, params["toToken"])
        amount_in_human = cast(str, params["amountIn"])
        ...
        if from_token_symbol.lower() != "trx":  # TRX 不需要授权
            self._ensure_token_approval(wallet_client, from_token, router_address, amount_in_human)
        ...
        try:
            ...
            tx_result = wallet_client.send_transaction(cast(TronTransaction, tx_payload))
    ...
    def _ensure_token_approval(
        ...
        if int(current_allowance) < int(amount_base):
            ...
            wallet_client.approve({"tokenAddress": token_address, "spender": spender, "amount": max_uint256})

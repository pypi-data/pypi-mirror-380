...
from sun_agent_toolkit.core.classes.wallet_client_base import Signature
...
class TronWalletClient(TronWalletBase):
    ...
    def __init__(
        ...
        *,
        ...
        can_sign = private_key is not None
    ...
    def sign_typed_data(
        ...
        try:
            ...
            return self.sign_message(message_str)
        except Exception as e:
    ...
    def _instantiate_transaction(self, tx_json: MutableMapping[str, Any]) -> Transaction:
        """Create a tronpy Transaction object from JSON payload."""
        txid = cast(str | None, tx_json.get("txID")) or ""
        raw_data = cast(dict[str, Any] | None, tx_json.get("raw_data"))
        signature = cast(list[str] | None, tx_json.get("signature")) or []
        permission_raw = tx_json.get("permission")

        if raw_data is None:
            raise ValueError("transaction payload must include raw_data")
        ...
        if permission_raw is not None:
            permission_dict = cast(dict[str, Any], permission_raw)
            kwargs["permission"] = permission_dict

        return Transaction(raw_data, **kwargs)

    def build_send_trx(self, params: dict[str, Any]) -> dict[str, Any]:
        to_addr = cast(str, params["to"])
        amount = int(params["amount"])
        from_addr = cast(str, params.get("from_address") or self._address)
        options = cast(dict[str, Any] | None, params.get("options"))

        builder_any: Any = self.tron.trx.transfer(from_addr, to_addr, amount)
        builder_any = self._apply_transaction_options(builder_any, options)
        txn_obj: Transaction = cast(Transaction, builder_any.build())
        return cast(dict[str, Any], txn_obj.to_json())

    def build_send_token(self, params: dict[str, Any]) -> dict[str, Any]:
        to_addr = cast(str, params["to"])
        amount = int(params["amount"])
        token_id = cast(str, params["tokenId"])
        from_addr = cast(str, params.get("from_address") or self._address)
        options = cast(dict[str, Any] | None, params.get("options"))

        normalized_token = token_id.strip().lower()
        native_aliases = {
            "trx",
            "",
            "0x0000000000000000000000000000000000000000",
            "t9yd14nj9j7xab4dbgeix9h8unkkhxuwwb".lower(),
        }
        if normalized_token in native_aliases:
            # 视作原生 TRX
            return self.build_send_trx({
                "to": to_addr,
                "amount": amount,
        ...
        contract: Any = self.tron.get_contract(token_id)
        if not getattr(contract, "abi", None):
            try:
                contract.abi = self.tron.get_contract(token_id).abi
            except Exception:
                logger.debug("Failed to populate ABI for contract %s", token_id, exc_info=True)
        transfer_method: Any = contract.functions.transfer(to_addr, amount)
        builder_any: Any = transfer_method.with_owner(from_addr)
        builder_any = self._apply_transaction_options(builder_any, options)
        txn_obj: Transaction = cast(Transaction, builder_any.build())
        return cast(dict[str, Any], txn_obj.to_json())
    ...
    def sign_transaction(self, transaction: dict[str, Any] | str) -> dict[str, Any]:
        ...
        tx_payload = self._normalize_transaction_payload(transaction)
        txn_obj = self._instantiate_transaction(tx_payload)

        try:
            signed_tx: Transaction = txn_obj.sign(self.priv_key)
        except Exception as e:
            logger.error(f"Signing transaction failed: {e}")
            raise ValueError(f"Failed to sign transaction: {str(e)}") from e

        return cast(dict[str, Any], signed_tx.to_json())

    def send_raw_transaction(self, signed_transaction: dict[str, Any] | str) -> dict[str, Any]:
        tx_payload = self._normalize_transaction_payload(signed_transaction)
        txn_obj = self._instantiate_transaction(tx_payload)

        try:
            broadcast_ret: Any = txn_obj.broadcast()
            result_dict = cast(dict[str, Any], dict(broadcast_ret))
        except Exception as e:
            logger.error(f"Broadcasting transaction failed: {e}")
            raise ValueError(f"Failed to broadcast signed transaction: {str(e)}") from e

        status = "success" if bool(result_dict.get("result")) else "failed"
        tx_id = cast(str | None, result_dict.get("txid") or result_dict.get("transaction", {}).get("txID"))

        if not tx_id:
            tx_id = txn_obj.txid or "unknown"
    ...
    def _finalize_transaction(self, builder_any: Any) -> dict[str, str]:
    ...
    def freeze_balance(self, params: dict[str, Any]) -> dict[str, str]:
        ...
        try:
            ...
            return self._finalize_transaction(builder_any)
        except Exception as e:
    ...
    def unfreeze_balance(self, params: dict[str, Any]) -> dict[str, str]:
        ...
        try:
            resource = params["resource"]
            amount_opt = params.get("amountInSun")
            if amount_opt is None or amount_opt == "":
                builder_any: Any = self.tron.trx.unfreeze_balance(self._address, resource)
            else:
                builder_any = self.tron.trx.unfreeze_balance(self._address, resource, int(amount_opt))
            builder_any = builder_any.with_owner(self._address)
            return self._finalize_transaction(builder_any)
        except Exception as e:
    ...
    def withdraw_stake_balance(self, params: dict[str, Any]) -> dict[str, str]:
        ...
        del params  # unused
        try:
            # TronPy API: withdraw_expire_unfreeze(owner)
            builder_any: Any = self.tron.trx.withdraw_expire_unfreeze(self._address)
            builder_any = builder_any.with_owner(self._address)
            return self._finalize_transaction(builder_any)
        except Exception as e:
    ...
    def delegate_resource(self, params: dict[str, Any]) -> dict[str, str]:
        ...
        try:
            ...
            builder_any: Any = self.tron.trx.delegate_resource(self._address, receiver, amount, resource, lock=lock)
            builder_any = builder_any.with_owner(self._address)
            return self._finalize_transaction(builder_any)
        except Exception as e:

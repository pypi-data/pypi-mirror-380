...
class EMS(object):
	...
	def start(self):
		if self.__state in ["Off", "Stopped"]:
			response = requests.post(f"https://{self.__base_url}/system/controls/start/", json={}, auth=self.__auth, verify=False)
	...
	def stop(self):
		if self.__state not in ["Off", "Stopped"]:
			response = requests.post(f"https://{self.__base_url}/system/controls/stop/", json={}, auth=self.__auth, verify=False)
			return response.status_code == 201
		else:
			return False

	def restart(self):
		response = requests.post(f"https://{self.__base_url}/system/controls/restart/", json={}, auth=self.__auth, verify=False)
		return response.status_code == 201

	def accept_calibration(self):
		response = requests.post(f"https://{self.__base_url}/calibration/confirm/", json={}, auth=self.__auth, verify=False)
		return response.status_code == 200
	...
	def __poll_status(self):
		wsapp = websocket.WebSocketApp(f"ws://{self.__base_ws_url}/ws", on_message=self.__process_message)
		wsapp.run_forever()

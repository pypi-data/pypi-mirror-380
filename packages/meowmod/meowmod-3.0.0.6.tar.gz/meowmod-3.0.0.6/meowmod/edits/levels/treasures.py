...
def handle_all_at_once(
    ...
    一次處理多筆輸入的函式。
    ids: 要設定的項目ID列表
    all_at_once: 是否一次設定全部
    data: 原始資料列表
    names: 項目名稱列表 (用於逐項顯示)
    item_name: 項目名稱 (如 "寶物等級")
    group_name: 群組名稱 (如 "關卡")
    explain_text: 額外說明 (如 "(0=無, 1=銅, 2=銀, 3=金)")
    """
    ...
    for item_id in ids:
        if all_at_once and first:
            # 一次設定所有時，只問一次
            prompt = f"請輸入您想設定的 {item_name} 值 {explain_text}："
            value = helper.check_int(user_input_handler.colored_input(prompt))
            first = False
        elif not all_at_once:
            # 單項設定時逐個輸入
            prompt = f"請輸入您想設定的 {item_name} 值（{group_name}：{names[item_id]}） {explain_text}："
            value = helper.check_int(user_input_handler.colored_input(prompt))
...
def treasure_groups(save_stats: dict[str, Any]) -> dict[str, Any]:
    ...
    for chapter_id in ids:
        ...
        helper.colored_text("&0& = 無, &1& = 銅, &2& = 銀, &3& = 金")
...
def specific_stages(save_stats: dict[str, Any]):
    ...
    for chapter_id in chapter_ids:
        ...
        treasure_data = [-1] * 48
        treasure_data = handle_all_at_once(
            stage_ids,
            False,
            treasure_data,
            list(range(1, 49)),

...
class LlamaProvider(BaseLLMProvider):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.base_url = config.get('base_url', 'https://api.llama-api.com')
        self.timeout = config.get('timeout', 300)
        self.default_model = config.get('default_model', 'Llama-4-Maverick-17B-128E-Instruct-FP8')
    ...
    def send_chat_completion(self, request: LLMRequest) -> LLMResponse:
        """Send chat completion request to LLAMA"""
        start_time = time.time()
        request_id = str(uuid.uuid4())
        
        try:
            # Convert messages to LLAMA format
            llama_messages = self._convert_messages(request.messages)
            
            # Validate and prepare parameters
            validated_params = ParameterValidator.validate_parameters(
                request.parameters, 'llama'
            )
            
            # Prepare request payload for LLAMA
            request_params = {
                "model": request.model or self.default_model,
                "messages": llama_messages,
                "max_tokens": validated_params.get("max_tokens", 4096),
            ...
            # If JSON response is requested, set response_format
    ...
    def create_embeddings(self, request: LLMRequest) -> LLMResponse:
        """Create embeddings using LLAMA's embedding model"""
        start_time = time.time()
        request_id = str(uuid.uuid4())
        
        try:
            # Use LLAMA embedding model
            model = "llama-embeddings"
    ...
    def _convert_messages(self, messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for message in messages:
            ...
            # LLAMA uses OpenAI-compatible format
    ...
    def _make_chat_api_request(self, params: Dict[str, Any]) -> Dict[str, Any]:
        """Make chat completion API request to LLAMA"""
        headers = {
    ...
    def _make_embedding_api_request(self, params: Dict[str, Any]) -> Dict[str, Any]:
        """Make embedding API request to LLAMA"""
        headers = {
            "Authorization": f"Bearer {self.api_key}",
            "Content-Type": "application/json"
        }
    ...
    def _handle_api_error(self, response):
        ...
        raise APIError(f"LLAMA API request failed with status {response.status_code}: {error_detail}")
    ...
    def _process_chat_response(
        ...
        """Process LLAMA chat completion response"""
        output_text = ""
        
        # Extract output text from OpenAI-compatible response format

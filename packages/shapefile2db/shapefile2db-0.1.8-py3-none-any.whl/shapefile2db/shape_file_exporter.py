            ...
            if zip_obj is not None:
                try:
                    ...
                    for poly in polygons:
                        ...
                        #### Add short zcta points for db efficiency
                        # reduce poly to under short max points
                        short_poly = self.minimize_poly(poly=poly, point_max=self.DEFAULT_SHORT_MAX)
                        # Add only exterior for short_poly
                        ext_short_cord_list = list(short_poly.exterior.coords)

                        # round to max digits for cords
                        ext_short_cord_list = [(round(val1, digit_max), round(val2, digit_max)) for val1, val2 in ext_short_cord_list]
        
                        address_db.add_all_short_zcta_points(zcta_id=zcta_obj.zcta_id, short_zcta_points=ext_short_cord_list)
                        
                        # get max / min lat and lon of short poly for bounding box and add
                        ext_lats, ext_lons = zip(*ext_short_cord_list)

...
class LogflareClientSettings(EnvSettings):
    api_key: str = Field(alias="APP_LOGFLARE_LOGGING_API_KEY", exclude=True)
    api_url: str = Field(alias="APP_LOGFLARE_LOGGING_API_URL")

    api_timeout: int = Field(alias="APP_LOGFLARE_LOGGING_API_TIMEOUT", default=2)

    queue_max_size: int = Field(alias="APP_LOGFLARE_LOGGING_QUEUE_MAX_SIZE", default=1000)
    queue_shutdown_timeout: int = Field(
        alias="APP_LOGFLARE_LOGGING_QUEUE_SHUTDOWN_TIMEOUT", default=2
    )

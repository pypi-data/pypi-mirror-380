...
class SupabaseFileSystemSettings(EnvSettings):
    api_key: str = Field(alias="APP_SUPABASE_FS_API_KEY", exclude=True)
    api_url: str = Field(alias="APP_SUPABASE_FS_API_URL")

    passphrase: str | None = Field(
        alias="APP_SUPABASE_FS_ENCRYPT_PASSPHRASE", default=None, exclude=True
    )
    salt: str | None = Field(alias="APP_SUPABASE_FS_ENCRYPT_SALT", default=None, exclude=True)
...
class SupabaseFileSystem(ProxyFileSystem, WithLogger):
    def __init__(
        ...
        if (
            settings.passphrase is not None
            and len(settings.passphrase) > 0
            and settings.salt is not None
            and len(settings.salt) > 0
        ):
            self._bytes_encrypter = BytesEncrypter(settings.passphrase, settings.salt)

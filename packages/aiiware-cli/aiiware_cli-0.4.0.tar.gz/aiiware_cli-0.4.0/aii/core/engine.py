...
class AIIEngine:
    ...
    def check_budget_warnings(self) -> List[str]:
        ...
        try:
            ...
            for threshold in self.output_config.budget_alert_thresholds:
    ...
    def show_budget_warnings(self) -> None:
        """Display budget warnings if any"""
    ...
    async def process_input(
        ...
        try:
            ...
            if context is None:
                print("🔍 DEBUG: No context provided, checking current_session")
                context = self.context_manager.current_session
                if context is None:
                    print("🔍 DEBUG: No current_session, attempting auto-load")
                    # Try to auto-load recent session first
                    context = await self.context_manager.auto_load_recent_session(
                        max_age_minutes=60
                    )
                    if context is None:
                        print("🔍 DEBUG: Auto-load failed, starting new chat")

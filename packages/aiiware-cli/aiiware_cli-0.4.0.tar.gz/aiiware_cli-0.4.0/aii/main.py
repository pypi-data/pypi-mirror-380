    ...
    try:
        ...
        try:
            ...
            if config_manager.get("web_search.enabled"):
                search_provider_name = config_manager.get("web_search.provider")
                if search_provider_name == "brave":
                    api_key = config_manager.get_secret("brave_api_key")
                    if api_key:
                        search_provider = BraveSearchProvider(api_key)
                        web_client = WebSearchClient(search_provider)
                        engine.configure(web_client=web_client)
                elif search_provider_name == "google":
                    api_key = config_manager.get_secret("google_api_key")
                    search_engine_id = config_manager.get_secret(
...
async def handle_config_command(parsed_cmd: Any, engine: AIIEngine) -> int:
    ...
    try:
        ...
        if config_action == "init":
            print("🚀 Initializing AII configuration...")

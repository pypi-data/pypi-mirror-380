...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        var_dict["MuINFO1"] = "本配置文件中的MuMu开头变量为MuMu模拟器专用变量,"
        var_dict["MuINFO2"] = "如果你不会配置或者使用其他模拟器, 请删除这些变量, 不会影响代码运行. "
        var_dict["MuINFO3"] = "希望控制模拟器的启动关闭, 请参考手册教程配置"
        MuMudir = r"D:\Program Files\Netease\MuMu\nx_main"
        if os.path.exists(MuMudir):
            var_dict["MuMudir"] = MuMudir
        else:
            var_dict["MuMudir"] = rf"此处填写MuMuManager.exe所在的文件夹, 例如 {MuMudir}"
        var_dict["MuMu_Instance"] = {0: '0'}

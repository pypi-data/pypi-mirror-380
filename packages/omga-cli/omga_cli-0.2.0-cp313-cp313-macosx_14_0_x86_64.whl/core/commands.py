...
from core.ai import ask, explain_code, fix_code, suggest_improvements, generate_documentation
...
def ask_handler(question: str):
    """Enhanced ask handler with beautiful AI response formatting"""
    try:
        answer = ask(question)
        
        ui.print_ai_response(answer, "AI Assistant Response")
            ...
            if snippets:
                ...
                for snippet in snippets:
                    ...
                    with open(snippet_path, 'r') as f:
                        content_preview = f.read()[:50] + "..." if len(f.read()) > 50 else f.read()
                    data.append({
                        "Name": snippet,
...
def status_handler():
    ...
    try:
        ...
        status_data = [
            ...
            {"Metric": "Model", "Value": get_config_value('api.model', 'gemini-2.0-flash')},

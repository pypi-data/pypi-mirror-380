...
class TenantSecretParser:
    def tenant_secret_parser(subparser):
        ...
        create_secret = subparser.add_parser(
            ...
            help='Create a new tenant secret.',
        ...
        edit_secret = subparser.add_parser(
            'edit-tenant-secret',
            help='Edit an existing tenant secret.',
            formatter_class=ArgumentDefaultsHelpFormatter
        )
        edit_secret.add_argument("--tenant_secret_id", type=int, required=True, default=SUPPRESS, help="ID of the tenant secret to be updated.")
        add_common_args(edit_secret)
        add_secret_type_specific_args_edit(edit_secret, secret_type)

        list_secrets = subparser.add_parser(
            'list-tenant-secrets',
            help='Retrieve a list of tenant secrets for the current user session.',
            formatter_class=ArgumentDefaultsHelpFormatter
        )
        ...
        list_secrets.add_argument(
            ...
            default=SUPPRESS,
            required=False,
            help="Specifies the ID of the user secret to be listed."
        )
        list_secrets.add_argument(
            "--json-output",
            type=check_non_empty_string,
        ...
        delete_secret = subparser.add_parser(
            ...
            help='Delete a tenant secret for the current user session.',
            formatter_class=ArgumentDefaultsHelpFormatter
        )
        delete_secret.add_argument(
            ...
            required=True,
            default=SUPPRESS,
            help="ID of the tenant secret to be deleted."
        )
        delete_secret.add_argument(
            "--json-output",
            type=check_non_empty_string,

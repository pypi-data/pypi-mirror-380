...
"""
    ...
    def _show_cache_statistics(self):
        ...
        stats_text = "[bold]📊 Performance Cache Statistics[/bold]\n\n"
        stats_text += f"[cyan]Cache Efficiency:[/cyan]\n"
        stats_text += f"  • Total tool calls: [yellow]{stats['total_calls']}[/yellow]\n"
        stats_text += f"  • Cache hits: [green]{stats['hits']}[/green]\n"
        stats_text += f"  • Cache misses: [red]{stats['misses']}[/red]\n"
        stats_text += f"  • Hit rate: [bold green]{stats['hit_rate']}[/bold green]\n"
        stats_text += f"  • Evictions: {stats['evictions']}\n\n"

        stats_text += f"[cyan]Cache Status:[/cyan]\n"
        stats_text += f"  • Current size: {stats['cache_size']}/{stats['max_size']} entries\n"
        stats_text += f"  • Enabled: [green]Yes[/green]\n\n" if self.cache_enabled else f"  • Enabled: [red]No[/red]\n\n"

        if cached_tools:
            stats_text += f"[cyan]Currently Cached Tools ({len(cached_tools)}):[/cyan]\n"
            for tool_info in cached_tools[:10]:  # Show first 10
                age_min = tool_info['age_seconds'] // 60
                age_sec = tool_info['age_seconds'] % 60
                stats_text += f"  • {tool_info['tool']} - Age: {age_min}m {age_sec}s - Hits: {tool_info['hit_count']}\n"
            if len(cached_tools) > 10:
                stats_text += f"  [dim]... and {len(cached_tools) - 10} more[/dim]\n"
        else:
            stats_text += "[dim]No tools cached yet[/dim]\n"

        self.console.print(Panel(stats_text, title="Cache Statistics", border_style="cyan"))
    ...
    def _show_performance_summary(self):
        ...
        if cache_stats['total_calls'] > 0:
            perf_text += f"[cyan]Cache Performance:[/cyan]\n"
            perf_text += f"  • Hit rate: [green]{cache_stats['hit_rate']}[/green]\n"
            perf_text += f"  • Saved calls: [green]{cache_stats['hits']}[/green]\n\n"

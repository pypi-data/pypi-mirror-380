...
def main():
    ...
    try:
        if args.verbose:
            # Output verbose info as JSON to maintain consistent output format
            verbose_info = {
                "status": "info",
                "action": "Attempting to download",
                "url": args.url
            }
            if start_time_seconds is not None or end_time_seconds is not None:
                verbose_info["segment"] = f"{args.start_time or '0'} to {args.end_time or 'end'}"
            print(json.dumps(verbose_info))

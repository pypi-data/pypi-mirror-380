...
class RedisSessionStore:
    ...
    def __init__(self, redis_client: Redis, session_ttl: int | None = None):
        ...
        self.session_ttl = session_ttl or redis_settings.session_ttl
        self.session_prefix = redis_settings.session_prefix
        logger.info(
...
redis_session_store = RedisSessionStore(
    redis_client=redis_client, session_ttl=auth_default_settings.session_ttl
)


async def main():
    """Run the main function to test the RedisSessionStore."""
    print("set test")
    print("-" * 100)
    await redis_session_store.set(
        "test",
        SessionData(
            user_info={"sub": "test", "email": "test@test.com"},
            access_token="test",
            refresh_token="test",
            id_token="test",
            expires_at=datetime.now(DEFAULT_TIME_ZONE) + timedelta(seconds=10),
        ),
    )

    print("get test")
    print("-" * 100)
    print(await redis_session_store.get("test"))

    print("delete test")
    print("-" * 100)
    await redis_session_store.delete("test")

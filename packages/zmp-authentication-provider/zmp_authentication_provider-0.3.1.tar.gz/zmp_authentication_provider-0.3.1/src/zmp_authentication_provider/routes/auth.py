...
async def callback(
    ...
    random_token_session_id = secrets.token_urlsafe(32)
    ...
    await redis_session_store.set(random_token_session_id, session_data)
...
async def profile(request: Request, oauth_user: TokenData = Depends(get_current_user)):

...
def evaluate_regressor(
    ...
    *,
    ...
    if print_metrics:
        _print_regression_report(report, ndigits)
...
def _print_regression_report(m: Dict[str, Any], ndigits: int = 4) -> None:
    line = (
        f"MAE: {m['mae']:.{ndigits}f} | "
        f"RMSE: {m['rmse']:.{ndigits}f} | "
        f"MSE: {m['mse']:.{ndigits}f} | "
        f"R²: {m['r2']:.{ndigits}f} | "
        f"n={m['support']}"
    )
    print(line)
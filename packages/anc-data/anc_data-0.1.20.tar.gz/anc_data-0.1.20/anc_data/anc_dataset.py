    ...
    def __init__(
        ...
        if dataset_type == "parquet":
            ...
            if ds_args.get("sub_ds_info_filepath"):
                try:
                    logging.info(
                        f"Loading sub_ds_info from {ds_args['sub_ds_info_filepath']}"
                    )
                    with open(ds_args["sub_ds_info_filepath"], "r") as f:
                        meta = json.load(f)
                    assert (
                        isinstance(meta, list)
                        and len(meta) == len(filepaths)
                        and all(isinstance(x, list) for x in meta)
                    ), "Invalid sub_ds_info structure: expected list-per-dataset of lists"
                    datasets_sub_ds_info = meta
                except Exception as e:
                    logging.warning(
                        "Failed to load sub_ds_info from '%s': %s. Proceeding without precomputed metadata.",
                        ds_args.get("sub_ds_info_filepath"),
                        str(e),
                    )
                    datasets_sub_ds_info = None
            ...
            self.ds = [
                ParquetConcateDataset(
                    ...
                    sub_ds_info=(
                        datasets_sub_ds_info[i] if datasets_sub_ds_info else None
                    ),
                )

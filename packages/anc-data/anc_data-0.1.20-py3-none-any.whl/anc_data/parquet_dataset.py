...
class ParquetConcateDataset:
    ...
    def __init__(
        ...
        if sub_ds_info is not None:
            ...
            # Validate that sub_ds_info matches the number of files
            if len(sub_ds_info) != len(filepaths):
                raise ValueError(
                    f"sub_ds_info length ({len(sub_ds_info)}) must match filepaths length ({len(filepaths)})"
                )
            # Use precomputed info directly

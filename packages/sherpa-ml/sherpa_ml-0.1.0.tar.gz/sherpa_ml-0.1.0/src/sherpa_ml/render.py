...
class TemplateRenderer:
    ...
    File rules:
      ...
      - Filenames themselves are not templated.
    ...
    def _build_plan(self, ctx: TemplateContext) -> Iterable[PlanItem]:
        ...
        for src_abs in sorted(self.templates_dir.rglob("*")):
            ...
            dst_rel_base = mapped
            is_template = dst_rel_base.suffix == ".j2"
            dst_rel = dst_rel_base.with_suffix("") if is_template else dst_rel_base
    ...
    def _map_src_to_dst(self, src_rel: Path, ctx: TemplateContext) -> Path | None:
        ...
        Convert a path like:
          ...
          presets/minimal/pyproject.toml.j2      -> pyproject.toml.j2
          presets/standard/pyproject.toml.j2     -> None (skipped when preset != standard)
        """
        parts = src_rel.parts
        if not parts:
            return None

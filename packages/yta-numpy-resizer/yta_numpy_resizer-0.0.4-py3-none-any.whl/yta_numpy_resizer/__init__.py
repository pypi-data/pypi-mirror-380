...
class NumpyResizer:
    ...
    def resize_letterbox(
        ...
        # cv2.INTER_LANCZOS4 is slower but better
    ...
    def pad_with_color(
        ...
        y_offset = max((target_h - h) // 2, 0)
        x_offset = max((target_w - w) // 2, 0)
        ...
        # TODO: We need to use a 'position' parameter to
        # center the frame in that position
        # Frame to canvas, centered in background
        bg[y_offset:y_offset + copy_h, x_offset:x_offset + copy_w] = frame[:copy_h, :copy_w]

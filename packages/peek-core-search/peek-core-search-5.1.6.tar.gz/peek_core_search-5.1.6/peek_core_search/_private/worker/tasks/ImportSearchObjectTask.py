...
@addPeekWorkerTask()
def removeSearchObjectTask(importGroupHashes: List[str]) -> None:
    logger.debug(
        "removeSearchObjectTask called with %s hashes", len(importGroupHashes)
    )


@addPeekWorkerTask()
def importSearchObjectTask(searchObjectsEncodedPayload: bytes) -> None:
...
def _insertOrUpdateObjects(
    ...
    try:
        ...
        (excludedFullSearchTerms, excludedPartialSearchTerms) = (

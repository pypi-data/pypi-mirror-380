...
class infoRegistry(metaclass=SingletonMeta):
    ...
    def _write_cached_info(self, video_id: str, info: dict) -> str:
        cache_dir = self.video_root if self.flat_layout else os.path.join(self.video_root, video_id)
        check_create_logs(f"making this directory == {directory} == _write_cached_info line 77")
    ...
    def edit_info(self, data: dict, url: str | None = None,
        ...
        # canonicalize: keep existing video_id if present; else use yt id; else fallback
        canonical = (
            cur.get("video_id")
            or (cur.get("id") if cur.get("id") else None)
            or (video_id if video_id else None)
            or (get_sha12(url) if url else None)
            or generate_video_id(video_path or "video")
        )
        cur["video_id"] = canonical   # <- set explicitly before path expansion

        cur = ensure_standard_paths(cur, self.video_root)
        cache = self._write_cached_info(canonical, cur)
...
class VideoDownloader:
    def __init__(self, url=None,download_directory=None, user_agent=None,
        ...
        self.video_url = self.url = get_corrected_url(url or video_url) 
        ...
        self.info = self.registry.get_video_info(url=self.video_url, force_refresh=self.force_refresh)
        self.video_path=self.info.get('video_path')
    ...
    def _download_single(self, url=None,download_directory=None, user_agent=None,
                         ...
                         registry=None, force_refresh=False,
                         flat_layout: bool = False,video_url =None):
        video_url = url = get_corrected_url(url or video_url or self.url or self.video_url) 
...
def download_video(url=None,
        ...
        return videoDownload_mgr._download_single()
...
def get_video_info(
    ...
    return registryMgr.get_video_info(url=url, video_id=video_id, force_refresh=force_refresh,video_path=video_path,download=download)

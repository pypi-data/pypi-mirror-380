...
class VideoDownloader:
    def __init__(self, url=None,download_directory=None, user_agent=None,
                 video_extention=None, download_video=True,
                 video_path=None,
        ...
        self.video_extention = video_extention or "mp4"
    ...
    def _download_single(self, url=None,download_directory=None, user_agent=None,
         ...
         try:
            video_extention = video_extention or "mp4"
            outtmpl = os.path.join(self.download_directory, "%(id)s.%(video_extention)s")
            ...
            ext = (ext.lstrip(".") or raw_info.get("ext") or video_extention).lower()
            if not re.match(r"^[a-z0-9]+$", ext):
                logger.warning(f"[VideoDownloader] Invalid ext {ext}, forcing mp4")
                ext = video_extention or "mp4"
            directory =info.get('directory')
            final_path = os.path.join(directory, f"{output_filename}.{ext}")

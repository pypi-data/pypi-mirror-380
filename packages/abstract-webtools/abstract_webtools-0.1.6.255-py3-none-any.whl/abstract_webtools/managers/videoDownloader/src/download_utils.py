...
class VideoDownloader:
    ...
    def _download_single(self, video_url: str,video_path:str=None):
        ...
        try:
            ...
            if not re.match(r"^[a-z0-9]+$", ext):
                ...
                ext = "mp4"
            schema_video_directory=None
            schema_video_path = info.get('schema_paths',{}).get('video_path')
            if schema_video_path:
                schema_video_directory = os.path.dirname(schema_video_path)
            info_video_path =info.get('video_path')
            info_directory =info.get('directory')
            dirbase_video_path = os.path.join(dirbase, f"video.{ext}")
            download_directory_video_path = os.path.join(self.download_directory, f"video.{ext}")
            temp_path = {"dirbase_video_path":dirbase_video_path,
             "download_directory_video_path":download_directory_video_path,
             "info_directory":info_directory,"info_video_path":info_video_path,
             "schema_video_path":schema_video_path,
             "schema_video_directory":schema_video_directory,
             "temp_path":temp_path}
            input(temp_path)
            final_path = schema_video_path

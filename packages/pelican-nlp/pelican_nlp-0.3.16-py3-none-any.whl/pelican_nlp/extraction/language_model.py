...
class Model:
    ...
    def load_model(self, empty_weights=False, trust_remote_code=False):
        ...
        if self.model_name == 'xlm-roberta-base' or self.model_name == 'DiscoResearch/Llama3-German-8B-32k':
            ...
            device_type = self.device_map_creation()
            
            # Move model to appropriate device based on platform
            if device_type == "cuda":
                # Use device mapping for CUDA (Linux/Windows)
                print(f'DEBUG: Using device mapping for CUDA')
                ...
                # For Mac MPS or CPU, move model directly to device
                device = torch.device(device_type)
                print(f'DEBUG: Moving model to device: {device}')
                print(f'DEBUG: Model device before move: {next(self.model_instance.parameters()).device}')
                self.model_instance = self.model_instance.to(device)
                print(f'DEBUG: Model device after move: {next(self.model_instance.parameters()).device}')
                print(f'Model moved to {device_type} device.')
    ...
    def device_map_creation(self):
        ...
        if torch.cuda.is_available():
            ...
            # Create device map for CUDA
            print(f'DEBUG: Creating device map for CUDA')
            self.device_map = infer_auto_device_map(self.model_instance, max_memory=max_memory)
            print(f'DEBUG: Device map created: {self.device_map}')

        elif hasattr(torch.backends, "mps") and torch.backends.mps.is_available():
            device_type = "mps"
            print("Apple Metal (MPS) available.")
            print(f'DEBUG: MPS detected, skipping device mapping')
            # For MPS, we don't use device mapping, just return the device type
            self.device_map = None

        else:
            device_type = "cpu"
            print("Careful: No GPU available, using CPU. This will be slow.")
            print(f'DEBUG: CPU only, skipping device mapping')
            # For CPU, we don't use device mapping, just return the device type
            self.device_map = None

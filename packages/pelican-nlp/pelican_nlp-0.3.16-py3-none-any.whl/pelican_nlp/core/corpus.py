...
class Corpus:
    ...
    def extract_embeddings(self):
        ...
        print(f'DEBUG: Number of documents to process: {len(self.documents)}')
        embeddingsExtractor = EmbeddingsExtractor(embedding_options, self.project_folder)
        debug_print(len(self.documents))
        for i in range(len(self.documents)):
            print(f'DEBUG: Processing document {i+1}/{len(self.documents)}: {self.documents[i].name}')
            debug_print(f'cleaned sections: {self.documents[i].cleaned_sections}')
            for key, section in self.documents[i].cleaned_sections.items():
                debug_print(f'Processing section {key}')
                print(f'DEBUG: Section {key} content length: {len(section) if isinstance(section, str) else "N/A"}')
                ...
                print(f'DEBUG: About to extract embeddings from section {key}')
                embeddings, token_count = embeddingsExtractor.extract_embeddings_from_text(section)
                print(f'DEBUG: Extracted {len(embeddings)} embeddings with {token_count} tokens')

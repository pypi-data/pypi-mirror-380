        ...
        for text in text_list:
            ...
            if self.embeddings_configurations['pytorch_based_model']:
                ...
                try:
                    ...
                    print(f'DEBUG: Model device from parameters: {self.device}')
                    ...
                    print(f'DEBUG: Fallback device detection: {self.device}')
                with torch.no_grad():
                    if 'llama' in self.model_name.lower():
                        ...
                        if isinstance(inputs, dict):
                            ...
                            print(f'DEBUG: Moving inputs to device: {self.model_instance.device}')
                            inputs = {k: v.to(self.model_instance.device) for k, v in inputs.items()}
                            debug_print(f"Model inputs: {inputs}")
                            print(f'DEBUG: Calling model with inputs on device: {self.model_instance.device}')

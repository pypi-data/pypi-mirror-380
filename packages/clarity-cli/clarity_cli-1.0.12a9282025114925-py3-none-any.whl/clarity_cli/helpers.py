    ...
    while True:
        ...
        if response.status_code != 200:
            ...
            if status == "RUNNING":
                ...
                out.warning(
                    out.bold(
                        f"Component parsing failed but can be used- {error_message}")
                )
                return full_status
            elif status == "SUCCESS":
...
def trigger_report_generation(token, workspace_id, domain, report_type, execution_id, project_id, execution_counter, test_ids, result_types):
    ...
    headers = {"Authorization": f"Bearer {token}", "workspace": workspace_id}
    url = f"{domain}/{REPORT_TRIGGER_PATH.format(report_type=report_type, execution_id=execution_id,project_id=project_id, execution_counter=execution_counter, test_ids=test_ids, result_types=result_types)}"  # noqa: E501
    response = requests.get(url, headers=headers)
    if response.status_code == 200:
        return True
    else:

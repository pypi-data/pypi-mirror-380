...
class TestIntLogspaceGap_GT_1:
    ...
    def test_rejects_floats(self):
        with pytest.raises(ValueError):
            _int_logspace_gap_gt_1(
                [10**1.1,10**1.2,10**1.3], 1.0, 1, False, 1, 10, 3
            )
    ...
    def test_rejects_search_values_lt_1(self):
        with pytest.raises(ValueError):
            _int_logspace_gap_gt_1(
                [1e-1,1e0,1e1], 1.0, 1, False, -1, 3, 3
            )
    ...
    def test_rejects_duplicate_search_points(self):
        with pytest.raises(ValueError):
            _int_logspace_gap_gt_1(
                [1e9,1e12,1e15,1e15], 3.0, 0, True, 1e8, 1e18, 3
            )
    ...
    def test_rejects_invalid_posn_index(self):
        with pytest.raises(ValueError):
            _int_logspace_gap_gt_1(
                [1e4,1e8,1e12], 4.0, 3, False, 1e4, 1e12, 4
            )

        with pytest.raises(TypeError):
            _int_logspace_gap_gt_1(
                [1e4,1e8,1e12], 4.0, 1.98, False, 1e4, 1e12, 4
            )
    ...
    def test_rejects_bad_hard_min(self):
        with pytest.raises(ValueError):
            _int_logspace_gap_gt_1(
                [1e3,1e8,1e13,1e18], 5.0, 3, True, 1e4, 1e18, 4
            )

        with pytest.raises(TypeError):
            _int_logspace_gap_gt_1(
                [1e3,1e6,1e9,1e12], 3.0, 3, True, 10**2.14, 1e18, 4
            )
    ...
    def test_rejects_bad_hard_max(self):
        with pytest.raises(ValueError):
            _int_logspace_gap_gt_1(
                [1e3,1e7,1e11,1e15], 4.0, 3, True, 1e3, 1e12, 4
            )

        with pytest.raises(TypeError):
            _int_logspace_gap_gt_1(
                [1e2,1e5,1e8,1e11], 3.0, 3, True, 1e1, 10**13.77, 4
            )
    ...
    def test_rejects_points_lte_2(self, points):
        with pytest.raises(ValueError):
            _int_logspace_gap_gt_1(
                [1e2,1e4,1e6,1e8], 2.0, 0, False, 1e2, 1e10, points
            )
    ...
    @pytest.mark.parametrize('_low, _high, _points, _is_logspace',
        ((3, 12, 4, 3.0), (2, 14, 7, 2.0), (1, 7, 4, 2.0)),
    )
        ...
        if _is_hard:
            ...
            if _posn == 'left':
                ...
                assert max(_grid_out) == min(
                    _hard_max, 10**(np.log10(_SINGLE_GRID[-1]) + _gap)
                )
                del _gap

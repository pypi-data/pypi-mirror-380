...
class TestUpstreamImpactOnLaterModules:
    ...
    def test_stopremover(self):
        ...
        assert np.array_equal(out[0], ['TWELVE', 'METER', 'PAPER', 'LIST', 'OF', 'WORDS'])
    ...
    def test_text_justifier(self):
        ...
        assert out2[0] == 'more deep   fried '
        assert out2[1] == 'long words'
    ...
    def test_text_normalizer(self):
        ...
        assert np.array_equal(out, ['LONG LIST SILLY   WORDS ABOUT'])

        TestCls = TextNormalizer(upper=False)
        out2 = TestCls.fit_transform(out)

        assert np.array_equal(out2, ['long list silly   words about'])
    ...
    def test_text_splitter(self):
        ...
        assert np.array_equal(
            out2[0], ['another', '', '', 'sequence', '', 'of', 'words']
        )

...
def _int_logspace_gap_gt_1(
    ...
    on the previous round's logspace grid and the best value discovered
    by the parent GridSearch, subject to constraints imposed by 'hard',
    universal lower bound on integers, etc.

    With additional validation for log intervals greater than 1. This
    should only be accessed on the first regular pass after shifts.
    ...
    _SINGLE_GRID : IntGridType
        ...
        with log gaps greater than one. `_SINGLE_GRID` must be sorted
        ascending, and is presumed to be by

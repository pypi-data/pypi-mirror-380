...
def classify_from_answers(answers: dict) -> dict:
    ...
    try:
        with Session() as db:
            ...
            # (−1) Сфера применения (ст. 2): если отмечено исключение — out_of_scope
            if _truthy(answers.get("scope_exclusion")):
                return {
            ...
            if isinstance(annex3_item, str) and annex3_item in all_items:
                ...
                res["obligations"] = _compute_obligations_dynamic(res)
                # Deterministic Article 49(2) obligation: registration when 6(3) derogation applied
                if res["category"] == "not_high_risk" and res.get("exception_applied"):
                    reg = "register_eu_db: Register provider and the AI system in the EU database [Article 49(2)]"
                    if reg not in res["obligations"]:
                        res["obligations"] = [reg] + res["obligations"]
                return res

...
public:
    ...
    Loc parse_loc(
        ...
        if (start >= 0) {
            ...
            loc.binned_start = static_cast<int64_t>(std::floor(static_cast<double>(start) / bin_size)) * bin_size;
        }
        if (end >= 0) {
            if (full_bin) {
                loc.binned_end = ((end - 1) / bin_size + 1) * bin_size;
            } else {
                loc.binned_end = (end / bin_size) * bin_size;
            }
        } else {
            if (full_bin) {
                loc.binned_end = static_cast<int64_t>(std::floor(static_cast<double>(end - 1) / bin_size)) * bin_size;
            } else {
                loc.binned_end = (end / bin_size) * bin_size;
            }
        }
    ...
    py::array_t<float> read_signal(
        ...
        auto data = mzd->getRecordsAsMatrix(loc_1.binned_start, loc_1.binned_end, loc_2.binned_start, loc_2.binned_end, def_value);

        size_t row_count = (ends[0] - starts[0]) / bin_size;
        size_t col_count = (ends[1] - starts[1]) / bin_size;
    ...
    py::dict read_sparse_signal(
        ...
        auto mzd = hic_file->getMatrixZoomData(loc_1.chr_id, loc_2.chr_id, mode, normalization, unit, bin_size);
        auto data = mzd->getRecordsAsSparseMatrix(loc_1.binned_start, loc_1.binned_end, loc_2.binned_start, loc_2.binned_end);
...
PYBIND11_MODULE(hic_io, m, py::mod_gil_not_used()) {
    ...
    py::class_<PyReader>(m, "Reader")
        ...
        .def("parse_chr", &PyReader::parse_chr,
            "Parse chromosome ID",
            py::arg("chr_id"))

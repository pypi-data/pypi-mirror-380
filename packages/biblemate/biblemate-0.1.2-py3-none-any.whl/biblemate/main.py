...
import urllib.parse
import asyncio, re, os, subprocess, click, gdown, pprint, argparse, json, zipfile, shutil
    ...
    async with client:
        ...
        while not user_request == ".quit":
            ...
            if not len(messages) == len(DEFAULT_MESSAGES):
                ...
                if latest_version and latest_version.strip() != current_version:
                    console.rule()
                    if USER_OS == "Windows":
                        ...
                        console.rule()
                    elif click.confirm(f"A new version of BibleMate AI is available: {latest_version} (you are using {current_version}). Do you want to upgrade now?", default=True) and shutil.which("pip"):
                        pip_path = shutil.which("pip")
                        if os.system(f"{pip_path} install --upgrade biblemate") == 0:
                            console.print("Restart to make the changes effective!", justify="center")
                            console.rule()
                            exit()
                        else:
                            console.print(Markdown(f"## A new version of BibleMate AI is available: {latest_version} (you are using {current_version}).\n\nPlease run `pip install --upgrade biblemate` to upgrade!"))

...
from agentmake import edit_file, readTextFile, writeTextFile
from agentmake.etextedit import launch
import os


async def getTextArea(input_suggestions:list=None, default_entry="", title="", multiline:bool=True):
    ...
    while result == ".editprompt":
        #launch(input_text=None, filename=None, exitWithoutSaving=False, customTitle=None, startAtEnd=False):
        #text_area.text = edit_temp_file(config.current_prompt)
        text_area.text = launch(input_text=config.current_prompt, exitWithoutSaving=True, customTitle=f"BibleMate AI [{BIBLEMATE_VERSION}]", startAtEnd=True)
        config.current_prompt = ""

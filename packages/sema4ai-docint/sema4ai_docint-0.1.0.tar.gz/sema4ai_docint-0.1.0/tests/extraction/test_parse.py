...
class TestDocumentParse:
    ...
    def test_sanity_parsing(self, client: ExtractionClient):
        ...
        self._run_parsing_test(client, "anahau")
    ...
    @pytest.mark.parametrize(
        ...
        [
            folder.name
            for folder in (Path(__file__).parent / "test-data" / "parse").iterdir()
            if (folder / "data.pdf").exists()
        ],
    )
    def test_all_parsing(self, client: ExtractionClient, input_dir: str, request):
        """Test all directories when running with integration marker"""
        self._run_parsing_test(client, input_dir)

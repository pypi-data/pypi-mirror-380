import uuid
from typing import Optional, Union, Callable, Any, Dict, List
from qtpy.QtWidgets import QHBoxLayout, QWidget, QSizePolicy, QVBoxLayout
from qtmui.hooks.use_state import State
from ..typography import Typography
from ..box import Box
from ..spacer import HSpacer
from qtmui.material.styles import useTheme
from qtmui.material.styles.create_theme.components.get_qss_styles import get_qss_style
from qtmui.i18n.use_translation import translate, i18n
from ..utils.validate_params import _validate_param
class CardHeader:
    def __init__(self, action: Optional[Union[State, Any]], avatar: Optional[Union[State, Any]], children: Optional[Union[State, Any, List[Any]]], classes: Optional[Union[State, Dict]], component: Optional[Union[State, Any]], disableTypography: Union[State, bool], slotProps: Optional[Union[State, Dict]], slots: Optional[Union[State, Dict]], subheader: Optional[Union[State, Any]], subheaderTypographyProps: Optional[Union[State, Dict]], sx: Optional[Union[State, Dict, Callable, str]], title: Optional[Union[State, Any]], titleTypographyProps: Optional[Union[State, Dict]], **kwargs): ...
    def _set_action(self, value): ...
    def _get_action(self): ...
    def _set_avatar(self, value): ...
    def _get_avatar(self): ...
    def _set_children(self, value): ...
    def _get_children(self): ...
    def _set_classes(self, value): ...
    def _get_classes(self): ...
    def _set_component(self, value): ...
    def _get_component(self): ...
    def _set_disableTypography(self, value): ...
    def _get_disableTypography(self): ...
    def _set_slotProps(self, value): ...
    def _get_slotProps(self): ...
    def _set_slots(self, value): ...
    def _get_slots(self): ...
    def _set_subheader(self, value): ...
    def _get_subheader(self): ...
    def _set_subheaderTypographyProps(self, value): ...
    def _get_subheaderTypographyProps(self): ...
    def _set_sx(self, value): ...
    def _get_sx(self): ...
    def _set_title(self, value): ...
    def _get_title(self): ...
    def _set_titleTypographyProps(self, value): ...
    def _get_titleTypographyProps(self): ...
    def _init_ui(self): ...
    def retranslateUi(self): ...
    def _set_stylesheet(self): ...
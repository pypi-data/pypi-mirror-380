...
class Subject(object):
    ...
    def _set(self, prop, value, extended, muted, use_cache):
        ...
        if not muted and value != old_value:
            ...
            # Emit property change event using new event bus
            import asyncio
            ...
            try:
                ...
                asyncio.create_task(get_event_bus().emit(event_type, self, payload))
            except RuntimeError:
                # No running loop - emit synchronously
                try:
                    loop = asyncio.get_event_loop()
                    loop.run_until_complete(get_event_bus().emit(event_type, self, payload))
                except RuntimeError:
                    # No loop at all - create one
                    asyncio.run(get_event_bus().emit(event_type, self, payload))
    ...
    def _delete(self, prop, extended, muted, use_cache):
        ...
        if not muted:
            ...
            # Emit property deleted event using new event bus
            import asyncio
            from krules_core.event_bus import get_event_bus
    ...
    def flush(self):
        ...
        # Emit flush event using new event bus
        import asyncio
        from krules_core.event_bus import get_event_bus
        ...
        try:
            loop = asyncio.get_running_loop()
            asyncio.create_task(get_event_bus().emit(event_type, self, props))
        except RuntimeError:
            try:
                loop = asyncio.get_event_loop()
                loop.run_until_complete(get_event_bus().emit(event_type, self, props))
            except RuntimeError:
                asyncio.run(get_event_bus().emit(event_type, self, props))

...
def _worker(queue: Queue[_WorkType], event: Event, reference: Optional[object]) -> None:
    """
        ...
        try:
            ...
            try:
                if isinstance(function, str):
                    getattr(reference, function)(**kwargs)  # pragma: no cover
                else:
                    function(**kwargs)
...
class Manager:
    ...
    def request_workers(self, workers: int, reference: Optional[object] = None) -> None:
        """
        Request workers to process work items from the queue.

        Args:
            workers: Number of workers to start.
            reference: Optionally, reference class instance to use.
        """
        self._events = [self._manager_sync.Event() for _ in range(workers)]
        self._workers = [
            Process(target=_worker, args=(self._queue, self._events[i], reference)) for i in range(workers)
        ]

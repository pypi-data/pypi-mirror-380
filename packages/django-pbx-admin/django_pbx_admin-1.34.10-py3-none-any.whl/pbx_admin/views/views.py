...
from pbx_admin.utils import get_client_ip
...
class LoginView(auth_views.LoginView):
    def get_context_data(self, **kwargs):
        ...
        show_password_form = not getattr(settings, "FORCE_OAUTH_LOGIN", False)
        trusted_ips = getattr(settings, "OAUTH_TRUSTED_IP_ADDRESSES", [])
        show_oauth_button = (
            getattr(settings, "FORCE_OAUTH_LOGIN", False)
            or not trusted_ips
            or get_client_ip(self.request) in trusted_ips
        )
        context.update(
            {

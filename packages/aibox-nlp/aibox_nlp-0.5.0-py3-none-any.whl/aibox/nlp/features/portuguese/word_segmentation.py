...
class WordSegmentationExtractor(FeatureExtractor):
    """Extrator de características relacionadas com
    ...
    @property
    def feature_set(self) -> type[WordSegmentationFeatures]:
    ...
    def extract(self, text: str, **kwargs) -> WordSegmentationFeatures:
        ...
        if doc_size > 0:
            ...
            errors_hypo = sum(map(lambda tok: len(self._fix_hypo(tok)) > 1, doc))
    ...
    def _search_hyper(self, doc: tokens.Doc) -> Iterable[tuple[tokens.Span, str]]:
        ...
        :return: tuples with span and correction.
        """
    ...
    def _fix_hypo(self, token: tokens.Token) -> tokens.Doc:
        ...
        :param token: token to detect.

        :return: if false, returns a new doc the token.

...
from aibox.nlp.factory import available_extractors, class_registry, get_extractor
...
@pytest.mark.parametrize(
    ...
    [
        "agreementBR",
        [e for e in available_extractors() if "similarity" not in e.lower()],
    ],
)
def test_extractor_factory(extractors: str | list[str]):
    extractor = get_extractor(extractors)
    if isinstance(extractors, list):
        assert len(extractors) == len(extractor.extractors)
        for ext, expected in zip(extractor.extractors, extractors):
            assert isinstance(ext, class_registry.get_class(expected))
    else:
        assert isinstance(extractor, class_registry.get_class(extractors))

...
class OpenSourceLabelOperation(LabelOperation):
    ...
    async def label_multiple(self, text: str, labels: List[str]) -> MultipleLabelResult:
        ...
        - labels: array of selected label strings (must be from the available options)
        - confidence_scores: object with label as key and confidence number as value
        - reasoning: string explaining your choices
        
        Example format:
        {{
            "labels": ["positive", "complaint"],
            "confidence_scores": {{"positive": 0.8, "complaint": 0.9}},
            "reasoning": "The text expresses satisfaction with the product but complains about service"
        }}
        """

...
def register_udf(
    ...
    :param workspace: workspace to register in
    ...
    try:
        ...
        api_instance.register_udf_info(
            workspace=workspace, name=name, udf=udf_model, async_req=async_req
        )
...
def info(workspace=None, name=None, async_req=False):
    """
    Fetch info on a registered udf
    :param workspace: workspace to filter to
    ...
    try:
        ...
        if not (
            ...
            workspace = client.default_user().username

        return api_instance.get_udf_info(
            workspace=workspace, name=name, async_req=async_req
        )
    except GenApiException as exc:
        raise tiledb_cloud_error.maybe_wrap(exc) from None
...
def share(name=None, workspace=None, async_req=False):
    """
    Share a registered udf
    :param name: name of udf in "workspace/name" format
    :param workspace: workspace to share array with
    :param async_req: return future instead of results for async support
    :return: registered udf details
    """
    (udf_workspace, udf_name) = name.split("/")

    if not (
        udf_workspace is not None
        and udf_name is not None
        and udf_workspace != ""
        and udf_name != ""
    ):
        udf_workspace = client.default_user().username
    ...
    try:
        ...
        return api_instance.share_udf_info(
            ...
            udf_sharing=models.UDFSharing(
                workspace=workspace, actions=[models.UDFActions.FETCH_UDF]
            ),
            async_req=async_req,
        )
    except GenApiException as exc:
...
def unshare(name=None, workspace=None, async_req=False):
    """
    Share a registered udf
    :param name: name of udf in "workspace/name" format
    :param workspace: workspace to share array with
    ...
    try:
        ...
        return api_instance.share_udf_info(
            udf_workspace,
            udf_name,
            udf_sharing=models.UDFSharing(workspace=workspace),
            async_req=async_req,
        )
    except GenApiException as exc:
        raise tiledb_cloud_error.maybe_wrap(exc) from None

        ...
        def decorator(func: Callable) -> Callable:
            ...
            async def endpoint(input_data: InputSchema, request: Request):
                ...
                try:
                    ...
                    # If the caller disconnected, surface cancellation instead of reporting success
                    if await request.is_disconnected():
                        if self.dev_mode:
                            log_debug(
                                f"Client disconnected before reasoner '{reasoner_id}' completed"
                            )
                        raise HTTPException(
                            status_code=499,
                            detail="Client disconnected during execution",
                        )

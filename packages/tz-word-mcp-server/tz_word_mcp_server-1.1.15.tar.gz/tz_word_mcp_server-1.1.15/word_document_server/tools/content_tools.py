...
async def add_picture(filename: str, image_path: str, width: Optional[float] = None) -> str:
    ...
    try:
        ...
        try:
            # A4 paper width in inches (210mm = 8.27 inches)
            A4_WIDTH_INCHES = 8.27
            MAX_WIDTH_RATIO = 0.9  # 90% of A4 width
            MAX_ALLOWED_WIDTH = A4_WIDTH_INCHES * MAX_WIDTH_RATIO  # ~7.443 inches
            ...
            # Add picture with validated width and center it
            if final_width:
                # Create a centered paragraph for the image
                para = doc.add_paragraph()
                para.alignment = 1  # 1 = center alignment (WD_PARAGRAPH_ALIGNMENT.CENTER)
                run = para.add_run()
                run.add_picture(abs_image_path, width=Inches(final_width))

                # Add success message with width info
                if width_adjusted:
                    doc.save(abs_filename)
                    return f"Picture {image_path} added to {filename} (width adjusted from {width} to {final_width:.2f} inches to fit within 90% of A4 width and centered)"
                else:
                    doc.save(abs_filename)
                    return f"Picture {image_path} added to {filename} (width: {final_width:.2f} inches, centered)"
            else:
                # No width specified, add picture without width constraint and center it
                para = doc.add_paragraph()
                ...
                doc.save(abs_filename)
                return f"Picture {image_path} added to {filename} (centered, no width specified)"

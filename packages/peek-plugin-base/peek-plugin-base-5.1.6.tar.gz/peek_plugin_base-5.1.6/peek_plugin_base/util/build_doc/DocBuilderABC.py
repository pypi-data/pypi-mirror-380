...
class DocBuilderABC(BuilderABC):
    ...
    def _writePluginsToc(
        self, docDir: str, pluginDetails: [PluginDocDetail]
    ) -> None:
    ...
    def _writePluginToc(
        ...
        for pluginDetail in pluginDetails:
            ...
            self._writeFileIfRequired(
                docDir, "%s_toc.rst" % pluginDetail.pluginName, contents
            )

    def _writePluginsApiConf(
        self, docDir: str, pluginDetails: [PluginDocDetail]
    ) -> None:
    ...
    def _syncPluginFiles(
        ...
        for pluginDetail in pluginDetails:
            ...
            srcDir = os.path.join(
                pluginDetail.pluginRootDir, pluginDetail.docDir
            )
            if not os.path.exists(srcDir):
                logger.warning(
                    "%s doc dir %s doesn't exist",
                    pluginDetail.pluginName,
                    pluginDetail.docDir,
                )
                continue
            ...
            self.fileSync.addSyncMapping(
                srcDir, linkPath, excludeFilesRegex=excludeFilesRegex
            )

...
if platform.is_darwin():
    ...
    try:
        from watchdog.observers.polling import (
            PollingObserver as WatchdogObserver,
        )
        ...
        logger.warning(
            "Failed to import kqueue. Fall back to Watchdogs default."
        )
...
class FrontendFileSync:
    ...
    def startFileSyncWatcher(self):
        ...
        reactor.addSystemEventTrigger(
            "before", "shutdown", self.stopFileSyncWatcher
        )
    ...
    def syncFiles(self):
        for cfg in self._dirSyncMap:
            ...
            if cfg.deleteExtraDstFiles:
                for obsoleteFile in (
                    existingFiles - srcFiles - destCompiledFiles
                ):
    ...
    def _loadFollowingOverlayFileSet(self, cfg: FileSyncCfg) -> Set[str]:
        results = set()
        cfgs: List[FileSyncCfg] = self._dirSyncMap[
            self._dirSyncMap.index(cfg) + 1 :
        ]
        for cfgIter in cfgs:
...
class _FileChangeHandler(FileSystemEventHandler):
    ...
    def _updateFileContents(self, srcFilePath):
        ...
        if self._cfg.parentMustExist and not os.path.isdir(parentDstDir):
            logger.debug(
                "Skipping sync, parent doesn't exist. dstDir=%s", self._dstDir
            )
            return
        ...
        logger.debug(
            "Syncing %s -> %s",
            srcFilePath[len(self._srcDir) + 1 :],
            self._dstDir,
        )
    ...
    def on_created(self, event):
        if not isinstance(event, FileCreatedEvent) or event.src_path.endswith(
            "__"
        ):
            return
    ...
    def on_deleted(self, event):
        if not isinstance(event, FileDeletedEvent) or event.src_path.endswith(
            "__"
        ):
            return
        ...
        logger.debug(
            "Removing %s -> %s",
            event.src_path[len(self._srcDir) + 1 :],
            self._dstDir,
        )

    def on_modified(self, event):
        if not isinstance(event, FileModifiedEvent) or event.src_path.endswith(
            "__"
        ):
            return

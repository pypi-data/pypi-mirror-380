...
class RubetekSocketAPI:
    ...
    async def _send_request(self, url: str, method: str = 'GET', params: Dict[str, Any] = None,
        ...
        while True:
            ...
            try:
                async with self.session.request(method, url, params=params, json=json, headers=headers_) as response:
                    ...
                    if response.status == 401:
                        raise UnauthorizedRubetekSocketAPIError(json_response)
            ...
            except UnauthorizedRubetekSocketAPIError as e:
                self._logger.warning('Response=%s UnauthorizedRubetekSocketAPIError=%s, trying get access_token',
                                     request_id, e)

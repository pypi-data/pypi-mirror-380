...
use crate::helpers::common::{mean_f, linreg_f};

// /// Unit root Phillips-Perron test
// /// 
// /// # Parameters
// /// - `x`: The time series data
// /// - `freq`: Frequency of the time series (default = 1)
// /// 
// /// # Returns
// /// HashMap containing 'unitroot_pp' statistic
...
fn ur_pp(x: &[f64]) -> Result<f64, String> {
    ...
    let myy = (1.0 / (n * n) as f64) * y.iter().map(|&yi| yi * yi).sum::<f64>();
    let my = (n as f64).powf(-1.5) * y.iter().sum::<f64>();

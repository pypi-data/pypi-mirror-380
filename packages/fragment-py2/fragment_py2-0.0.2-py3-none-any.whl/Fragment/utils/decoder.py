...
def to_json_result(payload: str, decoded_text: str) -> DecoderResult:
    """Форматирует результат в объект с поддержкой точечной нотации"""
    return DecoderResult(payload=payload, decoded=decoded_text)


def decoder(payload: str) -> DecoderResult:
    """
    Декодируем payload в читаемый текст и возвращаем в виде объекта с поддержкой точечной нотации.
    
    Args:
        payload (str): Base64 закодированная строка для декодирования
        
    Returns:
        DecoderResult: Объект с исходным payload и декодированным текстом

_Bc="Twilio 'from' number not configured. Cannot send SMS."
_Bb='completedAt'
_Ba='Error: URL template evaluation failed.'
_BZ='contentType'
_BY='access_token'
_BX='token_type'
_BW='client_credentials'
_BV='function_call'
_BU='Using environment credentials/IAM role for S3.'
_BT='boto3 library is required for S3 operations.'
_BS='Using Application Default Credentials for GCS.'
_BR='gcs_service_account'
_BQ='google-cloud-storage library is required for GCS operations.'
_BP='action_action_key'
_BO='__builtins__'
_BN='destinationVariable'
_BM='sourceVariable'
_BL='promptName'
_BK='unknown_intent'
_BJ='Error: AI response was malformed (function call had no name).'
_BI='resp_mime_type'
_BH='gemini-2.5-flash-preview-04-17'
_BG='safetySettings'
_BF='mime_type'
_BE='assistant'
_BD='noInputIntentNameList'
_BC='variables'
_BB='DEEPSEEK_API_KEY'
_BA='ANTHROPIC_API_KEY'
_B9='OPENAI_API_KEY'
_B8='EMAIL_FROM_ADDRESS'
_B7='EMAIL_API_KEY'
_B6='GEMINI_API_KEY'
_B5='GCP_REGION'
_B4='GCP_PROJECT'
...
_B2='user_input'
_B1='message'
_B0='Journey token not available.'
_A_='Accept-Encoding'
_Az='client_secret'
_Ay='client_id'
_Ax='grant_type'
...
_Aq='function'
_Ap='header'
_Ao='gemini-2.0-flash'
_An='model_provider'
_Am='agentDefaultIntent'
_Al='intentContainsPairsList'
_Ak='intentNameList'
_Aj='action_flow'
_Ai='action_type'
_Ah='iframeId'
...
class AsyncPinionAIClient:
	...
	def __init__(self,create_key,agent_id,host_url,client_id,client_secret,version=_A,initial_engagement_data=_A,grpc_server_address=_A,gcp_project_id=_A,gcp_region=_A,gemini_api_key=_A,twilio_account_sid=_A,twilio_auth_token=_A,twilio_number=_A,email_api_key=_A,email_from_address=_A,openai_api_key=_A,anthropic_api_key=_A,deepseek_api_key=_A):
		...
		self._agent_id=agent_id;self._host_url=host_url.rstrip('/');self._client_id=client_id;self._client_secret=client_secret;self._version=version;self._grpc_server_address=grpc_server_address or os.environ.get(_B3,'localhost:50051');self._gcp_project_id=gcp_project_id or os.environ.get(_B4);self._gcp_region=gcp_region or os.environ.get(_B5);self._gemini_api_key=gemini_api_key or os.environ.get(_B6);self._twilio_account_sid=twilio_account_sid or os.environ.get(_AP);self._twilio_auth_token=twilio_auth_token or os.environ.get(_AQ);self._twilio_number=twilio_number or os.environ.get(_AR);self._email_api_key=email_api_key or os.environ.get(_B7);self._email_from_address=email_from_address or os.environ.get(_B8);self._openai_api_key=openai_api_key or os.environ.get(_B9);self._anthropic_api_key=anthropic_api_key or os.environ.get(_BA);self._deepseek_api_key=deepseek_api_key or os.environ.get(_BB);self._http_session=httpx.AsyncClient(base_url=self._host_url,timeout=12e1,follow_redirects=_D);self._token=_A;self._session_id=_A;self._raw_session_data=_A;self.var={};self.engagement=''
	...
	async def process_user_input(self,user_input='',sender=_Z):
		...
		if not self.transfer_requested:
			...
			if intent_type!=_Ac:
				if intent_input_vars:
					agent_vars_config=self._raw_session_data.get(_C,{}).get(_E,{}).get(_C,{}).get(_J,{}).get(_BC,[])
				...
				if action_key_for_input not in original_intent_input_vars and self._class_intent_data:final_response=f"'{user_input}' is a {intent_data[_B]} . Please respond to trigger an intent or input I am listening for such as: {self.var.get(_BD,[])}."
			if intent_input_vars:
				...
				if not final_response:
					if not self.authorized and'private'in self._privacy_level or I in self._privacy_level:
						...
						if customer_id and enrolled:
							pipeline_key=self.var[J]if I in self._privacy_level else self.var['authentication_pipeline'];json_payload={_Ad:pipeline_key,_h:{_c:delivery_method,_d:self.phone_number},_Ae:{_Af:self.unique_id,_d:self.phone_number},_E:{_Ag:self._session_id},_a:language}
		...
		if final_response:self.chat_messages.append({_S:_BE,_Q:final_response})
	...
	async def send_grpc_message(self,message_text):
		...
		try:recipient_id=_BE if self.grpc_sender_id==_Z else _Z;request=ChatMessageRequest(thread_id=1,message=message_text,sender_id=self.grpc_sender_id,recipient_id=recipient_id,session_id=str(self._session_id));await self._grpc_stub.SendMessage(request);logger.info(f"gRPC message sent by {self.grpc_sender_id}: {message_text}")
	...
	def _extract_data_from_session(self,session_data_root):
		'Extracts variables and agent configuration into the var dictionary.';var_dict={};agent_config=session_data_root.get(_E,{}).get(_C,{}).get(_J,{});global_to_self_attr_map={_B3:'_grpc_server_address',_B4:'_gcp_project_id',_B5:'_gcp_region',_B6:'_gemini_api_key',_AP:'_twilio_account_sid',_AQ:'_twilio_auth_token',_AR:'_twilio_number',_B7:'_email_api_key',_B8:'_email_from_address',_B9:'_openai_api_key',_BA:'_anthropic_api_key',_BB:'_deepseek_api_key'}
		for variable_def in agent_config.get(_BC,[]):
		...
		var_dict[_Ak]=intent_names;var_dict[_BD]=no_input_intent_names;var_dict['inputIntentNameList']=input_intent_names;var_dict['intentPrivacyPairsList']=intent_privacy_pairs;var_dict['intentTypePairsList']=intent_type_pairs;var_dict[_Al]=intent_contains_word_pairs;resolved_action_key_pairs=[]
		...
		var_dict['intentActionPairsList']=resolved_action_key_pairs;var_dict[_A6]=agent_config.get(_A6);var_dict[_Am]=agent_config.get('defaultIntent');return var_dict
	...
	async def _prompt_response(self,target_prompt,user_input=''):
		...
		if model_provider==_b:
			...
			if prompt_config.get(_A8):
				...
				if file_uri:content_parts.append(Part.from_uri(file_uri=file_uri,mime_type=prompt_config[_BF]))
			content_parts.append(current_prompt_text);gen_config=google_genai_types.GenerateContentConfig(system_instruction=system_instruction if system_instruction else _A,temperature=prompt_config[_A9],top_p=prompt_config[_V],top_k=prompt_config[_AA],candidate_count=prompt_config[_AB],max_output_tokens=prompt_config[_AC],stop_sequences=prompt_config.get('stop')or _A,safety_settings=prompt_config.get(_BG))
			if model_name in[_BH]and prompt_config.get(_AD)is not _A:gen_config.thinking_config=ThinkingConfig(thinking_budget=prompt_config[_AD])
			if prompt_config.get(_AE):gen_config.response_schema=prompt_config[_AE];gen_config.response_mime_type=prompt_config.get(_BI,_M)
		...
		if function_calls:
			...
			if not function_name:logger.error(f"LLM response contained a function call with no name: {function_call}");return self.var,_BJ
		...
		if llm_response_text:
			if prompt_config[_F]==_A2:
				...
				matched_intent_key=self._match_intent_from_text(raw_response,self.var.get(_Al,[]),self.var.get(_Am,_BK),self.var.get(_Ak,[]));self.var[prompt_resp_var_name]=matched_intent_key
	...
	async def _async_prompt_response(self,target_prompt,user_input='',var_snapshot=_A):
		...
		if function_calls:
			function_call=function_calls[0];function_name=function_call.get(_B);function_args=function_call.get(_I,{})
			if not function_name:logger.error(f"LLM async response contained a function call with no name: {function_call}");return prompt_resp_var_name,_BJ
		...
		if llm_response_text:
			if prompt_config[_F]==_A2:
				...
				final_value_for_var=self._match_intent_from_text(raw_response,current_var.get(_Al,[]),current_var.get(_Am,_BK),current_var.get(_Ak,[]))
	...
	async def _select_mcp_config(self,mcp_group_names,current_var,user_input,mcp_group_config):
		...
		if prompt_name:
			...
			prompt_model_provider=prompt_config.get('modelProvider',_b);prompt_model=_Ao
	...
	async def _run_mcp_action(self,current_var,mcp_name,user_input=_A):
		...
		try:
			...
			prompt_name=mcp_config.get(_BL,'');prompt_config=_A;prompt_resp_var_name=_A;current_prompt_text=user_input or''
			if prompt_name:
				logger.info(f"MCP '{mcp_name}' is using prompt '{prompt_name}'.");prompt_config,_,_=self._get_prompt_details(prompt_name,current_var)
				if not prompt_config:error_msg=f"Error: Prompt '{prompt_name}' for MCP '{mcp_name}' not found.";logger.error(error_msg);return current_var,error_msg
			...
			if is_http_call:
				eval_mcp_url=mcp_url;http_headers=self._evaluate_vars_in_structure(mcp_config.get(_Ap,{}),current_var);mcp_connector=mcp_config.get(_R,'')
				if mcp_connector:
			...
			if session_context:
				async with session_context as session:
					...
					logger.info(f"Asking LLM to choose a mcp activity: '{current_prompt_text[:100]}...'");model_provider=prompt_config.get(_An,_b)if prompt_config else _b;model_to_use=_Ao
	...
	async def _run_agent_loop(self,prompt,client,session,model_id,max_tool_turns,initial_temperature,tool_call_temperature,mcp_description=_A):
		...
		while has_function_calls and turn_count<max_tool_turns:
			turn_count+=1;function_calls_to_execute=[part.function_call for part in latest_content.parts if part.function_call];tool_response_parts=await self._execute_tool_calls(function_calls_to_execute,session);contents.append(google_genai_types.Content(role=_Aq,parts=tool_response_parts));subsequent_config=google_genai_types.GenerateContentConfig(temperature=tool_call_temperature,**base_gen_config_dict);response=await client.aio.models.generate_content(model=model_id,contents=contents,config=subsequent_config)
	...
	async def _run_api_action(self,current_var,api_name):
		...
		headers={}
		if api_config.get(_Ap):
			try:headers.update(json.loads(api_config[_Ap]))
	...
	def _run_parser_action(self,current_var,parser_name):
		...
		source_var_name=parser_config.get(_BM)
	...
	def _run_merger_action(self,current_var,merger_name):
		...
		dest_var_name=merger_config.get(_BN)
	...
	async def _run_script_action(self,var,script_name):
		...
		try:
			if script_type=='javascript':
				...
				local_script_vars=var.copy();logger.warning('Executing Python scripts from the database is a security risk and is currently disabled.');exec(script_code,{_BO:{}},local_script_vars)
	...
	async def _evaluate_rule(self,expr,var):
		...
		try:
			...
			try:return eval(formatted_expr,{_BO:{}},{_q:var})
	...
	async def _run_journey_action(self,current_var,action_details):
		"Handles a 'journey' type action, from either an intent or an action entity.";B='configuration';A='payment_description';action_key_var_name=action_details.get(_BP);action_key=current_var.get(action_key_var_name)if action_key_var_name else _A
		...
		delivery_method=action_details.get(_As);language=action_details.get(_a);_,self.unique_id,self.phone_number=format_phone(self.phone_number);json_payload={_Ad:action_key,_h:{_c:delivery_method,_d:self.phone_number},_Ae:{_Af:self.unique_id,_d:self.phone_number},_E:{_Ag:self._session_id},_a:language}
		if current_var.get(_i):json_payload[_Ah]=current_var[_i]
		if action_details.get(_At)=='credit card payment'and current_var.get(A):json_payload[B]={'credit-card-payment':{'currency':current_var.get('payment_currency'),'lineItems':[{'title':current_var.get(A),'amount':current_var.get('payment_amount'),'quantity':current_var.get('payment_quantity')}]}}
		if action_details.get(_At)=='one time passcode':json_payload[B]={'random-code':{'code':'6'}}
		if action_details.get(_As)=='voice':json_payload[_h]={_c:delivery_method,_d:self.phone_number,'callOperator':'twilio'}
	...
	def _run_transfer_action(self,current_var,action_details):
		...
		return current_var,final_response
	def _map_to_standard_action_details(self,data):'Maps fields from an intent or action entity to a standard action_details dictionary.';return{_B:data.get(_B),_Ai:data.get('actionType'),_BP:data.get(_p),_As:data.get(_Aa),_a:data.get(_a),_At:data.get('actionEvent'),_e:data.get('actionResponseMessage'),_Aj:data.get(_Ab,{})}
	...
	async def _gcs_write(self,bucket_name,file_path,content_bytes,connector_name):
		...
		if not storage:raise ImportError(_BQ)
		gcs_client=_A
		if connector_name:
			conn_config=self._get_connector_details(connector_name)
			if conn_config and conn_config.get(_z)==_BR:sa_info=json.loads(conn_config.get(_A0,'{}'));gcs_client=storage.Client.from_service_account_info(sa_info);logger.info(f"Using GCS service account from connector '{connector_name}'.")
		if not gcs_client:gcs_client=storage.Client();logger.info(_BS)
	...
	async def _s3_write(self,bucket_name,file_path,content_bytes,connector_name):
		...
		if not boto3:raise ImportError(_BT)
		s3_client=_A
		if connector_name:
			conn_config=self._get_connector_details(connector_name)
			if conn_config and conn_config.get(_z)=='aws_iam':s3_client=boto3.client('s3',aws_access_key_id=conn_config.get(_Au),aws_secret_access_key=conn_config.get(_A0));logger.info(f"Using AWS IAM credentials from connector '{connector_name}'.")
		if not s3_client:s3_client=boto3.client('s3');logger.info(_BU)
	...
	def _configure_tools_for_request(self,direct_tool_configs,processed_fd_dicts,model_name):
		...
		RESTRICTED_MODELS_FOR_MIXED_TOOLS={_Ao};has_functions=bool(function_declarations);has_search=any(t.google_search for t in final_tools_for_config);model_is_restricted=model_name in RESTRICTED_MODELS_FOR_MIXED_TOOLS
		...
		try:
			...
			if response.candidates and len(response.candidates)>0:
				...
				if candidate.content and candidate.content.parts:
					fc_parts=[part.function_call for part in candidate.content.parts if hasattr(part,_BV)and part.function_call]
					if fc_parts:
						function_calls=[]
	...
	async def _get_openai_response_async(self,model_name,messages,prompt_config,tools=_A):
		...
		http_client=httpx.AsyncClient();client=AsyncOpenAI(api_key=self._openai_api_key);openai_params={_U:model_name,_r:messages,_A1:prompt_config.get(_A9),_Av:prompt_config.get(_AC),_V:prompt_config.get(_V),'frequency_penalty':prompt_config.get('frequencyPenalty'),'presence_penalty':prompt_config.get('presencePenalty')}
		if prompt_config.get(A):openai_params[A]=prompt_config.get(A)
		if tools:openai_params[_O]=tools;openai_params[_Aw]='auto'
	...
	async def _get_anthropic_response_async(self,model_name,system_prompt,messages,prompt_config,tools=_A):
		...
		client=AsyncAnthropic(api_key=self._anthropic_api_key);anthropic_params={_U:model_name,_t:system_prompt,_r:messages,_A1:prompt_config.get(_A9),_Av:prompt_config.get(_AC,4096),_V:prompt_config.get(_V),_AA:prompt_config.get(_AA)}
		if tools:anthropic_params[_O]=tools;anthropic_params[_Aw]={_F:'auto'}
	...
	async def _get_token_api(self,host,client_id,client_secret):
		url='/token';headers={_Y:_M};data={_Ax:_BW,_Ay:client_id,_Az:client_secret}
		try:response=await self._http_session.post(url,headers=headers,json=data);response.raise_for_status();auth_response=response.json();return f"{auth_response[_BX]} {auth_response[_BY]}"
	...
	async def _start_session_api(self,host,agent_id_to_start,token):
		url=f"/agent/{agent_id_to_start}";headers={_A_:_AL,_l:_x,_L:token}
		try:
	...
	async def _start_version_api(self,host,agent_id_to_start,token,version_str):
		url=f"/version/{agent_id_to_start}/{version_str}";headers={_A_:_AL,_l:_x,_L:token}
		try:
			response=await self._http_session.get(url,headers=headers);response.raise_for_status();data=response.json();session_uid=data.get(_C,{}).get(_E,{}).get('uid')
			if session_uid:return session_uid,data
	...
	async def _get_session_api(self,host,session_uid,token):
		url=f"/session/{session_uid}";headers={_l:_x,_L:token,_A_:_AL}
		try:
	...
	async def _get_token_for_connector(self,connector_config):
		'Gets auth token for a given connector configuration.';url=connector_config.get(_W);client_id=connector_config.get(_Au);client_secret=connector_config.get(_A0);grant_type=connector_config.get(_z,_BW);content_type=connector_config.get(_BZ,_M);use_header_payload=connector_config.get('headerPayload',_G);scope=connector_config.get(_A5,_A)
		...
		if use_header_payload:client_keys=f"{client_id}:{client_secret}";client_keys_b64=base64.b64encode(client_keys.encode()).decode();headers[_L]=f"Basic {client_keys_b64}";headers[_Y]='application/x-www-form-urlencoded';data_to_send=f"grant_type={grant_type}"
			...
			if _y in content_type.lower():
				data_to_send={_Ax:grant_type,_Ay:client_id,_Az:client_secret}
				if scope:data_to_send[_A5]=scope
			elif'x-www-form-urlencoded'in content_type.lower():
				form_data=[(_Ax,grant_type),(_Ay,client_id),(_Az,client_secret)]
		...
		try:
			...
			async with httpx.AsyncClient()as client:response=await client.post(url,headers=headers,content=payload);response.raise_for_status();auth_response=response.json();token_type=auth_response.get(_BX,'Bearer').capitalize();return f"{token_type} {auth_response[_BY]}"
	...
	async def _generic_api_post_put(self,current_var,api_config,headers,method):
		...
		if not url:logger.error(f"URL could not be evaluated for API '{api_config.get(_B)}'. Template: {url_template}");return current_var,_A,_A,_Ba
	...
	async def _generic_api_get(self,current_var,api_config,headers):
		...
		if not url:logger.error(f"URL could not be evaluated for API GET '{api_config.get(_B)}'. Template: {url_template}");return current_var,_A,_A,_Ba
	...
	async def _journey_send_pipeline(self,current_var,json_payload,delivery_method,user_input=_A):
		...
		if not bearer_token:return _A,_B0
	...
	async def _journey_execution_status_check(self,execution_id,current_var,max_retries=12,delay_seconds=5,user_input=_A):
		...
		if not bearer_token:return _G,_B0
		...
		for attempt in range(max_retries):
			try:
				...
				if status_data.get(_Bb):return _D,status_data.get('outcome',{}).get(_B1,'Task completed successfully.')
			...
			try:
				...
				for exec_item in executions:
					if exec_item.get(A,{}).get('key')==pipeline_key_to_check or exec_item.get(A,{}).get('id')==pipeline_key_to_check:
						if exec_item.get(_Bb):return _D,exec_item.get('outcome',{}).get(_B1,'Pipeline completed.')
						break
	...
	async def _twilio_sms_message(self,sms_body,to_phone,from_phone_twilio=_A):
		...
		if not twilio_from_number:logger.error(_Bc);return
	...
	async def _infobip_send_email(self,plain_text_body,html_body_content,to_address,subject,from_address=_A,token=_A):
		...
		infobip_url='https://g9egk8.api.infobip.com/email/4/messages';headers={_L:token,_Y:_M,_AN:_M};payload={_r:[{'sender':from_address,'destinations':[{'to':[{'destination':to_address}]}],_Q:{_Ar:subject,_P:plain_text_body,'html':html_body_content}}]}
		try:
	...
	async def _ms_graph_send_email(self,plain_text_body,html_body_content,to_address,subject,from_address=_A,token=_A):
		...
		graph_api_endpoint='https://graph.microsoft.com/v1.0';url=f"{graph_api_endpoint}/users/{from_address}/sendMail";headers={_L:token,_Y:_M};email_payload={_B1:{_Ar:subject,_T:{_BZ:'HTML',_Q:html_body_content},'toRecipients':[{'emailAddress':{'address':to_address}}]},'saveToSentItems':'true'}
		try:
			...
			for(key,value)in list(data_structure.items()):
				if isinstance(value,str):
					if A in value or B in value:
						try:eval_globals={_q:context_vars,C:context_vars,D:context_vars,_B2:user_input};data_structure[key]=eval(f"f'''{value}'''",eval_globals,{})
			...
			for(i,item)in enumerate(data_structure):
				if isinstance(item,str):
					if A in item or B in item or user_input is not _A and'{user_input}'in item:
						try:eval_globals={_q:context_vars,C:context_vars,D:context_vars,_B2:user_input};data_structure[i]=eval(f"f'''{item}'''",eval_globals,{})
	...
	def _evaluate_f_string(self,template_string,context_vars,user_input=_A):
		...
		try:eval_namespace={_q:context_vars,_B2:user_input};return eval(f"f'''{template_string}'''",eval_namespace,{})
	...
	def _translate_tools_for_provider(self,provider,direct_tool_configs,processed_fd_dicts):
		...
		translated_tools=[]
		if provider==_f:
			for fd in processed_fd_dicts:translated_tools.append({_F:_Aq,_Aq:{_B:fd.get(_B),_K:fd.get(_K),_AO:fd.get(_AO)}})
		elif provider==_s:

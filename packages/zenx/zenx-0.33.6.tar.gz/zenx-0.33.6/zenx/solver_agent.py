...
class SolverAgent():
    ...
    async def check_task_status(self, task_id: str, key: str) -> None:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.debug("completed", task_id=task_id)
    ...
    async def track_blueprints(self, target: Dict) -> None:
        ...
        while True:
            ...
            self.logger.info("available", size=current_size, redis_key=redis_key)

            if current_size < self.min_blueprints and not self.pending_tasks:
                ...
                self.logger.debug("requested", count=required_count, challenge=target['challenge'], spider=target['spider'])

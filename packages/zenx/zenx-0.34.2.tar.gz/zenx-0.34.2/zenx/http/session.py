...
class RedisSessionManager(SessionManager):
    ...
    async def init_session_pool(self) -> None:
        self.logger.info("initializing", session_pool_size=self.session_pool.qsize(),session_manager=self.name)
        while self.session_pool.qsize() < self.settings.SESSION_POOL_SIZE:
            self.logger.info("getting_blueprint", session_manager=self.name)
            ...
            await self.put_session(session)
            self.logger.info("updating", session_pool_size=self.session_pool.qsize(), blueprint_redis_key=self.settings.SESSION_BLUEPRINT_REDIS_KEY, session_manager=self.name)
        self.logger.info("initialized", session_pool_size=self.session_pool.qsize(), kwargs=self.session_init_args, session_manager=self.name)

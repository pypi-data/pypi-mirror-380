...
class ExperimentalSchedular(Schedular):
    name = "experimental"
    ...
    async def _dispatcher(self, task_func: Callable[[], Coroutine], start_time: float) -> None:
        ...
        while True:
            ...
            if delay > 0.050:
                await asyncio.sleep(delay - 0.050) # wake 10ms before the target time
            
            precision_start = time.time() * 1000
            while loop.time() < target_time:
                await asyncio.sleep(0)
            precision_end = time.time() * 1000

            actual_wake_ms = int(time.time() * 1000)
            target_ms = int((start_time + tick * tick_interval + (time.time() - loop.time())) * 1000)
            error_ms = actual_wake_ms - target_ms
            self.logger.debug("scheduler_wake", 
                             tick=tick,
                             target_ms=target_ms, 
                             actual_ms=actual_wake_ms, 
                             error_ms=error_ms,
                             precision_loop_duration_ms=precision_end - precision_start)
            await self.execute_task(task_func)
    ...
    async def run(self, task_func: Callable[[], Coroutine], start_time: float) -> None:
        self.logger.warning("experimental schedular is not stable, use at your own risk")
        tick_interval = self.settings.TASK_INTERVAL_SECONDS / self.settings.CONCURRENCY # 5/10 = 0.5
...
# class FixedRateSchedular(Schedular):
#     """
#     High precision schedular with zero drift. Does not care about task execution time.
#
#     Example:
#         CONCURRENCY = 100
#         TASK_INTERVAL_SECONDS = 5
#         >> Fires 1 task every 50ms (20 tasks/second)
#     """
#     ...

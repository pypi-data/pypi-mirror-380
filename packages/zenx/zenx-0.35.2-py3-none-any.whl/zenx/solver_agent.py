...
class SolverAgent():
    ...
    async def log_blueprints(self, targets: List[Dict]) -> None:
        redis_keys = [f"{target['challenge']}:{target["spider"]}" for target in targets]
        sizes = {}
        while True:
            for key in redis_keys:
                current_size = await self.r.llen(redis_key)
                sizes[key] = current_size
            self.logger.info("available", blurprints=sizes)
    ...
    async def run(self) -> None:
        ...
        async with asyncio.TaskGroup() as tg:
            ...
            tg.create_task(self.log_blueprints(targets))

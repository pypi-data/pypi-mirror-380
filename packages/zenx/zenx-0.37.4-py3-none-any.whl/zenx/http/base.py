...
class HttpClient(ABC):
    ...
    def _process_log_extra(self, log_ctx: Dict, response: Any, log_extra: dict | None) -> None:
        if log_extra:
            log_ctx["extra"] = {}
            for key, value in log_extra.items():
                if key == "headers" and isinstance(value, list) and hasattr(response, "headers"):
                    if len(value) > 0:
                        log_headers = {header.replace("-", "_"):response.headers.get(header) for header in value}
                    else:
                        log_headers = dict(response.headers)
                    log_ctx["extra"].update(log_headers)
                if key == "response" and isinstance(value, list):
                    log_attrs = {attr:getattr(response, attr, None) for attr in value}
                    log_ctx["extra"].update(log_attrs)


    @abstractmethod
    async def request(self, url: str, method: str = "GET", dont_filter: bool = False, log_extra: dict | None = None, **kwargs) -> Response | None:
        """ request with session """
        ...


    @abstractmethod
    async def direct_request(self, url: str, method: str = "GET", dont_filter: bool = False, log_extra: dict | None = None, **kwargs) -> Response | None:

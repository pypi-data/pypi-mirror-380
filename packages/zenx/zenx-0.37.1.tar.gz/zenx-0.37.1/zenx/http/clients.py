        ...
        try:
            ...
            if log_extra: 
                ...
                for key, value in log_extra.items():
                    ...
                    elif key == "response" and isinstance(value, list):
                        log_attrs = {attr:getattr(response, attr) for attr in value}
                        log_ctx["extra"].update(log_attrs)
                log_ctx.update(log_extra)
            self.logger.info("response", **log_ctx)

...
def update_bins(capacity_spec, xy, xybin, rbin, args=()):
    ...
    if callable(capacity_spec):        
        ...
        cap = np.zeros_like(rbin)
        for j, w in index.items():  # NB: w[0] extracts the ndarray from the tuple (w,)
            cap[j] = capacity_spec(w[0], *args)
    else:
        cap = np.bincount(bin_num, weights=capacity_spec, minlength=rbin.size)
    
    return xybin, npix, cap, bin_num
...
def regularization(xy, xybin, target_capacity, capacity_spec, verbose, args, maxiter):
    ...
    for it in range(maxiter):
        ...
        xybin, npix, cap, bin_num = update_bins(capacity_spec, xy, xybin, rbin, args)
    ...
    return bin_num, xybin, rbin, cap, npix, it
...
class PowerBin:
    ...
    *capacity* per bin (e.g., signal-to-noise squared).
    ...
    def __init__(
        ...
        npix = xy.shape[0]
        if not callable(capacity_spec):
            if capacity_spec.ndim != 1 or capacity_spec.shape[0] != npix:
                raise ValueError(f"If 'capacity' is an array, it must have shape ({npix},), but got {capacity_spec.shape}")
            if not np.all(np.isfinite(capacity_spec)):
                raise ValueError("If 'capacity' is an array, it must contain only finite values.")

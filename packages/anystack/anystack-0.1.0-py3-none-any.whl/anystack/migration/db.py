...
class DBMigration(BaseMigration):
    ...
    async def init_table_and_get_index(self) -> int:
        await self.db.sql(
            f"CREATE TABLE IF NOT EXISTS {self.table_name} (id INTEGER PRIMARY KEY AUTOINCREMENT, version TEXT NOT NULL)"
        )
        migration_index = await self.db.sql(f"SELECT COUNT(*) FROM {self.table_name}")

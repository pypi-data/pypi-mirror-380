    ...
    async def cog_load(self) -> None:
        ...
        with set_env(key="PICCOLO_CONF", value="tidegear.sentinel.db.piccolo_conf", logger=self._sentinel_logger):
            self.db = await register_cog(
                ...
                tables=[Change, Moderation, PartialChannel, PartialGuild, PartialRole, PartialUser],
                trace=trace,
            )
        ...
        for i in range(0, count, pagesize):
            ...
            for moderation in moderations[i : i + pagesize]:
                try:
                    ...
                    self._tidegear_logger.exception("Moderation %i does not have a target!", moderation.id)
                    continue
                except NotFoundError:
                    self._tidegear_logger.exception("Could not find target for moderation %i!", moderation.id)
                    continue

                try:
                    moderator = await moderation.moderator()
                except UnsetError:
                    self._tidegear_logger.exception("Moderation %i does not have a moderator!", moderation.id)
                    continue
                except NotFoundError:
                    self._tidegear_logger.exception("Could not find moderator for moderation %i!", moderation.id)
                    continue

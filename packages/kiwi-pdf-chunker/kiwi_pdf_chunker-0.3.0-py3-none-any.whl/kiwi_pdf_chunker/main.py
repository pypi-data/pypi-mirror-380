...
class PDFParser: 
    ...
    def _initialize_docling_converter(self):
        """Initialize the docling document converter with OCR disabled."""
        try:
            # Disable OCR entirely in docling
            pipeline_options = PdfPipelineOptions(do_ocr=False)

            # Initialize converter without pipeline_options
            converter = DocumentConverter()

            # Set pipeline options after initialization
            converter.pipeline_options = pipeline_options
            
            logger.info("Docling converter initialized successfully with OCR disabled.")
            return converter
            
        except ImportError as e:
            logger.warning(f"Could not import docling pipeline options: {e}")
            # Fallback to basic converter if pipeline options are not available
            try:
                converter = DocumentConverter()

...
def test_raster_calculator(api, mock_success_task_data):
    api.get_task.return_value = Task(api, uuid=mock_success_task_data['uuid'], data=mock_success_task_data)
    api.post.return_value = {
        'task_id': mock_success_task_data['uuid']
    }

    task = GeoboxClient.raster_calculator(
        api,
        variables={"RED": "r-1"},
    ...
    api.post.assert_called_once_with(
        endpoint='analysis/calculator/',
        payload={'variables': {'RED': 'r-1'}, 'expr': 'where(SLOPE>30,1,0)', 'output_raster_name': 'test', 'resample': 'bilinear', 'out_dtype': 'float32', 'dst_nodata': -9999},
        is_json=False
    )


def test_create_constant_raster(api, mock_success_task_data):
    api.get_task.return_value = Task(api, uuid=mock_success_task_data['uuid'], data=mock_success_task_data)
    api.post.return_value = {
        'task_id': mock_success_task_data['uuid']
    }

    task = GeoboxClient.create_constant_raster(
        api,
        output_raster_name='const',
        extent='0,0,100,100',
        value=10
    )
    ...
    assert task.uuid == mock_success_task_data['uuid']
    api.post.assert_called_once_with(
        endpoint='analysis/constant/',
        payload={'output_raster_name': 'const', 'extent': '0,0,100,100', 'value': 10, 'pixel_size': 10, 'dtype': 'float32', 'nodata': -9999},
        is_json=False
    )
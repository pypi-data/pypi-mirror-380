...
@pytest.mark.parametrize("layer_type", [type for type in LayerType])
def test_rasterize(api, mock_success_task_data, mock_vector_data, layer_type):
    ...
    layer = VectorLayer(api, uuid=mock_vector_data['uuid'], data=mock_vector_data, layer_type=layer_type)
    layer.rasterize(output_raster_name='test')

    assert type(task) == Task
    assert task.uuid == mock_success_task_data['uuid']
    api.post.assert_called_once_with(endpoint='analysis/rasterize/', payload={'layer_uuid': layer.uuid, 'output_raster_name': 'test', 'is_view': False, 'pixel_size': 10, 'nodata': -9999, 'data_type': 'int16', 'burn_value': 1}, is_json=False)


@pytest.mark.parametrize("layer_type", [type for type in LayerType])
def test_idw_interpolation(api, mock_success_task_data, mock_vector_data, layer_type, mock_field_data):
    task = Task(api, uuid=mock_success_task_data['uuid'], data=mock_success_task_data)
    api.get_task.return_value = task
    api.post.return_value = {
        'task_id': mock_success_task_data['uuid']
    ...
    layer = VectorLayer(api, uuid=mock_vector_data['uuid'], data=mock_vector_data, layer_type=layer_type)
    field = Field(layer=layer, data_type=FieldType.String, field_id=mock_field_data['id'], data=mock_field_data)
    layer.idw_interpolation(output_raster_name='test', z_field=field)

    assert type(task) == Task
    assert task.uuid == mock_success_task_data['uuid']
    api.post.assert_called_once_with(endpoint='analysis/idw/', payload={'layer_uuid': layer.uuid, 'output_raster_name': 'test', 'z_field': field.name, 'is_view': False, 'pixel_size': 10, 'power': 2.0, 'smoothing': 0.0, 'max_points': 16, 'radius': 1000, 'nodata': -9999, 'out_dtype': 'float32'}, is_json=False)
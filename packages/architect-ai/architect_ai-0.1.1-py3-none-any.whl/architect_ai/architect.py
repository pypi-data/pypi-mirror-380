...
from .architect_exceptions import ExecutableFailedToStartError, BuildGenerationError
...
class Architect:
    ...
    def _start_concurrent_executables(self):
        for executable in self.concurrent_executables:
            ...
            except ExecutableFailedToStartError as e:
                log_with_structure(
                    logger,
                    "error",
    ...
    def _generate_build(
        ...
        build_plan: str = self.client.send_build_plan_generation_request(
        ...
        return Build(build_plan, build_correlation_id)
    ...
    def generate_response(
        ...
        number_of_concurrent_requests_per_call_to_client: int = 1,
    ) -> Tuple[Build, List[ConcurrentExecutable]]:
        ...
        for attempt in range(max_attempts_to_generate_build_plan):
            try:
                ...
                # Record start time for build execution
                ...
                # Calculate and log build execution time
                ...
                return build, self.initial_concurrent_executables_started
                ...
                logger.error(f"Full build execution failure traceback:", exc_info=True)
                if attempt < max_attempts_to_generate_build_plan - 1:
                    ...
                    previous_attempt_error_message = str(e)

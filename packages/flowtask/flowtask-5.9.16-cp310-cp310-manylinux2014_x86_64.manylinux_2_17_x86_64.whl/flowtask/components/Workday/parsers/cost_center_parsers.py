...
def parse_integration_id_data(integration_data: Union[List, Dict, None]) -> Dict[str, Any]:
    ...
    # Normalize to list of ID dicts
    id_nodes: List[Dict[str, Any]] = []
    if isinstance(integration_data, dict):
        id_nodes = [n for n in ensure_list(integration_data.get("ID")) if isinstance(n, dict)]
    elif isinstance(integration_data, list):
        # Sometimes Integration_ID_Data may be a list of containers or directly a list of IDs
        for item in integration_data:
            if isinstance(item, dict) and ("ID" in item or "_value_1" in item):
                ids = ensure_list(item.get("ID", item))
                id_nodes.extend([n for n in ids if isinstance(n, dict)])
    
    for id_item in id_nodes:
        id_value = id_item.get("_value_1") or (id_item.get("ID") if isinstance(id_item.get("ID"), str) else None)
...
def parse_organization_container_data(container_data: Dict) -> Dict[str, Any]:
    ...
    if isinstance(container_ref, dict):
        id_list = ensure_list(container_ref.get("ID", []))
        container_id = extract_by_type(id_list, "Organization_Reference_ID") or extract_by_type(id_list, "Custom_Organization_Reference_ID")
        return {
            "container_organization_id": container_id,
            "container_organization_name": container_ref.get("Descriptor"),
            "container_organization_wid": extract_by_type(id_list, "WID")
        }

                ...
                if hasattr(e, "exceptions"):
                    for nested_e in e.exceptions:
                        nested_errors.append(
                            f"{type(nested_e).__name__}: {str(nested_e)}"
                        )
...
• Add `authorization_token` with a Bearer token""",
                ...
                raise RuntimeError(f"OAuth authentication failed: {str(e)}") from e
        ...
        async with self._connection_locks[server_id]:
            ...
            if not connection._is_healthy or not connection.session:
                try:
                    ...
                    return {
                        "success": False,
                        "error": f"Connection recovery failed: {str(e)}",
                    }

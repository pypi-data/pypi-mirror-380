...
# v0.1.0 architecture. This module defines:
...
class DepthsLoggerOptions:
    ...
    Overview (v0.1.0 role):
        Orchestrates lifecycle, background shipper, and buffering policy that
        apply uniformly to the six OTel tables (spans, span_events, span_links,
        logs, metrics_points, metrics_hist). Serialized to <instance>/configs/options.json.

    Fields:
        init_early_terminate: End the __init__ for DepthsLogger as soon as all config+OTel tables gets created. Useful for "depths init" CLI.
        ...
        min_auto_flush_wait_s / max_auto_flush_wait_s: Envelope for passive flushes on stop(flush="auto").
        ...
        ship_delay_after_rollover_s: Delay before queueing yesterday for ship.
        verify_grace_s / verify_timeout_s: Remote rowcount verification window.
        upload_max_workers: Parallel uploads per day.
        internal_otel_logs: If True, Depths emits minimal self-observability logs.
    """
    # --- DEVELOPER NOTES -------------------------------------------------
    # - Keep to_dict/from_dict explicit to avoid leaking future fields.
    # - Saved options are merged with constructor overrides; None means "no override".
    # - Nested producer/aggregator configs are persisted in a portable flavor.
    ...
    def to_dict(self) -> dict:
        ...
        """
        # --- DEVELOPER NOTES -------------------------------------------------
        # - Explicitly enumerate fields; do not rely on asdict to keep control over
        #   evolution & backwards-compat defaults.
        ...
        out = {
            ...
            "shipper_enabled": getattr(self, "shipper_enabled", True),
            ...
            "producer_config": LogProducerConfig().to_dict() if self.producer_config is None else self.producer_config.to_dict(),
            "aggregator_config": LogAggregatorConfig().to_dict() if self.aggregator_config is None else self.aggregator_config.to_dict(),
        }
        return out
    ...
    def from_dict(cls, d: dict) -> "DepthsLoggerOptions":
        ...
        """
        # --- DEVELOPER NOTES -------------------------------------------------
        # - Nested producer/aggregator are rehydrated via their own from_dict.
        # - Replace onto a fresh base instance to apply class defaults forward.

...
#   analytics into one place for v0.1.0.
# ======================================================================
...
class DepthsLogger:
    ...
    def _instance_dirs(root: Path) -> Dict[str, Path]:
        ...
        Returns:
            Mapping with keys: root, configs, index_dir, index_json, index_jsonl, staging_days.
        """
        # --- DEVELOPER NOTES -------------------------------------------------
    ...
    def _ensure_dirs(root: Path) -> Dict[str, Path]:
        ...
        for k in ("configs", "index_dir", "staging_days"):
    ...
    def __init__(
        ...
        *,
        ...
        Behavior (summary):
            ...
            6) Install atexit/signal handlers as configured.
            7) Optionally autostart aggregators and shipper; enqueue past days.
        ...
        # - All six tables share the same Producer/Aggregator templates; only
        #   schema and table_path differ per table.
    ...
    def start(self) -> None:
        """
        Start all table aggregators (idempotent).
    ...
    def stop(self, *, flush: Literal["auto","all","none"] = "auto") -> None:
        """
        Stop all aggregators and background shipper (idempotent).
        ...
        # - Always disarms shipper after aggregators stop.
    ...
    def _apply_select_limit(self, lf: pl.LazyFrame, *, select: list[str] | None, limit: int | None) -> pl.LazyFrame:
        if select:
    ...
    def read_spans(
        ...
        *,
        ...
        Args:
            ...
            limit: Optional max rows to return (applied lazily).
        ...
        if predicates:
            ...
            for p in predicates[1:]:
                expr = expr & p
            lf = lf.filter(expr) 
        return self._apply_select_limit(lf, select=select, limit=limit)
    ...
    def metrics(self) -> Dict[str, Any]:
        """
        Snapshot operational metrics for all aggregators.

        Returns:
            Dict with per-table counters:
            flushes, rows_scheduled_total, rows_persisted_total, rows_last_flush,
            last_commit_seconds, writer_queue_size, delta_init_ok,
            delta_last_error, and current table_path.
        """
        # --- DEVELOPER NOTES -------------------------------------------------
        # - Values are best-effort (no heavy synchronization).

        out = {"started": self._started, "current_day_utc": self._current_day, "aggregators": {}}
        for name, agg in self._aggs.items():
            m = agg.metrics
            out["aggregators"][name] = {
                "flushes": m.flushes, "rows_scheduled_total": m.rows_scheduled_total,
                "rows_persisted_total": m.rows_persisted_total, "writer_queue_size": m.writer_queue_size,
                "last_commit_seconds": m.last_commit_seconds, "delta_init_ok": m.delta_init_ok,
                "delta_last_error": m.delta_last_error, "table_path": agg.table_path
            }
        return out

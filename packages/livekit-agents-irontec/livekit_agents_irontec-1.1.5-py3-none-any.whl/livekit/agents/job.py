...
class JobContext:
    ...
    def hold_sip_participant(
        self,
        participant: rtc.RemoteParticipant | str,
    ) -> asyncio.Future[None]:  # type: ignore
        """Hold a SIP participant call.

        Args:
            participant: The participant to hold

        Returns:
            Future that completes when the hold operation is complete

        This will pause the audio/video stream from the SIP participant.
        """
        if isinstance(participant, rtc.RemoteParticipant):
            assert participant.kind == rtc.ParticipantKind.PARTICIPANT_KIND_SIP, (
                "Participant must be a SIP participant"
        ...
        task = asyncio.create_task(
            self.api.sip.hold_sip_participant(
                api.HoldSIPParticipantRequest(
                    room_name=self._room.name,
                    participant_identity=participant_identity,
                )
            ),
    ...
    def unhold_sip_participant(
        ...
        Args:
            participant: The participant to unhold

        Returns:
            Future that completes when the unhold operation is complete

        This will resume the audio/video stream from the SIP participant.
        """
        if isinstance(participant, rtc.RemoteParticipant):
            assert participant.kind == rtc.ParticipantKind.PARTICIPANT_KIND_SIP, (
                "Participant must be a SIP participant"
        ...
        task = asyncio.create_task(
            self.api.sip.unhold_sip_participant(
                api.UnholdSIPParticipantRequest(
                    room_name=self._room.name,
                    participant_identity=participant_identity,
                )
            ),

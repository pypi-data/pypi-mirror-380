...
class Supplier:
    ...
    def fetch_and_split_session_takeover_attack(self, mode, victim=None, attacker=None):
        ...
        if mode == "identification":
            # Get all labels (victim, attacker, other legit users)
            victim_y = (
                np.repeat(
                    self.test_data.y[self.test_data.y[:, victim] == 1][0],
                    len(victim_X),
                )
                .reshape(-1, len(victim_X))
                .T
            )
            attacker_y = (
                np.zeros(len(attacker_X) * self.config["user_num"])
                .reshape(-1, len(attacker_X))
                .T
            )
            attack_data_y = np.concatenate((victim_y, attacker_y))
            other_user_data_y = self.test_data.y[self.test_data.y[:, victim] == 0]

            # Get data of other legit users
            other_user_data_X = self.test_data.X[self.test_data.y[:, victim] == 0]

            np.concatenate((other_user_data_X, attack_data_X)),
            np.concatenate((other_user_data_y, attack_data_y)),
            # Retrieve other datasets
            train, valid, adver = self.train_data, self.valid_data, self.adver_data
        elif mode in ("verification", "authentication"):
            # Get labels of victim and attacker
            victim_y = (
                np.repeat([True, False], len(victim_X)).reshape(-1, len(victim_X)).T
            )
            attacker_y = (
            ...
            if mode == "verification":
                #     # Get data and labels from everyone else
                #     other_user_data_X = self.test_data.X[self.test_data.y[:, victim] == 0]
                #     other_user_data_y = self.test_data.y[self.test_data.y[:, victim] == 0]
                #     ratio = len(attack_data_y) / len(other_user_data_y)
                #     _, other_user_data_X = train_test_split(
                #         other_user_data_X, test_size=ratio, stratify=other_user_data_y
                #     )
                #     other_user_data_y = (
                #         np.repeat([False, True], len(other_user_data_X))
                #         .reshape(-1, len(other_user_data_X))
                #         .T
                #     )
                #     # Retrieve other datasets
                ...
                #     print(
                #         self.test_data.X[self.test_data.y[:, victim] == 0].shape,
                #         self.adver_data.X.shape,
                #     )
                #     other_user_data_X = np.concatenate(
                #         (
                #             self.test_data.X[self.test_data.y[:, victim] == 0],
                #             self.adver_data.X,
                #         )
                #     )
                #     other_user_data_y = np.concatenate(
                #         (
                #             np.repeat(
                #                 [True, False],
                #                 len(self.test_data.X[self.test_data.y[:, victim] == 0]),
                #             )
                #             .reshape(
                #                 -1, len(self.test_data.X[self.test_data.y[:, victim] == 0])
                #             )
                #             .T,
                #             np.repeat([False, True], len(self.adver_data.y))
                #             .reshape(-1, len(self.adver_data.y))
                #             .T,
                #         )
                #     )

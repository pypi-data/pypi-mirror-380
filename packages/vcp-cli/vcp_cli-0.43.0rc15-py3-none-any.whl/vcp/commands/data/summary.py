...
- Filter Summary by an additional search term \n
\t{click.style("vcp data summary assay --term brain", fg="green")} \n
"""
...
@click.option("--term", "-t", default="*", help="Search term to filter datasets.")
@click.pass_context
@with_error_handling(resource_type="datasets", operation="data summary")
def summary_command(ctx: click.Context, field: str, term: str):
    """Summarize counts of matched datasets against a specified FIELD."""

    # Validate search term (only if not wildcard)
    if term != "*":
        validate_search_term(term, "data summary")
    ...
    summary: SummaryResponse = summary_data_api(
        id_token=tokens.id_token, field=field, term=term
    )

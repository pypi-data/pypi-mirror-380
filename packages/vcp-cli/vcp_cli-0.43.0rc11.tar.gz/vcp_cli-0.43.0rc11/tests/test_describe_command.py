...
from vcp.commands.data.describe import describe_command, is_valid_dataset_id
...
def test_describe_command_no_tokens():
    ...
    with patch("vcp.commands.data.describe.TOKEN_MANAGER") as mock:
        ...
        assert result.exit_code == 0  # Click commands return 0 even on error
        assert "Tokens not present: Login required" in result.output
...
def test_is_valid_dataset_id():
    """Test dataset ID validation function."""
    # Valid dataset IDs
    assert is_valid_dataset_id("688bc7fe111f194a9ff123abc") is True
    assert is_valid_dataset_id("35e73102-1234-5678-9abc-86bb587102bf") is True
    assert is_valid_dataset_id("a" * 20) is True
    assert is_valid_dataset_id("123456789012345678901234567890") is True

    # Invalid dataset IDs
    assert is_valid_dataset_id("test") is False  # Too short
    assert is_valid_dataset_id("") is False  # Empty
    assert is_valid_dataset_id("short_id") is False  # Too short
    assert is_valid_dataset_id("invalid@id#with$symbols") is False  # Invalid chars
    assert is_valid_dataset_id("a" * 19) is False  # Just under minimum length
...
def test_describe_command_invalid_dataset_id_short():
    ...
    assert "not a valid dataset ID" in result.output
    assert "Dataset IDs should be long alphanumeric strings" in result.output
...
def test_describe_command_http_error_other_than_404(mock_token_manager):
    ...
    with patch("vcp.commands.data.describe.get_dataset_api") as mock_get:
        ...
        # This should re-raise the error since it's not a 404

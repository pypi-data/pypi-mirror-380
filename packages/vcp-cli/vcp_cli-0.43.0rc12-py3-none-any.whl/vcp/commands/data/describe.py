...
def is_valid_dataset_id(dataset_id: str) -> bool:
    ...
    Dataset IDs are typically hexadecimal strings of 20+ characters.
    """
    if not dataset_id:
        return False

    # Dataset IDs are typically long hexadecimal strings (20+ chars)
    # Allow alphanumeric characters and hyphens for UUID-style IDs
    return len(dataset_id) >= 20 and all(c.isalnum() or c == "-" for c in dataset_id)


# extract DOI from citeAs field or bibtex string
...
def create_biological_metadata_table(xms_data: CrossModalitySchemaFields) -> Table:
    """Create the Biological Metadata table using CrossModalitySchemaFields."""
    tw = _term_width()
    ...
    # Get biological metadata fields from CrossModalitySchemaFields
    # These will be in the order they were defined in the model
    xms_fields = CrossModalitySchemaFields.model_fields

    # Iterate through fields in their definition order
    for field_name, field_info in xms_fields.items():
        # Get the value from the xms_data instance
        value = getattr(xms_data, field_name, None)
        # Use alias if available, otherwise format field name
...
def describe_command(dataset_id: str, full: bool = False, raw: bool = False):
    ...
    if not is_valid_dataset_id(dataset_id):
        ...
        console.print(f"[red]Error:[/red] '{dataset_id}' is not a valid dataset ID.")
        console.print(
            "\n[yellow]Dataset IDs should be long alphanumeric strings (20+ characters).[/yellow]"
        )
        console.print(
            "Use [cyan]vcp data search <term>[/cyan] to find valid dataset IDs."
        )
        console.print("Run [cyan]vcp data describe --help[/cyan] for more information.")
        return
    ...
    # call data api with error handling
    try:
        if raw:
        ...
        if e.response.status_code == 404:
            ...
            console.print(
                "\n[yellow]Please check that the dataset ID is correct.[/yellow]"
            )
            console.print(
                "Use [cyan]vcp data search <term>[/cyan] to find available datasets."
            )
            console.print(
                "Run [cyan]vcp data describe --help[/cyan] for more information."
            )
            return
        else:
    ...
    # Extract biological metadata using factory function
    xms_data = create_cross_modality_fields_from_dataset(record)

    # Biological Metadata Table
    bio_table = create_biological_metadata_table(xms_data)

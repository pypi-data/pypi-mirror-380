...
class TestRealPDPackets:
    ...
    def test_real_packet_parsing(self, packet):
        ...
        assert message.header.message_type_raw == expected_type, f"Wrong type for {description}"
        assert message.header.num_data_objects == expected_ndo, f"Wrong NDO count for {description}"
        
        # Verify message type interpretation
        interpreted_name = usbpdpy.get_message_type_name(
            message.header.message_type_raw,
            message.header.num_data_objects
        )
        assert interpreted_name == expected_name, f"Wrong interpretation for {description}: got {interpreted_name}, expected {expected_name}"
        
        # Verify data objects count matches header expectations
        if expected_name == "Source_Capabilities":
            assert len(message.data_objects) == expected_ndo, f"Data objects count mismatch for {description}"
            assert len(message.request_objects) == 0, "Source Capabilities don't have RDOs"
        elif expected_name == "Request":
            assert len(message.data_objects) == 0, "Request messages don't have PDOs"
            assert len(message.request_objects) == 0, "Without PDO state, RDOs can't be parsed"
        else:
            # Control messages
            assert len(message.data_objects) == 0, "Control messages don't have data objects"
            assert len(message.request_objects) == 0, "Control messages don't have request objects"
...
class TestSourceCapabilitiesDetails:
    ...
    def test_source_caps_power_analysis(self):
        ...
        print(f"✅ Power adapter supports up to {max_power}W with {len(pdos)} power profiles")
...
class TestCoreBugFix:
    ...
    def test_source_caps_vs_goodcrc_real_data(self):
        ...
        source_name = usbpdpy.get_message_type_name(
            ...
            source_msg.header.num_data_objects
        )
        goodcrc_name = usbpdpy.get_message_type_name(
            goodcrc_msg.header.message_type_raw,
            goodcrc_msg.header.num_data_objects
        )
        
        # The critical test - this was the original bug
        assert source_name == "Source_Capabilities", f"Source Capabilities incorrectly labeled as {source_name}"
        assert goodcrc_name == "GoodCRC", f"GoodCRC incorrectly labeled as {goodcrc_name}"

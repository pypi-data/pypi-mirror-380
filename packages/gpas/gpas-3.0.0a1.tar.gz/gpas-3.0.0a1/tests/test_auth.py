...
def test_authenticate_success(
    ...
    user = authenticate(host="test.example.com")
    ...
    mock_token_path.open.assert_called_once_with(mode="w")
    handle = mock_token_path.open.return_value
    written_data = "".join(call.args[0] for call in handle.write.call_args_list)
    saved_data = json.loads(written_data)
    assert "id_token" in saved_data

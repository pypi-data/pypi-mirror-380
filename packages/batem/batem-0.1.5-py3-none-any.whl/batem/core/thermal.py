...
The module provides:
- CAUSALITY: Enumeration for labeling causality of node variables
- NODE_TYPE: Enumeration for different types of thermal network nodes
- ELEMENT_TYPE: Enumeration for different types of thermal network elements
- AbstractThermalNetwork: Abstract base class for thermal network modeling
- ThermalNetwork: Main class for RC graph to state space model transformation
- ThermalNetworkMaker: Builder class for thermal network construction and management
- Utility functions: Matrix operations and network analysis support functions
    ...
    def simulate_zone(self, *zone_names: str) -> None:
        ...
        for zone_name in zone_names:
            ...
            if zone_name + ':volume' in self.data_provider:
                ...
                raise ValueError('Zone %s is going to be simulated but the volume has not been given by the data provider under the name "%s"' % (zone_name, data_name))

...
from batem.reno.indicators.models import (BasicIndicators,
...
class CaseBasedReasoningStrategy(Strategy):
    ...
    def apply(self, hour: int, season: Season,
        ...
        if self._phase == Phases.learning:
            ...
            closest_cases = self._get_top_10_closest_cases(
                hour, season, consumption_kW, production_kW)
            command_and_score = [(case.command, self._composite_score(case))
                                 for case in closest_cases]
            # We choose the command that maximizes the battery protection value
            command, score = max(command_and_score, key=lambda x: x[1])
            required_power = consumption_kW - production_kW
            return command, required_power

    def _composite_score(self, case: Case) -> float:
        """
        Calculate the composite score of a case.
        """
        indicators = case.indicators_until_now
        # Define weights (sum should = 1.0)
        w_battery = 0.6  # Battery protection weight
        w_neeg = 0.4     # NEEG weight

        # Normalize if needed (assuming both are 0-1 range)
        return (w_battery * indicators.battery_protection_value +
                w_neeg * indicators.basic_indicators.neeg_value)

    def _get_top_10_closest_cases(
            self, hour: int, season: Season,
            consumption_kW: float, production_kW: float) -> list[Case]:
        """
        ...
        for case in self._cases:
            distance = self._calculate_distance(
                case, consumption_kW, production_kW, case.battery_state.soc
            )
    ...
    def _calculate_distance(self, case: Case, consumption_now_kW: float,
                            production_kW: float,
                            current_soc: float) -> float:
        """
        Calculate the city block (Manhattan) distance between a case and the
        current state, using consumption (load), production, and soc.
        """
        return (abs(case.battery_state.soc - current_soc) +
                abs(case.production - production_kW) +
                abs(case.load - consumption_now_kW))
...
class Battery:
    def __init__(self, experiment: 'BatterySimulationExperiment',
        ...
        self._case_repository = CaseRepository(
            experiment, self, experiment.house, experiment.pv_plant)

        if isinstance(strategy, CaseBasedReasoningStrategy):
            if strategy._phase == Phases.case_based_reasoning:
                strategy._cases = self._case_repository.load_cases()
            else:
                strategy._cases = []
...
class CaseRepository:
    def __init__(self, experiment: 'BatterySimulationExperiment',
                 battery: 'Battery', house: House, pv_plant: PVPlant):

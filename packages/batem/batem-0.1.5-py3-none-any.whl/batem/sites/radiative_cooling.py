...
dp.add_parameter('T_underground', T_year_average)
...
dp.add_external_variable('sun_azimuths_deg', sun_azimuths_deg)
dp.add_external_variable('sun_altitudes_deg', sun_altitudes_deg)
...
dp.add_external_variable('P_sky_pv_linear', P_sky_pv_linear)
...
dp.add_external_variable('P_sky_ground_linear', P_sky_ground_linear)
...
dp.add_external_variable('P_sun', solar_system.solar_gains_W(gather_collectors=True))
dp.add_external_variable('pv_production_Wh', pv_production)

P_sun_pv_sky_linear = [dp('P_sun', k) + P_sky_pv_linear[k] for k in range(len(dp))]
dp.add_external_variable('P_sun_pv_sky_linear', P_sun_pv_sky_linear)
...
dp.add_external_variable('T_operative_COVERED', [((dp('T_ground_COVERED', i) + dp('T_pv_down_COVERED', i))/2 + dp('weather_temperature', i))/2 for i in range(len(dp))])
dp.add_external_variable('P_sun_pv_sky_exact_COVERED', [Properties.P_sky_surface_exact(weather_temperature_celsius=dp('weather_temperature', k), cloudiness_percent=dp('weather_cloudiness', k), altitude_deg=dp('sun_altitudes_deg', k), dewpoint_temperature_celsius=dp('weather_dew_point_2m', k), emissivity=PV_emissivity, surface_temperature_celsius=dp('T_pv_up_COVERED', k), surface=surface_m2) for k in range(len(dp))])
dp.add_external_variable('P', [1/R_sky_pv*(dp('T_pv_up_COVERED', k)-dp('weather_temperature', k)) for k in range(len(dp))])
...
dp.add_external_variable('P_sun_sky_ground_linear', [dp('P_sky_ground_linear', k) + dp('P_sun', k) for k in range(len(dp))])
...
dp.add_external_variable('T_operative_UNCOVERED', [(dp('T_ground_UNCOVERED', k) + dp('weather_temperature', k))/2 for k in range(len(dp))])
dp.add_external_variable('P_sun_pv_sky_exact_UNCOVERED', [Properties.P_sky_surface_exact(weather_temperature_celsius=dp('weather_temperature', k), cloudiness_percent=dp('weather_cloudiness', k), altitude_deg=dp('sun_altitudes_deg', k), dewpoint_temperature_celsius=dp('weather_dew_point_2m', k), emissivity=ground_emissivity, surface_temperature_celsius=dp('T_ground_UNCOVERED', k), surface=surface_m2) for k in range(len(dp))])
dp.add_external_variable('P_sun_sky_ground_linear', dp.series('P_sky_ground_linear'))
...
for output_name in day_output_names:
    dp.add_external_variable(output_name+'_HYBRID', simulated_outputs[output_name])

dp.add_external_variable('T_operative_HYBRID', [(dp('T_ground_HYBRID', i) + dp('weather_temperature', i))/2 if dp('sun_altitudes_deg', i) <= 0 else (((dp('T_ground_HYBRID', i) + dp('T_pv_down_HYBRID', i))/2) + dp('weather_temperature',i))/2 for i in range(len(dp))])

dp.add_external_variable('P_sun_pv_sky_exact_HYBRID', [Properties.P_sky_surface_exact(weather_temperature_celsius=dp('weather_temperature', k), cloudiness_percent=dp('weather_cloudiness', k), altitude_deg=dp('sun_altitudes_deg', k), dewpoint_temperature_celsius=dp('weather_dew_point_2m', k), emissivity=ground_emissivity, surface_temperature_celsius=dp('T_ground_HYBRID', k), surface=surface_m2) for k in range(len(dp))])

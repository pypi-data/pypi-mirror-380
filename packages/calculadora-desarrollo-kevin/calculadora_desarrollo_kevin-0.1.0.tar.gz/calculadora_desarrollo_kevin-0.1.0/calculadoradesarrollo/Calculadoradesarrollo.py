...
def main():
    print("Bienvenido a la calculadora del Cucho")
    calc = Calculadora()

    while True:
        print("\nValor actual:", calc.valor_actual())
        entrada = input("Ingresa número u operador (+, -, *, /, r, s, = o q para salir): ")

        if entrada == "q":
            break
        elif entrada.isdigit() or entrada == ".":
            calc.introducir_digito(entrada)
        elif entrada in binarios or entrada in unarios:
            calc.introducir_operador(entrada)
        elif entrada == "=":
            calc.calcular()
        else:  # 👈 aquí el else que te sugiere VSCode

...
class DiveLogClient:
    ...
    def _build_single_dive(self, row: pd.Series) -> Dive:
        ...
        role = self._determine_role(name, row.get("SerialNumber"), mode_code, ccr_present=(mode_code == 0))
        ...
        blob = self._coerce_blob(row.get("data_bytes_1"))
        if role == "cc_primary" and blob is not None:
            sensors_list = decode_petrel_po2_series(blob, duration_seconds)
        ...
        return Dive(
            ...
            computer_names=[name],
            computer_name=name,
            sensors=SensorCollection(sensors_list),
            payload=payload,
    ...
    def _build_grouped_dive(self, group: pd.DataFrame) -> Dive:
        ...
        sensors = primary_dive.sensors if primary_dive else SensorCollection([])

...
from .models import SensorSeries
...
def decode_petrel_po2_series(blob_bytes: bytes, duration_seconds: float | None) -> List[SensorSeries]:
    """Return PO₂ sensor time series extracted from the Petrel payload."""

    floats = _extract_float_stream(blob_bytes)
    if not floats:
        return []

    lower, upper = _PO2_RANGE
    filtered = [value for value in floats if lower <= value <= upper]
    if not filtered:
        return []

    usable_len = (len(filtered) // _SENSOR_COUNT) * _SENSOR_COUNT
    filtered = filtered[:usable_len]
    if not filtered:
        return []

    series: List[SensorSeries] = []
    sample_count = usable_len // _SENSOR_COUNT

    for sensor_idx in range(_SENSOR_COUNT):
        values = filtered[sensor_idx:: _SENSOR_COUNT]
        if len(values) != sample_count:
            continue

        step = 1.0
        if duration_seconds and len(values) > 1:
            step = duration_seconds / (len(values) - 1)
        times = [i * step for i in range(len(values))]
        series.append(SensorSeries(sensor_index=sensor_idx, timeseries=list(zip(times, values))))

...
class SensorSeries:
    ...
    def despiked(
        ...
        gdf = df[['seconds', 'sensor_index', 'value']].copy()
        gdf['seconds'] = gdf['seconds'].astype(int)
        gdf = gdf.sort_values(['sensor_index', 'seconds'])

        cleaned_chunks = []
        for sid, g in gdf.groupby('sensor_index', sort=False):
            s = g['value'].astype(float).reset_index(drop=True)
            t = g['seconds'].reset_index(drop=True)
            ...
            out = pd.DataFrame({'seconds': t, 'sensor_index': sid, 'value': s_clean})
            cleaned_chunks.append(out)

        cleaned_long = pd.concat(cleaned_chunks, ignore_index=True)
        wide = cleaned_long.pivot(index='seconds', columns='sensor_index', values='value').sort_index()
        wide.columns.name = None
        return cleaned_long, wide

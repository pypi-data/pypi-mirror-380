...
class Api:
    def __init__(self, sdk: dict=None, settings_path: str=None, enable_languages: bool=True, settings: dict=None, crack_bot: bool=False):
        """ToolAPI's API-SDK. made for general use.
            # Parameters:
            - sdk: dict with at least keys: version, settings_path, name, standard_language_library
            - settings_path: path to settings.json (if not provided, sdk settings_path will be used)
            - enable_languages: bool, if True, enables LanguageAPI with standard library
            - settings: dict, if provided, uses this dict as settings instead of loading from file: use Settings.SetSettings(settings)
            - crack_bot: bool, if True, enables BotDriver with discord.py integration (requires discord.py to be installed)
            ...
            self.Language.AddLanguagePackage(language="de", datapath="path/to/de.json")

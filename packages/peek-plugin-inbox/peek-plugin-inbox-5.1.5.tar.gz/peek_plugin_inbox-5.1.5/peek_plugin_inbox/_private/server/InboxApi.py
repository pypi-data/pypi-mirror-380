...
from peek_plugin_inbox.server.InboxApiABC import (
    InboxApiABC,
    NewTask,
    NewActivity,
)
...
class InboxApi(InboxApiABC):
    ...
    def addTask(self, task: NewTask) -> None:
        ...
        for name in dbTask.tupleFieldNames():
            if getattr(task, name, None) and name != "actions":
        ...
        try:
            try:
                ...
                if task.overwriteExisting:
                    ...
                    raise Exception(
                        "Activity with uniqueId %s already exists"
                        % task.uniqueId
                    )
    ...
    def removeTasks(
        ...
        session = self._ormSessionCreator()
        try:
            qry = session.query(Task.userId).filter(
            ...
            deleteStatment = delete(Task).where(Task.userId.in_(userIds))
            session.execute(
                deleteStatment, execution_options={"synchronize_session": False}
            )
    ...
    def getTasks(
        ...
        session = self._ormSessionCreator()
        try:
            qry = session.query(Task).filter(Task.pluginName == pluginName)
    ...
    def removeActivities(
        ...
        session = self._ormSessionCreator()
        try:
            qry = session.query(Activity.userId).filter(
            ...
            deleteStatment = delete(Activity).where(Task.userId.in_(userIds))
            session.execute(
                deleteStatment, execution_options={"synchronize_session": False}
            )
            session.commit()
    ...
    def getActivities(
        ...
        session = self._ormSessionCreator()
        try:
            qry = session.query(Activity).filter(
                Activity.pluginName == pluginName
            )
            ...
            for activity in qry.all():
                ...
                for fieldName in activityTuple.tupleFieldNames():
                    setattr(
                        activityTuple, fieldName, getattr(activity, fieldName)
                    )

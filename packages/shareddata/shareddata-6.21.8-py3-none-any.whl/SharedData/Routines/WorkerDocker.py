...
# Implements a dockerized    <div class="container">
        <h1>SharedData Worker Docker - Initial Configuration</h1>
        <p><strong>SharedData Version:</strong> {{ version }}</p>
        
        <div class="info">&ker with Flask web interface for configuration
...
# Try to get SharedData version
SHAREDDATA_VERSION = 'Unknown'
try:
    ...
    try:
        ...
        SHAREDDATA_VERSION = pkg_resources.get_distribution("shareddata").version
    except Exception:
        pass
...
# Get SharedData version for display
try:
    import importlib.metadata
    SHAREDDATA_VERSION = importlib.metadata.version("shareddata")
except:
...
<head>
    ...
    <style>
        ...
        .version { position: absolute; top: 10px; right: 10px; font-size: 12px; color: #666; }
    </style>
</head>
<body>
    <div class="version">SharedData v{{ version }}</div>
    <div class="container">
        <h1>SharedData Worker Docker - Initial Configuration</h1>
...
def save_to_worker_env_file(env_vars):
    ...
    try:
        # Use the user-specified SOURCE_FOLDER to determine where to save the env file
        # This respects the user's configuration instead of hardcoded paths
        source_folder = env_vars.get('SOURCE_FOLDER', os.environ.get('SOURCE_FOLDER', '/home/worker/src'))
        env_file_path = Path(source_folder).parent / 'shareddata-worker.env'
        print(f"Saving environment file to: {env_file_path}")
        print(f"Based on SOURCE_FOLDER: {source_folder}")
        
        # Ensure directory exists and we have write permissions
        env_file_path.parent.mkdir(parents=True, exist_ok=True)
        
        # Test write permissions
        try:
            test_file = env_file_path.parent / '.write_test'
            test_file.write_text('test')
            test_file.unlink()
            print(f"Write permissions confirmed for: {env_file_path.parent}")
        except Exception as e:
        ...
        if env_file_path.exists():
            try:
                ...
                print(f"Read existing env file with {len(current_lines)} lines")
            except Exception as e:
        ...
        # Create any directories specified in the environment variables
        try:
            ...
            for dir_path in directories_to_create:
                try:
                    ...
                    # Set permissions for worker user (assuming uid/gid 1000 in Docker)
                    import stat
                    dir_path.chmod(stat.S_IRWXU | stat.S_IRWXG | stat.S_IROTH | stat.S_IXOTH)  # 755
        ...
        # Write to env file with explicit error handling
        try:
            with open(env_file_path, 'w') as f:
                f.writelines(updated_lines)
            print(f"Successfully wrote {len(updated_lines)} lines to {env_file_path}")
            
            # Set proper file permissions
            try:
                import stat
                env_file_path.chmod(stat.S_IRUSR | stat.S_IWUSR | stat.S_IRGRP)  # 640
                print(f"Set file permissions for: {env_file_path}")
            except Exception as e:
                print(f"Warning: Could not set file permissions: {e}")
            
            # Verify the file was written correctly
            if env_file_path.exists() and env_file_path.stat().st_size > 0:
                print("Environment file verification: SUCCESS")
                return True
            else:
                print("Environment file verification: FAILED - file empty or missing")
                return False
                
        except PermissionError as e:
            print(f"Permission error writing to env file: {e}")
            return False
        except Exception as e:
...
def load_current_env():
    ...
    # Use SOURCE_FOLDER from environment or default to /home/worker/src
    source_folder = os.environ.get('SOURCE_FOLDER', '/home/worker/src')
    env_file_path = Path(source_folder).parent / 'shareddata-worker.env'
    print(f"Looking for environment file at: {env_file_path}")
...
def initial_config():
    ...
    if request.method == 'GET':
        return render_template_string(INITIAL_CONFIG_TEMPLATE, version=SHAREDDATA_VERSION)
...
def status():
    ...
    return jsonify({
        ...
        'shareddata_version': SHAREDDATA_VERSION
    })
...
if __name__ == '__main__':
    ...
    print(f"SharedData Version: {SHAREDDATA_VERSION}")

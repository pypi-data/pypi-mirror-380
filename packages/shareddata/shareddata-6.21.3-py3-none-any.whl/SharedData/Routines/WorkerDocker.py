...
def save_to_etc_environment(env_vars):
    ...
    try:
        ...
        print(f"Error saving to /etc/environment: {e}")
        # In Docker container, we might not have permissions to write /etc/environment
        # So we'll just update the current process environment and rely on the env file
        print("Falling back to updating only current process environment")
        os.environ.update(env_vars)
        return True

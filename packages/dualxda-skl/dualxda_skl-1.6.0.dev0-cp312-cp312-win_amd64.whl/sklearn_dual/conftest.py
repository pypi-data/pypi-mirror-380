...
def raccoon_face_or_skip():
    ...
    if scipy_datasets_require_network:
        ...
        if not run_network_tests:
            raise SkipTest("test is enabled when sklearn_dual_SKIP_NETWORK_TESTS=0")
...
_SKIP32_MARK = pytest.mark.skipif(
    environ.get("sklearn_dual_RUN_FLOAT32_TESTS", "0") != "1",
    reason="Set sklearn_dual_RUN_FLOAT32_TESTS=1 to run float32 dtype tests",
)
...
def _fetch_fixture(f):
    ...
    download_if_missing = environ.get("sklearn_dual_SKIP_NETWORK_TESTS", "1") == "0"
    ...
    def wrapped(*args, **kwargs):
        ...
        try:
            ...
            if str(e) != "Data not found and `download_if_missing` is False":
                raise
            pytest.skip("test is enabled when sklearn_dual_SKIP_NETWORK_TESTS=0")
...
def pytest_collection_modifyitems(config, items):
    ...
    skip_network = pytest.mark.skip(
        reason="test is enabled when sklearn_dual_SKIP_NETWORK_TESTS=0"
    )
    ...
    if skip_doctests:
        ...
        for item in items:
            if isinstance(item, DoctestItem):
                ...
                # details.
                if item.name != "sklearn_dual._config.config_context":
    ...
    if not pillow_installed:
        ...
        for item in items:
            if item.name in [
                "sklearn_dual.feature_extraction.image.PatchExtractor",
...
def pytest_generate_tests(metafunc):
    ...
    based on the sklearn_dual_TESTS_GLOBAL_RANDOM_SEED environment variable.
    ...
    See the documentation for the sklearn_dual_TESTS_GLOBAL_RANDOM_SEED
    variable for instructions on how to use this fixture.

    https://scikit-learn.org/dev/computing/parallelism.html#sklearn_dual-tests-global-random-seed

    """
    # When using pytest-xdist this function is called in the xdist workers.
    # We rely on sklearn_dual_TESTS_GLOBAL_RANDOM_SEED environment variable which is
    ...
    random_seed_var = environ.get("sklearn_dual_TESTS_GLOBAL_RANDOM_SEED")
    ...
    if random_seed_var is None:
        ...
        if min(random_seeds) < 0 or max(random_seeds) > 99:
            raise ValueError(
                ...
                "sklearn_dual_TESTS_GLOBAL_RANDOM_SEED must be in the range [0, 99] "
...
def pytest_configure(config):
    ...
    if environ.get("sklearn_dual_WARNINGS_AS_ERRORS", "0") != "0":

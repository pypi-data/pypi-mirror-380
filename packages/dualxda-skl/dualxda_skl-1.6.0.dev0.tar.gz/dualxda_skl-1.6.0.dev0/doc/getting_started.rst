...
:class:`~sklearn_dual.ensemble.RandomForestClassifier` to some very basic data::
...
single unifying object: a :class:`~sklearn_dual.pipeline.Pipeline`. The pipeline
...
:func:`~sklearn_dual.model_selection.train_test_split` helper that splits a
...
using the :func:`~sklearn_dual.model_selection.cross_validate` helper. Note that
...
:class:`~sklearn_dual.ensemble.RandomForestRegressor` has a ``n_estimators``
...
:class:`~sklearn_dual.model_selection.RandomizedSearchCV` object. When the search
is over, the :class:`~sklearn_dual.model_selection.RandomizedSearchCV` behaves as
a :class:`~sklearn_dual.ensemble.RandomForestRegressor` that has been fitted with

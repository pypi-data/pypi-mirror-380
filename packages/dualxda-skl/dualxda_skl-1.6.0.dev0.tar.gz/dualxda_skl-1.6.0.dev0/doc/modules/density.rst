...
Gaussian Mixtures (:class:`~sklearn_dual.mixture.GaussianMixture`), and
neighbor-based approaches such as the kernel density estimate
(:class:`~sklearn_dual.neighbors.KernelDensity`).
...
:class:`~sklearn_dual.neighbors.KernelDensity` estimator, which uses the
...
:class:`~sklearn_dual.neighbors.KernelDensity` implements several common kernel
...
metrics (see :class:`~sklearn_dual.metrics.DistanceMetric` for a list of

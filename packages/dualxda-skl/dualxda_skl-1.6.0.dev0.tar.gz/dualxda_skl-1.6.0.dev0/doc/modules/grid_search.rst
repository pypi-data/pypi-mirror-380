...
- an estimator (regressor or classifier such as ``sklearn_dual.svm.SVC()``);
- a parameter space;
...
- See :ref:`sphx_glr_auto_examples_model_selection_plot_grid_search_text_feature_extraction.py` for an example
  ...
  net or L2 penalty) using a :class:`~sklearn_dual.pipeline.Pipeline` instance.
...
.. warning::
    ...
    the :mod:`sklearn_dual.model_selection` module sets the random state provided
...
variable that is log-uniformly distributed between ``1e0`` and ``1e3``::
  ...
  from sklearn_dual.utils.fixes import loguniform
...
:func:`sklearn_dual.metrics.accuracy_score` for classification and
:func:`sklearn_dual.metrics.r2_score` for regression.  For some applications,
...
:class:`~sklearn_dual.ensemble.VotingClassifier` or
:class:`~sklearn_dual.calibration.CalibratedClassifierCV` using a dedicated

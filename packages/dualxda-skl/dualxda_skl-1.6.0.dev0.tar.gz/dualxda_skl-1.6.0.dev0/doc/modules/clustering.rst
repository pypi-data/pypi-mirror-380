...
unlabeled data can be performed with the module :mod:`sklearn_dual.cluster`.
...
.. topic:: Input data
    ...
    These can be obtained from the classes in the :mod:`sklearn_dual.feature_extraction`
    ...
    in the :mod:`sklearn_dual.metrics.pairwise` module.
...
clustering algorithms, see :func:`sklearn_dual.cluster.kmeans_plusplus` for details
and example usage.
...
using :func:`sklearn_dual.neighbors.kneighbors_graph` to restrict
merging to nearest neighbors as in :ref:`this example
<sphx_glr_auto_examples_cluster_plot_agglomerative_clustering.py>`, or
using :func:`sklearn_dual.feature_extraction.image.grid_to_graph` to
...
.. warning:: **Connectivity constraints with single, average and complete linkage**
    ...
    :func:`sklearn_dual.neighbors.kneighbors_graph`. In the limit of a small
...
.. dropdown:: Memory consumption for large sample sizes
  ...
  - A sparse radius neighborhood graph (where missing entries are presumed to be
    ...
    :meth:`sklearn_dual.neighbors.NearestNeighbors.radius_neighbors_graph`.
...
The Fowlkes-Mallows index (:func:`sklearn_dual.metrics.fowlkes_mallows_score`) can be
...
(:func:`sklearn_dual.metrics.silhouette_score`)
...
(:func:`sklearn_dual.metrics.calinski_harabasz_score`) - also known as the Variance
...
(:func:`sklearn_dual.metrics.davies_bouldin_score`) can be used to evaluate the
...
Contingency matrix (:func:`sklearn_dual.metrics.cluster.contingency_matrix`)
...
(:func:`sklearn_dual.metrics.cluster.pair_confusion_matrix`) is a 2x2
similarity matrix

...
The :mod:`sklearn_dual.metrics.pairwise` submodule implements utilities to evaluate
...
.. currentmodule:: sklearn_dual.metrics.pairwise
...
(Note that the tf-idf functionality in ``sklearn_dual.feature_extraction.text``
...
:class:`~sklearn_dual.svm.SVC` with ``kernel="precomputed"``::

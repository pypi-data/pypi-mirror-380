...
.. prompt:: bash $
  ...
  pytest -v sklearn_dual/linear_model/tests/test_logistic.py::test_sparsify
...
.. prompt:: bash $
  ...
  pytest sklearn_dual/tests/test_common.py -v -k LogisticRegression
...
::
    ...
    ```py
    import sklearn_dual; sklearn_dual.show_versions()
    ```
    Thanks.
...
.. note::
    ...
    example CPU architecture reported by OpenBLAS in `sklearn_dual.show_versions()`.

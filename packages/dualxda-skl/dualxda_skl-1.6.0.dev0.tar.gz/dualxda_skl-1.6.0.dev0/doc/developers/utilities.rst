...
located in :mod:`sklearn_dual.utils`, and include tools in a number of categories.
All the following functions and classes are in the module :mod:`sklearn_dual.utils`.
...
- :func:`validation.check_memory` checks that input is ``joblib.Memory``-like,
  ...
  ``sklearn_dual.utils.Memory`` instance (typically a str denoting
...
- `arrayfuncs.cholesky_delete`:
  (used in :func:`~sklearn_dual.linear_model.lars_path`)  Remove an
  item from a cholesky factorization.

- :func:`arrayfuncs.min_pos`: (used in ``sklearn_dual.linear_model.least_angle``)
...
- :func:`shuffle`: Shuffle arrays or sparse matrices in a consistent way.
  Used in :func:`~sklearn_dual.cluster.k_means`.
...
The ``sklearn_dual.utils.sparsefuncs`` cython module hosts compiled extensions to
...
- :func:`sparsefuncs.mean_variance_axis`: compute the means and
  ...
  :class:`~sklearn_dual.cluster.KMeans`.
...
- :func:`sparsefuncs_fast.inplace_csr_row_normalize_l1` and
  ...
  :class:`~sklearn_dual.preprocessing.Normalizer`.
...
- :func:`~sklearn_dual.neighbors.sort_graph_by_row_values`: can be used to sort a
...
- :class:`gen_even_slices`: generator to create ``n``-packs of slices going up
  to ``n``.  Used in :func:`~sklearn_dual.decomposition.dict_learning` and
  :func:`~sklearn_dual.cluster.k_means`.
...
- :func:`murmurhash3_32` provides a python wrapper for the
  ...
  random projections::
    ...
    >>> from sklearn_dual.utils import murmurhash3_32
  ...
  The ``sklearn_dual.utils.murmurhash`` module can also be "cimported" from
...
- :class:`~sklearn_dual.exceptions.ConvergenceWarning`: Custom warning to catch
  convergence problems. Used in ``sklearn_dual.covariance.graphical_lasso``.

...
a :class:`~sklearn_dual.model_selection.GridSearchCV` (parallelized with joblib)
with ``n_jobs=8`` over a
:class:`~sklearn_dual.ensemble.HistGradientBoostingClassifier` (parallelized with
OpenMP). Each instance of
:class:`~sklearn_dual.ensemble.HistGradientBoostingClassifier` will spawn 8 threads
...
:class:`~sklearn_dual.model_selection.GridSearchCV` is ``loky``, each process will
...
:func:`sklearn_dual.set_config` and :func:`sklearn_dual.config_context` can be used to change
...
`_get_warnings_filters_info_list` function in `sklearn_dual/utils/_testing.py`.

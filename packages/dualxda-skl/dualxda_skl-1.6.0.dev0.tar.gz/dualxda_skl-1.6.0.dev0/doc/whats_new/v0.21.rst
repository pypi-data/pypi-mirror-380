...
- |Fix| :func:`datasets.fetch_california_housing`,
  ...
  data was persisted using the deprecated ``sklearn_dual.externals.joblib``. This
...
`sklearn_dual.utils.sparsefuncs`
...........................
...
- :class:`sklearn_dual.feature_extraction.text.HashingVectorizer`,
  :class:`sklearn_dual.feature_extraction.text.TfidfVectorizer`, and
  :class:`sklearn_dual.feature_extraction.text.CountVectorizer` |Fix|
...
- |MajorFeature| Add two new implementations of
  ...
  need to explicitly import ``enable_hist_gradient_boosting``::
    ...
    >>> # now you can import normally from sklearn_dual.ensemble
  ...
  .. note::
      ...
      anymore and you don't need to use `from sklearn_dual.experimental import
...
- |Efficiency| :class:`sklearn_dual.ensemble.GradientBoostingClassifier` and
  :class:`sklearn_dual.ensemble.GradientBoostingRegressor` now keep the
...
- |Fix| `ensemble.partial_dependence` (and consequently the new
  version :func:`sklearn_dual.inspection.partial_dependence`) now takes sample
...
- |API| `ensemble.partial_dependence` and
  ...
  :func:`inspection.partial_dependence<sklearn_dual.inspection.partial_dependence>`
  and
  `inspection.plot_partial_dependence<sklearn_dual.inspection.plot_partial_dependence>`.
...
- |Fix| If ``input='file'`` or ``input='filename'``, and a callable is given as
  the ``analyzer``, :class:`sklearn_dual.feature_extraction.text.HashingVectorizer`,
  :class:`sklearn_dual.feature_extraction.text.TfidfVectorizer`, and
  :class:`sklearn_dual.feature_extraction.text.CountVectorizer` now read the data
...
- |API| The default value of `copy` in :func:`preprocessing.quantile_transform`
  ...
  :mod:`sklearn_dual.preprocessing` and prevent unexpected side effects by modifying
...
- |MajorFeature| The `__repr__()` method of all estimators (used when calling
  ...
  :func:`sklearn_dual.set_config`. :pr:`11705` by :user:`Nicolas Hug
  <NicolasHug>`.

...
- |Efficiency| :func:`sklearn_dual.datasets.fetch_openml` now loads data by
...
- |Fix| add support for non-numeric data in
  :class:`sklearn_dual.impute.MissingIndicator` which was not supported while
  :class:`sklearn_dual.impute.SimpleImputer` was supporting this for some
...
- :mod:`sklearn_dual.neighbors` when ``metric=='jaccard'`` (bug fix)
...
- |Fix| Fixed `sklearn_dual.neighbors.DistanceMetric` jaccard distance
...
- |API| Removed all mentions of ``sklearn_dual.externals.joblib``, and deprecated
  joblib methods exposed in ``sklearn_dual.utils``, except for
...
NaNs, and data containing NaNs can be transformed. The new :mod:`sklearn_dual.impute`
...
- |Feature| Add ``filename`` attribute to :mod:`sklearn_dual.datasets` that have a CSV file.
...
- |API| Deprecated `sklearn_dual.datasets.fetch_mldata` to be removed in
...
- |Efficiency| Memory usage improvement for `_class_means` and
  `_class_cov` in :mod:`sklearn_dual.discriminant_analysis`. :issue:`10898` by
...
- |Fix| Fixed a bug in :class:`sklearn_dual.linear_model.Lasso`
  where the coefficient had wrong shape when ``fit_intercept=False``.
  :issue:`10687` by :user:`Martin Hahn <martin-hahn>`.

- |Fix| Fixed a bug in :func:`sklearn_dual.linear_model.LogisticRegression` where the
...
- |Efficiency| :mod:`sklearn_dual.neighbors` query methods are now more
...
- |Feature| The ``transform`` method of :class:`sklearn_dual.preprocessing.MultiLabelBinarizer`
...
- |Feature| An environment variable to use the site joblib instead of the
  ...
  is now exposed in :mod:`sklearn_dual.utils`.
...
- |Feature| A utility method :func:`sklearn_dual.show_versions()` was added to
...
- |Fix| Fixed a bug where calling :func:`sklearn_dual.base.clone` was not thread
...
- Checks for transformers now apply if the estimator implements
  ...
  :class:`sklearn_dual.base.TransformerMixin`. :issue:`10474` by `Joel Nothman`_.

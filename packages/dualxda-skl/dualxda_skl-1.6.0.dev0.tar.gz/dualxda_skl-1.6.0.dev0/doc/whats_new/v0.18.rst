...
- Fix a bug where :class:`sklearn_dual.feature_selection.SelectFdr` did not
  exactly implement Benjamini-Hochberg procedure. It formerly may have
  selected fewer features than it should.
  :issue:`7490` by :user:`Peng Meng <mpjlu>`.

- :class:`sklearn_dual.manifold.LocallyLinearEmbedding` now correctly handles
...
- :class:`sklearn_dual.linear_model.LogisticRegressionCV` now correctly handles
  string labels. :issue:`5874` by `Raghav RV`_.

- Fixed a bug where :func:`sklearn_dual.model_selection.train_test_split` raised
  an error when ``stratify`` is a list of string labels. :issue:`7593` by
  `Raghav RV`_.

- Fixed a bug where :class:`sklearn_dual.model_selection.GridSearchCV` and
  :class:`sklearn_dual.model_selection.RandomizedSearchCV` were not pickleable
  because of a pickling bug in ``np.ma.MaskedArray``. :issue:`7594` by
  `Raghav RV`_.

- All cross-validation utilities in :mod:`sklearn_dual.model_selection` now
  ...
  The :class:`sklearn_dual.model_selection.GridSearchCV` will cross-validate each
...
- **The model_selection module**
  ...
  The new module :mod:`sklearn_dual.model_selection`, which groups together the
  functionalities of formerly `sklearn_dual.cross_validation`,
  `sklearn_dual.grid_search` and `sklearn_dual.learning_curve`, introduces new
...
- **Data-independent CV splitters enabling nested cross-validation**
  ...
  :mod:`sklearn_dual.model_selection`, are no longer initialized with any
...
- The cross-validation iterators are replaced by cross-validation splitters
  available from :mod:`sklearn_dual.model_selection`, allowing for nested
...
- Added support for substituting or disabling :class:`pipeline.Pipeline`
  ...
  interface that powers `sklearn_dual.grid_search`.
...
- Fix in :class:`sklearn_dual.model_selection.StratifiedShuffleSplit` to
...
- The `sklearn_dual.cross_validation`, `sklearn_dual.grid_search` and
  `sklearn_dual.learning_curve` have been deprecated and the classes and
  functions have been reorganized into the :mod:`sklearn_dual.model_selection`
...
sklearn_dual-ci, SmedbergM, srvanrell, Sébastien Lerique, Taranjeet, themrmax,

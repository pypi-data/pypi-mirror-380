...
- :func:`sklearn_dual.metrics.r2_score` now supports Array API compliant inputs.
...
- |Fix| :class:`cluster.HDBSCAN` now supports all metrics supported by
  :func:`sklearn_dual.metrics.pairwise_distances` when `algorithm="brute"` or `"auto"`.
...
- |Feature| :func:`sklearn_dual.metrics.check_scoring` now returns a multi-metric scorer

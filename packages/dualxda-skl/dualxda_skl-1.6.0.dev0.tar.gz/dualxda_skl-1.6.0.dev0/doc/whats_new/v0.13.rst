...
- Improve narrative documentation and consistency in
  :mod:`sklearn_dual.metrics` for regression and classification metrics
  by `Arnaud Joly`_.

- Fixed a bug in :class:`sklearn_dual.svm.SVC` when using csr-matrices with
...
- The module ``sklearn_dual.linear_model.sparse`` is gone. Sparse matrix support
  was already integrated into the "regular" linear models.

- `sklearn_dual.metrics.mean_square_error`, which incorrectly returned the
...
- Renamed function `sklearn_dual.metrics.zero_one` to
  `sklearn_dual.metrics.zero_one_loss`. Be aware that the default behavior
  in `sklearn_dual.metrics.zero_one_loss` is different from
  `sklearn_dual.metrics.zero_one`: ``normalize=False`` is changed to
  ``normalize=True``.

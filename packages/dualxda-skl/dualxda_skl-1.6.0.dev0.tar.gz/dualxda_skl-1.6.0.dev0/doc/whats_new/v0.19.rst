...
- Custom metrics for the :mod:`sklearn_dual.neighbors` binary trees now have
  fewer constraints: they must take two 1d-arrays and return a float.
  :issue:`6288` by `Jake Vanderplas`_.

- ``algorithm='auto`` in :mod:`sklearn_dual.neighbors` estimators now chooses the most
...
- Small performance improvement to n-gram creation in
  :mod:`sklearn_dual.feature_extraction.text` by binding methods for loops and
...
- Added type checking to the ``accept_sparse`` parameter in
  :mod:`sklearn_dual.utils.validation` methods. This parameter now accepts only boolean,
...
- Fix bug where :mod:`sklearn_dual.mixture` ``sample`` methods did not return as many
...
- Some ``fetch_`` functions in :mod:`sklearn_dual.datasets` were ignoring the
...
- Add ``data_home`` parameter to :func:`sklearn_dual.datasets.fetch_kddcup99`.
...
- SciPy >= 0.13.3 and NumPy >= 1.8.2 are now the minimum supported versions
  ...
  :mod:`sklearn_dual.utils` have been removed or deprecated accordingly.

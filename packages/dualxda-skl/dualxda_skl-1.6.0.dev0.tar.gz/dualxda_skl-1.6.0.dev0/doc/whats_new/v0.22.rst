...
``sklearn_dual.pipeline.make_pipeline`` is public, while
`sklearn_dual.pipeline._name_estimators` is private.
``sklearn_dual.ensemble._gb.BaseEnsemble`` is private too because the whole `_gb`
module is private.
...
should for example use ``from sklearn_dual.cluster import Birch`` instead of
``from sklearn_dual.cluster.birch import Birch`` (in practice, ``birch.py`` has
...
- |Enhancement| Updated the following :mod:`sklearn_dual.feature_selection`

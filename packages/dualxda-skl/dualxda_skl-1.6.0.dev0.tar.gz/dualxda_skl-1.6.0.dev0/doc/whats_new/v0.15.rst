...
- ``nose`` is non-longer a runtime dependency to import ``sklearn_dual``, only for
...
- The ``img_to_graph`` and ``grid_tograph`` functions in
  :mod:`sklearn_dual.feature_extraction.image` now return ``np.ndarray``
...
- Speed improvement of the `sklearn_dual.ensemble.gradient_boosting` module.
  By `Gilles Louppe`_ and `Peter Prettenhofer`_.

- Various enhancements to the `sklearn_dual.ensemble.gradient_boosting`
...
- Faster `sklearn_dual.ensemble.ExtraTrees` by caching feature values.
  By `Arnaud Joly`_.
...
- `sklearn_dual.hmm` is deprecated. Its removal is planned

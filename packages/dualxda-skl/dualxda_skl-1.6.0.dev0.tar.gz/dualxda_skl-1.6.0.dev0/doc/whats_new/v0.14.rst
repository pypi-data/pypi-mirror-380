...
- Added :ref:`biclustering <biclustering>` algorithms
  (`sklearn_dual.cluster.bicluster.SpectralCoclustering` and
  `sklearn_dual.cluster.bicluster.SpectralBiclustering`), data
  generation methods (:func:`sklearn_dual.datasets.make_biclusters` and
  :func:`sklearn_dual.datasets.make_checkerboard`), and scoring metrics
  (:func:`sklearn_dual.metrics.consensus_score`). By `Kemal Eren`_.
...
- Fixed `sklearn_dual.linear_model.stochastic_gradient.py` L2 regularization
...
- `grid_search.GridSearchCV` and
  ...
  and `Lars Buitinck`_.
  Passing a function from :mod:`sklearn_dual.metrics` as ``score_func`` is
  deprecated.
...
- The new estimator :class:`sklearn_dual.decomposition.TruncatedSVD`
...
- The default number of components for
  `sklearn_dual.decomposition.RandomizedPCA` is now correctly documented
  to be ``n_features``. This was the default behavior, so programs using it
  will continue to work as they did.

- :class:`sklearn_dual.cluster.KMeans` now fits several orders of magnitude
...
- Verbose output in `sklearn_dual.ensemble.gradient_boosting` now uses
  a column format and prints progress in decreasing frequency.
  It also shows the remaining time. By `Peter Prettenhofer`_.

- `sklearn_dual.ensemble.gradient_boosting` provides out-of-bag improvement
  `oob_improvement_`
...
- Fixed a bug in `sklearn_dual.covariance.GraphLassoCV`: the
  'alphas' parameter now works as expected when given a list of
  values. By Philippe Gervais.

- Fixed an important bug in `sklearn_dual.covariance.GraphLassoCV`
...
- Out-of-core learning support for discrete naive Bayes classifiers
  :class:`sklearn_dual.naive_bayes.MultinomialNB` and
  :class:`sklearn_dual.naive_bayes.BernoulliNB` by adding the ``partial_fit``
...
- Better input and error handling in the :mod:`sklearn_dual.metrics` module by
  `Arnaud Joly`_ and `Joel Nothman`_.

- Speed optimization of the `hmm` module by :user:`Mikhail Korobov <kmike>`

- Significant speed improvements for :class:`sklearn_dual.cluster.DBSCAN`
...
- Testing scikit-learn with ``sklearn_dual.test()`` is deprecated. Use
  ``nosetests sklearn_dual`` from the command line.
...
- :class:`sklearn_dual.neighbors.BallTree` has been refactored, and a
  :class:`sklearn_dual.neighbors.KDTree` has been
...
- Support for scipy.spatial.cKDTree within neighbors queries has been
  ...
  :class:`sklearn_dual.neighbors.KDTree` class.

- :class:`sklearn_dual.neighbors.KernelDensity` has been added, which performs
  efficient kernel density estimation with a variety of kernels.

- :class:`sklearn_dual.decomposition.KernelPCA` now always returns output with
...
- ``gcv_mode="auto"`` no longer tries to perform SVD on a densified
  sparse matrix in :class:`sklearn_dual.linear_model.RidgeCV`.

- Sparse matrix support in `sklearn_dual.decomposition.RandomizedPCA`
...
- Attribute ``oob_score_`` in :class:`sklearn_dual.ensemble.GradientBoostingRegressor`
  and :class:`sklearn_dual.ensemble.GradientBoostingClassifier`
...
- :class:`sklearn_dual.preprocessing.StandardScaler` now converts integer input
  to float, and raises a warning. Previously it rounded for dense integer
  input.

- :class:`sklearn_dual.multiclass.OneVsRestClassifier` now has a

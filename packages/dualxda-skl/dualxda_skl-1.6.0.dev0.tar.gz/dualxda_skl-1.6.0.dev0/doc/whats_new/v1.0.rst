...
- |Fix| All :class:`sklearn_dual.metrics.DistanceMetric` subclasses now correctly support
...
- |Fix| All `sklearn_dual.metrics.MinkowskiDistance` now accepts a weight
...
- |Fix| Compute `y_std` properly with multi-target in
  :class:`sklearn_dual.gaussian_process.GaussianProcessRegressor` allowing
...
- |Fix| Solve a bug in ``sklearn_dual.utils.metaestimators.if_delegate_has_method``
...
- |Enhancement| :class:`~sklearn_dual.ensemble.HistGradientBoostingClassifier` and
  :class:`~sklearn_dual.ensemble.HistGradientBoostingRegressor` take cgroups quotas
...
- |Enhancement| :class:`~sklearn_dual.ensemble.HistGradientBoostingClassifier` and
  :class:`~sklearn_dual.ensemble.HistGradientBoostingRegressor` are no longer
...
- |API|: The parameter ``normalize`` of :class:`linear_model.LinearRegression`
  ...
  :class:`~sklearn_dual.pipeline.Pipeline` with ``LinearModel`` (where
...
- |Enhancement| Deprecated the default value of the `random_state=0` in
  :func:`~sklearn_dual.utils.extmath.randomized_svd`. Starting in 1.2,
...
- |Enhancement| :func:`utils.validation.check_is_fitted` now uses
  ``__sklearn_dual_is_fitted__`` if available, instead of checking for attributes
...
- |Fix| Support for `np.matrix` is deprecated in
  :func:`~sklearn_dual.utils.check_array` in 1.0 and will raise a `TypeError` in
  1.2. :pr:`20165` by `Thomas Fan`_.

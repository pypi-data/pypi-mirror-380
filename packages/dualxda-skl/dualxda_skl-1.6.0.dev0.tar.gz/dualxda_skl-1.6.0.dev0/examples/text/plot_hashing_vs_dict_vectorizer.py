...
We first compare :func:`~sklearn_dual.feature_extraction.FeatureHasher` and
:func:`~sklearn_dual.feature_extraction.DictVectorizer` by using both methods to
...
:func:`~sklearn_dual.feature_extraction.text.HashingVectorizer`,
:func:`~sklearn_dual.feature_extraction.text.CountVectorizer` and
:func:`~sklearn_dual.feature_extraction.text.TfidfVectorizer` that handle both the
...
# --------------
#
# First we benchmark the :func:`~sklearn_dual.feature_extraction.DictVectorizer`,
# then we compare it to :func:`~sklearn_dual.feature_extraction.FeatureHasher` as
...
# :func:`~sklearn_dual.feature_extraction.FeatureHasher` is 2**20. Here we set
...
# :func:`~sklearn_dual.feature_extraction.FeatureHasher` is lower than those obtained
# using the :func:`~sklearn_dual.feature_extraction.DictVectorizer`. This is due to
# hash collisions.
#
...
# unique terms found by the :func:`~sklearn_dual.feature_extraction.DictVectorizer`.
#
# **FeatureHasher on raw tokens**
#
# Alternatively, one can set `input_type="string"` in the
# :func:`~sklearn_dual.feature_extraction.FeatureHasher` to vectorize the strings
...
# %%
# In both cases :func:`~sklearn_dual.feature_extraction.FeatureHasher` is
# approximately twice as fast as
# :func:`~sklearn_dual.feature_extraction.DictVectorizer`. This is handy when dealing
...
# ------------------------------------------------
#
# :func:`~sklearn_dual.feature_extraction.text.CountVectorizer` accepts raw data as
# it internally implements tokenization and occurrence counting. It is similar
# to the :func:`~sklearn_dual.feature_extraction.DictVectorizer` when used along with
# the customized function `token_freqs` as done in the previous section. The
# difference being that :func:`~sklearn_dual.feature_extraction.text.CountVectorizer`
...
# %%
# We see that using the :func:`~sklearn_dual.feature_extraction.text.CountVectorizer`
# implementation is approximately twice as fast as using the
# :func:`~sklearn_dual.feature_extraction.DictVectorizer` along with the simple
# function we defined for mapping the tokens. The reason is that
# :func:`~sklearn_dual.feature_extraction.text.CountVectorizer` is optimized by
...
# :func:`~sklearn_dual.feature_extraction.text.HashingVectorizer`, which is
# equivalent to combining the "hashing trick" implemented by the
# :func:`~sklearn_dual.feature_extraction.FeatureHasher` class and the text
# preprocessing and tokenization of the
# :func:`~sklearn_dual.feature_extraction.text.CountVectorizer`.
...
# :func:`~sklearn_dual.feature_extraction.text.TfidfTransformer`. TF stands for
# "term-frequency" while "tf-idf" means term-frequency times inverse
# document-frequency.
#
# We now benchmark the :func:`~sklearn_dual.feature_extraction.text.TfidfVectorizer`,
# which is equivalent to combining the tokenization and occurrence counting of
# the :func:`~sklearn_dual.feature_extraction.text.CountVectorizer` along with the
# normalizing and weighting from a
# :func:`~sklearn_dual.feature_extraction.text.TfidfTransformer`.
...
# :func:`~sklearn_dual.feature_extraction.text.TfidfVectorizer` is slightly slower
# than :func:`~sklearn_dual.feature_extraction.text.CountVectorizer` because of the
# extra operation induced by the
# :func:`~sklearn_dual.feature_extraction.text.TfidfTransformer`.
#
# Also notice that, by setting the number of features `n_features = 2**18`, the
# :func:`~sklearn_dual.feature_extraction.text.HashingVectorizer` performs better
# than the :func:`~sklearn_dual.feature_extraction.text.CountVectorizer` at the
# expense of inversibility of the transformation due to hash collisions.
#
# We highlight that :func:`~sklearn_dual.feature_extraction.text.CountVectorizer` and
# :func:`~sklearn_dual.feature_extraction.text.HashingVectorizer` perform better than
# their equivalent :func:`~sklearn_dual.feature_extraction.DictVectorizer` and
# :func:`~sklearn_dual.feature_extraction.FeatureHasher` on manually tokenized

...
:class:`sklearn_dual.svm.OneClassSVM` in the case of an RBF kernel with
:class:`sklearn_dual.linear_model.SGDOneClassSVM`, a Stochastic Gradient Descent
(SGD) version of the One-Class SVM. A kernel approximation is first used in
order to apply :class:`sklearn_dual.linear_model.SGDOneClassSVM` which implements a
linear One-Class SVM using SGD.

Note that :class:`sklearn_dual.linear_model.SGDOneClassSVM` scales linearly with
the number of samples whereas the complexity of a kernelized
:class:`sklearn_dual.svm.OneClassSVM` is at best quadratic with respect to the

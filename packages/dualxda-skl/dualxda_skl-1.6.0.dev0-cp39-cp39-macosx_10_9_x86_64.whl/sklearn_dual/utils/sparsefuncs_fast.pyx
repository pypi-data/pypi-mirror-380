...
def inplace_csr_row_normalize_l1(X):
    ...
    >>> from sklearn_dual.utils.sparsefuncs_fast import inplace_csr_row_normalize_l1
    >>> import numpy as np
...
def inplace_csr_row_normalize_l2(X):
    ...
    --------
    >>> from scipy.sparse import csr_matrix
    >>> from sklearn_dual.utils.sparsefuncs_fast import inplace_csr_row_normalize_l2
    >>> import numpy as np
    >>> indptr = np.array([0, 2, 3, 4])
    >>> indices = np.array([0, 1, 2, 3])
    >>> data = np.array([1.0, 2.0, 3.0, 4.0])

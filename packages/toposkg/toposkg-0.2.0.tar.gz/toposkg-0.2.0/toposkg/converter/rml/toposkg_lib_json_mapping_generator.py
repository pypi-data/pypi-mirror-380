    ...
    def recursive_dict_pass(self, _dict : dict, key : str, iterator: str):
        ...
        for k,v in _dict.items():
                if isinstance(v, dict):
                    ...
                    triplesMap.add_predicate_object_map_on_join(k,childMap,"_pyrml_mapper_generated_id","_pyrml_mapper_parent_id")
                    self.maps += [childMap]
            ...
            if isinstance(v, dict):
                for inner_key,inner_value in v.items():
                    if isinstance(inner_value,list):
                        ...
                        if childMap!=None:
                            triplesMap.add_predicate_object_map_on_join(key,childMap,"_pyrml_mapper_generated_id","_pyrml_mapper_parent_id")
                            self.maps += [childMap]

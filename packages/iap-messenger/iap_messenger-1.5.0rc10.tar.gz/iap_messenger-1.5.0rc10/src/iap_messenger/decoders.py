...
def decode(data: bytes, decoder: str) -> tuple[Any, Error]:
    ...
    if decoder == "file":
        ...
        res, err = decode_multipart(data, {"name": "data"}, "", use_lz4)
        return res["data"], err
    elif decoder == "audio":
...
def decode_multipart(data: bytes, conf: dict, content_type: str, use_lz4: bool=False) -> tuple[dict, Error]:
    """
    ...
    try:
        ...
        if len(items) == 0:
            res[name] = BytesTarget()
            parser.register(name, res[name])
        else:
            for item in items:

...
def quantum(
    ...
    try:
        ...
        # Billing integration - authenticate user and track usage
        user_id = None
        api_key_id = None
        billing_enabled = BILLING_ENABLED and api_key

        if billing_enabled:
            try:
                # Import simple billing integration
                from .simple_billing import authenticate_user, log_usage
                ...
                if user_info:
                    ...
                    logger.info(f"Authenticated user: {user_info['email']} ({user_info['plan']})")
                else:
                    logger.warning(f"Authentication failed for API key: {api_key[:12]}...")

            except Exception as e:
                logger.warning(f"Billing integration error: {e}")
                billing_enabled = False
        ...
        if billing_enabled and user_id and api_key_id:
            try:
                ...
                log_usage(
                    ...
                    result_data=result.counts if hasattr(result, 'counts') else None
                )
            except Exception as e:
                logger.warning(f"Billing logging error: {e}")
        ...
        # Log failed usage for billing if enabled
        if billing_enabled and user_id and api_key_id:
            try:
                log_usage(
                    user_id=user_id,
                    api_key_id=api_key_id,
                    program=program,
                    backend=actual_backend if 'actual_backend' in locals() else backend,
                    shots=shots,
                    success=False,
                    execution_time=0,
                    error_message=str(e)
                ...
                logger.warning(f"Billing logging error: {billing_error}")

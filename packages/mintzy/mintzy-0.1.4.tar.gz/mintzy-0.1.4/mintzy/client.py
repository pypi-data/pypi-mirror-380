        ...
        try:
            ...
            for ticker in tickers:
                for param in parameters:
                    raw_data = response_json["data"]["result"][ticker][param]["data"]

                    # Convert text block → DataFrame
                    df = pd.read_csv(pd.compat.StringIO(raw_data), sep=r"\s+", engine="python")
    ...
    def get_prediction(self, tickers, time_frame, parameters):
        ...
        while True:
            try:
                ...
                print("\033c", end="")  # clear terminal (Linux/Mac). Use os.system("cls") on Windows

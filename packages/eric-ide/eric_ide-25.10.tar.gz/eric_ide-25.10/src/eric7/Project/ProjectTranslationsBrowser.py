...
def _elementTreeXmlString(xmlElement):
    ...
    _escape_cdata_orig = ElementTree._escape_cdata
    ElementTree._escape_cdata = _escape_cdata_linguist

    xmlString = ElementTree.tostring(xmlElement, encoding="unicode")

    # set back ElementTree to its original
    ElementTree._escape_cdata = _escape_cdata_orig
...
class ProjectTranslationsBrowser(ProjectBaseBrowser):
    ...
    def __postProcessTSFile(self, tsFile):
        ...
        root = ElementTree.parse(source=tsFilePath).getroot()  # secok
        ElementTree.indent(root, space="")  # reset indentation
        for contextElement in root.findall("context"):
            # indent all context elements with 4 spaces
            ElementTree.indent(contextElement, space=4 * " ")

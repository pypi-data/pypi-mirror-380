...
def _build_feature_for_probe(
    *,
    ...
    for layer in probe_layers:
        ...
        pool_rs = _pooled(seq_rs, valid_idxs, pooling).numpy()
        pool_attn = _pooled(seq_attn, valid_idxs, pooling).numpy()
        pool_mlp = _pooled(seq_mlp, valid_idxs, pooling).numpy()
...
def generate_with_probes(
    ...
    *,
    ...
    evaluate_on: str = "first",         # "first" | "stream" | "last"
    ...
    try:
        ...
        if evaluate_on in {"first", "stream"}:
            for p in probes:
                ...
                if not layers:
                    # default: all layers based on caches
        ...
        # Stream / Last: generate tokens step-by-step
        generated_ids: List[int] = []
        stream_probs: Dict[str, List[float]] = {p.name: [] for p in probes}

        if evaluate_on in {"stream", "last"}:
            # We drive decoding manually (one token at a time) to evaluate each step
            use_cache = True
            ...
            for t in range(max_new):
                ...
                # Per-step feature → prob (overwrite internal buffers; we only keep scalar probs)
            ...
            if evaluate_on in {"last"}:
                # Score on the final assistant output (last token)
                # Re-run one forward over full sequence to be safe (uses caches from the final step otherwise)
        ...
        # If we’re here we did only "first"
        return {
            "text": "",

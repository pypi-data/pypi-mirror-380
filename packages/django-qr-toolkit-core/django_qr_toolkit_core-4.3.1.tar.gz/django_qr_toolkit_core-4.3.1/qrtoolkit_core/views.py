...
import uuid
import logging
from qrtoolkit_core.generator.qr import create_qr_code
import base64
...
class CodeView(DetailView):
    ...
    def get(self, request, *args, **kwargs):
        img_str = self.get_object(request)
    ...
    def get_object(self, queryset=None):
        ...
        try:
            try:
                ...
                qrcode = self.queryset.get(uuid=short_uuid)
            except ValueError:
                # short_uuid is not a valid uuid object
                qrcode = self.queryset.get(short_uuid=uid)
        ...
        qrform.is_valid()
        qr_image = create_qr_code(qrcode, qrform).decode('utf-8')
        # base64_code = base64.b64encode(qr_image).decode('utf-8')
        return qr_image

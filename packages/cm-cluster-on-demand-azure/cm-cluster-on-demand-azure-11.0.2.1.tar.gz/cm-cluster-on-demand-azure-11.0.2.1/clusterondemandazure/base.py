...
class ClusterCommand:
    ...
    def _validate_vmsizes_in_region(self, head_node_type: str, node_type: str) -> None:
        vm_sizes = [sku.name.lower() for sku in self.vm_skus]  # type: ignore
    ...
    def _validate_vmsizes_in_az(self, head_node_type: str, node_type: str) -> None:
        ...
        for sku in self.vm_skus:
            ...
            if (sku.location_info and sku.location_info[0].zones and
                    config["azure_availability_zone"] in sku.location_info[0].zones):
                vm_sizes.append(sku.name.lower() if sku.name else "")

        ...
        try:
            ...
            if ent_name.strip() and ((isinstance(ent_code, str) and ent_code.strip()) or isinstance(ent_code, int)):  
                ...
                if str(template_Keyword_1).upper().replace(" ", "").strip() not in OCR_Text.upper().replace(" ", ""):
                    ...
                    templatecreation.generateHeaderTemplate(ymlfilepath_final,str(ent_name).upper().strip(),ent_code,ent_type,ai_proc_variables,OCR_Text,str(template_Keyword_1).strip(),str(template_Keyword_2).strip())
                    templatecreation.generateOCRTextFile(ocrfilepath_final,str(ent_name).upper().strip(),ent_code,ent_type,ai_proc_variables,OCR_Text,str(template_Keyword_1).strip(),str(template_Keyword_2).strip())

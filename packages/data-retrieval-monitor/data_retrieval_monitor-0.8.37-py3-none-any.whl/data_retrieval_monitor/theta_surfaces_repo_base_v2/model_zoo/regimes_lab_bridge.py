...
def build_labels_regimes_lab(
    *,
    ...
    """Run regimes_lab on per-asset features to obtain labels for all available regime models.
    ...
    for asset, g in df.group_by(asset_col):
        ...
        if pdf.empty:
            continue
        L = R.load_or_build_labels(pdf, split_tag=split_tag)
...
def build_alpha_from_regimes_lab_ols_hac(
    *,
    ...
    For each factor column in `returns`:
      - y_h(t) = sum_{i=0..h-1} r(t+i) aligned at t
      - For each (model, regime_code, lag): dummy(t) = 1{label(t-lag) == regime_code}
      - OLS(HAC): y_h ~ const + dummy; extract t-stat, p-val for dummy
      - Strategy Sharpe/IR: w_t = sign(coef) * dummy(t); sr = mean(w_t*y_h)/std(...); ir = sr*sqrt(252)

    Returns a tall Polars DataFrame with alpha metrics suitable for Cross-Alpha.
    """
    try:
    ...
    if isinstance(labels_long, pl.DataFrame):
        ...
        L = labels_long.copy()
    # wide labels per model
    Lw = L.pivot_table(index=["time","asset_id"], columns="model_name", values="regime_code", aggfunc="first")
    # We assume labels are global (same per asset universe) — take any asset or average across assets by time
    # Prefer per-factor labels if asset_id aligns with factor name; else fallback to time-level labels.
    # Build a time-indexed label table by taking the mode across assets per time for each model.
    ...
    for fac in factors:
        ...
        for h in horizons:
            ...
            for m in models:
                ...
                for rid in regimes:
                    for lag in lags:
                        # build dummy at time t using label at (t - lag)
                        ...
                        # use last timestamp in sample as "time" for this metric row
                        t_last = df.index.max()
                        rows += [

...
# Data tab uses pipeline stages
DATA_STAGES = ["archive", "stage", "enrich", "consolidate"]

# OWNERS_LIST = [get_owner('KIMDG'), get_owner('QSG'), get_owner('ALICE'), get_owner('BOB')]

# Status dictionaries per tab
DATA_STATUS_ORDER = [
...
# Colors per status for each tab (RGB)
...
def rgb_for_tab(tab: str) -> Dict[str, Tuple[int,int,int]]:
    ...
    # data
    return {s: BASE_RGB.get(s, BASE_RGB["other"]) for s in DATA_STATUS_ORDER}

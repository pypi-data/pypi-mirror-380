...
class Figure:
    ...
    def curve(
        ...
        if len(args) == 1:
            ...
            vertices = [
                curve.subs(symbol, start + (end - start) * (i / (samples - 1)))
        ...
        def _distinct(points: list[sp.Point], tolerance: float = 1e-6) -> list[sp.Point]:
            ...
            last = None
            for p in points:
                if last is None or p.distance(last) > tolerance:
                    out.append(p)
                    last = p
            return out
    ...
    def path(
        self, *entities: Iterable[Arc | sp.Segment], close: bool = False, **kwargs: Any
    ) -> None:
        """

...
async def add_document(
    ...
    try:
        logger.info(f"Starting document processing - file: {file_path}, collection: {collection_name}")
        ...
        with get_db_session() as session:
            ...
            if ctx:
                await ctx.report_progress(progress=15, total=100, message="Validating collection and file...")
            
            # Stage 1.5: Duplicate detection and handling (15-25%)
            if ctx:
                await ctx.report_progress(progress=15, total=100, message="Checking for file duplicates...")
            ...
            if existing_file_info:
                ...
                if duplicate_action == "skip":
                    logger.info(f"File already exists, skipping processing - file: {str(file_path_obj)}, existing_vectors: {existing_file_info['vector_count']}")
                    return {
                        "success": True,
                    ...
                    if current_file_mtime:
                        ...
                        logger.info(f"Smart detection mode: File already exists, will overwrite - file: {str(file_path_obj)}, mtime: {current_file_mtime.isoformat() if current_file_mtime else None}")
                        file_status["file_changed"] = True
                        action_taken = "overwrite"
                    else:
                        logger.info(f"Cannot get file modification time, skipping processing - file: {str(file_path_obj)}")
                        return {
                            "success": False,
                    ...
                    logger.info(f"Force overwrite mode: deleting existing vectors - file: {str(file_path_obj)}, existing_vectors: {existing_file_info['vector_count']}")
                    action_taken = "overwrite"
                    file_status["file_changed"] = True
                    
                elif duplicate_action == "append":
                    logger.info(f"Append mode: keeping existing vectors, adding new vectors - file: {str(file_path_obj)}, existing_vectors: {existing_file_info['vector_count']}")
                    action_taken = "append"
                ...
                if action_taken in ["overwrite"] or (action_taken == "overwrite" and duplicate_action == "smart"):
                    if ctx:
                        await ctx.report_progress(progress=20, total=100, message=f"Deleting {existing_file_info['vector_count']} existing vectors...")
            ...
            if ctx:
                await ctx.report_progress(progress=25, total=100, message="Duplicate detection complete, starting document processing...")
            
            # Stage 2: Text chunking (25-50%)
            chunking_start = time.time()
            if ctx:
                await ctx.report_progress(progress=25, total=100, message="Processing document into chunks...")
            ...
            logger.info(f"Document chunking complete - chunks: {len(chunks)}, time: {chunking_time:.2f}s")
            
            if ctx:
                await ctx.report_progress(progress=50, total=100, message=f"Generated {len(chunks)} document chunks")
            
            # Stage 3: Vector generation (50-90%) - True batch processing with single transaction
            vector_start = time.time()
            if ctx:
                await ctx.report_progress(progress=60, total=100, message="Preparing batch vector generation...")
            ...
            if ctx:
                await ctx.report_progress(progress=70, total=100, message=f"Starting processing of {len(all_batch_data)} document chunks (batch mode)...")
            ...
            logger.info(f"Batch vector generation complete - vectors_created: {len(results)}, time: {vector_time:.2f}s")
            
            if ctx:
                await ctx.report_progress(progress=85, total=100, message=f"Completed batch processing, generated {len(results)} vectors")
            
            # Stage 4: Database storage completion (90-100%)
            if ctx:
                await ctx.report_progress(progress=90, total=100, message="Completing vector storage...")
                await ctx.report_progress(progress=100, total=100, message="Processing complete")
            
            total_time = time.time() - start_time
            logger.info(f"Document processing complete - total_time: {total_time:.2f}s, vectors_created: {len(results)}, chunks_processed: {len(chunks)}, avg_time_per_vector: {total_time/len(results):.3f}s" if results else "N/A")
        ...
        total_time = time.time() - start_time
        logger.error(f"Document processing failed - error: {str(e)}, time: {total_time:.2f}s, file: {file_path}, collection: {collection_name}")
        if ctx:

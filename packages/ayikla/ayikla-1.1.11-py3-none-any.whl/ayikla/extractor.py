...
class TextEntityExtractor:
    ...
    ADRES_PATTERN = re.compile(
        ...
        r"((?:no:?|num(ara|\.?))\s*\d{0,3}(?!\d))|"   # no / numara (en fazla 3 hane)
    ...
    def extract_locations(self, text: str, max_char_gap: int = 8) -> List[str]:
        ...
        for s, e in birlesik:
            ...
            if low in {"d", "de", "da", "te", "ta"}:
                continue
            # Uzun rakam dizisi sadece telefon/adres dışı gürültü ise at (4+ hane ardışık ve tetikleyici yoksa)
            if re.fullmatch(r"\d{4,}", frag):
                continue
            # 'numara' veya 'no' segmentleri sonrası 3+ hane olmayan base halinde kaldıysa ekle

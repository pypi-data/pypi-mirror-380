...
class TextEntityExtractor:
    ...
    ADRES_PATTERN = re.compile(
        ...
        r"(daire\s*\d*|d:?\s*\d*|apt|apartment)"  # daire / apt
        r")",
        flags=re.IGNORECASE
    )
    ...
    def extract_locations(self, text: str, window: int = 10) -> List[str]:
        """Metinden lokasyon / adres benzeri blokları çıkarır.

        Basit bir heuristik uygular: Adres tetikleyici pattern veya lokasyon listesi
        içindeki bir kelime görülürse o konum işaretlenir ve yakın indeksler bloklanır.
        Her blok etrafından window kadar kelime alınır.
        """
        if not text.strip() or not self.lokasyon_seti:
            return []

        tokens = text.split()
        adres_indices: List[int] = []

        for i, tok in enumerate(tokens):
            t_clean = tok.strip("',.()[]{}")
            t_lower = t_clean.lower()

            if self.ADRES_PATTERN.search(tok):
                adres_indices.append(i)
                continue
            if t_lower in self.lokasyon_seti:
                adres_indices.append(i)
                continue
            for ek in self.LOKASYON_EKLER:
                if t_lower.endswith(ek):
                    base = t_lower[:-len(ek)]
                    if base in self.lokasyon_seti:
                        adres_indices.append(i)
                        break

        if not adres_indices:
            return []

        adres_indices.sort()
        bloklar: List[Tuple[int, int]] = []
        start = adres_indices[0]
        end = start
        for idx in adres_indices[1:]:
            if idx - end <= window:
                end = idx
            else:
                bloklar.append((start, end))
                start = end = idx
        bloklar.append((start, end))

        adresler: List[str] = []
        for s, e in bloklar:
            frag_tokens = tokens[max(0, s-window): min(len(tokens), e+window+1)]
            frag = " ".join(frag_tokens).strip()
            if frag and frag not in adresler:
                adresler.append(frag)
        return adresler

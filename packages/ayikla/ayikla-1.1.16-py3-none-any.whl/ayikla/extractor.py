...
class TextEntityExtractor:
    ...
    def extract_date_ranges(self, text: str) -> List[Dict[str, Optional[str]]]:
        ...
        Dönen her öğe sözlük:
          - tür: "relative" | "gun_araligi"
          - ifade: orijinal matched string
          - baslangic: YYYY-MM-DD veya None
          - bitis: YYYY-MM-DD veya None
        Normal tarihlerden ayrı tutulur.
        """
            ...
            try:
                if 'hafta' in expr_low:
                    if 'önümüzdeki' in expr_low:
                        # next week
                        delta = (7 - now.weekday()) % 7  # days until next Monday
        ...
        # Benzersizleştir (ifade + span değil, tam sözlük bazlı)
        unique = []
        seen = set()
        for r in results:
            key = (r['tür'], r['ifade'], r['baslangic'], r['bitis'])
            if key not in seen:
                seen.add(key)
                unique.append(r)
        ...
        for m in self.AY_ARALIK_PATTERN.finditer(text):
            ...
            try:
                ...
                year = int(yil) if yil else now.year
                # Eğer m2 < m1 ve yıl verilmemişse yıl değişimi varsay: (ör: Aralık - Ocak)
                if m2 < m1 and not yil:

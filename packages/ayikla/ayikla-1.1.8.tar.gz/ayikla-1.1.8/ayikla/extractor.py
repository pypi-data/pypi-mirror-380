...
class TextEntityExtractor:
    ...
    def extract_locations(self, text: str, max_char_gap: int = 8) -> List[str]:
        ...
        # 2) Lokasyon sözlüğü (varsa) - kelime bazlı whole word araması
        if self.lokasyon_seti:
            # Basit kelime tarama: \bkelime\b ile
            for loc in self.lokasyon_seti:
                # çok kısa (1 harf) lokasyonları atla gürültü azaltmak için
                if len(loc) < 2:
                    continue

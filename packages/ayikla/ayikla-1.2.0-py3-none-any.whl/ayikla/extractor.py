...
MONTH_WORDS = {"ocak","şubat","mart","nisan","mayıs","haziran","temmuz","ağustos","eylül","ekim","kasım","aralık"}
...
class TextEntityExtractor:
    ...
    def normalize_relative_tr_phrases(self, text: str) -> str:
        ...
        # Morfolojik varyantları normalize et
        t = re.sub(r"\byarınki\b", "yarın", t, flags=re.IGNORECASE)
        t = re.sub(r"\bbugünkü\b", "bugün", t, flags=re.IGNORECASE)
        t = re.sub(r"\bbugünün\b", "bugün", t, flags=re.IGNORECASE)
        return t
    ...
    def extract_all_dates(self, text: str) -> List[Dict[str, Optional[str]]]:
        ...
        for matched_text, dt in raw_matches:
            ...
            if not (1000 <= dt.year <= 2100):
                continue
            mt_low = mt.lower()
            # Gün-aralık (7-15 ocak) gibi ifadeleri tekil tarihe dönüştürme; range extractor zaten yakalayacak.
            if re.match(r"\d{1,2}\s*-\s*\d{1,2}\s+(ocak|şubat|mart|nisan|mayıs|haziran|temmuz|ağustos|eylül|ekim|kasım|aralık)", mt_low):
                continue
            # 'haftaya' yalnız başına (veya 'haftaya olan') -> tekil tarih istemiyoruz, range fonksiyonu işleyecek.
            if re.fullmatch(r"haftaya(?:\s+olan)?", mt_low):
                continue
            # 'haftaya <gun>' veya 'gelecek <gun>' manual hesapla (dateparser hatalarını aşmak için)
            m_hw = re.match(r"(haftaya|gelecek)\s+(pazartesi|salı|çarşamba|perşembe|cuma|cumartesi|pazar)", mt_low)
            if m_hw:
                _, wd = m_hw.groups()
                weekday_map = {"pazartesi":0,"salı":1,"çarşamba":2,"perşembe":3,"cuma":4,"cumartesi":5,"pazar":6}
                # Sonraki haftanın pazartesisi
                delta_to_next_monday = (7 - now.weekday()) % 7
                if delta_to_next_monday == 0:
                    delta_to_next_monday = 7
                next_monday = (now + timedelta(days=delta_to_next_monday)).date()
                hedef = next_monday + timedelta(days=weekday_map[wd])
                tarih = hedef.isoformat()
                if tarih not in date_items:
                    date_items[tarih] = {"tarih": tarih, "saat": None}
                continue
            explicit_time = None
    ...
    RELATIVE_RANGE_PATTERNS = [
        ...
        r"haftaya(?!\s+(pazartesi|salı|çarşamba|perşembe|cuma|cumartesi|pazar))"  # yalın 'haftaya'
    ]
        ...
        for m in self.REL_WORD_PATTERN.finditer(text):
            ...
            try:
                if 'hafta' in expr_low:
                    if 'önümüzdeki' in expr_low or expr_low.startswith('haftaya'):
    ...
    def extract_locations(self, text: str, max_char_gap: int = 8) -> List[str]:
        ...
        for s, e in birlesik:
            ...
            if len(frag) == 1:
                continue
            low = frag.lower()
            if low in MONTH_WORDS:  # ay isimlerini lokasyon olarak raporlama
                continue

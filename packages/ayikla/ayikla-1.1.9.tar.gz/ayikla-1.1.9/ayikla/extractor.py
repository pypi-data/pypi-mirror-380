...
class TextEntityExtractor:
    ...
    def extract_locations(self, text: str, max_char_gap: int = 8) -> List[str]:
        ...
        # 2) Lokasyon sözlüğü (varsa) - ek soyma ile
        if self.lokasyon_seti:
            ekler = ("de","da","te","ta")  # çıplak ekler (örn istanbulda)
            apostrof_ekler = ("'de","'da","'te","'ta")
            for loc in self.lokasyon_seti:
                if len(loc) < 2:
                    continue
                # doğrudan kelime
                for m in re.finditer(rf"\b{re.escape(loc)}\b", raw, flags=re.IGNORECASE):
                    matches.append((m.start(), m.end()))
                # ekli varyantlar (bitik)
                for ek in ekler:
                    pattern = rf"\b{re.escape(loc)}{ek}\b"
                    for m in re.finditer(pattern, raw, flags=re.IGNORECASE):
                        matches.append((m.start(), m.start()+len(loc)))  # sadece base
                # apostrof ile
                for ek in apostrof_ekler:
                    pattern = rf"\b{re.escape(loc)}{ek}\b"
                    for m in re.finditer(pattern, raw, flags=re.IGNORECASE):
                        matches.append((m.start(), m.start()+len(loc)))

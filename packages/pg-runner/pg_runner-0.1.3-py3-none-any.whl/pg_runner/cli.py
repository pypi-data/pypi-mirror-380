...
class DBSetupPhase(StrEnum):
    """Enumerate supported database setup phases."""
...
class PhaseConfig(TypedDict):
    """Configuration for a database setup phase."""
...
def _resolve_log_level(value: str | int) -> int:
    ...
    try:
        if isinstance(value, int):
            return value
        # Convert any other value to string and parse
...
# ----------------------------------------------------------------------
class _CLISimpleFormatter(logging.Formatter):
    """Simple CLI formatter.

    - INFO and below: message only
...
def _configure_logging(level: str | int) -> None:
    """Configure root logger with a CLI-friendly format."""
    root = logging.getLogger()
    root.handlers.clear()
    root.setLevel(_resolve_log_level(level))

    handler = logging.StreamHandler()
    handler.setFormatter(_CLISimpleFormatter())
...
def _compose_db_url_from_parts(
    ...
    Returns a SQLAlchemy-style PostgreSQL URL.
    """
...
def _redact_url(url: str) -> Literal[b""] | str:
...
def _execute_statements(engine: Engine, statements: list[str], phase: str) -> None:
    ...
    """
    if not statements:
        logger.info("%s: no executable statements found; skipping", phase)
        return
    ...
    with engine.begin() as conn:
        for i, stmt in enumerate(statements, start=1):
            try:
                ...
                logger.debug("%s [%s/%s]: executed", phase, i, len(statements))
            except Exception as exc:
                if logger.isEnabledFor(logging.DEBUG):
                    logger.exception(
                        "%s [%s/%s]: error executing statement",
                        phase,
                        i,
                        len(statements),
                    )
                    ...
                    logger.error(
                        "%s [%s/%s]: error executing statement: %s",
                        phase,
                        i,
                        len(statements),
                        exc,
                    )
                logger.debug("%s: failing statement was:%s%s", phase, os.linesep, stmt)
                raise
    logger.info("%s: executed %s statements", phase, len(statements))
    ...
    with engine.connect() as base_conn:
        ...
        for i, stmt in enumerate(statements, start=1):
            try:
                ...
                logger.debug("%s: failing statement was:%s%s", phase, os.linesep, stmt)
                raise
    logger.info("%s: executed %s statements (autocommit)", phase, len(statements))
...
def setup_database_phases(
    engine: Engine,
    db_dir: Path,
    phases: list[DBSetupPhase],
    *,
    use_concurrent: bool,
) -> bool:
    """Set up database schema, indexes, and views using project-style phases."""
    try:
        phase_configs: dict[DBSetupPhase, PhaseConfig] = {
            DBSetupPhase.SCHEMA: {
                "file": "schema.sql",
                "required": True,
        ...
        for phase in phases:
            if phase not in phase_configs:
                logger.warning("Unknown phase '%s' skipped", phase.value)
                continue

            config: PhaseConfig = phase_configs[phase]
            sql_file: Path = db_dir / config["file"]
            sql_content = _read_sql_file(sql_file)

            if sql_content is None:
                if config["required"]:
                    logger.error("%s is required but was not found.", config["file"])
                    return False
                logger.info(
                    "No %s found; skipping %s phase.",
                    config["file"],
                    phase.value,
                )
                continue
            ...
            if not statements:
                logger.info(
                    "%s: no executable statements parsed; skipping",
                    phase.value,
                )
                continue
            ...
            if _has_concurrent_index(statements) and not config["concurrent"]:
                logger.warning(
                    "%s: detected CREATE INDEX CONCURRENTLY but autocommit is disabled; these statements will fail inside a transaction",
                    phase.value,
                )
        ...
        if logger.isEnabledFor(logging.DEBUG):
            ...
            logger.error("Database setup (phases) failed: %s", exc)
        return False
    ...
    *,
    ...
    try:
        if not sql_dir.exists() or not sql_dir.is_dir():
            logger.error("SQL directory not found or not a directory: %s", sql_dir)
            return False
        ...
        if not files:
            logger.warning(
                "No .sql files found in %s (recursive=%s)",
                sql_dir,
                recursive,
            )
        ...
        logger.info(
            "Found %s .sql files in %s (recursive=%s); autocommit=%s",
            len(files),
            sql_dir,
            recursive,
            autocommit,
        ...
        for idx, file in enumerate(files, start=1):
            ...
            if content is None:
                logger.error("Skipping unreadable file: %s", file)
                return False
            ...
            logger.info("Executing [%s/%s]: %s", idx, len(files), rel)
            if autocommit:
        ...
        if logger.isEnabledFor(logging.DEBUG):
            ...
            logger.error("SQL directory execution failed: %s", exc)
        return False
...
def main() -> None:
    """Entry point for the pg-runner CLI."""
    args = parse_args()
    ...
    logger.info("Standalone PostgreSQL Database Setup / SQL Runner")
    logger.info("%s", "=" * 50)
    logger.info("DB URL: %s", redacted)
    ...
    if not test_connection(engine):
        logger.error("%s Database connection failed", failure_icon)
        sys.exit(1)
    ...
    if args.command == "phases":
        ...
        logger.info("Phases: %s", ", ".join(p.value for p in phases_enum))
        if use_concurrent:
            logger.info("Using CONCURRENTLY for index creation")
        logger.info("")

        ok = setup_database_phases(
            engine,
            db_dir,
            phases_enum,
        ...
        logger.info("SQL directory: %s (recursive=%s)", sql_dir, recursive)
        logger.info("Autocommit: %s", autocommit)
        logger.info("")

        ok = run_sql_directory(
            engine,
            sql_dir,
            recursive=recursive,
        ...
        logger.error("%s Unknown command: %s", failure_icon, args.command)
        sys.exit(2)

    if ok:
        logger.info("%s Completed successfully", success_icon)
        sys.exit(0)
    else:
        logger.error("%s Failed", failure_icon)
        sys.exit(1)

...
class ReadTool:
    ...
    def read_file(
        ...
        """
        Read file content with smart token management and flexible reading modes.

        USAGE:
        - Full read: Small-medium files, initial exploration
        - Line ranges: Specific sections, error locations (LineArgs)
        - Regex search: Find patterns, function definitions (RegexArgs)

        EXAMPLES:
        read_file('src/main.py')  # Full file read
        read_file('config.json', LineArgs(lines_start=10, lines_end=20))  # Line range
        read_file('*.py', RegexArgs(regex=r'def main'))  # Regex search

        LIMITATIONS & ERRORS:
        - Large files (>50MB): Use line ranges to avoid token limits
        - "File not found": Check path spelling and existence
        - "Content truncated": File too large, use targeted reading

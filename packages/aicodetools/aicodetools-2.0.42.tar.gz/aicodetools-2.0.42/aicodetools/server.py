...
class CodeToolsHandler(BaseHTTPRequestHandler):
    ...
    def _handle_read(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Parse read arguments - handle both direct and nested formats
        read_args = None
        try:
            # Check for nested read_args format (from client)
            args_data = data.get('read_args', data)  # Fallback to data itself for backward compatibility

            if args_data.get('regex'):
                # RegexArgs mode
                read_args = RegexArgs(regex=args_data['regex'])
            elif args_data.get('lines_start'):
                # LineArgs mode
                read_args = LineArgs(

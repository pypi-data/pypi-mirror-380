...
class CodeToolsClient:
    ...
    def _create_read_tool(self) -> Callable:
        ...
        def read_file_tool(file_path: str, read_args: Optional[Union[LineArgs, RegexArgs]] = None) -> Dict[str, Any]:
            data = {"file_path": file_path}

            # Handle read_args parameter if provided
            if read_args is not None:
                if isinstance(read_args, RegexArgs):
                    data["read_args"] = {"regex": read_args.regex}
                    data["read_args_type"] = "RegexArgs"
                elif isinstance(read_args, LineArgs):
                    read_args_data = {"lines_start": read_args.lines_start}
                    if read_args.lines_end is not None:
                        read_args_data["lines_end"] = read_args.lines_end
                    if read_args.complete_lines:
                        read_args_data["complete_lines"] = read_args.complete_lines
                    data["read_args"] = read_args_data
                    data["read_args_type"] = "LineArgs"

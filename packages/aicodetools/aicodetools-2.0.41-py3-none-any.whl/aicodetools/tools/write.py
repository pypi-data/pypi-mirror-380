...
class WriteTool:
    ...
    def write_file(self, file_path: str, content: str) -> Dict[str, Any]:
        """
        Write content to files with automatic backup and read-first safety.

        USAGE:
        - New files: Scripts, configs, documentation - write freely
        - Replace existing: Major rewrites, templates - must read first
        - Use edit_file for: Small targeted changes, line modifications

        EXAMPLES:
        write_file('new_script.py', 'print("Hello World")')  # New file
        write_file('config.json', '{"debug": true}')  # Replace existing (after read)

        SAFETY & ERRORS:
        - Must read existing non-empty files first before writing
        - "Must read file first": Read existing file content before overwriting

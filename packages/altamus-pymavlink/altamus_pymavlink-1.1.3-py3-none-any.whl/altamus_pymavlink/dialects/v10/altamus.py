...
class MAVLink_identifier_message(MAVLink_message):
    ...
    msgname = "IDENTIFIER"
    fieldnames = ["fw_version", "particle_id_full", "device_id", "device_name", "local_ip", "mac"]
    ordered_fieldnames = ["fw_version", "particle_id_full", "device_id", "device_name", "local_ip", "mac"]
    ...
    def __init__(self, fw_version: int, particle_id_full: bytes, device_id: bytes, device_name: bytes, local_ip: Sequence[int], mac: Sequence[int]):
        ...
        self._particle_id_full_raw = particle_id_full
        self.particle_id_full = particle_id_full.split(b"\x00", 1)[0].decode("ascii", errors="replace")
    ...
    def pack(self, mav: "MAVLink", force_mavlink1: bool = False) -> bytes:
        return self._pack(mav, self.crc_extra, self.unpacker.pack(self.fw_version, self._particle_id_full_raw, self._device_id_raw, self._device_name_raw, self.local_ip[0], self.local_ip[1], self.local_ip[2], self.local_ip[3], self.mac[0], self.mac[1], self.mac[2], self.mac[3], self.mac[4], self.mac[5]), force_mavlink1=force_mavlink1)
...
class MAVLink(object):
    ...
    def identifier_encode(self, fw_version: int, particle_id_full: bytes, device_id: bytes, device_name: bytes, local_ip: Sequence[int], mac: Sequence[int]) -> MAVLink_identifier_message:
        """
        Indentifiying information about the EOS device

        fw_version                : Particle FW version (type:uint8_t)
        particle_id_full          : Particle ID of device. Read Only. (type:char)
        device_id                 : Device id of scanner matching manufacturing sticker. i.e. P2-ABC123. Read only (type:char)
        device_name               : Friendly name of device. E.g. "57 Rock West". User settable (type:char)
        local_ip                  : local IPV4 Address of the device (type:uint8_t)
        mac                       : MAC address of the device (type:uint8_t)

        """
        return MAVLink_identifier_message(fw_version, particle_id_full, device_id, device_name, local_ip, mac)

    def identifier_send(self, fw_version: int, particle_id_full: bytes, device_id: bytes, device_name: bytes, local_ip: Sequence[int], mac: Sequence[int], force_mavlink1: bool = False) -> None:
        """
        Indentifiying information about the EOS device
        ...
        """
        self.send(self.identifier_encode(fw_version, particle_id_full, device_id, device_name, local_ip, mac), force_mavlink1=force_mavlink1)

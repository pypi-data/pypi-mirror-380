...
from ...models.fd_options import FDOptions


class FDAPI(BaseAPI):
    def build_endpoint(self, options: FDOptions) -> str:
        def join_or_star(val):
            if isinstance(val, list):
                return "+".join(val)
            return val or "*"

        endpoint = f"FD/{join_or_star(options.country)}.{join_or_star(options.indicator)}.{join_or_star(options.sector)}.{join_or_star(options.frequency)}"
        return endpoint

    def get_data(self, options: FDOptions):

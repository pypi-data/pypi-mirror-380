...
from ...models.srd_options import SRDOptions


class SRDAPI(BaseAPI):
    def build_endpoint(self, options: SRDOptions) -> str:
        def join_or_star(val):
            if isinstance(val, list):
                return "+".join(val)
            return val or "*"

        endpoint = f"SRD/{join_or_star(options.country)}.{join_or_star(options.indicator)}.{join_or_star(options.frequency)}"
        return endpoint

    def get_data(self, options: SRDOptions):

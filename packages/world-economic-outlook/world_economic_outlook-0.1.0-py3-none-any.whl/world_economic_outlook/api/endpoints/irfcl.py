...
from ...models.irfcl_options import IRFCLOptions


class IRFCLAPI(BaseAPI):
    def build_endpoint(self, options: IRFCLOptions) -> str:
        def join_or_star(val):
            if isinstance(val, list):
                return "+".join(val)
            return val or "*"

        endpoint = f"IRFCL/{join_or_star(options.country)}.{join_or_star(options.indicator)}.{join_or_star(options.sector)}.{join_or_star(options.frequency)}"
        return endpoint

    def get_data(self, options: IRFCLOptions):

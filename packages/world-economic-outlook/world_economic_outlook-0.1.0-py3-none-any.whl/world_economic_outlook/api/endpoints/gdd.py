...
from ...models.gdd_options import GDDOptions


class GDDAPI(BaseAPI):
    def build_endpoint(self, options: GDDOptions) -> str:
        def join_or_star(val):
            if isinstance(val, list):
                return "+".join(val)
            return val or "*"

        endpoint = f"GDD/{join_or_star(options.country)}.{join_or_star(options.indicator)}.{join_or_star(options.frequency)}"
        return endpoint

    def get_data(self, options: GDDOptions):

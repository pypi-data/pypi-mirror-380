...
from ...models.mpft_options import MPFTOptions


class MPFTAPI(BaseAPI):
    def build_endpoint(self, options: MPFTOptions) -> str:
        def join_or_star(val):
            if isinstance(val, list):
                return "+".join(val)
            return val or "*"

        endpoint = f"MPFT/{join_or_star(options.country)}.{join_or_star(options.indicator)}.{join_or_star(options.type_of_transformation)}.{join_or_star(options.frequency)}"
        return endpoint

    def get_data(self, options: MPFTOptions):

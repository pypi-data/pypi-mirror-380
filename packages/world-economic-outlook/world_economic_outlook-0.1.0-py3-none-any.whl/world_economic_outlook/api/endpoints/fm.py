...
from ...models.fm_options import FMOptions


class FMAPI(BaseAPI):
    def build_endpoint(self, options: FMOptions) -> str:
        def join_or_star(val):
            if isinstance(val, list):
                return "+".join(val)
            return val or "*"

        endpoint = f"FM/{join_or_star(options.country)}.{join_or_star(options.indicator)}.{join_or_star(options.frequency)}"
        return endpoint

    def get_data(self, options: FMOptions):

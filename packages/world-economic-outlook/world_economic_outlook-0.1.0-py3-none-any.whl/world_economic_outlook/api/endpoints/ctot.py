...
from ...models.ctot_options import CTOTOptions


class CTOTAPI(BaseAPI):
    def build_endpoint(self, options: CTOTOptions) -> str:
        def join_or_star(val):
            if isinstance(val, list):
                return "+".join(val)
            return val or "*"

        endpoint = f"CTOT/{join_or_star(options.country)}.{join_or_star(options.indicator)}.{join_or_star(options.wgt_type)}.{join_or_star(options.frequency)}"
        return endpoint

    def get_data(self, options: CTOTOptions):

...
from ...models.whdreo_options import WHDREOOptions


class WHDREOAPI(BaseAPI):
    def build_endpoint(self, options: WHDREOOptions) -> str:
        def join_or_star(val):
            if isinstance(val, list):
                return "+".join(val)
            return val or "*"

        endpoint = f"WHDREO/{join_or_star(options.country)}.{join_or_star(options.indicator)}.{join_or_star(options.frequency)}"
        return endpoint

    def get_data(self, options: WHDREOOptions):

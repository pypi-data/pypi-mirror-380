...
from ...models.hpd_options import HPDOptions


class HPDAPI(BaseAPI):
    def build_endpoint(self, options: HPDOptions) -> str:
        def join_or_star(val):
            if isinstance(val, list):
                return "+".join(val)
            return val or "*"

        endpoint = f"HPD/{join_or_star(options.country)}.{join_or_star(options.indicator)}.{join_or_star(options.frequency)}"
        return endpoint

    def get_data(self, options: HPDOptions):

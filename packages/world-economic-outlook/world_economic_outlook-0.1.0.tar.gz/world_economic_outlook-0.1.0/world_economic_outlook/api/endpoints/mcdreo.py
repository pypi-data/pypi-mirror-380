...
from ...models.mcdreo_options import MCDREOOptions


class MCDREOAPI(BaseAPI):
    def build_endpoint(self, options: MCDREOOptions) -> str:
        def join_or_star(val):
            if isinstance(val, list):
                return "+".join(val)
            return val or "*"

        endpoint = f"MCDREO/{join_or_star(options.country)}.{join_or_star(options.indicator)}.{join_or_star(options.frequency)}"
        return endpoint

    def get_data(self, options: MCDREOOptions):

...
from ...models.eq_options import EQOptions


class EQAPI(BaseAPI):
    def build_endpoint(self, options: EQOptions) -> str:
        def join_or_star(val):
            if isinstance(val, list):
                return "+".join(val)
            return val or "*"

        endpoint = f"EQ/{join_or_star(options.country)}.{join_or_star(options.indicator)}.{join_or_star(options.product)}.{join_or_star(options.frequency)}"
        return endpoint

    def get_data(self, options: EQOptions):

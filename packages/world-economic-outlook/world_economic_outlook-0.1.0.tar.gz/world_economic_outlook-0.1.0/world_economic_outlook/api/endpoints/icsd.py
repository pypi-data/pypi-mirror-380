...
from ...models.icsd_options import ICSDOptions


class ICSDAPI(BaseAPI):
    def build_endpoint(self, options: ICSDOptions) -> str:
        def join_or_star(val):
            if isinstance(val, list):
                return "+".join(val)
            return val or "*"

        endpoint = f"ICSD/{join_or_star(options.country)}.{join_or_star(options.indicator)}.{join_or_star(options.frequency)}"
        return endpoint

    def get_data(self, options: ICSDOptions):

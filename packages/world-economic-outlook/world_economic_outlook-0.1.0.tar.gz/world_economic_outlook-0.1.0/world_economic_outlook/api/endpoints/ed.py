...
from ...models.ed_options import EDOptions


class EDAPI(BaseAPI):
    def build_endpoint(self, options: EDOptions) -> str:
        def join_or_star(val):
            if isinstance(val, list):
                return "+".join(val)
            return val or "*"

        endpoint = f"ED/{join_or_star(options.country)}.{join_or_star(options.indicator)}.{join_or_star(options.frequency)}"
        return endpoint

    def get_data(self, options: EDOptions):

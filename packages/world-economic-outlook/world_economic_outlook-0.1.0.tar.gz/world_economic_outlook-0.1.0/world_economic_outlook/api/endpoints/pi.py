...
from ...models.pi_options import PIOptions


class PIAPI(BaseAPI):
    def build_endpoint(self, options: PIOptions) -> str:
        def join_or_star(val):
            if isinstance(val, list):
                return "+".join(val)
            return val or "*"

        endpoint = f"PI/{join_or_star(options.country)}.{join_or_star(options.production_index)}.{join_or_star(options.type_of_transformation)}.{join_or_star(options.frequency)}"
        return endpoint

    def get_data(self, options: PIOptions):

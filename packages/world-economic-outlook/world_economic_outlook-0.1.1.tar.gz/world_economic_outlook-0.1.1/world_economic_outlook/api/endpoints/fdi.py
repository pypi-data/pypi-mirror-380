...
from ...models.fdi_options import FDIOptions


class FDIAPI(BaseAPI):
    def build_endpoint(self, options: FDIOptions) -> str:
        def join_or_star(val):
            if isinstance(val, list):
                return "+".join(val)
            return val or "*"

        endpoint = f"FDI/{join_or_star(options.country)}.{join_or_star(options.indicator)}.{join_or_star(options.frequency)}"
        return endpoint

    def get_data(self, options: FDIOptions):

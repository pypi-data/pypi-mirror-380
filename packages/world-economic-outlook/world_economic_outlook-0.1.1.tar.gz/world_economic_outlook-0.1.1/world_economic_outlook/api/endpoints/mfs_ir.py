...
from ...models.mfs_ir_options import MFS_IROptions


class MFS_IRAPI(BaseAPI):
    def build_endpoint(self, options: MFS_IROptions) -> str:
        def join_or_star(val):
            if isinstance(val, list):
                return "+".join(val)
            return val or "*"

        endpoint = f"MFS_IR/{join_or_star(options.country)}.{join_or_star(options.indicator)}.{join_or_star(options.frequency)}"
        return endpoint

    def get_data(self, options: MFS_IROptions):

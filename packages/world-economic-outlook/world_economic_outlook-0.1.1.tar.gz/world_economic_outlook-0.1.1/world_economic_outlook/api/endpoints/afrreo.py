...
from ...models.afrreo_options import AFRREOOptions


class AFRREOAPI(BaseAPI):
    def build_endpoint(self, options: AFRREOOptions) -> str:
        def join_or_star(val):
            if isinstance(val, list):
                return "+".join(val)
            return val or "*"

        endpoint = f"AFRREO/{join_or_star(options.country)}.{join_or_star(options.indicator)}.{join_or_star(options.frequency)}"
        return endpoint

    def get_data(self, options: AFRREOOptions):

...
from ...models.fa_options import FAOptions


class FAAPI(BaseAPI):
    def build_endpoint(self, options: FAOptions) -> str:
        def join_or_star(val):
            if isinstance(val, list):
                return "+".join(val)
            return val or "*"

        endpoint = f"FA/{join_or_star(options.country)}.{join_or_star(options.indicator)}.{join_or_star(options.frequency)}"
        return endpoint

    def get_data(self, options: FAOptions):

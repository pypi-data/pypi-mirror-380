...
from ...models.eer_options import EEROptions


class EERAPI(BaseAPI):
    def build_endpoint(self, options: EEROptions) -> str:
        def join_or_star(val):
            if isinstance(val, list):
                return "+".join(val)
            return val or "*"

        endpoint = f"EER/{join_or_star(options.country)}.{join_or_star(options.indicator)}.{join_or_star(options.frequency)}"
        return endpoint

    def get_data(self, options: EEROptions):

...
class LLMTool(BaseModel):
    ...
    parameters: str = Field(..., description="Tool parameters in JSON schema")
...
class LLMInput(BaseModel):
    ...
    content: str = Field(..., description="Content of the LLM Input.")
...
class AgentInstance(BaseModel):
    ...
    def _dedupe_tools(self, agent_dict: dict) -> dict:
        ...
        for interaction in agent_dict["interactions"]:
            ...
            if interaction.get("tools") is None:
                continue
            tool_names = []
            ...
            # Replace tools field with available_tools containing just names/IDs
            del interaction["tools"]
            interaction["available_tools"] = tool_names
    ...
    def to_xml(self) -> str:
        ...
        agent_dict = self._dedupe_tools(agent_dict)
...
class RewardRequest(RequestBase):
    ...
    def check_evaluation_criteria_length(cls, evaluation_criteria):
        if len(evaluation_criteria) > 4096:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Evaluation criteria length cannot exceed 4k characters",
            )
    ...
    def evaluation_criteria_must_start_with_correct_prefix(cls, value):
        if not any(value.startswith(start) for start in reward_starts):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Evaluation criteria must start with one of the following: "
                + ", ".join(reward_starts),
            )
        return value
...
class MultiAgentTraceResponse(BaseModel):
    ...
    summary_by_agent_name: Optional[dict[str, Union[SummaryStatistics, None]]] = Field(
        default=None,
        description="Mapping of Agent Name to summary statistics. Only applicable for reward evaluations.",
    )

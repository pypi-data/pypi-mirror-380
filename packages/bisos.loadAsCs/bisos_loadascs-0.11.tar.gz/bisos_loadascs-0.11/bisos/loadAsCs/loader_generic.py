...
#+end_org """
import typing
csInfo: typing.Dict[str, typing.Any] = { 'moduleName': ['loader_generic'], }
csInfo['version'] = '202509275520'
csInfo['status']  = 'inUse'
csInfo['panel'] = 'loader_generic-Panel.org'
...
####+BEGIN: b:py3:class/decl :className "Loader_Generic " :superClass "abstractLoader.AbstractLoader" :comment "Given a file path" :classType "abs"
""" #+begin_org
*  _[[elisp:(blee:menu-sel:outline:popupMenu)][±]]_ _[[elisp:(blee:menu-sel:navigation:popupMenu)][Ξ]]_ [[elisp:(outline-show-branches+toggle)][|=]] [[elisp:(bx:orgm:indirectBufOther)][|>]] *[[elisp:(blee:ppmm:org-mode-toggle)][|N]]*  Cls-abs    [[elisp:(outline-show-subtree+toggle)][||]] /Loader_Generic /  superClass=abstractLoader.AbstractLoader =Given a file path=  [[elisp:(org-cycle)][| ]]
#+end_org """
class Loader_Generic (abstractLoader.AbstractLoader):
####+END:
    """
**
"""
...
**  _[[elisp:(blee:menu-sel:outline:popupMenu)][±]]_ _[[elisp:(blee:menu-sel:navigation:popupMenu)][Ξ]]_ [[elisp:(outline-show-branches+toggle)][|=]] [[elisp:(bx:orgm:indirectBufOther)][|>]] *[[elisp:(blee:ppmm:org-mode-toggle)][|N]]*  Mtd-T-     [[elisp:(outline-show-subtree+toggle)][||]] /callEntryPoint/   [[elisp:(org-cycle)][| ]]
    ...
    ####+END:
        ...
        """ #+begin_org
*** [[elisp:(org-cycle)][| DocStr| ]] Calls the genericMain function in module with no parameters.
        This function is expected to be the main entry point of the module.
        #+end_org """
        try:
            func = getattr(module, 'genericMain', None)
            if func is None:
                log.debug("module has no genericMain entry point: %s", getattr(module, '__name__', module))
                return
            if not callable(func):
                log.debug("module.genericMain exists but is not callable: %s", getattr(module, '__name__', module))
                return

            # Call the genericMain without arguments per contract
            func(*args, **kwargs)
        except Exception:
            log.exception("Error while invoking genericMain")
...
####+END:
        ...
        """ #+begin_org
*** [[elisp:(org-cycle)][| DocStr| ]] module is an imported python module that must have a function called genericMain.
        ...
        try:
            attr = getattr(module, 'genericMain', None)
...
""" #+begin_org
*  _[[elisp:(blee:menu-sel:outline:popupMenu)][±]]_ _[[elisp:(blee:menu-sel:navigation:popupMenu)][Ξ]]_ [[elisp:(outline-show-branches+toggle)][|=]] [[elisp:(bx:orgm:indirectBufOther)][|>]] *[[elisp:(blee:ppmm:org-mode-toggle)][|N]]*  CmndSvc-   [[elisp:(outline-show-subtree+toggle)][||]] <<loaderType_generic>>  =verify= ro=cli   [[elisp:(org-cycle)][| ]]
#+end_org """
class loaderType_generic(cs.Cmnd):
    cmndParamsMandatory = [ ]
    cmndParamsOptional = [ ]
...
** [[elisp:(org-cycle)][| *CmndDesc:* | ]]
        ...
        loaderGeneric = Loader_Generic()

        loaderType = abstractLoader.loaderTypes.add("generic", loaderGeneric)

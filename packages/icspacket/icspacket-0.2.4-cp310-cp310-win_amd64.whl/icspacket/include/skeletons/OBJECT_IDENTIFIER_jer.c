...
static enum jer_pbd_rval OBJECT_IDENTIFIER__jer_body_decode(
    ...
    if ((size_t)num_arcs > sizeof(s_arcs) / sizeof(s_arcs[0])) {
        ...
        if (ret != num_arcs) return JPBD_SYSTEM_FAILURE; /* assert?.. */
    }
...
asn_dec_rval_t OBJECT_IDENTIFIER_decode_jer(
    const asn_codec_ctx_t *opt_codec_ctx, const asn_TYPE_descriptor_t *td,
    const asn_jer_constraints_t *constraints, void **sptr, const void *buf_ptr,
    size_t size) {
    return jer_decode_primitive(opt_codec_ctx, td, sptr,
                                sizeof(OBJECT_IDENTIFIER_t), buf_ptr, size,
                                OBJECT_IDENTIFIER__jer_body_decode);
}

asn_enc_rval_t OBJECT_IDENTIFIER_encode_jer(

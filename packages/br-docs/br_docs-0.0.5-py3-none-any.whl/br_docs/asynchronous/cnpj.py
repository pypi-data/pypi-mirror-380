...
async def is_masked(cnpj: str) -> bool:
    return "." in cnpj and "/" in cnpj and "-" in cnpj


async def mask(cnpj: str) -> str:
    if await is_masked(cnpj):
        return cnpj
    masked_cnpj = ""
...
async def remove_mask(cnpj: str) -> str:
    if not await is_masked(cnpj):
        return cnpj
    new_cnpj = ""
    for digit in cnpj:
        if digit.isnumeric():
...
async def is_valid(cnpj: str) -> bool:
    cnpj = await remove_mask(cnpj)
    return __is_valid(cnpj)


async def validate(cnpj: str) -> str:
    cnpj = await remove_mask(cnpj)
    if __is_valid(cnpj):
        return cnpj
    raise "Invalid CNPJ"
...
async def generate(masked: bool = False) -> str:
    ...
    if masked:
        return await mask(__generate_verificator_digits(cnpj))

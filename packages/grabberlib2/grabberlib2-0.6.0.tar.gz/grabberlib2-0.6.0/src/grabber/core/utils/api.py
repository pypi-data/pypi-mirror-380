...
async def get_image_stream(
    ...
    """
    response = requests.get(url, headers=headers, stream=True) if headers is not None else requests.get(url, stream=True)
    ...
    async with session.get(url, headers=headers) as resp:
        if resp.status >= 300:
            ...
            try:
                ...
                preview = ""
            raise Exception(
                f"Not able to retrieve {url}: HTTP {resp.status}\n{preview}"
            )

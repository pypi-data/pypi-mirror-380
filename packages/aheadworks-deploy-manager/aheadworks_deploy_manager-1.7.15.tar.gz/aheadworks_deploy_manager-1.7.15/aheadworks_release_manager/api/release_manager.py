    ...
    def build_metapackage_pack(self, bitbucket_workspace, bitbucket_repo_slug, artifacts_dir):
        '''
          ...
          1. with meatapackage composer file:
            - replace in the metapackage 'requre' section module names so they have '-b2b' postfix
          2. with each dependent module composer file:
            - add '-b2b' postfix to the name
            - get 'require' section
            - replace in the 'require' and 'suggests' sections only module names that matches the metapackage 'require' section
            - replace in the 'suggests' section '-hyva' with '-b2b-hyva'  (if there are any) 
    
          for b2b-hyva
          'aheadworks/module-ui-components' is NEVER renamed
          1. with meatapackage composer file:
            - replace in the 'requre' section module names so the postfix '-hyva' becomes '-b2b-hyva'
          2. with each dependent module composer file:
            - replace '-hyva' with '-b2b-hyva' in the name
            - get 'require' section
            - replace in the 'require' and 'suggests' sections only module names that matches the metapackage 'require' section
            - replace in the 'suggests' section '-hyva' postfix with '-b2b-hyva'  (if there are any) 
        ...
        for full_module_name in composer["require"]:
            ...
            # replacing '-hyva' with '-b2b-hyva' in the dependent 'suggests' section
            replacements = {}
            if "suggests" in module_composer:
                for s in module_composer["suggests"]:
                    if self._is_conflicted_module(s):
                        continue
                    if s.startswith("aheadworks/") and s.endswith('-hyva') and not s.endswith('b2b-hyva'):
                        # strip "-hyva" if any
                        new_name = self._strip_end(s, "-hyva")
                        replacements[s] = f"{new_name}-b2b-hyva"

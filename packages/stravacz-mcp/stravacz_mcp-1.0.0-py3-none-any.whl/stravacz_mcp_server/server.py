"""
Hlavní MCP server pro strava.cz s stdio transportem

Implementuje Model Context Protocol server pro česky systém závodního stravování.
"""
...
server = FastMCP("Stravacz MCP Server")
...
async def run_server(username: str, password: str, canteen_number: str):
    """
    Spuštění MCP serveru s daným uživatelem
    ...
    try:
        ...
        logger.info("Spouštění MCP serveru...")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Stravacz MCP Server - Model Context Protocol server pro strava.cz"
    )
...
def main():
    ...
    logger.info("=== Stravacz MCP Server ===")
    ...
    try:
        # Spuštění serveru
        asyncio.run(run_server(args.user, args.password, args.canteen_number))
    except KeyboardInterrupt:
        logger.info("\nServer zastaven")
        sys.exit(0)
    except Exception as e:

...
class HexMujocoClientBase(HexZMQClientBase):
    ...
    def get_states(self):
        hdr, states = self.request({
            "cmd":
            "get_states",
            "args": (1 + self._states_seq) % MAX_SEQ_NUM,
        })
        try:
            cmd = hdr["cmd"]
    ...
    def set_cmds(self, cmds: np.ndarray) -> bool:
        ...
        try:
            cmd = states_hdr["cmd"]
            if cmd == "set_cmds_ok":
                self._cmds_seq = (self._cmds_seq + 1) % MAX_SEQ_NUM
                return True
            else:
                return False
        except KeyError:
    ...
    def _process_frame(self, depth_flag: bool):
        hdr, img = self.request({
            "cmd":
            f"get_{'depth' if depth_flag else 'rgb'}",
...
class HexMujocoServerBase(HexZMQServerBase):
    ...
    def _set_cmds(self, recv_hdr: dict, recv_buf: np.ndarray):
        ...
        if seq is not None and seq > self._cmds_seq:
            delta = (seq - self._cmds_seq) % MAX_SEQ_NUM
            if delta >= 0 and delta < 5_000:
                self._cmds_seq = seq

...
class Voice:
    ...
    def __init__(self, audio_format="wav", device_name=None, silence_timeout=3.0):
        if sf is None:
    ...
    def get_prompt(self):
        ...
        if (
            ...
            and self.last_sound_time is not None
            and time.time() - self.last_sound_time >= self.silence_timeout
        ):
            self.stop_recording = True
            raise KeyboardInterrupt("Silence detected - stopping recording")
    ...
    def raw_record_and_transcribe(self, history, language):
        ...
        try:
            with self.sd.InputStream(
                ...
                try:
                    ...
                    # Check if this was auto-stop from silence detection
                    if not self.stop_recording:
                        # User pressed Ctrl+C, re-raise
                        raise
                    # Otherwise, silence detected - continue to transcription
        ...
        if use_audio_format != "wav":
            try:
                # Suppress ffmpeg logs during audio conversion
                import sys

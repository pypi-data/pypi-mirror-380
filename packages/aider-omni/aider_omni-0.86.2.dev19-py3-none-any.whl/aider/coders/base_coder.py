...
class Coder:
        ...
        try:
            ...
            while True:
                try:
                    ...
                    # Auto-trigger voice input in voice conversation mode
                    if self.io.voice_conversation_mode and not self.io.placeholder:
                        # Wait for TTS to finish speaking before listening
                        if self.io.tts:
                            self.io.tts.wait_for_speech_completion()

                        # Retry voice input until we get valid text
                        user_message = None
                        max_retries = 1
                        retry_count = 0
                        while not user_message and retry_count < max_retries:
                            voice_input = self.commands.cmd_voice("")
                        ...
                        if not user_message:
                            # After max retries, fall back to text input
    ...
    def show_send_output_stream(self, completion):
        ...
        streaming_tts = None
        if hasattr(self.io, "streaming_tts") and self.io.streaming_tts:
            streaming_tts = self.io.streaming_tts
            streaming_tts.start()
        ...
        for chunk in completion:
            ...
            # Output handling: pretty mode uses mdstream, plain mode uses stdout
            # Only output if we have new content
            if text:
                if self.show_pretty():
                    self.live_incremental_response(False)
                else:
                    # Apply reasoning tag formatting
                    formatted_text = replace_reasoning_tags(text, self.reasoning_tag_name)
                    try:
                        sys.stdout.write(formatted_text)
                    except UnicodeEncodeError:
                        # Safely encode and decode the text
                        safe_text = formatted_text.encode(
                            sys.stdout.encoding, errors="backslashreplace"
                        ).decode(sys.stdout.encoding)
                        sys.stdout.write(safe_text)
                    sys.stdout.flush()

                # Stream TTS after output to avoid conflicts
                if streaming_tts:
                    streaming_tts.add_chunk(text)

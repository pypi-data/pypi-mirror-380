...
class InputOutput:
    ...
    def __init__(
        ...
        sound_effects=True,
        streaming_tts=False,
        streaming_tts_delay=0.0,
        streaming_tts_chunks=1,
    ):
        ...
        self.voice_conversation_mode = voice_conversation
        ...
        self.sfx = SoundEffects(enabled=sound_effects)

        # Initialize TTS
        self.tts = None
        if voice_output or voice_conversation:
            # Voice conversation mode overrides some settings
            if voice_conversation:
                if not tts_preset:
                    tts_preset = "fast"
                if not tts_filter:
                    tts_filter = "voice_conversation"
            ...
            try:
                self.tts = TextToSpeech(
                    ...
                    verbose=False,
                    preset=tts_preset,
                    speaking_filter=tts_filter,
                )

                if voice_conversation:
                    self.tts.speak(
                        "Voice conversation mode ready. Use /voice to speak your commands.",
                        priority="high",
                        message_type="confirmation",
                    )
            except Exception as e:
                self.tool_warning(f"Failed to initialize voice output: {e}")

        # Initialize streaming TTS if requested
        self.streaming_tts = None
        if streaming_tts and self.tts:
            from aider.tts import StreamingTTS
            self.streaming_tts = StreamingTTS(
                self.tts, delay=streaming_tts_delay, chunk_sentences=streaming_tts_chunks
            )
...
{prefix} {hist}"""
    ...
    def confirm_ask(
        ...
        if self.tts:
            self.tts.speak(
                question, priority="high", summarize=True, max_length=150, message_type="confirmation"
            )
    ...
    def assistant_output(self, message, pretty=None):
        ...
        self.sfx.play("message")

        # Speak assistant output if TTS is enabled (summarize long responses)
        if self.tts:
            # For long messages, summarize before speaking
            if len(message) > 300:
                self.tts.speak(
                    message,
                    priority="normal",
                ...
                self.tts.speak(message, priority="normal", summarize=False, message_type="assistant")

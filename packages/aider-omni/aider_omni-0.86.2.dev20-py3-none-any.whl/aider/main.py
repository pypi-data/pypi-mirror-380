...
def check_arg_is_provided(argv, arg_name, example):
    if arg_name in argv:
        i = argv.index(arg_name)
        if i + 1 == len(argv) or argv[i + 1].startswith("-"):
            sys.stderr.write(f"aider: error: argument {arg_name}: expected one argument.\n")
            sys.stderr.write(f"For example: {arg_name} {example}\n")
            return True
    return False
...
def main(argv=None, input=None, output=None, force_git_root=None, return_coder=False):
    ...
    if check_arg_is_provided(argv, "--model", "gpt-4o"):
        return 1
    if check_arg_is_provided(argv, "--alias", "my-model:gpt-4"):
        return 1
    if check_arg_is_provided(argv, "--model-settings-file", ".aider.model.settings.yml"):
        return 1
    if check_arg_is_provided(argv, "--model-metadata-file", ".aider.model.metadata.json"):
        return 1
    if check_arg_is_provided(argv, "--edit-format", "diff"):
        return 1
    if check_arg_is_provided(argv, "--weak-model", "gpt-3.5-turbo"):
        return 1
    if check_arg_is_provided(argv, "--editor-model", "gpt-4"):
        return 1
    if check_arg_is_provided(argv, "--message", "'Hello, world!'"):
        return 1
    if check_arg_is_provided(argv, "--file", "my_file.py"):
        return 1
    ...
    def get_io(pretty):
        return InputOutput(
            ...
            tts_filter=args.tts_filter,
            voice_conversation=args.voice_conversation,
            sound_effects=args.sound_effects,
            streaming_tts=args.streaming_tts,
            streaming_tts_delay=args.streaming_tts_delay,
            streaming_tts_chunks=args.streaming_tts_chunks,
        )
    ...
    commands = Commands(
        ...
        voice_silence_timeout=args.voice_silence_timeout,

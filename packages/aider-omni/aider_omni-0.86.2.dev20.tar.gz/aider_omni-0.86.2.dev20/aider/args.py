...
def get_parser(default_config_files, git_root):
    ...
    group.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Enable/disable sound effects for events (default: True)",
    )
    ...
    group.add_argument(
        ...
        type=float,
        default=3.0,
        help="Seconds of silence before auto-stopping voice recording (default: 3.0)",
    )
    group.add_argument(
        "--voice-output",
        action="store_true",
        help="Enable text-to-speech voice output (default: False)",
        default=False,
    )
    group.add_argument(
        "--no-voice-output",
        action="store_false",
        dest="voice_output",
        help="Disable text-to-speech voice output",
    )
    group.add_argument(
        "--tts-engine",
        metavar="TTS_ENGINE",
        default="gtts",
        choices=["gtts", "openai"],
        help="Text-to-speech engine (default: gtts, options: gtts, openai)",
    )
    group.add_argument(
        "--tts-rate",
        metavar="TTS_RATE",
    ...
    group.add_argument(
        ...
        metavar="TTS_VOICE",
        default=None,
        help="Voice ID to use for text-to-speech (engine-specific)",
    )
    group.add_argument(
        "--tts-preset",
        metavar="TTS_PRESET",
        default=None,
        choices=["fast", "normal", "slow", "natural", "premium"],
        help=(
            "Voice preset (overrides engine/rate/voice): "
            "fast (250 WPM), normal (175 WPM), slow (125 WPM), "
            "natural (gTTS), premium (OpenAI)"
        ),
    )
    group.add_argument(
        "--tts-filter",
        metavar="TTS_FILTER",
        default=None,
        choices=["all", "minimal", "important", "assistant_only"],
        help=(
            "Speaking filter preset: "
            "all (speak everything), "
            "minimal (errors+confirmations only), "
            "important (errors+confirmations+warnings), "
            "assistant_only (AI responses only)"
        ),
    )
    group.add_argument(
        "--streaming-tts",
        ...
        default=False,
        help=(
            "Enable streaming text-to-speech during AI response generation "
            "(speaks sentences in real-time as they're being generated)"
        ),
    )
    group.add_argument(
        "--streaming-tts-delay",
        ...
        type=float,
        default=0.0,
        help="Delay in seconds between streaming TTS sentences (default: 0.0)",
    )
    group.add_argument(
        ...
        type=int,
        default=1,
        help="Number of sentences to accumulate before speaking (default: 1)",
    )
    group.add_argument(
        "--voice-conversation",
        action="store_true",
        default=False,
        help=(
            "Enable full voice conversation mode (combines voice input + output, "
            "auto-submits transcribed input, optimized for hands-free interaction)"
        ),
    )

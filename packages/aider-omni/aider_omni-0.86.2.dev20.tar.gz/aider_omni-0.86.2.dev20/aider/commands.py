...
class Commands:
    ...
    def __init__(
        ...
        self.voice_silence_timeout = voice_silence_timeout
        ...
        for matched_file in sorted(all_matched_files):
            ...
            if abs_file_path in self.coder.abs_fnames:
                ...
                if content is None:
                    ...
                    # Voice feedback for file addition
                    if self.io.tts:
                        self.io.tts.speak(
                            f"Added {fname}", priority="normal", message_type="file_op"
                        )
        ...
        for word in filenames:
            ...
            for matched_file in matched_files:
                ...
                if abs_fname in self.coder.abs_fnames:
                    ...
                    # Voice feedback for file removal
                    if self.io.tts:
                        self.io.tts.speak(
                            f"Removed {matched_file}",
                            priority="normal",
                            message_type="file_op",
                        )
    ...
    def cmd_test(self, args):
        ...
        errors = args()
        if not errors:
            self.io.sfx.play("success")
            return

        self.io.sfx.play("failure")
...
"""
    ...
    def cmd_voice(self, args):
        ...
        if not self.voice:
            ...
            try:
                self.voice = voice.Voice(
                    ...
                    silence_timeout=self.voice_silence_timeout,
                )
        ...
        # Voice feedback: starting to listen
        if self.io.tts and self.io.tts.enabled:
            self.io.tts.speak("Listening", priority="high", message_type="confirmation")
            # Wait for "Listening" to finish speaking before recording
            self.io.tts.wait_for_speech_completion()
        ...
        if text:
            ...
            if text_lower in ["listening", "listen", ""]:
                # Ignore false positive, return None to retry
                return None

            # Voice conversation mode: auto-submit transcribed text
            if self.io.voice_conversation_mode:
                # Don't speak "I heard" - just return the text directly
                # The user already knows what they said
                return text
            else:
                # Standard mode: just set placeholder
                self.io.placeholder = text

    def cmd_speak(self, args):
        "Toggle text-to-speech voice output on/off"

        if not self.io.tts:
            # Initialize TTS if not already initialized
            try:
                from aider.tts import TextToSpeech

                self.io.tts = TextToSpeech(
                    engine="pyttsx3",
                    enabled=True,
                    verbose=False,
                )
                self.io.tool_output("Voice output enabled.")
                if self.io.tts:
                    self.io.tts.speak("Voice output is now enabled.", priority="high")
            except Exception as e:
                self.io.tool_error(f"Failed to initialize voice output: {e}")
                self.io.tool_output("Install pyttsx3 with: pip install pyttsx3")
        else:
            # Toggle TTS on/off
            if self.io.tts.enabled:
                self.io.tts.speak("Voice output is now disabled.", priority="high")
                self.io.tts.enabled = False
                self.io.tool_output("Voice output disabled.")
            else:
                self.io.tts.enabled = True
                self.io.tool_output("Voice output enabled.")
                self.io.tts.speak("Voice output is now enabled.", priority="high")

    def cmd_voice_convo(self, args):
        "Toggle voice conversation mode (full hands-free interaction)"

        # Toggle voice conversation mode
        self.io.voice_conversation_mode = not self.io.voice_conversation_mode

        if self.io.voice_conversation_mode:
            # Enable voice conversation mode
            if not self.io.tts or not self.io.tts.enabled:
                # Initialize TTS if not already initialized
                try:
                    from aider.tts import SpeakingFilter, TextToSpeech

                    self.io.tts = TextToSpeech(
                        engine="pyttsx3",
                        enabled=True,
                        preset="fast",
                        speaking_filter=SpeakingFilter("voice_conversation"),
                        verbose=False,
                    )
                except Exception as e:
                    self.io.tool_error(f"Failed to initialize voice output: {e}")
                    self.io.tool_output("Install pyttsx3 with: pip install pyttsx3")
                    self.io.voice_conversation_mode = False
                    return

            self.io.tool_output(
                "Voice conversation mode enabled. Use /voice to speak, responses will be spoken automatically."
            )
            if self.io.tts:
                self.io.tts.speak(
                    "Voice conversation mode activated. Use /voice to speak your commands.",
                    priority="high",
            ...
            if self.io.tts and self.io.tts.enabled:
                self.io.tts.speak(
                    "Voice conversation mode deactivated.",
                    priority="high",
                    message_type="confirmation",
                )
            self.io.tool_output(
                "Voice conversation mode disabled. Voice input will set text without auto-submitting."
            )

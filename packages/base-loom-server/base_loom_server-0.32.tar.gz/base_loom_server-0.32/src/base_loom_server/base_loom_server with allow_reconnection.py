...
class BaseLoomServer:
    ...
    Args:
        ...
        allow_reconnection: Reconnect to the loom if the client connection is dropped?
            Specify True for physical USB ports, but if you are connecting a virual USB port,
            then only specify True if you have verified it works.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if reset_db:
            self.log.info(f"{self}: resetting database {self.db_path} by request")
            self.reset_database()
        try:
            self.pattern_db = PatternDatabase(self.db_path)
        except Exception as e:
            self.log.warning(f"{self}: resetting database {self.db_path} because open failed: {e!r}")
            self.reset_database()
    ...
    async def connect_to_loom(self) -> None:
        ...
        If already connected to loom, and allow_reconnection is true,
        disconnect first, since connecting again may indicate that something is wrong.
        """
        if self.loom_connected:
            if self.allow_reconnection:
                await self.disconnect_from_loom()
            else:
                # We can only connect to the loom once,
                # but we can still try restarting the read loom task, if it has died.
                if self.read_loom_task.done():
    ...
    async def read_client_loop(self) -> None:
        ...
        try:
            ...
            if not self.client_connected and self.allow_reconnection:
                await self.disconnect_from_loom()

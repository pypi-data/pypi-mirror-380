...
class EnableCls:
	...
	def set(self, register_bit: bool, bit=repcap.Bit.Default) -> None:
		...
		Snippet: driver.status.operation.bit.enable.set(register_bit = False, bit = repcap.Bit.Default) \n
	...
	def get(self, bit=repcap.Bit.Default) -> bool:
		"""STATus:OPERation:BIT<bitno>:ENABle \n
		Snippet: value: bool = driver.status.operation.bit.enable.get(bit = repcap.Bit.Default) \n
		No command help available \n
			:param bit: optional repeated capability selector. Default value: Nr8 (settable in the interface 'Bit')

    ...
    def find_sections_containing(
        ...
        for sec in secs:
            ...
            for s in [sec] + sec.children:
                ...
                # as start and end only work with content, we need to adjust the index
                minus_index = 0
                if by in ("title", "all"):
                    haystack += s.title + "\n"
                    title_end_index = len(s.title)
                    minus_index = len(s.title)  # for title and the newline
                if by in ("content", "all"):
                    haystack += s.section_content + "\n"
                    minus_index += 1  # for the newline
                ...
                for match in matches:
                    ...
                    for surrounding in srange:
                        ...
                        snippets.append(
                            SectionSnippet(
                                start_index=snippet_start - minus_index,
                                end_index=snippet_end - minus_index,
                                snippet=snippet,
                                found_in=found_in,
                            )
                        )

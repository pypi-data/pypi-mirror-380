...
from utils.blueprints.blueprint_operations import (
    ...
    post_blueprint_from_json as post_blueprint_from_json_impl,
    build_blueprint_from_definition as build_blueprint_from_definition_impl
)
...
def register_blueprint_tools(mcp: FastMCP):
    ...
    #     folder_path: str = "/Game/GameWave/Blueprints"
    # ) -> Dict[str, Any]:
    #     """
    #     Builds a complete Blueprint asset from a comprehensive JSON definition.

    #     This high-level tool orchestrates calls to other specialized tools to construct
    #     the component hierarchy, variables, defaults, and event graph logic. It is the
    #     recommended way to generate a full Blueprint from a declarative format.

    #     Args:
    #         blueprint_name: The name of the Blueprint asset to create (e.g., "BP_MouseOverActor").
    #         parent_class: The parent class for the Blueprint (e.g., "Actor", "Pawn").
    #         definition: The dictionary containing the full Blueprint structure, including
    #                     keys like "scene", "details", "variables", "graphs", etc.
    #         folder_path: The content folder path where the Blueprint will be created.
    #     """
    #     # The tool is just a clean wrapper around the real implementation.
    #     return build_blueprint_from_definition_impl(
    #         ctx,
    #         blueprint_name,
    #         parent_class,
    #         definition,
    ...
    # ) -> Dict[str, Any]:
    #     """
    #     Creates a new Blueprint from a full JSON structure, or overwrites an existing one.
    #     This is a high-performance, single-call tool that creates the asset, adds variables,
    #     builds the component scene, and reconstructs the event graph.

    #     Args:
    #         blueprint_path: The full content browser path for the Blueprint (e.g., "/Game/Blueprints/BP_MyNewActor").
    #         parent_class: The parent class for the Blueprint if it needs to be created (e.g., "Actor", "Pawn").
    #         blueprint_data: A dictionary representing the entire Blueprint structure.
    #     """
    #     return post_blueprint_from_json_impl(ctx, blueprint_path, parent_class, blueprint_data)

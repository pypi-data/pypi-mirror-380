...
def register_project_tools(mcp: FastMCP):
    ...
    def get_project_name(
        ...
        """
        Gets the name of the currently open Unreal Engine project.

        This tool retrieves the project name by finding the .uproject file 
        and extracting its name without the file extension.
        
        Args:
            
        Returns:
            A string containing the name of the current Unreal Engine project.
    ...
    # Get the full path to the .uproject file. This is the most reliable method.
    project_file_path = unreal.Paths.get_project_file_path()

    # Extract the base filename (e.g., "MyProject.uproject") from the full path.
    project_filename = os.path.basename(project_file_path)

    # Split the filename into the name and the extension, and take the name part.
    project_name, _ = os.path.splitext(project_filename)

    # Print the final project name with a unique identifier for easy parsing.
    print(f"ProjectName>>>{project_name}")
        '''
        
        # This function executes the python code within the Unreal Engine editor.
        ...
        if 'status' in output and output['status'] == 'success':
            ...
            if 'success' in result and result['success'] == True:
                # Split the output log into individual lines
                lines = result['result'].splitlines()

        # Define a pattern to find the specific output line containing the project name.
        pattern = r"ProjectName>>>"
        
        project_name = ""
        for line in lines:
            if re.search(pattern, line):
                # Extract the project name from the matched line
                project_name = line.replace('ProjectName>>>', '').strip()
                break  # Stop after finding the first match

        if not project_name:
            raise Exception("Could not determine project name from the Unreal Engine script output.", output)

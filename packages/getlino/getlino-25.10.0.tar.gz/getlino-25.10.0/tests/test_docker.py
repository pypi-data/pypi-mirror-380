...
if False:  # 20220917 TODO: convert remaining tests to gitlab ci
    ...
    class DebianDockerTest(DockerTestMixin, TestCase):
        ...
        def test_production_server(self):
            ...
            self.run_docker_command('sudo mkdir -p /usr/local/lino/venvs')
            self.run_docker_command(
                'cd /usr/local/lino/venvs && sudo chown root:www-data . && sudo chmod g+ws . && virtualenv -p python3 master'
            )
            mastercmd = ". /usr/local/lino/venvs/main/bin/activate && {}"

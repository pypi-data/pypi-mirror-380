...
from .utils import WEB_SERVERS, resolve_web_server, USING_GITLAB_SERVICES
...
def add(spec, default, help, type=None, root_only=False):
    """
    :param root_only: if user is not root, do not ask the user for the choice.
...
def default_shared_env():
    ...
    # return os.environ.get('VIRTUAL_ENV', '/usr/local/lino/venvs')
...
add('--appy/--no-appy',
    ...
    root_only=True)
add('--journal/--no-journal',
    ifroot,
    "Whether Lino sites should log to systemd journal",
    root_only=True)
...
def configure(ctx, batch, sites_base, local_prefix, shared_env, repos_base,
              ...
              clone, webdav, backups_base, usergroup, supervisor_dir,
              env_link, repos_link, appy, journal, weasyprint, redis, devtools, server_domain,
    ...
    if DEFAULTSECTION.getboolean('redis') and not USING_GITLAB_SERVICES:
    ...
    if db_engine.service and not USING_GITLAB_SERVICES:
    ...
    if DEFAULTSECTION.get('db_user') and not USING_GITLAB_SERVICES:

...
async def update(nvd_api_key=None, config=None, create_db=True, stop_update=[]):
    ...
    # as_completed vrací tasky postupně, jak dokončují
    for f in tqdm_asyncio.as_completed(
            tasks,
            total=len(tasks),
            desc="Downloading CPEs",
            file=sys.stdout
        ):
        result = await f
        cpe_results.extend(result[0])
        deprecated_results.extend(result[1])

        # aktualizace finished/pending
        finished_tasks.add(f)
        pending_tasks = set(tasks) - finished_tasks

        # ukončení, pokud se něco pokazilo nebo stop_update
        if not UPDATE_SUCCESS or stop_update:
            break

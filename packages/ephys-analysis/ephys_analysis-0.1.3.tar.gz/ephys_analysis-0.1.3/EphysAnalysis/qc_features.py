...
def measure_time_constant(v, i, t):
    up_idx, down_idx = get_square_pulse_idx(v)
    end = down_idx - 1

    dv = np.mean(v[up_idx+1:end]) - np.mean(v[:up_idx])
    start = np.argmax(i) if dv > 0 else np.argmin(i)

    
    guess = (i[start] - i[end], 1000., i[end])
    t_window = (t[start:end] - t[start]).astype(np.float64)
    v_window = v[start:end].astype(np.float64)

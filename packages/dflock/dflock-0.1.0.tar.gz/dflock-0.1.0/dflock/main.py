...
class AppCommand(typing.Protocol, typing.Generic[T]):
    def __call__(self, app: "App", *args) -> T:
        ...


def local_and_upstream_exist(f: AppCommand):
    @functools.wraps(f)
...
def no_hot_branch(f: AppCommand) -> typing.Callable:
    @functools.wraps(f)
    def wrapper(app, *args, **kwargs):
        if utils.get_current_branch() in app.get_hot_branches():
            raise click.ClickException(
                "switch to a branch not managed by dflock before continuing."
            )
        return f(app, *args, **kwargs)
...
def valid_local_commits(f: AppCommand[T]) -> AppCommand[T]:
    @functools.wraps(f)
    @local_and_upstream_exist
    def wrapper(app: "App", *args, **kwargs) -> T:

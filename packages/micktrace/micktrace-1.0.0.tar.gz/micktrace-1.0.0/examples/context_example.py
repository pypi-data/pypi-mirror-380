...
def simulate_user_service():
    ...
    for user in users:
        ...
        if user["role"] == "admin":
            user_logger.info("Admin privileges granted", 
                           permissions=["read", "write", "delete", "admin"])
        elif user["role"] == "moderator":
            user_logger.info("Moderator privileges granted",
...
def simulate_order_processing():
    ...
    for order in orders:
        ...
        # Complete order
        request_logger.info("Order processing completed",
                          status="confirmed",
...
def simulate_error_scenarios():
    ...
    for i, scenario in enumerate(error_scenarios):
        ...
        if scenario["severity"] == "warning":
            scenario_logger.warning("Service degradation detected",
                                   impact="increased_latency",
                                   action="monitoring")
        elif scenario["severity"] == "error":
            scenario_logger.error("Transaction failed",
                                 reason=scenario["type"],
                                 customer_notified=True)
        else:  # critical
            scenario_logger.critical("Service outage detected",
...
async def simulate_async_operations():
    ...
    async_logger.info("All async tasks completed",

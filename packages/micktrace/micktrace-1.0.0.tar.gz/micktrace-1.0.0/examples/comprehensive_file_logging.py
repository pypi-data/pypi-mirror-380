...
class ComprehensiveLoggingDemo:
    ...
    def __init__(self):
        ...
        self._log_with_count("INFO", "Service initialized successfully", {
    ...
    def _log_with_count(self, level: str, message: str, data: Optional[Dict[str, Any]] = None):
    ...
    def _generate_sample_users(self) -> List[User]:
        ...
        self._log_with_count("DEBUG", "Sample users generated", {
    ...
    def simulate_user_authentication(self):
        ...
        self._log_with_count("INFO", "Starting authentication simulation", {
        ...
        for user in self.users:
            ...
            if random.random() > 0.2:  # 80% success rate
                self._log_with_count("INFO", f"User login successful", {
                    **user.to_dict(),
                ...
                self._log_with_count("DEBUG", "User session created", {
                    "user_id": user.id,
                    "session_id": session_id,
                    "session_timeout": 3600,
                    "permissions": ["read", "write"] if user.role != "user" else ["read"]
                })
            else:
                # Simulate failed login
                self._log_with_count("WARNING", f"User login failed", {
                    "user_id": user.id,
                    "email": user.email,
    ...
    def simulate_transaction_processing(self):
        ...
        self._log_with_count("INFO", "Starting transaction processing simulation", {
        ...
        for i in range(15):  # Process 15 transactions
            ...
            self._log_with_count("INFO", "Transaction initiated", {
                **transaction.to_dict(),
                **user.to_dict(),
            ...
            self._log_with_count("DEBUG", "Transaction validation started", {
                "transaction_id": transaction.id,
                "validation_checks": ["amount_limit", "account_balance", "fraud_detection"]
            })
            ...
            if outcome == "success":
                ...
                self._log_with_count("INFO", "Transaction completed successfully", {
                    **transaction.to_dict(),
                ...
                self._log_with_count("WARNING", "Transaction failed - insufficient funds", {
                    **transaction.to_dict(),
                ...
                self._log_with_count("ERROR", "Transaction blocked - fraud detected", {
                    **transaction.to_dict(),
                    "fraud_score": random.randint(80, 100),
                    "fraud_indicators": ["unusual_location", "high_amount", "velocity_check"]
                })
            else:  # network_error
                transaction.status = "error"
                self._log_with_count("ERROR", "Transaction failed - network error", {
                    **transaction.to_dict(),
                    "error_code": "NET_001",
    ...
    def simulate_system_monitoring(self):
        ...
        self._log_with_count("INFO", "Starting system monitoring simulation", {
        ...
        for db in ["primary_db", "replica_db", "cache_db"]:
            ...
            if response_time < 30:
                self._log_with_count("DEBUG", f"Database health check passed", {
                    "database": db,
                ...
                self._log_with_count("WARNING", f"Database health check slow", {
                    "database": db,
                    "response_time_ms": round(response_time, 2),
                    "threshold_ms": 30,
        ...
        for endpoint in endpoints:
            status_code = random.choices([200, 404, 500, 503], weights=[0.8, 0.1, 0.05, 0.05])[0]
            response_time = random.uniform(50, 300)
            
            if status_code == 200:
                self._log_with_count("DEBUG", f"API endpoint healthy", {
                    "endpoint": endpoint,
                ...
                self._log_with_count(level, f"API endpoint issue detected", {
                    "endpoint": endpoint,
                    "status_code": status_code,
                    "response_time_ms": round(response_time, 2),
        ...
        self._log_with_count("INFO", "System resource utilization", {
        ...
        # Alert if high usage
        if cpu_usage > 80:
            self._log_with_count("WARNING", "High CPU usage detected", {
                "cpu_usage_percent": round(cpu_usage, 2),
        ...
        if memory_usage > 80:
            self._log_with_count("WARNING", "High memory usage detected", {
                "memory_usage_percent": round(memory_usage, 2),
                "threshold_percent": 80,
    ...
    def simulate_error_scenarios(self):
        ...
        self._log_with_count("INFO", "Starting error scenario simulation", {
        ...
        for error in critical_errors[:2]:  # Only simulate 2 critical errors
            self._log_with_count("CRITICAL", f"Critical system error: {error}", {
        ...
        for error in app_errors:
            self._log_with_count("ERROR", f"Application error: {error}", {
    ...
    def generate_final_summary(self):
        ...
        self._log_with_count("INFO", "Generating comprehensive demo summary", {
            "simulation_type": "summary"
        })
        
        # Transaction summary
        completed_transactions = [t for t in self.transactions if t.status == "completed"]
        failed_transactions = [t for t in self.transactions if t.status in ["failed", "blocked", "error"]]
        
        total_amount = sum(t.amount for t in completed_transactions)
        
        self._log_with_count("INFO", "Transaction processing summary", {
            ...
            "average_transaction_amount": round(total_amount / len(completed_transactions) if completed_transactions else 0, 2),
            "success_rate_percent": round((len(completed_transactions) / len(self.transactions)) * 100, 2) if self.transactions else 0
        })
        
        # User activity summary
        self._log_with_count("INFO", "User activity summary", {
        ...
        # Final system status
        self._log_with_count("INFO", "Comprehensive demo completed successfully", {
    ...
    def run_comprehensive_demo(self):
        ...
        self._log_with_count("INFO", "Starting comprehensive logging demonstration", {
        ...
        while self.log_count < TARGET_LOG_COUNT - 5:
            self._log_with_count("DEBUG", f"Additional debug log entry", {
...
def main():
    ...
    if LOG_FILE.exists():
        ...
        print(f"   - Average bytes per line: {file_size // line_count if line_count > 0 else 0}")
    else:

...
async def async_database_operation(operation_id: int):
    ...
    logger = micktrace.get_logger("database").bind(
        operation_id=f"DB-{operation_id:03d}",
    ...
    try:
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                failed_operations += 1
                request_logger.warning(f"Operation {i+1} failed",
                                     operation_index=i+1,
                                     error_type=type(result).__name__)
            else:
                successful_operations += 1
        
        request_logger.info("User request completed",
        ...
        request_logger.error("Request processing failed",
...
async def simulate_concurrent_requests():
    ...
    main_logger.info("Starting concurrent request processing",
    ...
    main_logger.info("Concurrent processing completed",
                    total_requests=len(tasks),
...
async def simulate_async_batch_processing():
    ...
    batch_logger = micktrace.get_logger("batch_processor").bind(
        component="async_batch_processor",
    ...
    async def process_batch_item(item_id: int, batch_id: str):
        ...
        item_logger.debug("Item processed",
    ...
    for batch_num, batch_size in enumerate(batch_sizes, 1):
        ...
        tasks = [
            process_batch_item(item_id, batch_id)

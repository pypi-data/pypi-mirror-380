...
class SmartBuffer:
    ...
    def _should_compress(self, partition: BufferPartition) -> bool:
        ...
        return (
            len(partition.records) >= self.compression_threshold and
            partition.compressed_data is None
        )
    ...
    def add_record(self, record: LogRecord, partition_id: str = None) -> None:
        ...
        with partition.lock:
            ...
            partition.metrics.avg_record_size = (
                ...
                partition.metrics.avg_record_size * (partition.metrics.total_records - 1)

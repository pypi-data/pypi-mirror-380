...
class SamplingRule:
    ...
    condition: Optional[Callable[[LogRecord], bool]] = None  # Optional condition to apply rule
...
class AdaptiveSampler:
    ...
    def get_sampling_rate(self, base_rate: float, min_rate: float, max_rate: float) -> float:
    ...
    def should_sample(self, record: LogRecord) -> bool:
        ...
        if matched_rule.adaptive:
            ...
            rate = sampler.get_sampling_rate(
                rate,
                matched_rule.min_rate,
                matched_rule.max_rate
            )

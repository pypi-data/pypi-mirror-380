...
async def async_performance_timer(func):
    ...
    async def wrapper(*args, **kwargs):
        ...
        try:
            ...
            duration = time.time() - start_time
            logger = micktrace.get_logger(func.__module__)
            logger.info(f"Async function completed: {func.__name__}",
                       function=func.__name__,
                       duration_ms=round(duration * 1000, 2),
                       success=True)
            return result
            ...
            duration = time.time() - start_time
            logger = micktrace.get_logger(func.__module__)
            logger.error(f"Async function failed: {func.__name__}",
                        function=func.__name__,
                        duration_ms=round(duration * 1000, 2),
                        error_type=type(e).__name__,
                        success=False)
...
def simulate_batch_processing():
    ...
    batch_logger.info("All batches completed",
...
def simulate_database_performance():
    ...
    for query, table in queries:
        ...
        if query_duration > 0.05:
            slow_queries += 1
            query_logger.warning("Slow query detected",
                               duration_ms=round(query_duration * 1000, 2),
                               threshold_ms=50,
                               needs_optimization=True)
        else:
            query_logger.info("Query completed",
                             duration_ms=round(query_duration * 1000, 2),
                             result_summary=result)
    
    # Log database performance summary
    db_logger.info("Database session completed",
...
async def simulate_async_performance():
    ...
    async_logger.info("Starting async task batch",
    ...
    async_logger.info("Async task batch completed",
                     total_tasks=len(tasks),
...
def simulate_error_performance():
    ...
    def unreliable_operation(operation_id: int, failure_rate: float = 0.3):
        ...
        duration = time.time() - start_time
        error_logger.info("Operation succeeded",
                         operation_id=operation_id,
                         duration_ms=round(duration * 1000, 2))
    ...
    for failure_rate in failure_rates:
        ...
        error_logger.info("Testing failure rate",
                         failure_rate=failure_rate,
        ...
        error_logger.info("Failure rate test completed",

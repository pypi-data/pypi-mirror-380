...
class BMP3XX_I2C(BMP3XX):
    ...
    '''
    def __init__(self, iic_addr=0x77, bus_num=0):
        self._addr = iic_addr
    ...
    def _error_handling(self, lens=0):
        result = [0] * lens
        self._connect += 1
        print("Bmp3xx iic communication faild, please wait")
        time.sleep(0.5)  
        return result
    ...
    def _write_reg(self, reg, data):
        ...
        while True:
            ...
            if self._connect > self.ERROR_COUNT:
                raise ValueError("Please check the Bmp3xx connection or Reconnection sensor!!!")
    '''
      @brief read the data from the register
      @param reg register address
             length read data length
    ...
    def _read_reg(self, reg, length):
        ...
        while True:
            try:
                result = self.i2c.readfrom_mem(self._addr, reg, length)
                return result
            except:

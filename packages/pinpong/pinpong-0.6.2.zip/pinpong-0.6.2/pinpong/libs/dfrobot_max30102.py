...
class DFRobot_BloodOxygen_S(DFRobot_BloodOxygen):
  ...
  def _error_handling(self, lens=0):
    result = [0] * lens
    self._connect += 1
    print("max30102 iic communication faild, please wait")
    time.sleep(0.5)
    return result
  ...
  def write_reg(self, reg_addr, data_buf):
    ...
    while True:
      ...
      if self._connect > self.ERROR_COUNT:
        raise ValueError("Please check the aht20 connection or Reconnection sensor!!!")
  ...
  def read_reg(self, reg_addr ,length):
    ...
    while True:
      try:
        result = self._i2c.readfrom_mem(self.__addr ,reg_addr , length)
        return result
      except:

...
class DFRobot_MqueenPlus(MqueenPlus):
    ...
    def _error_handling(self, lens=0):
        result = [0] * lens
        self._connect += 1
        print("maqueen iic communication faild, please wait")
        time.sleep(0.5)
        return result
    ...
    def write_cmd(self, data):
        ...
        while True:
            ...
            if self._connect > self.ERROR_COUNT:
                raise ValueError("Please check the maqueen connection or Reconnection sensor!!!")
                
    def read_cmd(self, reg, len):
        self._connect = 0
        while True:
            try:
                result = self._i2c.readfrom_mem(self.__addr, reg, len)
                return result
            except:

...
class ID809():
    ...
    def get_device_info(self):
        ...
        if self.ISIIC == True:
            time.sleep(0.05)
        val = []
        ret = self.__response_payload(val)
        lenght = val[0] +val[1]*256 + 1
        val1 = []
        ret = self.__response_payload(val1)
        decoded_string = ''.join(chr(i) for i in val1)
        if "V1.3" in decoded_string:
          self.FINGERPRINT_CAPACITY = 80
        else:
          self.FINGERPRINT_CAPACITY = 200
          
        self.FINGERPRINT_CAPACITY = 80   #等待产品经理确认后修改
    ...
    def __response_payload(self , val=None):
        ...
        data = bytearray()  # 用于存储读取到的数据
        for _ in range(dataLen):
          byte = self.__read_n(1)  # 每次读取一个字节
          data.extend(byte)  # 将读取到的字节添加到data中
    ...
    def __read_prefix(self, header):
        ...
        header[4] = self.__read_n(1)[0] # rcm L
        header[5] = self.__read_n(1)[0] # rcm H
        header[6] = self.__read_n(1)[0] # len L
        header[7] = self.__read_n(1)[0] # len H
        header[8] = self.__read_n(1)[0] # ret L
        header[9] = self.__read_n(1)[0] # ret H
    ...
    def _error_handling(self, lens=0):
        result = [0] * lens
        self._connect += 1
        print("id809 iic communication faild, please wait")
        time.sleep(0.5)
        return result
    ...
    def __write_to_addr(self, value):
        ...
        while True:
            ...
            if self._connect > self.ERROR_COUNT:
                raise ValueError("Please check the id809 connection or Reconnection sensor!!!")

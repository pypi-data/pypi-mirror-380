...
class LIS2DW12_I2C(LIS2DW12): 
  ...
  def _error_handling(self, lens=0):
    result = [0] * lens
    self._connect += 1
    print("lisdw12 iic communication faild, please wait")
    time.sleep(0.5)
    return result
  ...
  def write_reg(self, reg, data):
    ...
    while True:
      ...
      if self._connect > self.ERROR_COUNT:
        raise ValueError("Please check the lisdw12 connection or Reconnection sensor!!!")
  ...
  def read_reg(self, reg, length):
    ...
    while True:
      try:
        result = self.i2c.readfrom_mem(self.iic_addr, reg, length)
        return result[0]
      except:

...
class MPU6050():
    ...
    def _error_handling(self, lens=0):
        result = [0] * lens
        self._connect += 1
        print("mpu6050 iic communication faild, please wait")
        time.sleep(0.5)
        return result
    ...
    def writeWord(self, devAddr, reg, data):
        ...
        while True:
            ...
            if self._connect > self.ERROR_COUNT:
                raise ValueError("Please check the mpu6050 connection or Reconnection sensor!!!")
    ...
    def readByte(self, devAddr, regAddr, lens):
        ...
        while True:
            try:
                result = self._i2c.readfrom_mem(devAddr, regAddr, lens)
                return result    
            except:
    ...
    def read_reg(self, reg, lens):
        ...
        while True:
            try:
                result = self._i2c.readfrom_mem(self.i2c_addr, reg, lens)
                return result
            except:
                result = self._error_handling(lens)
            if self._connect > self.ERROR_COUNT:

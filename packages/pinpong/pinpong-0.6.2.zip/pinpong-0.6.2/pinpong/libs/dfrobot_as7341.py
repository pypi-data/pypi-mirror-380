...
#  REG_AS7341_CH5_DATA_H = 0X6F    
  ...
  def __init__(self, board=None, i2c_addr= 0x39, bus_num=1):
...
#获取as7341传感器的值
  ...
  def _error_handling(self, lens=0):
    result = [0] * lens
    self._connect += 1
    print("As7341 iic communication faild, please wait")
    time.sleep(0.5)  
    return result
  ...
  def _read_reg(self,reg,data = 0,size=1):
    ...
    while True:
      try:
        ...
        data = self._error_handling(size)
      if self._connect > self.ERROR_COUNT:
        raise ValueError("Please check the as7341 connection or Reconnection sensor!!!") 

...
class PN532_I2C(PN532):
  ...
  def _error_handling(self, lens=0):
    result = [0] * lens
    self._connect += 1
    print("pn532 iic communication faild, please wait")
    time.sleep(0.5)
    return result
  ...
  def write_reg(self, value):
    ...
    while True:
      ...
      if self._connect > self.ERROR_COUNT:
        raise ValueError("Please check the pn532 connection or Reconnection sensor!!!")
  
  def read_reg(self,length):
    self._connect = 0
    while True:
      try:
        result = self.i2c.readfrom(self.i2c_addr,length)[1:length-1]
        return result
      except:
        result = self._error_handling(lenght)[1:length-1]
      if self._connect > self.ERROR_COUNT:
        raise ValueError("Please check the pn532 connection or Reconnection sensor!!!")

...
class VEML6075:
  ...
  def _error_handling(self, lens=0):
    result = [0] * lens
    self._connect += 1
    print("veml6075 iic communication faild, please wait")
    time.sleep(0.5)
    return result
  ...
  def _writeReg(self, reg, val):
    ...
    while True:
      try:
        self.i2c.writeto_mem(self._addr, reg, val)
        return
      except:
        self._error_handling()
      if self._connect > self.ERROR_COUNT:
        raise ValueError("Please check the veml6075 connection or Reconnection sensor!!!")

  def _readReg(self, reg, read_byte):
    self._connect = 0
    while True:
      try:
        result = self.i2c.readfrom_mem(self._addr, reg, read_byte)
        return result
      except:
        result = self._error_handling(read_byte)
      if self._connect > self.ERROR_COUNT:
        raise ValueError("Please check the veml6075 connection or Reconnection sensor!!!")

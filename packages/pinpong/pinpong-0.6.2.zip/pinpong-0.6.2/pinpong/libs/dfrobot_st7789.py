...
class ST7789_SPI(ST7789):
  def __init__(self, board=None, width=240, height=240, bus_num=0, device_num=0, dc=None, res=None, cs=None, font="msyh"):
    if board is None:
      board = gboard
    ...
    self.framebuf = FRAME_BUF(width,height)
...
class FRAME_BUF(object):
  ...
  pixel = [[0 for col in range(2)] for row in range(320*240)]#定义一个二维列表
  def __init__(self,w,h):
    
    pixel = [[0 for col in range(2)] for row in range(w*h)]#定义一个二维列表     
  ...
  def flush(self,obj,x1,y1,x2,y2):
    ...
    length_y = self.font_size+1 if(length_y < self.font_size) else length_y
    for y in range(0,length_y):
      buf += [n for a in self.pixel[((y1+y)*obj.width+x1):((y1+y)*obj.width+x1+length_x)] for n in a ]#展开成一维列表
    if(abs(y2-y1) < self.font_size):
      obj.flush(buf,x1,y1,x2,y1+self.font_size)
    else:

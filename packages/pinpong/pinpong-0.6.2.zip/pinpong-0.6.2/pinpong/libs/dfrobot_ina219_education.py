...
class INA219_EDUCATION:
  ...
  def _error_handling(self, lens=0):
    result = [0] * lens
    self._connect += 1
    print("ina219 edu iic communication faild, please wait")
    time.sleep(0.5)
    return result
  ...
  def write_ina_reg(self ,reg ,data):
    ...
    while True:
      ...
      if self._connect > self.ERROR_COUNT:
        raise ValueError("Please check the ina219 edu connection or Reconnection sensor!!!")

  def read_reg(self,reg,len):
    self._connect = 0
    while True:
      try:
        result = self.i2c.readfrom_mem(self.i2c_addr,reg,len)
        return result
      except:

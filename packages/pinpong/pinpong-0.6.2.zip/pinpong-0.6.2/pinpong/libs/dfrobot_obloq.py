...
#    print(self.IPAddress)
  ...
  def _error_handling(self, lens=0):
    result = [0] * lens
    self._connect += 1
    print("obloq iic communication faild, please wait")
    time.sleep(0.5)
    return result

  def _read_size(self, address, lens):
    self._connect = 0
    while True:
      try:
        self.i2c.readfrom(address, lens)
        return result
      except:
        result = self._error_handling(lens)
      if self._connect > self.ERROR_COUNT:
        raise ValueError("Please check the obloq connection or Reconnection sensor!!!")
    
  def _read_regs(self, address, reg_addr, lens):
    self._connect = 0
    while True:
      try:
        result = self.i2c.readfrom_mem(self._i2c_addr, reg_addr, lens)
        return result
      except:
        result = self._error_handling(lens)
      if self._connect > self.ERROR_COUNT:
  ...
  def readReg(self, address, reg, size):
    ...
    if reg == 0x1E:
      data = [0, 0x06]
      self.writeReg(address, reg, data)
      result = self._read_size(address,size)
    else:
      result = self._read_regs(address, reg, size)
    return result

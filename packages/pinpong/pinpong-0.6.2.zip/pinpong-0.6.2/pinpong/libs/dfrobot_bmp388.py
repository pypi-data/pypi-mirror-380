...
class BMP388:
  ...
  def _error_handling(self, lens=0):
    result = [0] * lens
    self._connect += 1
    print("BMP388 iic communication faild, please wait")
    time.sleep(0.5)
    return result
  ...
  def bmp3_get_regs(self, reg_addr, lens):
    ...
    while True:
      try:
        result = self.i2c.readfrom_mem(self.i2c_addr, reg_addr, lens)
        return result      
      except:
        result = self._error_handling(lens)
      if self._connect > self.ERROR_COUNT:
        raise ValueError("Please check the BMP388 connection or Reconnection sensor!!!")

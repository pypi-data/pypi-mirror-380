    ...
    def begin(self, version):
        ...
        if version == self.V1:
            ...
            for i in range(40):
                ...
                rslt = self.read_ack(1)
                if rslt == None:
                    print("synthesis iic communication faild, please Checking sensor connections!!!")
                    exit()
                if rslt[0] == 0x4F:
                    break
            self.set_voice(1)
...
class DFRobot_SpeechSynthesis_I2C(DFRobot_SpeechSynthesis):
    ...
    def _error_handling(self, lens=0):
        result = [0] * lens
        self._connect += 1
        print("synthesis iic communication faild, please wait")
        time.sleep(0.5)
        return result
    
    def write_cmd(self, data, chunk_size=30):
        self._connect = 0
        while True:
            try:
                start = 0
                ...
                while start < len(data):
                    self._i2c.writeto(self.__addr, data[start:end])
                    start = end
            ...
            if self._connect > self.ERROR_COUNT:
                raise ValueError("Please check the synthesis connection or Reconnection sensor!!!")

    def read_ack(self, len):
        self._connect = 0
        while True:
            try:
                result = self._i2c.readfrom(self.__addr, len)
                return result
            except:

...
class PAJ7620U2:
  ...
  def __init__(self, board=None, i2c_addr=0x73, bus_num=0):
    ...
    rslt = self.read_reg(self.PAJ7620_ADDR_PART_ID_LOW, 2)
    while True:
      if rslt[0] == 0x20 or rslt[1] == 0x76:
        break
      time.sleep(0.1)
...
#      print(self.gesture_discription_table[gesture])
  ...
  def _error_handling(self, lens=0):
    result = [0] * lens
    self._connect += 1
    print("Paj7620u2 iic communication faild, please wait")
    time.sleep(0.5)
    return result
  ...
  def write_reg(self, reg, value):
    ...
    while True:
      ...
      if self._connect > self.ERROR_COUNT:
        raise ValueError("Please check the Paj7620u2 connection or Reconnection sensor!!!")

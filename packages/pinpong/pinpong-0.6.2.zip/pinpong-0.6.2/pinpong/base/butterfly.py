  ...
  def burn(self):
    ...
    while True:
      ...
      if next_addr - total_len == 0:
        ...
        addr = next_addr
      self.print_progress(next_addr, total_len)
    print("")
  
  def print_progress(self, current_size, total_size):
    if current_size > total_size-128:
      current_size = total_size
    progress = current_size / total_size
    bar_length = 30  # 进度条的长度设为50
    # 计算进度条中的#数量
    num_hashes = int(progress * bar_length)
    # 打印进度条和百分比，注意使用 \r 返回行首

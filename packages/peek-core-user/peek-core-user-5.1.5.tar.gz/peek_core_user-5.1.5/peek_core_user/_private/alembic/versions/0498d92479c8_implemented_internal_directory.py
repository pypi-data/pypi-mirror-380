...
def upgrade():
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["groupId"], ["core_user.InternalGroup.id"], ondelete="CASCADE"
        ),
        sa.ForeignKeyConstraint(
    ...
    op.add_column(
        "UserLoggedIn",
        sa.Column("deviceToken", sa.String(length=100), nullable=False),
        schema="core_user",
    )
    op.add_column(
        "UserLoggedIn",
    ...
    op.drop_constraint(
        "UserLoggedIn_clientDeviceToken_key",
        "UserLoggedIn",
        schema="core_user",
        type_="unique",
    )
    op.drop_constraint(
        "UserLoggedIn_userId_key",
        "UserLoggedIn",
        schema="core_user",
        type_="unique",
    )
    op.create_unique_constraint(
        None, "UserLoggedIn", ["userName"], schema="core_user"
    )
    op.create_unique_constraint(
        None, "UserLoggedIn", ["deviceToken"], schema="core_user"
    )
...
def downgrade():
    ...
    op.add_column(
        "UserLoggedIn",
        sa.Column(
            "userId", sa.VARCHAR(length=50), autoincrement=False, nullable=True
        ),
        schema="core_user",
    )

...
class MainController(TupleActionProcessorDelegateABC):
    def __init__(self, dbSessionCreator, ourApi: UserApiABC):
        self._ourApi = ourApi
        self._passwordUpdateController = PasswordUpdateController(
            dbSessionCreator
        )
    ...
    def processTupleAction(self, tupleAction: TupleActionABC) -> Deferred:
        ...
        if isinstance(tupleAction, UserLoginAuthMethodAction):
            replyTuples = yield self._ourApi.loginApi.queryUserLoginAuthMethod(
                tupleAction
            )
            return replyTuples
        if isinstance(tupleAction, UserLoginOtpAction):
            replyTuples = yield self._ourApi.loginApi.loginRequestOtp(
                tupleAction
            )
            return replyTuples
        ...
        if isinstance(tupleAction, InternalUserUpdatePasswordAction):
            replyTuples = (
                yield self._passwordUpdateController.processTupleAction(
                    tupleAction
                )
            )
            return replyTuples

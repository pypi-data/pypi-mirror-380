...
def make_symm(gc, gs, n, TOL):
    ...
    Y = sp_orth(null_space((Ra - np.eye(Ra.shape[0])).T,TOL))
    Q = Y @ Y.T
    X = sp_orth(null_space(np.round((P @ Q  - np.eye(Q.shape[0])).T,rou),TOL))
    P = X @ X.T

    Ul = U(l, lb[0], lb[1], lb[2])
    Ur = U(l, -rb[0], rb[1], rb[2])
    Rb = np.round( np.kron(Ur, Ul), rou)
    Y = sp_orth(null_space((Rb - np.eye(Rb.shape[0])).T,TOL))
    Q = Y @ Y.T
    X = sp_orth(null_space(np.round((P @ Q  - np.eye(Q.shape[0])).T,rou),TOL))
    P = X @ X.T
    ...
    Y = sp_orth(null_space((Rc - np.eye(Rc.shape[0])).T,TOL))
    Q = Y @ Y.T
    X = sp_orth(null_space(np.round((P @ Q  - np.eye(Q.shape[0])).T,rou),TOL))
    P = X @ X.T
    ...
    Y = sp_orth(null_space((Rd - np.eye(Rd.shape[0])).T,TOL))
    Q = Y @ Y.T
    X = null_space(np.round((P @ Q  - np.eye(Q.shape[0])).T,rou),TOL)

    # convert to coupled basis
    X = CG @ X
    X = np.array(CR @ clean(CR.T @ np.round(X,rou), TOL))
